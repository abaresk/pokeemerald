...
struct WishFutureKnock
{
    ...
    u8 weatherDuration;
    u8 knockedOffMons[2]; // Each battler is represented by a bit. The array entry is dependent on
                          // the battler's side.
};
...
struct BattleStruct
{
    ...
    u8 palaceFlags; // First 4 bits are "is < 50% HP and not asleep" for each battler, last 4 bits
                    // are selected moves to pass to AI
    u8 field_93;    // related to choosing pokemon?
    u8 wallyBattleState;
    u8 wallyMovesState;
    u8 wallyWaitFrames;
    u8 wallyMoveFrames;
    u8 lastTakenMove[MAX_BATTLERS_COUNT * 2
                     * 2]; // Last move that a battler was hit with. This field seems to erroneously
                           // take 16 bytes instead of 8.
    u16 hpOnSwitchout[2];
    ...
    u8 lastTakenMoveFrom[MAX_BATTLERS_COUNT * MAX_BATTLERS_COUNT
                         * 2]; // a 3-D array [target][attacker][byte]
    u16 castformPalette[MAX_BATTLERS_COUNT][16];
    u8 field_180; // weird field, used in battle_main.c, once accessed as an array of u32
                  // overwriting the field below
    u8 field_181;
    u8 field_182;
    u8 field_183;
    ...
    u16 arenaStartHp[2];
    u8 arenaLostPlayerMons; // Bits for party member, lost as in referee's decision, not by
                            // fainting.
    u8 arenaLostOpponentMons;
    u8 alreadyStatusedMoveAttempt; // As bits for battlers; For example when using Thunder Wave on
                                   // an already paralyzed pokemon.
};
...
#define IS_BATTLER_OF_TYPE(battlerId, type) \
    ((gBattleMons[battlerId].type1 == type || gBattleMons[battlerId].type2 == type))
