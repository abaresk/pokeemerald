...
/*
  ...
  <Restrictions for H-DMA or DMA started with V-Blank>
    ...
    Calculate the number of the transferable DMA count based on this 42 cycles and the access cycles of the destination
  and source. For example, if both the CPU internal RAM --> VRAM have a one cycle access, then a 21 count DMA can occur.


  If RFU is used outside of these restrictions, problems, such as the loss of data caused by the failure of the AGB, as
  a clock slave, to notify that data has been received from the RFU, will occur. When this problem occurs, the REQ
  callback will send a REQ_commandID=ID_CLOCK_SLAVE_MS_CHANGE_ERROR_BY_DMA_REQ notification. (When using Link Manager,
...
#define ID_CLOCK_SLAVE_MS_CHANGE_ERROR_BY_DMA_REQ \
    0x00ff // When the AGB is the clock slave, the RFU generates an informational notice, and an automatically started
           ...
           // notified by a REQ callback when the exchange of this information (REQ command) fails.
...
#define RFU_MBOOT_DOWNLOADER_SERIAL_NO \
    0x0000 // The game serial number of the multi-boot downloader (programs that boot without a Game Pak)

#define RFU_API_BUFF_SIZE_RAM \
    0x0e64 // Necessary size for buffer specified by rfu_initializeAPI (fast communication version that operates the
           // library SIO interrupt routines in RAM)
#define RFU_API_BUFF_SIZE_ROM \
    0x0504 // Necessary size for buffer specified by rfu_initializeAPI (fast communication version that operates the
           // library SIO interrupt routines in ROM)
...
#define RFU_H_DMA_MAX_CPU_CYCLE \
    42 // Maximum number of CPU cycles for H-DMA or DMA that starts with V-Blank that can operate when using RFU

// Value to determine the level of the icon that displays the link strength of the wireless status (value of
// rfuLinkStatus->strength[0-3])
...
// #define CP_STATUS_SLOT_CLOSED           0x02               // Parent device is not in connection mode with child
// device #define CP_STATUS_DISCONNECTED          0x03               // Disconnected by parent device while connecting
...
#define REASON_DISCONNECTED \
    0x00 // State of complete disconnection with rfuLL_disconnect and no possibility of restoring the link (can detect
         // only child device)
...
#define ERR_REQ_CMD_CLOCK_SLAVE \
    (ERR_REQ_CMD | 0x0004) // Issuing the REQ command was attempted but failed because the AGB is the clock slave
#define ERR_REQ_CMD_IME_DISABLE \
    (ERR_REQ_CMD | 0x0006) // Issuing the REQ command was attempted but failed because the IME register is 0
...
#define ERR_ID_CHECK_IME_DISABLE \
    0xffffffff // ID check could not be executed because REG_IME=0 when executing rfu_REQBN_softReset_and_checkID
...
#define ERR_SUBFRAME_SIZE \
    0x0500 // The specified subframe size is either larger than the remaining communication frame size or smaller than
           // the link layer subframe header size
...
#define ERR_DATA_RECV \
    0x0700 // Error notification is issued in the following format.
           // "ERR_DATA_RECV | (slot number issuing error with 0x0010<<UNI) | (slot number issuing error with
           // 0x0001<<NI)" The actual error code is stored in gRfuSlotStatusNI or gRfuSlotStatusUNI[x]->recv.errorCode.

// Error code during NI-type communication (code entered into gRfuSlotStatusNI[x]->recv.errorCode)
#define ERR_RECV_BUFF_OVER \
    (ERR_DATA_RECV |       \
     0x0001) // The receive data size is larger than the receive buffer (Note: This error is used for both NI and UNI)
#define ERR_RECV_REPLY_SUBFRAME_SIZE \
    (ERR_DATA_RECV | 0x0002) // All frames are already in use by other communications, and a subframe for reception
                             // response could not be secured
...
// NI- and UNI-type shared slot stand-by data (same state as gRfuSlotStatusNI[x]->send or ->recv.state or
// gRfuSlotStatusUNI)
...
#define SLOT_STATE_RECV_IGNORE \
    (SLOT_RECV_FLAG | 0x009) // Ignore reception (refer to gRfuSlotStatusNI[x]->recv.errorCode for reason for ignoring)
...
struct UNISend {
    ...
    u8 dataReadyFlag; // Flag indicating transmission data is prepared (0: data sent; 1: transmission data ready)
                      //   Set with rfu_UNI_re_setSendData or rfu_UNI_readySendData, cleared when data is sent with
                      //   rfu_REQ_sendData.
...
struct UNIRecv {
    ...
    u8 newDataFlag;   // Flag indicating whether receive data has newly arrived (0: no newly arrived data; 1: newly
                      // arrived data)
                      //   The flag is cleared with rfu_UNI_clearRecvNewDataFlag; it is set when data is received with
                      //   rfu_REQ_recvData.
...
struct NIComm {
    ...
    u16 failCounter; // Count of failed transmissions/receptions (Count is increased when transmission/reception of data
                     // does not succeed within 1PF=16.7 ms)
    const u8* now_p[WINDOW_COUNT]; // Address of current send/receive (The data is divided into WINDOW_COUNT blocks and
                                   // sent in payloadSize units.)
    ...
    u8 bmSlot;                     // Expresses the current communication slot in bits
               //   (When sending from the Master, because multiple slaves can be specified with bmSlot, communications
               //   are terminated based on the failCounter for each child device)
    ...
    u8 bmSlotOrg;    // Expresses the communication source slot at the beginning of the transmission in bits (valid only
    ...
    u32 dataSize;    // Size of transmission/reception data
                     //   Calculated by subtracting link layer header size from subFrameSize specified by the
                     //   rfu_NI_setSendData function
};
...
struct RfuTgtData {
    ...
    u8 mbootFlag; // Flag to indicate whether or not the parent candidate   Flag to indicate whether or not the
                  // connection partner
    ...
    u8 gname[RFU_GAME_NAME_LENGTH +
             2]; // Game name of parent candidate                          Game name of connection partner
    u8 uname[RFU_USER_NAME_LENGTH +
             1]; // User name for parent candidate                         User name for connection partner
};
...
struct RfuLinkStatus {
    ...
    u8 getNameFlag;     // Parent: Flag to indicate whether or not game identification information was obtained from the
                        // child connected to this slot (") Child: Flag to indicate whether or not game identification
                        // information was sent to the parent connected to this slot (")
    u8 findParentCount; // Number of parent candidates discovered with rfu_REQ_xxxSearchParent, valid only with a child
                        // device
    ...
    u8 strength[RFU_CHILD_MAX]; // Link strength value (0x00 - 0xff; 0x00 is link loss. Updates with
                                // rfu_REQBN_watchLink)
    vu8 LLFReadyFlag; // Indicates whether the link layer frame to send next has be constructed (Flag is on when calling
                      // rfu_constructLLFrame, flag is cleared when sending by rfu_REQ_sendData.)
    u8 remainLLFrameSizeParent; // Remaining size of the link layer communication frame for the parent device
    u8 remainLLFrameSizeChild[RFU_CHILD_MAX]; // Remaining size of the link layer communication frame for each slot for
                                              // child device
    struct RfuTgtData
        partner[RFU_CHILD_MAX]; // Parent, Child: When there is a connection, the matrix element corresponding to the
                                // number of the connected slot
                                //   stores the game identification information for the connection partner.
                                // Child: Stores the game identification information for the parent candidate discovered
                                // when executing rfu_REQ_xxxxSearchParent.
...
// Used only by parent device. At the beginning of a MSC Callback that received notification that the data transmission
// completed, an ACK flag is obtained.
...
// Used only by child device. After establishing connection at the RFU level, configure transmission of child device
// game identification information in order to authenticate connection
...
// For Multi-boot
// Inherits the information about the link established by the downloader just after the program downloaded with
// multiboot starts up.
