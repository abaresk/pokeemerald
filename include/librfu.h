...
/*
  ...
  <Restrictions for H-DMA or DMA started with V-Blank>
    ...
    Calculate the number of the transferable DMA count based on this 42 cycles and the access cycles
  of the destination and source. For example, if both the CPU internal RAM --> VRAM have a one cycle
  access, then a 21 count DMA can occur.


  If RFU is used outside of these restrictions, problems, such as the loss of data caused by the
  failure of the AGB, as a clock slave, to notify that data has been received from the RFU, will
  occur. When this problem occurs, the REQ callback will send a
  REQ_commandID=ID_CLOCK_SLAVE_MS_CHANGE_ERROR_BY_DMA_REQ notification. (When using Link Manager,
  the LMAN call back will send a LMAN_msg=LMAN_MSG_CLOCK_SLAVE_MS_CHANGE_ERROR_BY_DMA notification.)
...
#define ID_MS_CHANGE_REQ                                                                           \
    0x0027 // When returned by the REQ callback, this ID indicates that execution of
           // rfu_REQ_changeMasterSlave has finished. This ID is returned by both the REQ callback
           // and the MSC callback. When returned by the MSC callback, this is notification that
           // after the AGB has been made into the clock slave, the MC_Timer expired and the RFU
           // returned the AGB to be the clock master.
...
#define ID_CLOCK_SLAVE_MS_CHANGE_ERROR_BY_DMA_REQ                                                  \
    0x00ff // When the AGB is the clock slave, the RFU generates an informational notice, and an
           // automatically started DMA, such as HDMA, is generated at the instant the AGB is being
           // returned as the clock master. This ID is notified by a REQ callback when the exchange
           // of this information (REQ command) fails.
...
#define RFU_ID                                                                                     \
    0x00008001 // ID number of AGB-RFU device: ID returned by execution of
               // rfu_REQBN_softReset_and_checkID.

#define RFU_MBOOT_DOWNLOADER_SERIAL_NO                                                             \
    0x0000 // The game serial number of the multi-boot downloader (programs that boot without a Game
           // Pak)

#define RFU_API_BUFF_SIZE_RAM                                                                      \
    0x0e64 // Necessary size for buffer specified by rfu_initializeAPI (fast communication version
           // that operates the library SIO interrupt routines in RAM)
#define RFU_API_BUFF_SIZE_ROM                                                                      \
    0x0504 // Necessary size for buffer specified by rfu_initializeAPI (fast communication version
           // that operates the library SIO interrupt routines in ROM)
...
#define RFU_H_DMA_MAX_CPU_CYCLE                                                                    \
    42 // Maximum number of CPU cycles for H-DMA or DMA that starts with V-Blank that can operate
       // when using RFU

// Value to determine the level of the icon that displays the link strength of the wireless status
// (value of rfuLinkStatus->strength[0-3])
#define RFU_LINK_ICON_LEVEL4_MAX                                                                   \
    255 // 90% -100%  (If the value is betwen 229 and 255, the icon level is 4)
#define RFU_LINK_ICON_LEVEL4_MIN 229
#define RFU_LINK_ICON_LEVEL3_MAX                                                                   \
    228 // 50% - 89%  (If the value is betwen 127 and 228, the icon level is 3)
#define RFU_LINK_ICON_LEVEL3_MIN 127
#define RFU_LINK_ICON_LEVEL2_MAX                                                                   \
    126 // 10% - 49%  (If the value is betwen 25 and 126, the icon level is 2)
#define RFU_LINK_ICON_LEVEL2_MIN 25
#define RFU_LINK_ICON_LEVEL1_MAX                                                                   \
    24 //  0% -  9%  (If the value is betwen 0 and 24, the icon level is 1)
...
// AvailSlot (number of slots available for connection) used in u16 availSlotFlag in
...
// #define CP_STATUS_SLOT_CLOSED           0x02               // Parent device is not in connection
// mode with child device #define CP_STATUS_DISCONNECTED          0x03               // Disconnected
// by parent device while connecting #define CP_STATUS_UNKNOWN               0xff               //
// Cannot read status due to REQ-API execution error
...
// #define RC_STATUS_FAILED                0x01               // Connection restore failure
// (meaningless to try anymore) #define RC_STATUS_SEARCHING_PARENT      0x02               //
// Searching for parent device #define RC_STATUS_UNKNOWN               0xff               // Cannot
// read status due to REQ-API execution error

// Value of u8 *linkLossReason argument for rfu_REQBN_watchLink (Reason for Link Loss)
#define REASON_DISCONNECTED                                                                        \
    0x00 // State of complete disconnection with rfuLL_disconnect and no possibility of restoring
         // the link (can detect only child device)
#define REASON_LINK_LOSS                                                                           \
    0x01 // State of link loss that allows for the possibility for the link to be restored
...
#define ERR_REQ_CMD_CLOCK_DRIFT                                                                    \
    (ERR_REQ_CMD | 0x0001) // Clock drift occurs when a REQ command is sent
...
#define ERR_REQ_CMD_CLOCK_SLAVE                                                                    \
    (ERR_REQ_CMD | 0x0004) // Issuing the REQ command was attempted but failed because the AGB is
                           // the clock slave
#define ERR_REQ_CMD_IME_DISABLE                                                                    \
    (ERR_REQ_CMD |                                                                                 \
        0x0006) // Issuing the REQ command was attempted but failed because the IME register is 0

#define ERR_PID_NOT_FOUND                                                                          \
    0x0100 // The specified PID does not exist in the gRfuLinkStatus->partner[0-3].id list
...
#define ERR_ID_CHECK_IME_DISABLE                                                                   \
    0xffffffff // ID check could not be executed because REG_IME=0 when executing
               // rfu_REQBN_softReset_and_checkID

// Error codes returned by rfu_getConnectParentStatus and rfu_CHILD_getConnectRecoveryStatus
#define ERR_REQ_CMD_ID                                                                             \
    (ERR_REQ_CMD | 0x0010) // A required REQ command was not executed within this function execution
...
#define ERR_MODE_NOT_CONNECTED                                                                     \
    (ERR_MODE | 0x0001) // RFU is not in connection mode (parent or child)
...
#define ERR_SLOT_NOT_SENDING                                                                       \
    (ERR_SLOT | 0x0003) // The specified slot is not currently sending (Note: This is returned by
                        // rfu_UNI_re_setSendData)
#define ERR_SLOT_TARGET (ERR_SLOT | 0x0004) // The specified target slot is not right

#define ERR_SUBFRAME_SIZE                                                                          \
    0x0500 // The specified subframe size is either larger than the remaining communication frame
           // size or smaller than the link layer subframe header size
...
#define ERR_DATA_RECV                                                                              \
    0x0700 // Error notification is issued in the following format.
           // "ERR_DATA_RECV | (slot number issuing error with 0x0010<<UNI) | (slot number issuing
           // error with 0x0001<<NI)" The actual error code is stored in gRfuSlotStatusNI or
           // gRfuSlotStatusUNI[x]->recv.errorCode.

// Error code during NI-type communication (code entered into gRfuSlotStatusNI[x]->recv.errorCode)
#define ERR_RECV_BUFF_OVER                                                                         \
    (ERR_DATA_RECV | 0x0001) // The receive data size is larger than the receive buffer (Note: This
                             // error is used for both NI and UNI)
#define ERR_RECV_REPLY_SUBFRAME_SIZE                                                               \
    (ERR_DATA_RECV | 0x0002) // All frames are already in use by other communications, and a
                             // subframe for reception response could not be secured

// Error code during UNI-type communication (code entered into gRfuSlotStatusUNI[x]->recv.errorCode)
#define ERR_RECV_DATA_OVERWRITED                                                                   \
    (ERR_DATA_RECV | 0x0008) // Received new data when gRfuSlotStatusUNI[x]->recv.newDataFlag=1
...
// Definition data for NI-(Numbered Information) and UNI-(Un-Numbered Information) type
// communication

// Definition data common to NI- and UNI-type communication
#define LLF_P_SIZE                                                                                 \
    87 // Maximum link layer communication frame size for the Parent (Maximum number of bytes per
       // transmission)
#define LLF_C_SIZE 16 //      "                                "     for the Child
#define LLSF_P_HEADER_SIZE                                                                         \
    3 // Header size used by one link layer subframe (NI or UNI frame) for the Parent
#define LLSF_C_HEADER_SIZE                                                                         \
    2 //      "                                "                       for the Child
...
// NI- and UNI-type shared slot stand-by data (same state as gRfuSlotStatusNI[x]->send or
// ->recv.state or gRfuSlotStatusUNI)
...
#define SLOT_STATE_SEND_START                                                                      \
    (SLOT_BUSY_FLAG | SLOT_SEND_FLAG | LCOM_NI_START)                  // Start transmission
#define SLOT_STATE_SENDING (SLOT_BUSY_FLAG | SLOT_SEND_FLAG | LCOM_NI) // Transmitting
#define SLOT_STATE_SEND_LAST                                                                       \
    (SLOT_BUSY_FLAG | SLOT_SEND_FLAG | LCOM_NI_END) // Verify transmission end
#define SLOT_STATE_SEND_NULL                                                                       \
    (SLOT_BUSY_FLAG | SLOT_SEND_FLAG |                                                             \
...
#define SLOT_STATE_RECV_SUCCESS_AND_SENDSIDE_UNKNOWN                                               \
    (SLOT_RECV_FLAG | 0x008) // The data was received successful, but because the end of the
                             // communication was incomplete, whether the transmission side status
                             // is successful or failed is unknown
#define SLOT_STATE_RECV_IGNORE                                                                     \
    (SLOT_RECV_FLAG | 0x009) // Ignore reception (refer to gRfuSlotStatusNI[x]->recv.errorCode for
                             // reason for ignoring)
...
struct UNISend
{
    ...
    u8 dataReadyFlag; // Flag indicating transmission data is prepared (0: data sent; 1:
                      // transmission data ready)
                      //   Set with rfu_UNI_re_setSendData or rfu_UNI_readySendData, cleared when
                      //   data is sent with rfu_REQ_sendData.
    u8 bmSlot;        // Expresses transmission destination slot as bits
    u16 payloadSize;  // Payload size of 1 transmission. "size" specified by rfu_UNI_setSendData or
                      // rfu_UNI_re_setSendData is used.
...
struct UNIRecv
{
    u16 state;      // Slot communication state (SLOT_STATE_READY, SLOT_STATE_RECEIVING,
                    // SLOT_STATE_RECV_IGNORE)
    u16 errorCode;  // Error code during reception
    u16 dataSize;   // Size of receive data
    u8 newDataFlag; // Flag indicating whether receive data has newly arrived (0: no newly arrived
                    // data; 1: newly arrived data)
                    //   The flag is cleared with rfu_UNI_clearRecvNewDataFlag; it is set when data
                    //   is received with rfu_REQ_recvData.
    u8 dataBlockFlag; // Flag for unreferenced receive data overwrite block (default is 0 Note: not
                      // used)
};
...
struct NIComm
{
    ...
    u16 failCounter; // Count of failed transmissions/receptions (Count is increased when
                     // transmission/reception of data does not succeed within 1PF=16.7 ms)
    const u8 *now_p[WINDOW_COUNT]; // Address of current send/receive (The data is divided into
                                   // WINDOW_COUNT blocks and sent in payloadSize units.)
    ...
    u8 bmSlot;                     // Expresses the current communication slot in bits
               //   (When sending from the Master, because multiple slaves can be specified with
               //   bmSlot, communications are terminated based on the failCounter for each child
               //   device)
    ...
    u8 bmSlotOrg; // Expresses the communication source slot at the beginning of the transmission in
                  // bits (valid only for transmitting device)
    u8 dataType;  // Transmission/reception data type (0: User data; 1: Game identification
                  // information)
    u16 payloadSize; // Payload size for one transmission
    u32 dataSize;    // Size of transmission/reception data
                  //   Calculated by subtracting link layer header size from subFrameSize specified
                  //   by the rfu_NI_setSendData function
};
...
struct RfuTgtData
{
    ...
    u8 mbootFlag; // Flag to indicate whether or not the parent candidate   Flag to indicate whether
                  // or not the connection partner
                  //   is multiboot program for download                      is multiboot program
                  //   for download
    u16 serialNo; // Game serial number of parent candidate                 Game serial number of
                  // connection partner
    u8 gname[RFU_GAME_NAME_LENGTH + 2]; // Game name of parent candidate Game name of connection
                                        // partner
    u8 uname[RFU_USER_NAME_LENGTH + 1]; // User name for parent candidate User name for connection
                                        // partner
};
...
struct RfuLinkStatus
{
    ...
    u8 getNameFlag; // Parent: Flag to indicate whether or not game identification information was
                    // obtained from the child connected to this slot (") Child: Flag to indicate
                    // whether or not game identification information was sent to the parent
                    // connected to this slot (")
    u8 findParentCount; // Number of parent candidates discovered with rfu_REQ_xxxSearchParent,
                        // valid only with a child device
    u8 watchInterval;   // Link monitor execution interval (16.7 ms unit AGB picture frame)
    u8 strength[RFU_CHILD_MAX]; // Link strength value (0x00 - 0xff; 0x00 is link loss. Updates with
                                // rfu_REQBN_watchLink)
    vu8 LLFReadyFlag; // Indicates whether the link layer frame to send next has be constructed
                      // (Flag is on when calling rfu_constructLLFrame, flag is cleared when sending
                      // by rfu_REQ_sendData.)
    u8 remainLLFrameSizeParent; // Remaining size of the link layer communication frame for the
                                // parent device
    u8 remainLLFrameSizeChild[RFU_CHILD_MAX]; // Remaining size of the link layer communication
                                              // frame for each slot for child device
    struct RfuTgtData
        partner[RFU_CHILD_MAX]; // Parent, Child: When there is a connection, the matrix element
                                // corresponding to the number of the connected slot
                                //   stores the game identification information for the connection
                                //   partner.
                                // Child: Stores the game identification information for the parent
                                // candidate discovered when executing rfu_REQ_xxxxSearchParent.
    struct RfuTgtData my; // The device's own game identification information (unrelated to the
                          // element slot value)
};
...
// Arguments with "bm..." specify slots of the form (0x01 << slot number) that are the object of a
// function operation.

// librfu_rfu
// API Initialization and Initial Settings
// API Initialization
u16 rfu_initializeAPI(
    u32 *APIBuffer, u16 buffByteSize, IntrFunc *sioIntrTable_p, bool8 copyInterruptToRam);
// Set Timer Interrupt
...
// RFU Link Management
// Link Monitoring
u16 rfu_REQBN_watchLink(
    u16 reqCommandId, u8 *bmLinkLossSlot, u8 *linkLossReason, u8 *parentBmLinkRecoverySlot);
// Link Disconnect
...
// Used only by parent device. At the beginning of a MSC Callback that received notification that
// the data transmission completed, an ACK flag is obtained.
...
// Used only by child device. After establishing connection at the RFU level, configure transmission
// of child device game identification information in order to authenticate connection
...
// For Multi-boot
// Inherits the information about the link established by the downloader just after the program
// downloaded with multiboot starts up.
