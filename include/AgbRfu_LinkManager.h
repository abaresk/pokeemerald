...
#define LMAN_ERROR_NOW_LINK_RECOVERY \
    5 // New settings were ignored because link recovery was under way when current link recovery was ON and new
      // settings turned link recovery OFF.
...
// Details of messages and the related parameters returned by the u8 msg, u8 param_count, and lman.param[0-1] arguments
// of the LMAN callback generated by the operation of the Link Manager.
//        msg name                                          msg No.    param qty    param[0]                    param[1]
//        Description
#define LMAN_MSG_INITIALIZE_COMPLETED \
    0x00 //    0            -                            -            Generated when RFU reset and initial settings are
         //    completed
#define LMAN_MSG_NEW_CHILD_CONNECT_DETECTED \
    0x10 //    1        Bit indicating slot                -            Generated when new child device connection was
         //    detected at RFU level.
//          with detected connection
#define LMAN_MSG_NEW_CHILD_CONNECT_ACCEPTED \
    0x11 //    1        Bit indicating slot                -            Generated when game identification information
         //    from child device connected at RFU level is received and accepted (game serial numbers match).
//          where connection was accepted
#define LMAN_MSG_NEW_CHILD_CONNECT_REJECTED \
    0x12 //    1        Bit indicating slot                -            Generated when the connection from the child
         //    device connected at RFU level is rejected (game identification information reception failed or game
         //    serial numbers do not match).
//          where connection was rejected
#define LMAN_MSG_SEARCH_CHILD_PERIOD_EXPIRED \
    0x13 //    0            -                            -            Generated when SearchChild operation time expires.
#define LMAN_MSG_END_WAIT_CHILD_NAME \
    0x14 //    0            -                            -            Generated when reception of game identification
         //    information from all child devices completes after the SearchChild operation time expires.
#define LMAN_MSG_PARENT_FOUND \
    0x20 //    1        Bit indicating                     -            Generated when valid (game serial numbers match)
         //    parent devices are found during SearchParent.
//          rfuLinkStatus->partner[x] index number storing the valid parent devices (game serial number matches) from
//          among the discovered parent devices.
#define LMAN_MSG_SEARCH_PARENT_PERIOD_EXPIRED \
    0x21 //    0            -                            -            Generated when SearchParent time expires.
#define LMAN_MSG_CONNECT_PARENT_SUCCESSED \
    0x22 //    1        Connected slot number            -            Generated when connection with parent device at
         //    RFU level succeeds.
#define LMAN_MSG_CONNECT_PARENT_FAILED \
    0x23 //    1        Connection failure reason        -            Generated when connection with parent device at
         //    RFU level fails.
#define LMAN_MSG_CHILD_NAME_SEND_COMPLETED \
    0x24 //    0            -                            -            Generated when transmission of the child's game
         //    identification information to the parent device succeeds after connection with parent device at RFU level
         //    succeeds.
#define LMAN_MSG_CHILD_NAME_SEND_FAILED_AND_DISCONNECTED \
    0x25 //    0        -                            -            Generated when transmission of the child's game
         //    identification information to the parent device fails after connection with parent device at RFU level
         //    succeeds.
#define LMAN_MSG_LINK_LOSS_DETECTED_AND_DISCONNECTED \
    0x30 //    1        Bit indicating slot                -            Generated when a link cut is detected and that
         //    slot disconnects. (Generated only when link recovery is OFF.)
...
#define LMAN_MSG_LINK_LOSS_DETECTED_AND_START_RECOVERY \
    0x31 //    1        Bit indicating slot                -            Generated when a link cut is detected and the
         //    link recovery process starts. (Generated only when link recovery is ON.)
//         where link cut was detected and link recovery was started
#define LMAN_MSG_LINK_RECOVERY_SUCCESSED \
    0x32 //    1        Bit indicating slot                -            Generated when link recovery succeeds.
         //    (Generated only when link recovery is ON.)
//        where link recovery succeeded
#define LMAN_MSG_LINK_RECOVERY_FAILED_AND_DISCONNECTED \
    0x33 //    1        Bit indicating slot                -            Generated when link recovery fails and that slot
         //    disconnects. (Generated only when link recovery is ON.)
//        where link recovery failed and that disconnected
#define LMAN_MSG_LINK_DISCONNECTED_BY_USER \
    0x40 //    1    Generated on disconnection by execution of rfu_REQ_disconnect by disconnected user.
//         Slot indicated in bits    *Note: If rfu_REQ_disconnect is used for disconnection during link recovery in the
//         child device, the link recovery is also suspended, but the only message returned is DISCONNECTED_BY_USER.
#define LMAN_MSG_CHANGE_AGB_CLOCK_SLAVE \
    0x41 //    0            -                            -            Generated when the AGB-RFU clock switches to AGB
         //    clock slave after a successful connection or link recovery in the child device. (This message is not
         //    generated when the AGB switches to a clock slave when an MSC callback completes.)
#define LMAN_MSG_CHANGE_AGB_CLOCK_MASTER \
    0x45 //    0            -                            -            Generated when the AGB-RFU intercommunication
         //    clock is switched to the AGB clock master in the child device.
#define LMAN_MSG_RFU_POWER_DOWN \
    0x42 //    0            -                            -            Generated when the RFU enters power conservation
         //    mode with rfu_LMAN_powerDownRFU.
#define LMAN_MSG_MANAGER_STOPPED \
    0x43 //    0            -                            -            Generated when the Link Manager is halted by
         //    rfu_LMAN_stopLMAN(0). (This message is not generated during SearchChild, SearchParent, ConnectParent, and
         //    LinkRecovery. A message (-PERIOD_EXPIRED, -SUCCESSED, -FAILED) corresponding to the operation completion
         //    is returned.)
#define LMAN_MSG_MANAGER_FORCED_STOPPED_AND_RFU_RESET \
    0x44 //    0            -                            -            Generates when the Link Manager is forcibly halted
         //    by rfu_LMAN_stopLMAN(1) and RFU is reset.

#define LMAN_MSG_RECV_DATA_REQ_COMPLETED \
    0x50 //    0            -                            -            Generated when the execution of rfu_REQ_recvData
         //    completes. (This message is not notification of data reception from a RFU.)

#define LMAN_MSG_REQ_API_ERROR \
    0xf0 //    2        REQ_commandID                    REQ_result     REQ-API resulted in error. This message is also
         //    generated by an REQ-API executed by either the link manager or the user.
#define LMAN_MSG_WATCH_DOG_TIMER_ERROR \
    0xf1 //     0            -                            -            Generated when a MSC callback does not occur even
         //     after 6 seconds pass when the AGB is the clock slave.
#define LMAN_MSG_CLOCK_SLAVE_MS_CHANGE_ERROR_BY_DMA \
    0xf2 //     0            -                            -            Generated when an automatic starting DMA, such as
         //     HDMA, is issued at the same time the RFU attempts to send notification and to return the AGB to the
         //     clock master while the AGB is the clock slave and the exchange of that information (REQ command) fails.
#define LMAN_MSG_LMAN_API_ERROR_RETURN \
    0xf3 //  1        Return error code                -            Generated when LMAN-API execution returns an error.
#define LMAN_MSG_RFU_FATAL_ERROR \
    0xff //    0            -                            -            Generated when the Link Manager cannot recongize a
         //    RFU because of the execution of rfu_REQBN_softReset_and_checkID.

// Value of lman.childClockSlave_flag
#define RFU_CHILD_CLOCK_SLAVE_OFF \
    0 // The child device is not operating in AGB clock slave mode. (The child device is in this mode in cases such as
      // when the child has not established a connection or during link recovery.)
#define RFU_CHILD_CLOCK_SLAVE_ON \
    1 // The child device is operating in AGB clock slave mode. (Child automatically enters this mode when a connection
      // is established.)
...
typedef struct InitializeParametersTag {
    ...
    u16 availSlot_flag; // Use RFU-API constant "AVAIL_SLOT1-4" to specify the maximum number of child devices (1 - 4)
                        // that can be connected to a parent device.
    ...
    u16 linkRecovery_period; // Time to spend on the link recovery process (x 16.7 ms)  Note: Runs for unlimited time
                             // when specifying 0.

    // Setting for NI-type data transmit/receive period
    u16 NI_failCounter_limit; //  Limit for failCounter during NI type data transmit/receive (x 16.7 ms) Note: Runs for
                              //  unlimited time when specifying 0.
} INIT_PARAM;
