...
static void sub_8064734(void) {
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_MULTI) &&
        ...
        TryShinyAnimation(BATTLE_PARTNER(gActiveBattler),
        ...
        if (gSprites[gUnknown_03005D7C[gActiveBattler]].callback == SpriteCallbackDummy &&
            ...
            if (IsDoubleBattle() && !(gBattleTypeFlags & BATTLE_TYPE_MULTI)) {
                ...
                SetBattlerShadowSpriteCallback(
                    BATTLE_PARTNER(gActiveBattler),
                    GetMonData(&gEnemyParty[gBattlerPartyIndexes[BATTLE_PARTNER(gActiveBattler)]], MON_DATA_SPECIES));
            }
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_MULTI) {
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_BATTLE_TOWER) {
            ...
            if ((gLinkPlayers[GetBattlerMultiplayerId(gActiveBattler)].version & 0xFF) == VERSION_FIRE_RED ||
                ...
                trainerPicId =
                    PlayerGenderToFrontTrainerPicId(gLinkPlayers[GetBattlerMultiplayerId(gActiveBattler)].gender);
            }
        }
    } else {
        xPos = 176;
...
static void LinkOpponentHandleTrainerSlide(void) {
    ...
    gBattlerControllerFuncs[gActiveBattler] =
        CompleteOnBankSpriteCallbackDummy2; // this line is redundant, because LinkOpponentBufferExecCompleted changes
                                            // the battle battlerId function
    LinkOpponentBufferExecCompleted();
}
