...
void EvolutionScene(struct Pokemon* mon, u16 speciesToEvolve, bool8 canStopEvo, u8 partyID) {
    ...
    DecompressPicFromTable_2(&gMonFrontPicTable[currSpecies], gMonSpritesGfxPtr->sprites[1], currSpecies);
    ...
    // postEvo sprite
    DecompressPicFromTable_2(&gMonFrontPicTable[speciesToEvolve], gMonSpritesGfxPtr->sprites[3], speciesToEvolve);
...
static void CB2_EvolutionSceneLoadGraphics(void) {
    ...
    DecompressPicFromTable_2(&gMonFrontPicTable[postEvoSpecies], gMonSpritesGfxPtr->sprites[3], postEvoSpecies);
...
void TradeEvolutionScene(struct Pokemon* mon, u16 speciesToEvolve, u8 preEvoSpriteID, u8 partyID) {
    ...
    DecompressPicFromTable_2(&gMonFrontPicTable[speciesToEvolve], gMonSpritesGfxPtr->sprites[1], speciesToEvolve);
...
static void CreateShedinja(u16 preEvoSpecies, struct Pokemon* mon) {
    ...
    if (gEvolutionTable[preEvoSpecies][0].method == EVO_LEVEL_NINJASK && gPlayerPartyCount < 6) {
        ...
        SetMonData(&gPlayerParty[gPlayerPartyCount], MON_DATA_SPECIES,
                   (&gEvolutionTable[preEvoSpecies][1].targetSpecies));
        SetMonData(&gPlayerParty[gPlayerPartyCount], MON_DATA_NICKNAME,
...
static void Task_EvolutionScene(u8 taskID) {
    ...
    if (gMain.heldKeys == B_BUTTON && gTasks[taskID].tState == 8 && gTasks[sEvoGraphicsTaskID].isActive &&
        ...
        case 22: // try to learn a new move
            switch (gTasks[taskID].tLearnMoveState) {
                ...
                case 5:
                    if (!gPaletteFade.active) {
                        FreeAllWindowBuffers();
                        ShowSelectMovePokemonSummaryScreen(gPlayerParty, gTasks[taskID].tPartyID, gPlayerPartyCount - 1,
                ...
                case 6:
                    if (!gPaletteFade.active && gMain.callback2 == CB2_EvolutionSceneUpdate) {
                        ...
                        if (var == MAX_MON_MOVES) {
                            ...
                            if (IsHMMove2(move)) {
                                BattleStringExpandPlaceholdersToDisplayedString(
                                    gBattleStringsTable[STRINGID_HMMOVESCANTBEFORGOTTEN - BATTLESTRINGS_ID_ADDER]);
                                BattlePutTextOnWindow(gDisplayedStringBattle, 0);
...
static void Task_TradeEvolutionScene(u8 taskID) {
    ...
    switch (gTasks[taskID].tState) {
        ...
        case 15:
            if (!gTasks[sEvoGraphicsTaskID].isActive) {
                m4aMPlayAllStop();
                BeginNormalPaletteFade((1 << (gSprites[sEvoStructPtr->preEvoSpriteID].oam.paletteNum + 16)) | (0x4001C),
                                       0, 0x10, 0, RGB_WHITE);
                gTasks[taskID].tState++;
            }
            break;
            ...
            switch (gTasks[taskID].tLearnMoveState) {
                ...
                case 5:
                    if (!gPaletteFade.active) {
                        ...
                        ShowSelectMovePokemonSummaryScreen(gPlayerParty, gTasks[taskID].tPartyID, gPlayerPartyCount - 1,
                                                           CB2_TradeEvolutionSceneLoadGraphics, gMoveToLearn);
                        gTasks[taskID].tLearnMoveState++;
                    }
                    break;
                case 6:
