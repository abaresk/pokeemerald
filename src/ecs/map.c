...
// TODO(hondew): If map keys take up too much memory, make the map accept a
// generic numeric key type (i.e., let key size vary).
...
static int32_t get_bucket_count(
...
/* Get bucket for key */
static ecs_bucket_t *get_bucket(
    const ecs_map_t *map,
    ecs_map_key_t key)
{
...
static ecs_bucket_t *ensure_bucket(
    ecs_map_t *map,
    ecs_map_key_t key)
{
    if (!map->bucket_count)
