...
static const u8* const sCompatibilityMessages[] = { gDaycareText_GetAlongVeryWell, gDaycareText_GetAlong,
                                                    gDaycareText_DontLikeOther, gDaycareText_PlayOther };
...
static s32 GetParentToInheritNature(struct DayCare* daycare) {
    ...
    if (GetBoxMonData(&daycare->mons[parent].mon, MON_DATA_HELD_ITEM) != ITEM_EVERSTONE || Random() >= USHRT_MAX / 2) {
        return -1;
    }
...
static void _TriggerPendingDaycareEgg(struct DayCare* daycare) {
    ...
    // inherit nature
        ...
        u8 wantedNature =
            GetNatureFromPersonality(GetBoxMonData(&daycare->mons[parent].mon, MON_DATA_PERSONALITY, NULL));
        u32 personality;
