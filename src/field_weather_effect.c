...
static const struct SpriteSheet sCloudSpriteSheet = { .data = gWeatherCloudTiles,
...
static void CreateCloudSprites(void) {
    ...
    for (i = 0; i < NUM_CLOUD_SPRITES; i++) {
        ...
        if (spriteId != MAX_SPRITES) {
            ...
            SetSpritePosToMapCoords(sCloudSpriteMapCoords[i].x + 7, sCloudSpriteMapCoords[i].y + 7, &sprite->pos1.x,
                                    &sprite->pos1.y);
...
static void UpdateRainSprite(struct Sprite* sprite) {
    if (sprite->tState == 0) {
        ...
        if (sprite->tActive && (sprite->pos1.x >= -8 && sprite->pos1.x <= 248) && sprite->pos1.y >= -16 &&
            sprite->pos1.y <= 176)
...
static bool8 CreateRainSprite(void) {
    ...
    spriteId =
        CreateSpriteAtEnd(&sRainSpriteTemplate, sRainSpriteCoords[spriteIndex].x, sRainSpriteCoords[spriteIndex].y, 78);
...
static const union AnimCmd* const gSpriteAnimTable_839AB64[] = {
    gSpriteAnim_839AB34, gSpriteAnim_839AB3C, gSpriteAnim_839AB44,
    gSpriteAnim_839AB4C, gSpriteAnim_839AB54, gSpriteAnim_839AB5C,
};
    ...
    if (!gWeatherPtr->sandstormSwirlSpritesCreated) {
        for (i = 0; i < NUM_SWIRL_SANDSTORM_SPRITES; i++) {
            ...
            if (spriteId != MAX_SPRITES) {
                ...
                CalcCenterToCornerVec(gWeatherPtr->sprites.s2.sandstormSprites2[i], SPRITE_SHAPE(32x32),
                                      SPRITE_SIZE(32x32), ST_OAM_AFFINE_OFF);
