...
bool8 sub_811F960(void)
{
    ...
    switch (sMenu->tileLoadState)
    {
        case 0:
            ...
            for (i = 0; i < 6; i++)
            {
                CpuFastCopy(sMenu->frameTiles + TILE_SIZE_4BPP,
                    dest + TILE_SIZE_4BPP * (i + 1),
                    TILE_SIZE_4BPP);
            }
            CpuFastCopy(
                sMenu->frameTiles + TILE_SIZE_4BPP * 2, dest + TILE_SIZE_4BPP * 7, TILE_SIZE_4BPP);
            sMenu->tileLoadState++;
            break;
        default:
            ...
            for (i = 0; i < 6; i++)
            {
                CpuFastCopy(sMenu->frameTiles + TILE_SIZE_4BPP * 4,
                    dest + TILE_SIZE_4BPP * (i + 1),
                    TILE_SIZE_4BPP);
            }
            CpuFastCopy(
                sMenu->frameTiles + TILE_SIZE_4BPP * 5, dest + TILE_SIZE_4BPP * 7, TILE_SIZE_4BPP);
            sMenu->tileLoadState++;
            break;
        case 13:
            ...
            for (i = 0; i < 6; i++)
            {
                CpuFastCopy(sMenu->frameTiles + TILE_SIZE_4BPP * 7,
                    dest + TILE_SIZE_4BPP * (i + 1),
                    TILE_SIZE_4BPP);
            }
            CpuFastCopy(
                sMenu->frameTiles + TILE_SIZE_4BPP * 8, dest + TILE_SIZE_4BPP * 7, TILE_SIZE_4BPP);
            sMenu->tileLoadState++;
            return FALSE;
        case 14:
            return FALSE;
...
static void sub_811FC80(s16 x, s16 y, u16 baseTileTag, u16 basePaletteTag)
{
    ...
    if (spriteId != MAX_SPRITES)
    {
        ...
        CalcCenterToCornerVec(
            sMenu->menuTextSprite, SPRITE_SHAPE(32x16), SPRITE_SIZE(32x16), ST_OAM_AFFINE_OFF);
    }
    else
    {
        sMenu->menuTextSprite = NULL;
