...
// rom const data
static const u16 sSoundMovesTable[] = { MOVE_GROWL,         MOVE_ROAR,        MOVE_SING,   MOVE_SUPERSONIC,
                                        MOVE_SCREECH,       MOVE_SNORE,       MOVE_UPROAR, MOVE_METAL_SOUND,
                                        MOVE_GRASS_WHISTLE, MOVE_HYPER_VOICE, 0xFFFF };
...
void PressurePPLose(u8 target, u8 attacker, u16 move) {
    ...
    if (!(gBattleMons[attacker].status2 & STATUS2_TRANSFORMED) &&
        ...
        BtlController_EmitSetMonData(0, REQUEST_PPMOVE1_BATTLE + moveIndex, 0, 1,
...
void PressurePPLoseOnUsingImprison(u8 attacker) {
    ...
    if (imprisonPos != 4 && !(gBattleMons[attacker].status2 & STATUS2_TRANSFORMED) &&
        ...
        gActiveBattler = attacker;
        BtlController_EmitSetMonData(0, REQUEST_PPMOVE1_BATTLE + imprisonPos, 0, 1,
...
void PressurePPLoseOnUsingPerishSong(u8 attacker) {
    ...
    if (perishSongPos != MAX_MON_MOVES && !(gBattleMons[attacker].status2 & STATUS2_TRANSFORMED) &&
        ...
        gActiveBattler = attacker;
        BtlController_EmitSetMonData(0, REQUEST_PPMOVE1_BATTLE + perishSongPos, 0, 1,
...
u8 CheckMoveLimitations(u8 battlerId, u8 unusableMoves, u8 check) {
    ...
    for (i = 0; i < MAX_MON_MOVES; i++) {
        ...
        if (gBattleMons[battlerId].moves[i] == gDisableStructs[battlerId].disabledMove &&
            ...
            unusableMoves |= gBitTable[i];
        if (gBattleMons[battlerId].moves[i] == gLastMoves[battlerId] && check & MOVE_LIMITATION_TORMENTED &&
...
u8 DoFieldEndTurnEffects(void) {
    ...
    for (gBattlerAttacker = 0; gBattlerAttacker < gBattlersCount && gAbsentBattlerFlags & gBitTable[gBattlerAttacker];
         gBattlerAttacker++) {}
    for (gBattlerTarget = 0; gBattlerTarget < gBattlersCount && gAbsentBattlerFlags & gBitTable[gBattlerTarget];
         gBattlerTarget++) {}
            ...
            case ENDTURN_WISH:
                while (gBattleStruct->turnSideTracker < gBattlersCount) {
                    ...
                    if (gWishFutureKnock.wishCounter[gActiveBattler] != 0 &&
                        --gWishFutureKnock.wishCounter[gActiveBattler] == 0 && gBattleMons[gActiveBattler].hp != 0) {
            ...
            switch (gBattleStruct->turnEffectsTracker) {
                ...
                case ENDTURN_LEECH_SEED: // leech seed
                    if ((gStatuses3[gActiveBattler] & STATUS3_LEECHSEED) &&
                        ...
                        gBattlerTarget =
                            ...
                            STATUS3_LEECHSEED_BATTLER; // Notice gBattlerTarget is actually the HP receiver.
                ...
                case ENDTURN_ENCORE: // encore
                    if (gDisableStructs[gActiveBattler].encoreTimer != 0) {
                        if (gBattleMons[gActiveBattler].moves[gDisableStructs[gActiveBattler].encoredMovePos] !=
                                ...
                                .encoredMove) // pokemon does not have the encored move anymore
                        {
                            ...
                            gDisableStructs[gActiveBattler].encoreTimer = 0;
                                   ...
                                   gBattleMons[gActiveBattler].pp[gDisableStructs[gActiveBattler].encoredMovePos] ==
                                       0) {
                            gDisableStructs[gActiveBattler].encoredMove = 0;
                            gDisableStructs[gActiveBattler].encoreTimer = 0;
        ...
        case 0:
            while (gBattleStruct->wishPerishSongBattlerId < gBattlersCount) {
                ...
                if (gWishFutureKnock.futureSightCounter[gActiveBattler] != 0 &&
                    --gWishFutureKnock.futureSightCounter[gActiveBattler] == 0 && gBattleMons[gActiveBattler].hp != 0) {
        ...
        switch (gBattleStruct->faintedActionsState) {
            ...
            case 6:
                if (AbilityBattleEffects(ABILITYEFFECT_INTIMIDATE1, 0, 0, 0, 0) ||
                    AbilityBattleEffects(ABILITYEFFECT_TRACE, 0, 0, 0, 0) || ItemBattleEffects(1, 0, TRUE) ||
                ...
                if (gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION) {
                    ...
                    if (gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION) {
                        ...
                        } else // confusion dmg
                        {
                            ...
                            gBattleMoveDamage =
                                CalculateBaseDamage(&gBattleMons[gBattlerAttacker], &gBattleMons[gBattlerAttacker],
                                                    MOVE_POUND, 0, 40, 0, gBattlerAttacker, gBattlerAttacker);
            ...
            case CANCELLER_IN_LOVE: // infatuation
                if (gBattleMons[gBattlerAttacker].status2 & STATUS2_INFATUATION) {
                    gBattleScripting.battler =
                        CountTrailingZeroBits((gBattleMons[gBattlerAttacker].status2 & STATUS2_INFATUATION) >> 0x10);
                    if (Random() & 1) {
                        BattleScriptPushCursor();
                    } else {
...
u8 CastformDataTypeChange(u8 battler) {
    u8 formChange = 0;
    if (gBattleMons[battler].species != SPECIES_CASTFORM || gBattleMons[battler].ability != ABILITY_FORECAST ||
            ...
            if (gBattleMoveDamage) {
                switch (atkHoldEffect) {
                    ...
                    case HOLD_EFFECT_SHELL_BELL:
                        if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && gSpecialStatuses[gBattlerTarget].dmg != 0 &&
                            gSpecialStatuses[gBattlerTarget].dmg != 0xFFFF && gBattlerAttacker != gBattlerTarget &&
        ...
        case MOVE_TARGET_SELECTED:
            ...
            if (gSideTimers[side].followmeTimer && gBattleMons[gSideTimers[side].followmeTarget].hp)
                ...
                if (gBattleMoves[move].type == TYPE_ELECTRIC &&
                    AbilityBattleEffects(ABILITYEFFECT_COUNT_OTHER_SIDE, gBattlerAttacker, ABILITY_LIGHTNING_ROD, 0,
                                         0) &&
...
u8 IsMonDisobedient(void) {
    ...
    if (calc < obedienceLevel) {
        ...
        calc -= obedienceLevel;
        if (calc < obedienceLevel) {
            gBattleMoveDamage = CalculateBaseDamage(&gBattleMons[gBattlerAttacker], &gBattleMons[gBattlerAttacker],
                                                    MOVE_POUND, 0, 40, 0, gBattlerAttacker, gBattlerAttacker);
            gBattlerTarget = gBattlerAttacker;
