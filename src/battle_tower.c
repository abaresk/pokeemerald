...
// The challenge number at which an Apprentice can appear, depending on how many of their questions
// were answered
...
// Trainer IDs? Don't make sense as part of previous array, min/max relationship reversed and never
// accessed
        ...
        else
        {
            ...
            if (gSaveBlock2Ptr->frontier.curChallengeBattleNum + 1 < 7)
                gSaveBlock2Ptr->frontier
                    .trainerIds[gSaveBlock2Ptr->frontier.curChallengeBattleNum] =
                    gTrainerBattleOpponent_A;
        }
    }
}
...
u16 GetRandomScaledFrontierTrainerId(u8 challengeNum, u8 battleNum)
{
    ...
    if (challengeNum <= 7)
    {
        if (battleNum == 6)
        {
            // The last battle in each challenge has a jump in difficulty, pulls from a table with
            // higher ranges
    ...
    {
        // After challenge 7, trainer IDs always come from the last, hardest range, which is the
        // same for both trainer ID tables
...
// Unused
static void GetRandomScaledFrontierTrainerIdRange(
    u8 challengeNum, u8 battleNum, u16 *trainerIdPtr, u8 *rangePtr)
{
    u16 trainerId, range;
...
u8 GetFrontierTrainerFrontSpriteId(u16 trainerId)
{
    ...
    {
        if (gBattleTypeFlags & BATTLE_TYPE_RECORDED)
            return gFacilityClassToPicIndex[gApprentices[GetRecordedBattleApprenticeId()]
                                                .facilityClass];
        else
            return gFacilityClassToPicIndex
                [gApprentices[gSaveBlock2Ptr
...
u8 GetFrontierOpponentClass(u16 trainerId)
{
    ...
    else
    {
        ...
        {
            trainerClass =
                gFacilityClassToTrainerClass[gApprentices[GetRecordedBattleApprenticeId()]
                                                 .facilityClass];
        }
        else
        {
...
static void FillTrainerParty(u16 trainerId, u8 firstMonId, u8 monCount)
{
    ...
    else if (trainerId < TRAINER_RECORD_MIXING_APPRENTICE)
    {
        ...
        {
            if (gSaveBlock2Ptr->frontier.towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND]
                        .party[j]
                        .species != 0 &&
                gSaveBlock2Ptr->frontier.towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND]
                        .party[j]
                        .level <= level)
            {
                CreateBattleTowerMon2(&gEnemyParty[i],
                    &gSaveBlock2Ptr->frontier.towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND]
                         .party[j],
                    FALSE);
            }
    ...
    while (i != monCount)
    {
        ...
        {
            if (GetMonData(&gEnemyParty[j], MON_DATA_SPECIES, NULL) ==
        ...
        for (j = 0; j < MAX_MON_MOVES; j++)
        {
            ...
            if (gFacilityTrainerMons[monId].moves[j] == MOVE_FRUSTRATION)
                friendship =
                    0; // Frustration is more powerful the lower the pokemon's friendship is.
        }

        SetMonData(&gEnemyParty[i + firstMonId], MON_DATA_FRIENDSHIP, &friendship);
        SetMonData(&gEnemyParty[i + firstMonId],
            MON_DATA_HELD_ITEM,
            &gBattleFrontierHeldItems[gFacilityTrainerMons[monId].itemTableId]);
...
static void Unused_CreateApprenticeMons(u16 trainerId, u8 firstMonId)
{
    ...
    for (i = 0; i != 3; i++)
    {
        CreateMonWithEVSpread(
            &gEnemyParty[firstMonId + i], apprentice->party[i].species, level, fixedIV, 8);
        friendship = MAX_FRIENDSHIP;
        for (j = 0; j < MAX_MON_MOVES; j++)
        {
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        for (j = 0; j < MAX_MON_MOVES; j++)
            SetMonMoveAvoidReturn(
                &gEnemyParty[firstMonId + i], gFacilityTrainerMons[monId].moves[j], j);

        SetMonData(&gEnemyParty[firstMonId + i], MON_DATA_FRIENDSHIP, &friendship);
        SetMonData(&gEnemyParty[firstMonId + i],
            MON_DATA_HELD_ITEM,
            &gBattleFrontierHeldItems[gFacilityTrainerMons[monId].itemTableId]);
    }
}
...
void DoSpecialTrainerBattle(void)
{
    ...
    case SPECIAL_BATTLE_TOWER:
        ...
        case FRONTIER_MODE_LINK_MULTIS:
            gBattleTypeFlags |=
                BATTLE_TYPE_DOUBLE | BATTLE_TYPE_LINK | BATTLE_TYPE_MULTI | BATTLE_TYPE_x800000;
            FillFrontierTrainersParties(FRONTIER_MULTI_PARTY_SIZE);
            break;
        }
    ...
    case SPECIAL_BATTLE_EREADER:
        ...
        for (i = 0; i < 3; i++)
            CreateBattleTowerMon(
                &gEnemyParty[i], &gSaveBlock2Ptr->frontier.ereaderTrainer.party[i]);
    ...
    case SPECIAL_BATTLE_STEVEN:
        ...
        BattleSetup_ConfigureTrainerBattle(
            MossdeepCity_SpaceCenter_2F_EventScript_MaxieTrainer + 1);
        gApproachingTrainerId = 1;
        BattleSetup_ConfigureTrainerBattle(
            MossdeepCity_SpaceCenter_2F_EventScript_TabithaTrainer + 1);
...
static void sub_8163EE4(void)
{
    ...
    for (i = 0; i < MAX_FRONTIER_PARTY_SIZE; i++)
    {
        if (gSaveBlock2Ptr->frontier.selectedPartyMons[i] != 0)
            sub_80686FC(&gPlayerParty[gSaveBlock2Ptr->frontier.selectedPartyMons[i] - 1],
...
static void GetRecordMixFriendMultiPartnerParty(u16 trainerId)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (gSaveBlock2Ptr->frontier.towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND]
            ...
            gSaveBlock2Ptr->frontier.towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND]
                    .party[i]
                    .level <= GetFrontierEnemyMonLevel(lvlMode) &&
            gSaveBlock2Ptr->frontier.towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND]
                    .party[i]
                    .species != 0)
        {
...
static void LoadMultiPartnerCandidatesData(void)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        for (j = 0; j < 2; j++)
        {
            while (1)
            {
                ...
                if (j % 2 != 0 && gFacilityTrainerMons[gSaveBlock2Ptr->frontier.trainerIds[r10 - 1]]
                                          .itemTableId == gFacilityTrainerMons[monId].itemTableId)
                    continue;
    ...
    {
        if (gSaveBlock2Ptr->apprentices[i].lvlMode != 0 &&
            sApprenticeChallengeThreshold[gSaveBlock2Ptr->apprentices[i].numQuestions] / 7 <=
                challengeNum &&
            gSaveBlock2Ptr->apprentices[i].lvlMode - 1 == lvlMode)
        {
            k = 0;
    ...
    if (r10 != 0)
    {
        ...
        objEventTemplates[7].graphicsId =
    ...
    if (r10 != 0)
    {
        ...
        objEventTemplates[8].graphicsId =
            ...
            for (i = 0; i < PLAYER_NAME_LENGTH; i++)
                gStringVar3[i] =
                    gSaveBlock2Ptr->apprentices[trainerId - TRAINER_RECORD_MIXING_APPRENTICE]
                        .playerName[i];
            gStringVar3[i] = EOS;
            ConvertInternationalString(gStringVar3,
                gSaveBlock2Ptr->apprentices[trainerId - TRAINER_RECORD_MIXING_APPRENTICE].language);
        }
    }
...
static void ShowPartnerCandidateMessage(void)
{
    ...
    for (partnerId = 0; partnerId < ARRAY_COUNT(sPartnerTrainerTextTables); partnerId++)
    {
        if (sPartnerTrainerTextTables[partnerId].facilityClass ==
            GetFrontierTrainerFacilityClass(trainerId))
            break;
    }
            ...
            for (i = 0; i < PLAYER_NAME_LENGTH; i++)
                gStringVar1[i] =
                    gSaveBlock2Ptr->apprentices[trainerId - TRAINER_RECORD_MIXING_APPRENTICE]
                        .playerName[i];
            gStringVar1[i] = EOS;
            ConvertInternationalString(gStringVar1,
                gSaveBlock2Ptr->apprentices[trainerId - TRAINER_RECORD_MIXING_APPRENTICE].language);
            ConvertIntToDecimalStringN(gStringVar2,
                gSaveBlock2Ptr->apprentices[trainerId - TRAINER_RECORD_MIXING_APPRENTICE].number,
...
static void LoadLinkMultiOpponentsData(void)
{
    ...
    case 3:
        ...
        {
            ResetBlockReceivedFlags();
            memcpy(&gSaveBlock2Ptr->frontier.trainerIds,
                gBlockRecvBuffer,
                sizeof(gSaveBlock2Ptr->frontier.trainerIds));
...
static void SetTowerInterviewData(void)
{
    ...
    GetBattleTowerTrainerLanguage(
        &gSaveBlock2Ptr->frontier.towerInterview.opponentLanguage, gTrainerBattleOpponent_A);
    ...
    if (GetCurrentBattleTowerWinStreak(lvlMode, battleMode) > 55)
    {
        for (i = 0; i < monCount; i++)
        {
            ...
            if (!GetMonData(&gSaveBlock1Ptr->playerParty[partyIndex], ribbonType))
            {
                ...
                SetMonData(
                    &gSaveBlock1Ptr->playerParty[partyIndex], ribbonType, &gSpecialVar_Result);
    ...
    if (gSpecialVar_Result)
    {
        ...
        if (ribbons[0].count > NUM_CUTIES_RIBBONS)
        {
            TryPutSpotTheCutiesOnAir(
                &gSaveBlock1Ptr->playerParty[ribbons[0].partyIndex], ribbonType);
        }
    }
}
...
static void FillEReaderTrainerWithPlayerData(void)
{
    ...
    if (gSaveBlock2Ptr->playerGender != MALE)
    {
        ereaderTrainer->facilityClass = gTowerFemaleFacilityClasses
            [(gSaveBlock2Ptr->playerTrainerId[0] + gSaveBlock2Ptr->playerTrainerId[1] +
                 gSaveBlock2Ptr->playerTrainerId[2] + gSaveBlock2Ptr->playerTrainerId[3]) %
                ARRAY_COUNT(gTowerFemaleFacilityClasses)];
    }
    else
    {
        ereaderTrainer->facilityClass = gTowerMaleFacilityClasses
            [(gSaveBlock2Ptr->playerTrainerId[0] + gSaveBlock2Ptr->playerTrainerId[1] +
                 gSaveBlock2Ptr->playerTrainerId[2] + gSaveBlock2Ptr->playerTrainerId[3]) %
                ARRAY_COUNT(gTowerMaleFacilityClasses)];
    }
...
static void FillPartnerParty(u16 trainerId)
{
        ...
        for (i = 0; i < MULTI_PARTY_SIZE; i++)
        {
            do
            {
                j = Random32();
                     ...
                     sStevenMons[i].nature != GetNatureFromPersonality(j));
            CreateMon(&gPlayerParty[MULTI_PARTY_SIZE + i],
                sStevenMons[i].species,
                sStevenMons[i].level,
                sStevenMons[i].fixedIV,
                TRUE,
                i, // BUG: personality was stored in the 'j' variable. As a result, Steven's pokemon
                   // do not have the intended natures.
                OT_ID_PRESET,
                STEVEN_OTID);
            for (j = 0; j < PARTY_SIZE; j++)
                SetMonData(&gPlayerParty[MULTI_PARTY_SIZE + i],
                    MON_DATA_HP_EV + j,
                    &sStevenMons[i].evs[j]);
            for (j = 0; j < MAX_MON_MOVES; j++)
            ...
            SetMonData(&gPlayerParty[MULTI_PARTY_SIZE + i],
                MON_DATA_OT_NAME,
                gTrainers[TRAINER_STEVEN].trainerName);
            j = MALE;
    ...
    else if (trainerId < FRONTIER_TRAINERS_COUNT)
    {
        ...
        for (i = 0; i < FRONTIER_MULTI_PARTY_SIZE; i++)
        {
            ...
            {
                SetMonMoveSlot(
                    &gPlayerParty[MULTI_PARTY_SIZE + i], gFacilityTrainerMons[monId].moves[j], j);
                if (gFacilityTrainerMons[monId].moves[j] == MOVE_FRUSTRATION)
                    friendship = 0;
            }
    ...
    else if (trainerId < TRAINER_RECORD_MIXING_APPRENTICE)
    {
        ...
        {
            struct EmeraldBattleTowerRecord *record =
                &gSaveBlock2Ptr->frontier.towerRecords[trainerId];
            struct BattleTowerPokemon monData =
    ...
    else
    {
        ...
        for (i = 0; i < FRONTIER_MULTI_PARTY_SIZE; i++)
        {
            CreateApprenticeMon(&gPlayerParty[MULTI_PARTY_SIZE + i],
                &gSaveBlock2Ptr->apprentices[trainerId],
    ...
    struct RSBattleTowerRecord *src, struct EmeraldBattleTowerRecord *dst)
{
    ...
    else
    {
        ...
        // BUG: Reading outside the array. sRubyFacilityClassToEmerald has less than
        // FACILITY_CLASSES_COUNT entries.
...
void GetBattleTowerTrainerLanguage(u8 *dst, u16 trainerId)
{
    ...
    else if (trainerId < TRAINER_RECORD_MIXING_APPRENTICE)
    {
        if (gBattleTypeFlags & BATTLE_TYPE_RECORDED)
            ...
            *dst = gSaveBlock2Ptr->frontier.towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND]
                       .language;
    }
    else
    {
...
// Frontier Trainer parties are roughly scaled in difficulty with higher trainer IDs, so scale IVs
// as well Duplicated in Battle Dome as GetDomeTrainerMonIvs
static u8 GetFrontierTrainerFixedIvs(u16 trainerId)
{
    u8 fixedIv;
