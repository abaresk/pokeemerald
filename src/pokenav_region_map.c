...
static const struct CompressedSpriteSheet sCityZoomTextSpriteSheet[1] = { { gRegionMapCityZoomText_Gfx, 0x800, 6 } };

static const struct SpritePalette sCityZoomTilesSpritePalette[] = { { gRegionMapCityZoomTiles_Pal, 11 }, {} };

static const struct WindowTemplate sMapSecInfoWindowTemplate = {
    .bg = 1, .tilemapLeft = 17, .tilemapTop = 4, .width = 12, .height = 13, .paletteNum = 1, .baseBlock = 0x4C
};
...
static void DrawCityMap(struct Pokenav5Struct_2* state, int mapSecId, int pos) {
    int i;
    for (i = 0; i < NUM_CITY_MAPS && (sPokenavCityMaps[i].mapSecId != mapSecId || sPokenavCityMaps[i].index != pos);
         i++)
        ;
