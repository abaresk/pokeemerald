...
static void LoadCableCarSprites(void) {
    ...
    u8 playerGraphicsIds[2] = { OBJ_EVENT_GFX_RIVAL_BRENDAN_NORMAL, OBJ_EVENT_GFX_RIVAL_MAY_NORMAL };
    u16 rval = Random();
    u8 hikerGraphicsIds[4] = { OBJ_EVENT_GFX_HIKER, OBJ_EVENT_GFX_CAMPER, OBJ_EVENT_GFX_PICNICKER,
    ...
    if ((rval % 64) == 0) {
        spriteId = AddPseudoObjectEvent(hikerGraphicsIds[rval % 3], callbacks[gSpecialVar_0x8004],
                                        hikerCoords[gSpecialVar_0x8004][0], hikerCoords[gSpecialVar_0x8004][1], 0x6a);
        if (spriteId != MAX_SPRITES) {
            gSprites[spriteId].oam.priority = 2;
...
static void sub_8151214(void) {
    ...
    for (i = 0; i < 9; i++) {
        ...
        FillBgTilemapBufferRect(0, sCableCar->unk22[i][sCableCar->unk18], gUnknown_0203ABB0, gUnknown_0203ABB1, 1, 1,
                                17);
        gUnknown_0203ABB0 = (gUnknown_0203ABB0 + 1) % 32;
        FillBgTilemapBufferRect(0, sCableCar->unk22[i][sCableCar->unk18 + 1], gUnknown_0203ABB0, gUnknown_0203ABB1, 1,
                                1, 17);
    }
...
static void sub_8151388(void) {
    ...
    for (i = 0; i < 9; i++) {
        ...
        FillBgTilemapBufferRect(0, sCableCar->unk22[i][sCableCar->unk18], gUnknown_0203ABB3, gUnknown_0203ABB4, 1, 1,
                                17);
        gUnknown_0203ABB3 = (gUnknown_0203ABB3 + 1) % 32;
        FillBgTilemapBufferRect(0, sCableCar->unk22[i][sCableCar->unk18 + 1], gUnknown_0203ABB3, gUnknown_0203ABB4, 1,
                                1, 17);
    }
