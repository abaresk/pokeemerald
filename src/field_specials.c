...
static void DetermineCyclingRoadResults(u32 numFrames, u8 numBikeCollisions)
{
    ...
    if (numFrames < 3600)
    {
        ...
        ConvertIntToDecimalStringN(
            &gStringVar2[3], ((numFrames % 60) * 100) / 60, STR_CONV_MODE_LEADING_ZEROS, 2);
...
{
    if (!FlagGet(FLAG_SYS_CRUISE_MODE)
        || (*GetVarPointer(VAR_CRUISE_STEP_COUNT) += delta) < SS_TIDAL_MAX_STEPS)
    {
        return FALSE;
    }
    return TRUE;
...
void SpawnLinkPartnerObjectEvent(void)
{
    ...
    s16 x = 0;
    s16 y = 0;
    u8 movementTypes[] = { MOVEMENT_TYPE_FACE_UP,
    ...
    for (i = 0; i < gSpecialVar_0x8004; i++)
    {
        if (myLinkPlayerNumber != i)
        {
            ...
            SpawnSpecialObjectEventParameterized(linkSpriteId,
                movementTypes[j],
                240 - i,
                coordOffsets[j][0] + x + 7,
                coordOffsets[j][1] + y + 7,
                0);
...
static void LoadLinkPartnerObjectEventSpritePalette(u8 graphicsId, u8 localEventId, u8 paletteNum)
{
    ...
    {
        u8 obj = GetObjectEventIdByLocalIdAndMap(
            localEventId, gSaveBlock1Ptr->location.mapNum, gSaveBlock1Ptr->location.mapGroup);
...
void MauvilleGymPressSwitch(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sMauvilleGymSwitchCoords); i++)
    {
        if (i == gSpecialVar_0x8004)
            MapGridSetMetatileIdAt(sMauvilleGymSwitchCoords[i].x,
                sMauvilleGymSwitchCoords[i].y,
            ...
            switch (MapGridGetMetatileIdAt(x, y))
            {
                ...
                case METATILE_MauvilleGym_GreenBeamH3_Off:
                    MapGridSetMetatileIdAt(
                        x, y, METATILE_MauvilleGym_GreenBeamH3_On | METATILE_COLLISION_MASK);
                    break;
                case METATILE_MauvilleGym_GreenBeamH4_Off:
                    MapGridSetMetatileIdAt(
                        x, y, METATILE_MauvilleGym_GreenBeamH4_On | METATILE_COLLISION_MASK);
                    break;
                ...
                case METATILE_MauvilleGym_RedBeamH3_Off:
                    MapGridSetMetatileIdAt(
                        x, y, METATILE_MauvilleGym_RedBeamH3_On | METATILE_COLLISION_MASK);
                    break;
                case METATILE_MauvilleGym_RedBeamH4_Off:
                    MapGridSetMetatileIdAt(
                        x, y, METATILE_MauvilleGym_RedBeamH4_On | METATILE_COLLISION_MASK);
                    break;
                case METATILE_MauvilleGym_GreenBeamV1_On:
                    MapGridSetMetatileIdAt(
                        x, y, METATILE_MauvilleGym_PoleBottom_On | METATILE_COLLISION_MASK);
                    break;
                ...
                case METATILE_MauvilleGym_RedBeamV1_On:
                    MapGridSetMetatileIdAt(
                        x, y, METATILE_MauvilleGym_PoleBottom_Off | METATILE_COLLISION_MASK);
                    break;
                ...
                case METATILE_MauvilleGym_PoleBottom_On:
                    MapGridSetMetatileIdAt(
                        x, y, METATILE_MauvilleGym_GreenBeamV1_On | METATILE_COLLISION_MASK);
                    break;
                case METATILE_MauvilleGym_FloorTile:
                    if (MapGridGetMetatileIdAt(x, y - 1) == METATILE_MauvilleGym_GreenBeamV1_On)
                        MapGridSetMetatileIdAt(
                            x, y, METATILE_MauvilleGym_GreenBeamV2_On | METATILE_COLLISION_MASK);
                    else
                        MapGridSetMetatileIdAt(
                            x, y, METATILE_MauvilleGym_RedBeamV2_On | METATILE_COLLISION_MASK);
                    break;
                case METATILE_MauvilleGym_PoleBottom_Off:
                    MapGridSetMetatileIdAt(
                        x, y, METATILE_MauvilleGym_RedBeamV1_On | METATILE_COLLISION_MASK);
                    break;
                case METATILE_MauvilleGym_PoleTop_Off:
                    MapGridSetMetatileIdAt(
                        x, y, METATILE_MauvilleGym_PoleTop_On | METATILE_COLLISION_MASK);
                    break;
...
void MauvilleGymDeactivatePuzzle(void)
{
    ...
    {
        MapGridSetMetatileIdAt(
            switchCoords->x, switchCoords->y, METATILE_MauvilleGym_PressedSwitch);
        switchCoords++;
    }
    for (y = 12; y < 24; y++)
    {
...
static void Task_PetalburgGymSlideOpenRoomDoors(u8 taskId)
{
    if (sSlidingDoorNextFrameDelay[sSlidingDoorFrame] == sSlidingDoorNextFrameCounter)
    {
        PetalburgGymSetDoorMetatiles(
            gSpecialVar_0x8004, sPetalburgGymSlidingDoorMetatiles[sSlidingDoorFrame]);
        sSlidingDoorNextFrameCounter = 0;
...
static void PetalburgGymSetDoorMetatiles(u8 roomNumber, u16 metatileId)
{
    ...
    {
        MapGridSetMetatileIdAt(
            doorCoordsX[i] + 7, doorCoordsY[i] + 7, metatileId | METATILE_COLLISION_MASK);
        MapGridSetMetatileIdAt(
            doorCoordsX[i] + 7, doorCoordsY[i] + 8, (metatileId + 8) | METATILE_COLLISION_MASK);
    }
    DrawWholeMapView();
}
...
{
    return (u16)((gSaveBlock2Ptr->playerTrainerId[1] << 8) | gSaveBlock2Ptr->playerTrainerId[0])
           % 10;
}
...
void CableCarWarp(void)
{
    ...
    {
        SetWarpDestination(
            MAP_GROUP(ROUTE112_CABLE_CAR_STATION), MAP_NUM(ROUTE112_CABLE_CAR_STATION), -1, 6, 4);
    }
    else
    {
        SetWarpDestination(MAP_GROUP(MT_CHIMNEY_CABLE_CAR_STATION),
            MAP_NUM(MT_CHIMNEY_CABLE_CAR_STATION),
            -1,
            6,
            4);
    }
...
static void PCTurnOnEffect_1(s16 isPcTurnedOn, s8 dx, s8 dy)
{
    ...
    MapGridSetMetatileIdAt(gSaveBlock1Ptr->pos.x + dx + 7,
        gSaveBlock1Ptr->pos.y + dy + 7,
...
void RemoveCameraObject(void)
{
    ...
    RemoveObjectEventByLocalIdAndMap(
        OBJ_EVENT_ID_CAMERA, gSaveBlock1Ptr->location.mapNum, gSaveBlock1Ptr->location.mapGroup);
}
...
void BufferLottoTicketNumber(void)
{
    ...
    else if (gSpecialVar_Result >= 1000)
    {
        ...
        ConvertIntToDecimalStringN(gStringVar1 + 1,
            gSpecialVar_Result,
    ...
    else if (gSpecialVar_Result >= 100)
    {
        ...
        ConvertIntToDecimalStringN(gStringVar1 + 2,
            gSpecialVar_Result,
            STR_CONV_MODE_LEFT_ALIGN,
            CountDigits(gSpecialVar_Result));
    }
    else if (gSpecialVar_Result >= 10)
    {
        ...
        ConvertIntToDecimalStringN(gStringVar1 + 3,
            gSpecialVar_Result,
            STR_CONV_MODE_LEFT_ALIGN,
            CountDigits(gSpecialVar_Result));
    }
    else
    {
        ...
        ConvertIntToDecimalStringN(gStringVar1 + 4,
            gSpecialVar_Result,
            STR_CONV_MODE_LEFT_ALIGN,
            CountDigits(gSpecialVar_Result));
    }
...
bool8 InMultiPartnerRoom(void)
{
    if (gSaveBlock1Ptr->location.mapGroup
            == MAP_GROUP(BATTLE_FRONTIER_BATTLE_TOWER_MULTI_PARTNER_ROOM)
        && gSaveBlock1Ptr->location.mapNum
               == MAP_NUM(BATTLE_FRONTIER_BATTLE_TOWER_MULTI_PARTNER_ROOM)
...
void ShowDeptStoreElevatorFloorSelect(void)
{
    ...
    AddTextPrinterParameterized(
        sTutorMoveAndElevatorWindowId, 1, gText_ElevatorNowOn, xPos, 1, TEXT_SPEED_FF, NULL);

    xPos = GetStringCenterAlignXOffset(1, gDeptStoreFloorNames[gSpecialVar_0x8005], 64);
    AddTextPrinterParameterized(sTutorMoveAndElevatorWindowId,
        1,
        gDeptStoreFloorNames[gSpecialVar_0x8005],
        xPos,
        17,
        TEXT_SPEED_FF,
        NULL);
...
void ShowFrontierManiacMessage(void)
{
    ...
    static const u8 sFrontierManiacStreakThresholds[][FRONTIER_MANIAC_MESSAGE_COUNT - 1] = {
        ...
        [FRONTIER_MANIAC_BATTLE_PIKE] = { 13, 112 }, // BUG: 112 (0x70) is probably a mistake; the
                                                     // Pike Queen is battled twice well before that
    ...
    switch (facility)
    {
        ...
        case FRONTIER_MANIAC_BATTLE_DOME:
            if (gSaveBlock2Ptr->frontier.domeWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_50]
                >= gSaveBlock2Ptr->frontier
                       .domeWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_OPEN])
            {
                winStreak =
                    gSaveBlock2Ptr->frontier.domeWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_50];
            }
            else
            {
                winStreak = gSaveBlock2Ptr->frontier
                                .domeWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_OPEN];
            }
            break;
        case FRONTIER_MANIAC_BATTLE_FACTORY:
            if (gSaveBlock2Ptr->frontier.factoryWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_50]
                >= gSaveBlock2Ptr->frontier
                       .factoryWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_OPEN])
            {
                winStreak = gSaveBlock2Ptr->frontier
                                .factoryWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_50];
            }
            else
            {
                winStreak = gSaveBlock2Ptr->frontier
        ...
        case FRONTIER_MANIAC_BATTLE_PALACE:
            if (gSaveBlock2Ptr->frontier.palaceWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_50]
                >= gSaveBlock2Ptr->frontier
                       .palaceWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_OPEN])
            {
                winStreak = gSaveBlock2Ptr->frontier
                                .palaceWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_50];
            }
            else
            {
                winStreak = gSaveBlock2Ptr->frontier
    ...
    for (i = 0; i < FRONTIER_MANIAC_MESSAGE_COUNT - 1
                && sFrontierManiacStreakThresholds[facility][i] < winStreak;
         i++)
        ;
...
static const u8 *const sScrollableMultichoiceOptions[][MAX_SCROLL_MULTI_LENGTH] = {
    ...
    [SCROLL_MULTI_GLASS_WORKSHOP_VENDOR] = { gText_BlueFlute,
        gText_YellowFlute,
        gText_RedFlute,
        gText_WhiteFlute,
        gText_BlackFlute,
    ...
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_1] = { gText_KissPoster16BP,
        ...
        gText_Exit },
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_2] = { gText_LaprasDoll128BP,
        gText_SnorlaxDoll128BP,
        gText_VenusaurDoll256BP,
        gText_CharizardDoll256BP,
        gText_BlastoiseDoll256BP,
        gText_Exit },
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_VITAMIN_VENDOR] = { gText_Protein1BP,
        gText_Calcium1BP,
        gText_Iron1BP,
        gText_Zinc1BP,
        gText_Carbos1BP,
    ...
    [SCROLL_MULTI_BF_MOVE_TUTOR_2] = { gText_DefenseCurl16BP,
        ...
        gText_FirePunch48BP,
        gText_Exit },
    [SCROLL_MULTI_SS_TIDAL_DESTINATION] = { gText_SlateportCity,
        ...
        gText_FarawayIsland,
        gText_Exit },
    [SCROLL_MULTI_BATTLE_TENT_RULES] = { gText_BattleTrainers,
        gText_BattleBasics,
        gText_PokemonNature,
        gText_PokemonMoves,
        gText_Underpowered,
...
static void Task_ShowScrollableMultichoice(u8 taskId)
{
    ...
    template =
        CreateWindowTemplate(0, task->tLeft, task->tTop, task->tWidth, task->tHeight, 0xF, 0x64);
    ...
    task->tListTaskId = ListMenuInit(
        &gScrollableMultichoice_ListMenuTemplate, task->tScrollOffset, task->tSelectedRow);
...
static void ScrollableMultichoice_UpdateScrollArrows(u8 taskId)
{
    ...
    if (task->tMaxItemsOnScreen != task->data[1])
    {
        ...
        task->tScrollArrowId =
            AddScrollIndicatorArrowPair(&template, &sScrollableMultichoice_ScrollOffset);
    }
}
...
{
    static const u16 sFrontierChallenges[] = { FRONTIER_CHALLENGE(
                                                   FRONTIER_FACILITY_TOWER, FRONTIER_MODE_SINGLES),
...
void UpdateBattlePointsWindow(void)
{
    ...
    StringCopy(ConvertIntToDecimalStringN(
                   string, gSaveBlock2Ptr->frontier.battlePoints, STR_CONV_MODE_RIGHT_ALIGN, 4),
        gText_BP);
        ...
        switch (menu)
        {
            case SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_1:
                AddTextPrinterParameterized2(
                    0, 1, sFrontierExchangeCorner_Decor1Descriptions[selection], 0, NULL, 2, 1, 3);
                ...
                else
                {
                    ...
                    sScrollableMultichoice_ItemSpriteId = AddDecorationIconObject(
                        sFrontierExchangeCorner_Decor1[selection], 33, 88, 0, 5500, 5500);
                }
                break;
            case SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_2:
                AddTextPrinterParameterized2(
                    0, 1, sFrontierExchangeCorner_Decor2Descriptions[selection], 0, NULL, 2, 1, 3);
                ...
                else
                {
                    ...
                    FreeSpritePaletteByTag(5500);
                    sScrollableMultichoice_ItemSpriteId = AddDecorationIconObject(
                        sFrontierExchangeCorner_Decor2[selection], 33, 88, 0, 5500, 5500);
                }
                break;
            case SCROLL_MULTI_BF_EXCHANGE_CORNER_VITAMIN_VENDOR:
                AddTextPrinterParameterized2(0,
                    1,
                    sFrontierExchangeCorner_VitaminsDescriptions[selection],
                    0,
                    NULL,
                    2,
                    1,
            ...
            case SCROLL_MULTI_BF_EXCHANGE_CORNER_HOLD_ITEM_VENDOR:
                AddTextPrinterParameterized2(0,
                    1,
                    sFrontierExchangeCorner_HoldItemsDescriptions[selection],
                    0,
                    NULL,
                    2,
                    1,
...
static const u16 sBattleFrontier_TutorMoves1[] = { MOVE_SOFT_BOILED,
    MOVE_SEISMIC_TOSS,
    MOVE_DREAM_EATER,
    MOVE_MEGA_PUNCH,
    MOVE_MEGA_KICK,
...
static const u16 sBattleFrontier_TutorMoves2[] = { MOVE_DEFENSE_CURL,
    MOVE_SNORE,
    MOVE_MUD_SLAP,
    MOVE_SWIFT,
    MOVE_ICY_WIND,
...
static void ShowBattleFrontierTutorMoveDescription(u8 menu, u16 selection)
{
    ...
    if (menu == SCROLL_MULTI_BF_MOVE_TUTOR_1 || menu == SCROLL_MULTI_BF_MOVE_TUTOR_2)
    {
        ...
        if (menu == SCROLL_MULTI_BF_MOVE_TUTOR_2)
        {
            AddTextPrinterParameterized(sTutorMoveAndElevatorWindowId,
                1,
                sBattleFrontier_TutorMoveDescriptions2[selection],
                0,
                1,
                0,
                NULL);
        ...
        else
        {
            AddTextPrinterParameterized(sTutorMoveAndElevatorWindowId,
                1,
                sBattleFrontier_TutorMoveDescriptions1[selection],
                0,
                1,
                0,
                NULL);
...
void sub_813ADD4(void)
{
    ...
    if (taskId != 0xFF)
    {
        ...
        {
            AddTextPrinterParameterized5(task->tWindowId,
                1,
                sScrollableMultichoiceOptions[gSpecialVar_0x8004][scrollOffset + i],
                10,
                i * 16,
                TEXT_SPEED_FF,
                NULL,
        ...
        AddTextPrinterParameterized(
            task->tWindowId, 1, gText_SelectorArrow, 0, selectedRow * 16, TEXT_SPEED_FF, NULL);
...
static void ChangeDeoxysRockLevel(u8 rockLevel)
{
    ...
    TryGetObjectEventIdByLocalIdAndMap(
        1, gSaveBlock1Ptr->location.mapNum, gSaveBlock1Ptr->location.mapGroup, &objectEventId);
    ...
    Overworld_SetObjEventTemplateCoords(
        1, sDeoxysRockCoords[rockLevel][0], sDeoxysRockCoords[rockLevel][1]);
}
...
void CreateAbnormalWeatherEvent(void)
{
    ...
    if (FlagGet(FLAG_DEFEATED_KYOGRE) == TRUE)
    {
        VarSet(VAR_ABNORMAL_WEATHER_LOCATION,
            (randomValue % TERRA_CAVE_LOCATIONS) + TERRA_CAVE_LOCATIONS_START);
    }
    else if (FlagGet(FLAG_DEFEATED_GROUDON) == TRUE)
    {
        VarSet(VAR_ABNORMAL_WEATHER_LOCATION,
            (randomValue % MARINE_CAVE_LOCATIONS) + MARINE_CAVE_LOCATIONS_START);
    }
...
static void Task_CloseBattlePikeCurtain(u8 taskId)
{
    ...
    if (data[data[3]] == 0)
    {
        for (y = 0; y < 4; y++)
        {
            for (x = 0; x < 3; x++)
            {
                MapGridSetMetatileIdAt(gSaveBlock1Ptr->pos.x + x + 6,
                    gSaveBlock1Ptr->pos.y + y + 4,
                    x + 513 + y * 8 + data[3] * 32);
            }
        }
        DrawWholeMapView();
...
/*  Summary of the Lilycove Trainer Fan Club, because it's a little messy
    ...
    There are initially 4 members of the Fan Club (+ an interviewer), none of whom are fans of the
   player After becoming the champion there will be 8 members of the Fan Club, 3 of whom are
   automatically fans of the player After this point, if a club member is a fan of the player they
   will sit at the front table and comment on the player If they are not fans of the player, they
   will sit at the far table and can make comments about a different trainer (see
   BufferFanClubTrainerName)
    ...
    If the player has fewer than 3 fans, they may also gain fans by completing certain tasks enough
   times (see TryGainNewFanFromCounter) If the player has at least 5 fans, they can lose a fan every
   12 real-time hours, or more often if the timer variable is reset (see TryLoseFansFromPlayTime) If
   the player has only 1 fan left it cannot be lost
    ...
    VAR_FANCLUB_FAN_COUNTER, a bitfield for tracking the fans
      ...
      Bits 9-16: Flags for each of the 8 club members, set to 1 when theyre a fan of the player and
   0 when theyre not
...
#define SET_TRAINER_FAN_CLUB_COUNTER(count) \
    (FANCLUB_BITFIELD = (FANCLUB_BITFIELD & FANCLUB_FAN_FLAGS) | (count))
...
void TryLoseFansFromPlayTimeAfterLinkBattle(void)
{
    if (DidPlayerGetFirstFans())
    {
        ...
        gSaveBlock1Ptr->vars[VAR_FANCLUB_LOSE_FAN_TIMER - VARS_START] =
...
// Loop through the fan club members, and if theyre not a fan of the player there is a 50% chance
// for them to become a fan Stops when a fan is gained If no new fan was gained while looping, the
// last non-fan in the list becomes a fan If all the members are already fans of the player then
// this redundantly sets the first fan in the list to be a fan
static u16 PlayerGainRandomTrainerFan(void)
{
...
// Loops through the fan club members, and if theyre a fan of the player there is a 50% chance for
// them to stop being a fan Stops if a fan is removed, or if the player has only one fan left If no
// fan was lost while looping, the last current fan in the list will stop being a fan
...
// If the player has > 5 fans in the Trainer Fan Club, then lose 1 fan for every 12 hours since the
// last fan loss / timer reset
void TryLoseFansFromPlayTime(void)
{
    u8 i = 0;
    if (gSaveBlock2Ptr->playTimeHours < 999)
    {
        while (TRUE)
        {
            ...
            else if (gSaveBlock2Ptr->playTimeHours
                         - gSaveBlock1Ptr->vars[VAR_FANCLUB_LOSE_FAN_TIMER - VARS_START]
                     < 12)
            {
                return;
...
void BufferFanClubTrainerName(void)
{
    ...
    BufferFanClubTrainerName_(
        &gSaveBlock1Ptr->linkBattleRecords, whichLinkTrainer, whichNPCTrainer);
}

static void BufferFanClubTrainerName_(
    struct LinkBattleRecords *linkRecords, u8 whichLinkTrainer, u8 whichNPCTrainer)
{
