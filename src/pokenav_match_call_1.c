...
static const u8 sMatchCallOptionsNoCheckPage[] = { MATCH_CALL_OPTION_CALL, MATCH_CALL_OPTION_CANCEL };

static const u8 sMatchCallOptionsHasCheckPage[] = { MATCH_CALL_OPTION_CALL, MATCH_CALL_OPTION_CHECK,
...
static bool32 sub_81CB1D0(void) {
    ...
    if (!state->matchCallEntries[selection].isSpecialTrainer) {
        ...
        if (state->matchCallEntries[selection].headerId == MC_HEADER_WATTSON) {
            if (GetMatchCallMapSec(selection) == gMapHeader.regionMapSectionId && FlagGet(FLAG_BADGE05_GET) == TRUE) {
