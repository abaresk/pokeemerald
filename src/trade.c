    ...
    switch (gMain.state) {
        ...
        case 7:
            ...
            for (i = 0; i < sTradeMenuData->partyCounts[TRADE_PLAYER]; i++) {
                ...
                sTradeMenuData->partySpriteIds[TRADE_PLAYER][i] = CreateMonIcon(
                    GetMonData(mon, MON_DATA_SPECIES2), SpriteCB_MonIcon, (sTradeMonSpriteCoords[i][0] * 8) + 14,
                    (sTradeMonSpriteCoords[i][1] * 8) - 12, 1, GetMonData(mon, MON_DATA_PERSONALITY), TRUE);
            }

            for (i = 0; i < sTradeMenuData->partyCounts[TRADE_PARTNER]; i++) {
                struct Pokemon* mon = &gEnemyParty[i];
                sTradeMenuData->partySpriteIds[TRADE_PARTNER][i] =
                    CreateMonIcon(GetMonData(mon, MON_DATA_SPECIES2, NULL), SpriteCB_MonIcon,
        ...
        case 8:
            ...
            break;
        case 9:
            DrawHeldItemIconsForTrade(&sTradeMenuData->partyCounts[0], sTradeMenuData->partySpriteIds[0],
                                      TRADE_PARTNER);
            gMain.state++;
            break;
        case 10:
        ...
        case 13:
            ...
            sTradeMenuData->cursorSpriteIdx = CreateSprite(
                &gSpriteTemplate_832DC94, sTradeMonSpriteCoords[0][0] * 8 + 32, sTradeMonSpriteCoords[0][1] * 8, 2);
    ...
    switch (gMain.state) {
        ...
        case 7:
            ...
            for (i = 0; i < sTradeMenuData->partyCounts[TRADE_PLAYER]; i++) {
                struct Pokemon* mon = &gPlayerParty[i];
                sTradeMenuData->partySpriteIds[TRADE_PLAYER][i] = CreateMonIcon(
                    GetMonData(mon, MON_DATA_SPECIES2, NULL), SpriteCB_MonIcon, (sTradeMonSpriteCoords[i][0] * 8) + 14,
                    (sTradeMonSpriteCoords[i][1] * 8) - 12, 1, GetMonData(mon, MON_DATA_PERSONALITY), TRUE);
            }
        ...
        case 13:
            ...
            sTradeMenuData->cursorSpriteIdx = CreateSprite(
                &gSpriteTemplate_832DC94, sTradeMonSpriteCoords[sTradeMenuData->cursorPosition][0] * 8 + 32,
...
static bool8 BufferTradeParties(void) {
    ...
    switch (sTradeMenuData->bufferPartyState) {
        ...
        case 20:
            if (_GetBlockReceivedStatus() == 3) {
                Trade_Memcpy(sTradeMenuData->giftRibbons, gBlockRecvBuffer[id ^ 1],
...
static void PrintAndBufferIsThisTradeOkay(void) {
    DrawTextWindowAndBuffer6Bytes(sText_IsThisTradeOkay,
                                  (void*)(OBJ_VRAM0 + (sTradeMenuData->bottomTextTileStart * 32)), 24);
}
...
static void TradeMenuShowMonSummaryScreen(void) {
    if (!gPaletteFade.active) {
        // Player's party
        if (sTradeMenuData->cursorPosition < PARTY_SIZE)
            ShowPokemonSummaryScreen(PSS_MODE_LOCK_MOVES, gPlayerParty, sTradeMenuData->cursorPosition,
                                     sTradeMenuData->partyCounts[TRADE_PLAYER] - 1, CB2_ReturnToTradeMenu);
        // Partner's party
        else
            ShowPokemonSummaryScreen(PSS_MODE_LOCK_MOVES, gEnemyParty, sTradeMenuData->cursorPosition - PARTY_SIZE,
                                     sTradeMenuData->partyCounts[TRADE_PARTNER] - 1, CB2_ReturnToTradeMenu);
        FreeAllWindowBuffers();
    }
}
...
static bool32 CheckMonsBeforeTrade(void) {
    ...
    switch (CheckValidityOfTradeMons(aliveMons, sTradeMenuData->partyCounts[TRADE_PLAYER],
                                     sTradeMenuData->cursorPosition, sTradeMenuData->partnerCursorPosition)) {
...
static void DrawTradeMenuParty(u8 whichParty) {
    ...
    switch (sTradeMenuData->drawPartyState[whichParty]) {
        case 1:
            ...
            gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * PARTY_SIZE)]].data[2] =
                (sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE][0] +
                 sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE + 1][0]) /
                    2 * 8 +
                14;
            gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * PARTY_SIZE)]].data[4] =
                (sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE][1] * 8) - 12;
            ...
            break;
        case 2:
            if (gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * PARTY_SIZE)]].callback ==
                SpriteCB_MonIcon)
        ...
        case 3:
            ...
            CopyBgTilemapBufferToVram(1);
            gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * PARTY_SIZE)]].pos1.x =
                (sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE][0] +
                 sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE + 1][0]) /
                    2 * 8 +
                14;
            gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * PARTY_SIZE)]].pos1.y =
                (sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE][1] * 8) - 12;
            ...
            AddTextPrinterParameterized3((whichParty * 2) + 14, 0, (80 - nameStringWidth) / 2, 4, sTradeTextColors, 0,
                                         nickname);
            ...
            break;
        case 4:
            DrawTradeMenuPartyMonInfo(whichParty, partyIdx, sTradeMenuPartyMonBoxDimensions[whichParty][0] + 4,
...
int GetUnionRoomTradeMessageId(struct GFtgtGnameSub rfuPlayer, struct GFtgtGnameSub rfuPartner, u16 playerSpecies2,
                               u16 partnerSpecies, u8 requestedType, u16 playerSpecies, u8 isObedientBitSet) {
...
static void SetTradeBGAffine(void) {
    ...
    DoBgAffineSet(&affine, sTradeData->texX * 0x100, sTradeData->texY * 0x100, sTradeData->scrX, sTradeData->scrY,
                  sTradeData->sXY, sTradeData->sXY, sTradeData->alpha);
...
static void LoadTradeMonPic(u8 whichParty, u8 state) {
    ...
    switch (state) {
        case 0:
            ...
            if (whichParty == TRADE_PLAYER)
                HandleLoadSpecialPokePic_2(&gMonFrontPicTable[species], gMonSpritesGfxPtr->sprites[1], species,
                                           personality);
            else
                HandleLoadSpecialPokePic_DontHandleDeoxys(
                    &gMonFrontPicTable[species], gMonSpritesGfxPtr->sprites[whichParty * 2 + 1], species, personality);
    ...
    switch (state) {
        case 0:
            ...
            SetGpuReg(REG_OFFSET_BG2CNT,
                      BGCNT_PRIORITY(2) | BGCNT_CHARBASE(1) | BGCNT_16COLOR | BGCNT_SCREENBASE(18) | BGCNT_TXT512x256);
        ...
        case 1:
            ...
            SetGpuReg(REG_OFFSET_BG1CNT,
                      BGCNT_PRIORITY(2) | BGCNT_CHARBASE(0) | BGCNT_16COLOR | BGCNT_SCREENBASE(5) | BGCNT_TXT256x512);
            SetGpuReg(REG_OFFSET_BG2CNT,
                      BGCNT_PRIORITY(2) | BGCNT_CHARBASE(1) | BGCNT_16COLOR | BGCNT_SCREENBASE(18) | BGCNT_TXT256x512);
        ...
        case 4:
            ...
            SetGpuReg(REG_OFFSET_BG2CNT,
                      BGCNT_PRIORITY(3) | BGCNT_CHARBASE(1) | BGCNT_256COLOR | BGCNT_SCREENBASE(18) | BGCNT_AFF128x128);
            sTradeData->texX = 64;
            sTradeData->texY = 92;
            sTradeData->sXY = 32;
    ...
    switch (sTradeData->state) {
        ...
        case 11:
            if (++sTradeData->timer == 80) {
                sTradeData->pokeballSpriteId = CreateTradePokeballSprite(
                    sTradeData->pokePicSpriteIdxs[0], gSprites[sTradeData->pokePicSpriteIdxs[0]].oam.paletteNum, 120,
                    32, 2, 1, 0x14, 0xfffff);
                sTradeData->state++;
        ...
        case 37:
            if (!IsMonSpriteNotFlipped(sTradeData->monSpecies[TRADE_PLAYER])) {
                ...
                CalcCenterToCornerVec(&gSprites[sTradeData->pokePicSpriteIdxs[TRADE_PLAYER]], SPRITE_SHAPE(64x64),
                                      SPRITE_SIZE(64x64), ST_OAM_AFFINE_DOUBLE);
        ...
        case 65:
            if (gSprites[sTradeData->unk_D3].callback == SpriteCallbackDummy) {
                HandleLoadSpecialPokePic_2(&gMonFrontPicTable[sTradeData->monSpecies[TRADE_PARTNER]],
                                           gMonSpritesGfxPtr->sprites[3], sTradeData->monSpecies[TRADE_PARTNER],
        ...
        case 66:
            ...
            CreatePokeballSpriteToReleaseMon(sTradeData->pokePicSpriteIdxs[TRADE_PARTNER],
                                             gSprites[sTradeData->pokePicSpriteIdxs[TRADE_PARTNER]].oam.paletteNum, 120,
                                             84, 2, 1, 20, 0xFFFFF, sTradeData->monSpecies[TRADE_PARTNER]);
        ...
        case 72: // Only if in-game trade
            ...
            evoTarget =
                GetEvolutionTargetSpecies(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]], TRUE, ITEM_NONE);
            if (evoTarget != SPECIES_NONE) {
                TradeEvolutionScene(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]], evoTarget,
...
static bool8 AnimateTradeSequenceWireless(void) {
    ...
    switch (sTradeData->state) {
        ...
        case 125:
            if (gSprites[sTradeData->unk_91].animEnded) {
                DestroySprite(&gSprites[sTradeData->unk_91]);
                SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_TGT1_BG1 | BLDCNT_TGT1_OBJ | BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_BG2);
...
static void _CreateInGameTradePokemon(u8 whichPlayerMon, u8 whichInGameTrade) {
    ...
    CreateMon(pokemon, inGameTrade->species, level, 32, TRUE, inGameTrade->personality, OT_ID_PRESET,
              inGameTrade->otId);
...
static void CheckPartnersMonForRibbons(void) {
    ...
    u8 numRibbons = 0;
    for (i = 0; i < 12; i++) {
        numRibbons += GetMonData(&gEnemyParty[gSelectedTradeMonPositions[TRADE_PARTNER] % PARTY_SIZE],
