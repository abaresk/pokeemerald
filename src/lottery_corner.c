...
void PickLotteryCornerTicket(void) {
    ...
    for (i = 0; i < PARTY_SIZE; i++) {
        ...
        } else // pokemon are always arranged from populated spots first to unpopulated, so the moment a NONE species is
               // found, that's the end of the list.
            break;
    }
...
// lottery numbers go from 0 to 99999, not 65535 (0xFFFF). interestingly enough, the function that calls
// GetLotteryNumber shifts to u16, so it cant be anything above 65535 anyway.
...
// interestingly, this may have been the original lottery number set function, but GF tried to change it to 32-bit later
// but didnt finish changing all calls as one GetLotteryNumber still shifts to u16.
