...
static const u16 sWaveformOffsets[][72] = { { 0x0000,
                                                0x0004,
                                                0x0008,
                                                0x000C,
                                                0x0010,
...
static void BufferCryWaveformSegment(void)
{
    ...
    if (gPcmDmaCounter < 2)
        ...
        baseBuffer = gSoundInfo.pcmBuffer + (gSoundInfo.pcmDmaPeriod + 1 - gPcmDmaCounter) *
...
static void AdvancePlayhead(u8 windowId)
{
    ...
    for (i = 0; i < 7; i++)
        CopyToWindowPixelBuffer(
            windowId, sCryScreenBg_Gfx, TILE_SIZE_4BPP, offset + (i * TILE_SIZE_4BPP));
}
...
{
// Position is a bitfield containing the play start pos, the playhead pos, and which vertical slice
// half to draw
...
bool8 LoadCryMeter(struct CryScreenWindow *window, u8 windowId)
{
    ...
    case 1:
        ...
        sCryMeterNeedle->spriteId = CreateSprite(
            &sCryMeterNeedleSpriteTemplate, 40 + window->xPos * 8, 56 + window->yPos * 8, 1);
...
void FreeCryScreen(void)
{
    FreeSpritePaletteByTag(
        GetSpritePaletteTagByPaletteNum(gSprites[sCryMeterNeedle->spriteId].oam.paletteNum));
