...
enum { WINDOW_EARNED_SYMBOLS, WINDOW_BATTLE_RECORD, WINDOW_BATTLE_POINTS, WINDOW_DESCRIPTION, WINDOW_4, WINDOW_COUNT };

// Windows displayed in the facilities map view.
enum { MAP_WINDOW_0, MAP_WINDOW_NAME, MAP_WINDOW_DESCRIPTION, MAP_WINDOW_COUNT };
...
struct {
    ...
    { 28, 76, 132, 220 }, { 84, 132, 132, 220 }, { 80, 102, 20, 108 }, { 0, 16, 152, 240 }, { 108, 134, 20, 108 },
    { 24, 48, 20, 108 },  { 50, 66, 20, 36 },    { 66, 82, 32, 48 },   { 50, 66, 44, 60 },  { 66, 82, 56, 72 },
    { 50, 66, 68, 84 },   { 66, 82, 80, 96 },    { 50, 66, 92, 108 },
};
...
static const struct CompressedSpriteSheet sHeadsSpriteSheet[] = { { sHeadsGfx, 0x100, 4 }, {} };

static const struct SpritePalette sSpritePalettes[] = { { gFrontierPassCursor_Pal, 0 },
...
static const union AnimCmd sSpriteAnim_857151C[] = { ANIMCMD_FRAME(0, 0), ANIMCMD_END };

static const union AnimCmd sSpriteAnim_8571524[] = { ANIMCMD_FRAME(0, 0), ANIMCMD_END };

static const union AnimCmd sSpriteAnim_857152C[] = { ANIMCMD_FRAME(4, 0), ANIMCMD_END };

static const union AnimCmd sSpriteAnim_8571534[] = { ANIMCMD_FRAME(8, 0), ANIMCMD_END };

static const union AnimCmd sSpriteAnim_857153C[] = { ANIMCMD_FRAME(12, 0), ANIMCMD_END };

static const union AnimCmd sSpriteAnim_8571544[] = { ANIMCMD_FRAME(16, 0), ANIMCMD_END };

static const union AnimCmd sSpriteAnim_857154C[] = { ANIMCMD_FRAME(20, 0), ANIMCMD_END };

static const union AnimCmd sSpriteAnim_8571554[] = { ANIMCMD_FRAME(24, 0), ANIMCMD_END };

static const union AnimCmd sSpriteAnim_857155C[] = { ANIMCMD_FRAME(0, 45), ANIMCMD_FRAME(8, 45), ANIMCMD_JUMP(0) };

static const union AnimCmd sSpriteAnim_8571568[] = { ANIMCMD_FRAME(16, 45), ANIMCMD_FRAME(24, 45), ANIMCMD_JUMP(0) };

static const union AnimCmd* const sSpriteAnimTable_8571574[] = { sSpriteAnim_8571524, sSpriteAnim_857152C };

static const union AnimCmd* const sSpriteAnimTable_857157C[] = { sSpriteAnim_8571524, sSpriteAnim_857152C,
                                                                 sSpriteAnim_8571534, sSpriteAnim_857153C,
                                                                 sSpriteAnim_8571544, sSpriteAnim_857154C,
                                                                 sSpriteAnim_8571554 };

static const union AnimCmd* const sSpriteAnimTable_8571598[] = { sSpriteAnim_857155C, sSpriteAnim_8571568 };

static const union AffineAnimCmd sSpriteAffineAnim_85715A0[] = { AFFINEANIMCMD_FRAME(256, 256, 0, 0),
                                                                 AFFINEANIMCMD_END };

static const union AffineAnimCmd* const sSpriteAffineAnimTable_85715B0[] = { sSpriteAffineAnim_85715A0 };
...
static const u8* const sPassAreaDescriptions[] = {
    gText_ThereIsNoBattleRecord,     gText_CheckFrontierMap,         gText_CheckTrainerCard,
    gText_ViewRecordedBattle,        gText_PutAwayFrontierPass,      gText_CurrentBattlePoints,
    gText_CollectedSymbols,          gText_BattleTowerAbilitySymbol, gText_BattleDomeTacticsSymbol,
    gText_BattlePalaceSpiritsSymbol, gText_BattleArenaGutsSymbol,    gText_BattleFactoryKnowledgeSymbol,
    gText_BattlePikeLuckSymbol,      gText_BattlePyramidBraveSymbol, gText_EmptyString7,
};
...
static void VblankCb_FrontierPass(void) {
    if (sPassGfx->setAffine) {
        SetBgAffine(2, gUnknown_085713E0[sPassData->unkE - 1][0] << 8, gUnknown_085713E0[sPassData->unkE - 1][1] << 8,
                    gUnknown_085713E0[sPassData->unkE - 1][0], gUnknown_085713E0[sPassData->unkE - 1][1],
                    sPassGfx->unk2E, sPassGfx->unk30, 0);
    }
    LoadOam();
...
static u8 GetCursorAreaFromCoords(s16 x, s16 y) {
    ...
    for (i = 0; i < CURSOR_AREA_COUNT - 1; i++) {
        if (sPassAreasLayout[i].yStart <= y && sPassAreasLayout[i].yEnd >= y && sPassAreasLayout[i].xStart <= x &&
    ...
    if (!var) // Cursor did not change.
    {
        if (sPassData->cursorArea != CURSOR_AREA_NOTHING && gMain.newKeys & A_BUTTON) {
            if (sPassData->cursorArea <= CURSOR_AREA_RECORD) // Map, Card, Record
            {
                ...
                DestroyTask(taskId);
                // BUG. The function should return here. Otherwise, it can play the same sound twice and destroy the
                // same task twice.
            }
        }
...
static void sub_80C5F58(bool8 arg0, bool8 arg1) {
    ...
    if (arg1) {
        SetBgAffine(2, gUnknown_085713E0[sPassData->unkE - 1][0] << 8, gUnknown_085713E0[sPassData->unkE - 1][1] << 8,
                    ...
                    MathUtil_Inv16(Q_8_8(1.984375)), 0);
    } else {
        SetBgAffine(2, gUnknown_085713E0[sPassData->unkE - 1][0] << 8, gUnknown_085713E0[sPassData->unkE - 1][1] << 8,
                    gUnknown_085713E0[sPassData->unkE - 1][0], gUnknown_085713E0[sPassData->unkE - 1][1],
                    MathUtil_Inv16(Q_8_8(1)), MathUtil_Inv16(Q_8_8(1)), 0);
    }
}
...
static void LoadCursorAndSymbolSprites(void) {
    ...
    for (i = 0; i < NUM_FRONTIER_FACILITIES; i++) {
        if (sPassData->facilitySymbols[i] != 0) {
            ...
            spriteId = CreateSprite(&sprite, sPassAreasLayout[i + CURSOR_AREA_SYMBOL - 1].xStart + 8,
                                    sPassAreasLayout[i + CURSOR_AREA_SYMBOL - 1].yStart + 6, i + 1);
...
static void InitFrontierMapSprites(void) {
    ...
    spriteId = CreateSprite(&sSpriteTemplates_Cursors[1], sMapLandmarks[sMapData->cursorPos].x,
                            sMapLandmarks[sMapData->cursorPos].y, 1);
...
static void PrintOnFrontierMap(void) {
    ...
    AddTextPrinterParameterized3(MAP_WINDOW_DESCRIPTION, 1, 4, 0, sTextColors[0], 0,
...
static void HandleFrontierMapCursorMove(u8 direction) {
    ...
    AddTextPrinterParameterized3(MAP_WINDOW_NAME, 7, 4, (oldCursorPos * 16) + 1, sTextColors[1], 0,
                                 sMapLandmarks[oldCursorPos].name);
    AddTextPrinterParameterized3(MAP_WINDOW_NAME, 7, 4, (sMapData->cursorPos * 16) + 1, sTextColors[2], 0,
