...
static const u8* const sTrainerCardColorNames[] = { gText_BronzeCard, gText_CopperCard, gText_SilverCard,
                                                    gText_GoldCard };
...
static void Task_LinkupAwaitConnection(u8 taskId) {
    ...
    if (CheckLinkCanceledBeforeConnection(taskId) == TRUE || CheckLinkCanceled(taskId) == TRUE || playerCount < 2)
        return;
...
static void Task_LinkupExchangeDataWithLeader(u8 taskId) {
    ...
    if (gSpecialVar_Result == LINKUP_ONGOING)
        return;
    if (gSpecialVar_Result == LINKUP_DIFF_SELECTIONS || gSpecialVar_Result == LINKUP_WRONG_NUM_PLAYERS) {
        SetCloseLinkCallback();
        HideFieldMessageBox();
        gTasks[taskId].func = Task_StopLinkup;
    } else if (gSpecialVar_Result == LINKUP_PLAYER_NOT_READY || gSpecialVar_Result == LINKUP_PARTNER_NOT_READY) {
        CloseLink();
        HideFieldMessageBox();
        gTasks[taskId].func = Task_StopLinkup;
    } else {
...
static void SetLinkBattleTypeFlags(int linkService) {
    switch (linkService) {
        ...
        case USING_MULTI_BATTLE:
            ...
            break;
        case USING_BATTLE_TOWER:
            gBattleTypeFlags = BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_DOUBLE | BATTLE_TYPE_LINK | BATTLE_TYPE_TRAINER |
                               BATTLE_TYPE_MULTI;
            break;
    }
}
