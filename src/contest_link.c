...
static void Task_LinkContest_InitFlags(u8 taskId)
{
    ...
    for (i = 0;
         i < gNumLinkContestPlayers && (u32)(gLinkPlayers[i].version & 0xFF) - 1 > VERSION_RUBY - 1;
         i++)
        ;
            ...
            if (LinkContest_GetBlockReceivedFromAllPlayers())
            {
                for (i = 0; i < gNumLinkContestPlayers; i++)
                {
                    ...
                    StripPlayerAndMonNamesForLinkContest(
                        &gContestMons[i], gLinkPlayers[i].language);
                }
...
void Task_LinkContest_CommunicateRngRS(u8 taskId)
{
    switch (gTasks[taskId].tState)
    {
        case 0:
            ...
            {
                if (IsLinkTaskFinished()
                    && LinkContest_SendBlock(&gRngValue, sizeof(gRngValue)) == TRUE)
                    gTasks[taskId].tState++;
            }
            else
            {
    ...
    switch (gTasks[taskId].tState)
    {
        case 0:
            if (IsLinkTaskFinished())
            {
                ...
                if (LinkContest_SendBlock(&eContestantStatus[gContestPlayerMonIndex].currMove,
                        sizeof(eContestantStatus[gContestPlayerMonIndex].currMove))
                    == TRUE)
                    gTasks[taskId].tState++;
            }
            break;
...
void Task_LinkContest_CommunicateFinalStandings(u8 taskId)
{
    switch (gTasks[taskId].tState)
    {
        case 0:
            if (IsLinkTaskFinished())
            {
                if (LinkContest_SendBlock(gContestMonTotalPoints, sizeof(gContestMonTotalPoints))
                    == 1)
                    gTasks[taskId].tState++;
            }
            break;
        case 1:
            if (LinkContest_GetBlockReceivedFromAllPlayers())
            {
                memcpy(gContestMonTotalPoints,
                    gBlockRecvBuffer[gContestLinkLeaderIndex],
                    sizeof(gContestMonTotalPoints));
                gTasks[taskId].tState++;
            }
            break;
        case 2:
        ...
        case 4:
            if (LinkContest_GetBlockReceivedFromAllPlayers())
            {
                memcpy(gContestMonAppealPointTotals,
                    gBlockRecvBuffer[gContestLinkLeaderIndex],
        ...
        case 6:
            if (IsLinkTaskFinished())
            {
                if (LinkContest_SendBlock(gContestMonRound2Points, sizeof(gContestMonRound2Points))
                    == 1)
                    gTasks[taskId].tState++;
            }
            break;
        case 7:
            if (LinkContest_GetBlockReceivedFromAllPlayers())
            {
                memcpy(gContestMonRound2Points,
                    gBlockRecvBuffer[gContestLinkLeaderIndex],
                    sizeof(gContestMonRound2Points));
                gTasks[taskId].tState++;
            }
            break;
        case 9:
            if (IsLinkTaskFinished())
            {
                if (LinkContest_SendBlock(gContestFinalStandings, sizeof(gContestFinalStandings))
                    == 1)
                    gTasks[taskId].tState++;
            }
            break;
...
void Task_LinkContest_CommunicateAppealsState(u8 taskId)
{
    switch (gTasks[taskId].tState)
    {
        case 0:
            ...
            {
                if (LinkContest_SendBlock(
                        eContestantStatus, CONTESTANT_COUNT * sizeof(struct ContestantStatus))
                    == 1)
                    gTasks[taskId].tState++;
            }
            break;
        case 1:
            if (LinkContest_GetBlockReceivedFromAllPlayers())
            {
                memcpy(eContestantStatus,
                    gBlockRecvBuffer[gContestLinkLeaderIndex],
                    CONTESTANT_COUNT * sizeof(struct ContestantStatus));
                gTasks[taskId].tState++;
            }
            break;
        case 2:
        ...
        case 3:
            ...
            {
                if (LinkContest_SendBlock(
                        gContestResources->appealResults, sizeof(struct ContestAppealMoveResults))
                    == 1)
                    gTasks[taskId].tState++;
            }
            break;
        case 4:
            if (LinkContest_GetBlockReceivedFromAllPlayers())
            {
                memcpy(gContestResources->appealResults,
                    gBlockRecvBuffer[gContestLinkLeaderIndex],
                    sizeof(struct ContestAppealMoveResults));
                gTasks[taskId].tState++;
            }
            break;
        case 6:
            ...
            {
                if (LinkContest_SendBlock(
                        gContestResources->excitement, sizeof(struct ContestExcitement))
                    == 1)
                    gTasks[taskId].tState++;
            }
            break;
        case 7:
            if (LinkContest_GetBlockReceivedFromAllPlayers())
            {
                memcpy(gContestResources->excitement,
                    gBlockRecvBuffer[gContestLinkLeaderIndex],
                    sizeof(struct ContestExcitement));
                gTasks[taskId].tState++;
            }
            break;
        case 9:
...
void Task_LinkContest_CommunicateRound1Points(u8 taskId)
{
    ...
    switch (gTasks[taskId].tState)
    {
        case 0:
            if (IsLinkTaskFinished())
            {
                if (LinkContest_SendBlock(gContestMonRound1Points, sizeof(gContestMonRound1Points))
                    == 1)
                    gTasks[taskId].tState++;
            }
            break;
        case 1:
            if (LinkContest_GetBlockReceivedFromAllPlayers())
            {
                memcpy(gContestMonRound1Points,
                    gBlockRecvBuffer[gContestLinkLeaderIndex],
                    sizeof(gContestMonRound1Points));
                gTasks[taskId].tState++;
            }
            break;
        default:
