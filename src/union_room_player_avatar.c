...
static const u8 sMovement_UnionPlayerEnter[2] = { MOVEMENT_ACTION_FLY_DOWN,
...
{
    TrySpawnObjectEvent(sUnionRoomLocalIds[playerIdx],
...
{
    RemoveObjectEventByLocalIdAndMap(sUnionRoomLocalIds[playerIdx],
        gSaveBlock1Ptr->location.mapNum,
        gSaveBlock1Ptr->location.mapGroup);
}
...
static bool32 SetUnionRoomPlayerEnterExitMovement(u32 playerIdx, const u8 *movement)
{
    ...
    if (TryGetObjectEventIdByLocalIdAndMap(sUnionRoomLocalIds[playerIdx],
            gSaveBlock1Ptr->location.mapNum,
            gSaveBlock1Ptr->location.mapGroup,
            &objectId))
    {
...
static u8 GetNewFacingDirectionForUnionRoomPlayer(
    u32 direction, u32 playerIdx, struct GFtgtGname *gname)
{
...
{
    return IsObjectEventSpriteInvisible(
        UR_PLAYER_SPRITE_ID(playerIdx, direction) - UR_SPRITE_START_ID);
}
...
static void SpawnGroupMember(u32 playerIdx, u32 direction, u8 graphicsId, struct GFtgtGname *gname)
{
    ...
    SetUnionRoomObjectFacingDirection(
        direction, playerIdx, GetNewFacingDirectionForUnionRoomPlayer(direction, playerIdx, gname));
...
{
    s32 x, y;
    SetObjectEventSpriteAnim(
        UR_PLAYER_SPRITE_ID(playerIdx, direction) - UR_SPRITE_START_ID, UNION_ROOM_SPAWN_OUT);
    GetUnionRoomPlayerFacingCoords(playerIdx, direction, &x, &y);
    MapGridSetMetatileImpassabilityAt(x, y, FALSE);
}
...
static void AssembleGroup(u32 playerIdx, struct GFtgtGname *gname)
{
        ...
        TRUE)
    {
        ...
        SpawnGroupMember(playerIdx,
            0,
            GetUnionRoomPlayerGraphicsId(gname->playerGender, gname->unk_00.playerTrainerId[0]),
            gname);
    }
    for (i = 1; i < 5; i++)
    {
...
bool32 TryInteractWithUnionRoomMember(
    struct UnkStruct_Main0 *main0, s16 *directionPtr, s16 *playerIdxPtr, u8 *spriteIds)
{
    s16 x, y;
    s32 i, direction;
    struct UnkStruct_x20 *r4;
    ...
    for (i = 0, r4 = main0->arr; i < MAX_UNION_ROOM_PLAYERS; i++)
    {
        for (direction = DIR_NONE; direction <= DIR_EAST; direction++)
        {
            ...
            // Face player
            SetUnionRoomObjectFacingDirection(
                direction, i, sOppositeFacingDirection[GetPlayerFacingDirection()]);
...
static void SetUnionRoomObjectFacingDirection(s32 currDirection, s32 playerIdx, u8 newDirection)
{
    ...
    // TurnObjectEventSprite(UR_PLAYER_SPRITE_ID(playerIdx, currDirection) - UR_SPRITE_START_ID,
    // newDirection);
}

void UpdateUnionRoomMemberFacing(u32 currDirection, u32 playerIdx, struct UnkStruct_Main0 *main0)
{
    return SetUnionRoomObjectFacingDirection(currDirection,
        playerIdx,
        GetNewFacingDirectionForUnionRoomPlayer(
            currDirection, playerIdx, &main0->arr[playerIdx].gname_uname.gname));
}
