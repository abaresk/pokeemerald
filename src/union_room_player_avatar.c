...
static const u8 sUnionRoomObjGfxIds[GENDER_COUNT][MAX_UNION_ROOM_PLAYERS + 2] = {
    [MALE] = { OBJ_EVENT_GFX_MAN_3, OBJ_EVENT_GFX_BLACK_BELT, OBJ_EVENT_GFX_CAMPER, OBJ_EVENT_GFX_YOUNGSTER,
               OBJ_EVENT_GFX_PSYCHIC_M, OBJ_EVENT_GFX_BUG_CATCHER, OBJ_EVENT_GFX_MAN_4, OBJ_EVENT_GFX_MAN_5 },
    [FEMALE] = { OBJ_EVENT_GFX_WOMAN_5, OBJ_EVENT_GFX_HEX_MANIAC, OBJ_EVENT_GFX_PICNICKER, OBJ_EVENT_GFX_LASS,
                 OBJ_EVENT_GFX_LASS, OBJ_EVENT_GFX_GIRL_3, OBJ_EVENT_GFX_WOMAN_2, OBJ_EVENT_GFX_BEAUTY }
};

static const s16 sUnionRoomPlayerCoords[MAX_UNION_ROOM_PLAYERS][2] = { { 4, 6 },  { 13, 8 }, { 10, 6 }, { 1, 8 },
                                                                       { 13, 4 }, { 7, 4 },  { 1, 4 },  { 7, 8 } };

static const s8 sFacingDirectionOffsets[][2] = { [DIR_NONE] = { 0, 0 },
                                                 [DIR_SOUTH] = { 1, 0 },
                                                 [DIR_NORTH] = { 0, -1 },
                                                 [DIR_WEST] = { -1, 0 },
                                                 [DIR_EAST] = { 0, 1 } };

static const u8 sOppositeFacingDirection[] = { [DIR_NONE] = DIR_NONE,
                                               [DIR_SOUTH] = DIR_NORTH,
                                               [DIR_NORTH] = DIR_SOUTH,
                                               [DIR_WEST] = DIR_EAST,
                                               [DIR_EAST] = DIR_WEST };

static const u8 sNextFacingDirection[] = { [DIR_NONE] = DIR_SOUTH,
                                           [DIR_SOUTH] = DIR_WEST,
                                           [DIR_NORTH] = DIR_SOUTH,
                                           [DIR_WEST] = DIR_EAST,
                                           [DIR_EAST] = DIR_NORTH };
...
static const u16 sUnknown[] = { 0x2BF, 0x2C0, 0x2C1, 0x2C2, 0x2C3, 0x2C4, 0x2C5, 0x2C6 };

static const u8 sMovement_UnionPlayerExit[2] = { MOVEMENT_ACTION_FLY_UP, MOVEMENT_ACTION_STEP_END };

static const u8 sMovement_UnionPlayerEnter[2] = { MOVEMENT_ACTION_FLY_DOWN, MOVEMENT_ACTION_STEP_END };
...
static void CreateUnionRoomPlayerObjectEvent(u32 playerIdx) {
    TrySpawnObjectEvent(sUnionRoomLocalIds[playerIdx], gSaveBlock1Ptr->location.mapNum,
                        gSaveBlock1Ptr->location.mapGroup);
}

static void RemoveUnionRoomPlayerObjectEvent(u32 playerIdx) {
    RemoveObjectEventByLocalIdAndMap(sUnionRoomLocalIds[playerIdx], gSaveBlock1Ptr->location.mapNum,
                                     gSaveBlock1Ptr->location.mapGroup);
}
...
static bool32 SetUnionRoomPlayerEnterExitMovement(u32 playerIdx, const u8* movement) {
    ...
    if (TryGetObjectEventIdByLocalIdAndMap(sUnionRoomLocalIds[playerIdx], gSaveBlock1Ptr->location.mapNum,
                                           gSaveBlock1Ptr->location.mapGroup, &objectId)) {
        return FALSE;
    }
...
void CreateGroupMemberSpritesInvisible(u8* spriteIds, s32 playerIdx) {
    ...
    for (direction = DIR_NONE; direction <= DIR_EAST; direction++) {
        ...
        spriteIds[id] =
            CreateObjectSprite(OBJ_EVENT_GFX_MAN_4, id - UR_SPRITE_START_ID,
                               sUnionRoomPlayerCoords[playerIdx][0] + sFacingDirectionOffsets[direction][0],
                               sUnionRoomPlayerCoords[playerIdx][1] + sFacingDirectionOffsets[direction][1], 3, 1);
...
static void SpawnGroupMember(u32 playerIdx, u32 direction, u8 graphicsId, struct GFtgtGname* gname) {
    ...
    SetUnionRoomObjectFacingDirection(direction, playerIdx,
                                      GetNewFacingDirectionForUnionRoomPlayer(direction, playerIdx, gname));
...
static void AssembleGroup(u32 playerIdx, struct GFtgtGname* gname) {
    ...
    if (IsObjectEventSpriteInvisible(UR_PLAYER_SPRITE_ID(playerIdx, 0) - UR_SPRITE_START_ID) == TRUE) {
        if (IsUnionRoomPlayerFacingTileAt(playerIdx, 0, x, y) == TRUE ||
            IsUnionRoomPlayerFacingTileAt(playerIdx, 0, x2, y2) == TRUE) {
            return;
        }
        SpawnGroupMember(playerIdx, 0,
                         GetUnionRoomPlayerGraphicsId(gname->playerGender, gname->unk_00.playerTrainerId[0]), gname);
    }
    for (i = 1; i < 5; i++) {
        if (gname->child_sprite_gender[i - 1] == 0) {
            DespawnGroupMember(playerIdx, i);
        } else if (IsUnionRoomPlayerFacingTileAt(playerIdx, i, x, y) == FALSE &&
                   IsUnionRoomPlayerFacingTileAt(playerIdx, i, x2, y2) == FALSE) {
            SpawnGroupMember(playerIdx, i,
                             GetUnionRoomPlayerGraphicsId((gname->child_sprite_gender[i - 1] >> 3) & 1,
                                                          gname->child_sprite_gender[i - 1] & 7),
                             gname);
        }
    }
}
...
bool32 TryInteractWithUnionRoomMember(struct UnkStruct_Main0* main0, s16* directionPtr, s16* playerIdxPtr,
                                      u8* spriteIds) {
    s16 x, y;
    s32 i, direction;
    struct UnkStruct_x20* r4;
...
void UpdateUnionRoomMemberFacing(u32 currDirection, u32 playerIdx, struct UnkStruct_Main0* main0) {
    return SetUnionRoomObjectFacingDirection(
        currDirection, playerIdx,
        GetNewFacingDirectionForUnionRoomPlayer(currDirection, playerIdx, &main0->arr[playerIdx].gname_uname.gname));
}
