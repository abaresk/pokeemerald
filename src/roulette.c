...
static void FlashSelectionOnWheel(u8 selectionId)
{
    ...
    default:
    {
        ...
        {
        // The specific color of the poke it references doesn't matter, because the icons of a poke
        // share a palette So it just uses the first sprite ID of each
        case COL_WYNAUT:
            palOffset =
                gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_ORANGE_WYNAUT]].oam.paletteNum * 16;
            break;
        case COL_AZURILL:
            palOffset =
                gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_GREEN_AZURILL]].oam.paletteNum * 16;
            break;
        case COL_SKITTY:
            palOffset =
                gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_PURPLE_SKITTY]].oam.paletteNum * 16;
            break;
        case COL_MAKUHITA:
            palOffset =
                gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_ORANGE_MAKUHITA]].oam.paletteNum * 16;
            break;
        }
        if (numSelected == 1)
        {
