...
static EWRAM_DATA struct Roulette {
    ...
    u8 betSelection[BALLS_PER_ROUND]; // Because curBallNum is used as the only index, only the first element is ever
                                      // used (prev bet selections are never needed)
    u8 wheelDelayTimer;
    u8 wheelSpeed;
    u8 wheelDelay;
    s16 wheelAngle;
...
static const struct WindowTemplate sWindowTemplates[] = {
    { .bg = 0, .tilemapLeft = 3, .tilemapTop = 15, .width = 24, .height = 4, .paletteNum = 15, .baseBlock = 0xC5 },
...
static const struct YesNoFuncTable sYesNoTable_AcceptMinBet = { Task_AcceptMinBet, Task_DeclineMinBet };

static const struct YesNoFuncTable sYesNoTable_KeepPlaying = { Task_ContinuePlaying, Task_StopPlaying };
...
static void ProcessBetGridInput(u8 taskId) {
    ...
    if ((!(JOY_NEW(DPAD_UP)) || ((dirPressed = TRUE) && CanMoveSelectionInDir(&gTasks[taskId].tSelectionId, 0))) &&
        ...
        sRoulette->flashUtil.palettes[FLASH_ICON].available = sRoulette->flashUtil.palettes[FLASH_ICON_2].available =
...
// Odds of a lower value increase as play continues, if the player has Shroomish and/or Taillow in the party, and
...
static void Task_InitBallRoll(u8 taskId) {
    ...
    sRoulette->ballAngleAccel =
        ((sRoulette->ballAngleSpeed * 0.5f) - sRoulette->ballAngleSpeed) / S16TOPOSFLOAT(sRoulette->ballTravelDistFast);
...
static void Task_PrintPayout(u8 taskId) {
    ConvertIntToDecimalStringN(gStringVar1, (sRoulette->minBet * gTasks[taskId].tMultiplier), STR_CONV_MODE_LEFT_ALIGN,
                               2);
...
static void Task_EndTurn(u8 taskId) {
    ...
    sRoulette->flashUtil.palettes[FLASH_ICON].available = sRoulette->flashUtil.palettes[FLASH_ICON_2].available =
        sRoulette->flashUtil.palettes[FLASH_ICON_3].available = FALSE;
    gSprites[sRoulette->spriteIds[SPR_WHEEL_ICONS + sGridSelections[gTasks[taskId].tWinningSquare].spriteIdOffset]]
        .invisible = TRUE;
...
static u8 RecordHit(u8 taskId, u8 slotId) {
    u8 i, j;
    u32 columnFlags[NUM_BOARD_POKES] = { F_WYNAUT_COL | F_ORANGE_WYNAUT | F_GREEN_WYNAUT | F_PURPLE_WYNAUT,
    ...
    switch (selectionId) {
        ...
        default: {
            ...
            switch (GET_COL(selectionId)) {
                // The specific color of the poke it references doesn't matter, because the icons of a poke share a
                // palette So it just uses the first sprite ID of each
                case COL_WYNAUT:
            ...
            if (numSelected == 1) {
                ...
                if (!(sRoulette->hitFlags & sGridSelections[selectionId].flag)) {
                    ...
                    RouletteFlash_Add(&sRoulette->flashUtil, NUM_ROULETTE_SLOTS + 1,
                ...
                for (i = 0; i < NUM_BOARD_COLORS; i++) {
                    ...
                    if (!(sRoulette->hitFlags & sGridSelections[columnSlotId].flag)) {
                        ...
                        RouletteFlash_Add(&sRoulette->flashUtil, i + NUM_ROULETTE_SLOTS + 1,
...
static const struct SpritePalette sSpritePalettes[] = { { .data = sShadow_Pal, .tag = PALTAG_SHADOW },
...
static const struct CompressedSpriteSheet sSpriteSheet_WheelIcons = { .data = sWheelIcons_Gfx,
                                                                      .size = 0xC00,
                                                                      .tag = GFXTAG_WHEEL_ICONS };

static const union AnimCmd sAnim_WheelIcons[] = { ANIMCMD_FRAME(0, 0),  ANIMCMD_FRAME(32, 0), ANIMCMD_FRAME(64, 0),
                                                  ANIMCMD_FRAME(72, 0), ANIMCMD_FRAME(8, 0),  ANIMCMD_FRAME(40, 0),
                                                  ANIMCMD_FRAME(48, 0), ANIMCMD_FRAME(80, 0), ANIMCMD_FRAME(16, 0),
                                                  ANIMCMD_FRAME(24, 0), ANIMCMD_FRAME(56, 0), ANIMCMD_FRAME(88, 0),
                                                  ANIMCMD_END };
...
static const struct CompressedSpriteSheet sSpriteSheet_Headers = { .data = gRouletteHeaders_Gfx,
                                                                   .size = 0x1600,
                                                                   .tag = GFXTAG_HEADERS };

static const struct CompressedSpriteSheet sSpriteSheet_GridIcons = { .data = sGridIcons_Gfx,
                                                                     .size = 0x400,
                                                                     .tag = GFXTAG_GRID_ICONS };

static const union AnimCmd sAnim_Headers[] = { ANIMCMD_FRAME(0, 0),   ANIMCMD_FRAME(16, 0),  ANIMCMD_FRAME(32, 0),
                                               ANIMCMD_FRAME(48, 0),  ANIMCMD_FRAME(64, 0),  ANIMCMD_FRAME(80, 0),
                                               ANIMCMD_FRAME(96, 0),  ANIMCMD_FRAME(112, 0), ANIMCMD_FRAME(128, 0),
                                               ANIMCMD_FRAME(144, 0), ANIMCMD_FRAME(160, 0), ANIMCMD_END };

static const union AnimCmd sAnim_GridIcons[] = { ANIMCMD_FRAME(0, 0), ANIMCMD_FRAME(4, 0), ANIMCMD_FRAME(8, 0),
                                                 ANIMCMD_FRAME(12, 0), ANIMCMD_END };
...
static const union AnimCmd sAnim_Multiplier[] = { ANIMCMD_FRAME(0, 0),  ANIMCMD_FRAME(8, 0),  ANIMCMD_FRAME(16, 0),
                                                  ANIMCMD_FRAME(24, 0), ANIMCMD_FRAME(32, 0), ANIMCMD_END };

static const union AnimCmd* const sAnims_Multiplier[] = { sAnim_Multiplier };

static const union AnimCmd sAnim_BallCounter[] = { ANIMCMD_FRAME(0, 0), ANIMCMD_FRAME(2, 0), ANIMCMD_FRAME(4, 0),
                                                   ANIMCMD_FRAME(6, 0), ANIMCMD_FRAME(8, 0), ANIMCMD_END };
...
static const struct CompressedSpriteSheet sSpriteSheet_Ball = { .data = sBall_Gfx, .size = 0x200, .tag = GFXTAG_BALL };

static const union AnimCmd sAnim_Ball_RollFast[] = { ANIMCMD_FRAME(0, 5), ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(8, 5),
                                                     ANIMCMD_FRAME(4, 5), ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_Ball_RollMedium[] = { ANIMCMD_FRAME(0, 10), ANIMCMD_FRAME(4, 10), ANIMCMD_FRAME(8, 10),
                                                       ANIMCMD_FRAME(4, 10), ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_Ball_RollSlow[] = { ANIMCMD_FRAME(0, 15), ANIMCMD_FRAME(4, 15), ANIMCMD_FRAME(8, 15),
                                                     ANIMCMD_FRAME(4, 15), ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_Ball_StopOnFrame1[] = { ANIMCMD_FRAME(4, 2), ANIMCMD_FRAME(8, 5), ANIMCMD_FRAME(4, 5),
                                                         ANIMCMD_FRAME(12, 5), ANIMCMD_END };

static const union AnimCmd sAnim_Ball_StopOnFrame3[] = { ANIMCMD_FRAME(4, 2), ANIMCMD_FRAME(0, 4),  ANIMCMD_FRAME(4, 4),
                                                         ANIMCMD_FRAME(8, 4), ANIMCMD_FRAME(12, 4), ANIMCMD_END };

static const union AnimCmd sAnim_Ball_StopOnFrame4[] = { ANIMCMD_FRAME(0, 2), ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(8, 5),
                                                         ANIMCMD_FRAME(12, 5), ANIMCMD_END };

static const union AnimCmd sAnim_Ball_Still[] = { ANIMCMD_FRAME(12, 0), ANIMCMD_END };

static const union AnimCmd sAnim_Ball_StopOnFrame2[] = { ANIMCMD_FRAME(8, 2), ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(0, 5),
                                                         ANIMCMD_FRAME(12, 5), ANIMCMD_END };

static const union AnimCmd* const sAnims_Ball[] = { sAnim_Ball_RollFast,     sAnim_Ball_RollMedium,
                                                    sAnim_Ball_RollSlow,     sAnim_Ball_StopOnFrame1,
                                                    sAnim_Ball_StopOnFrame2, sAnim_Ball_StopOnFrame3,
                                                    sAnim_Ball_StopOnFrame4, sAnim_Ball_StopOnFrame4,
                                                    sAnim_Ball_Still };
...
static const struct CompressedSpriteSheet sSpriteSheet_WheelCenter = { .data = gRouletteCenter_Gfx,
                                                                       .size = 0x800,
...
static const struct CompressedSpriteSheet sSpriteSheet_ShroomishTaillow = { .data = sShroomishTaillow_Gfx,
                                                                            .size = 0xE00,
                                                                            .tag = GFXTAG_SHROOMISH_TAILLOW };

static const union AnimCmd sAnim_Shroomish[] = { ANIMCMD_FRAME(0, 6),  ANIMCMD_FRAME(16, 6), ANIMCMD_FRAME(32, 6),
                                                 ANIMCMD_FRAME(48, 6), ANIMCMD_FRAME(32, 6), ANIMCMD_FRAME(64, 6),
                                                 ANIMCMD_JUMP(2) };

static const union AnimCmd sAnim_Taillow_WingDown_Left[] = { ANIMCMD_FRAME(80, 10), ANIMCMD_END };

static const union AnimCmd sAnim_Taillow_WingDown_Right[] = { ANIMCMD_FRAME(80, 10, .hFlip = TRUE), ANIMCMD_END };

static const union AnimCmd sAnim_Taillow_FlapSlow_Left[] = { ANIMCMD_FRAME(80, 20), ANIMCMD_FRAME(96, 20),
                                                             ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_Taillow_FlapSlow_Right[] = { ANIMCMD_FRAME(80, 20, .hFlip = TRUE),
                                                              ANIMCMD_FRAME(96, 20, .hFlip = TRUE), ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_Taillow_FlapFast_Left[] = { ANIMCMD_FRAME(80, 10), ANIMCMD_FRAME(96, 10),
                                                             ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_Taillow_FlapFast_Right[] = { ANIMCMD_FRAME(80, 10, .hFlip = TRUE),
                                                              ANIMCMD_FRAME(96, 10, .hFlip = TRUE), ANIMCMD_JUMP(0) };

static const union AnimCmd* const sAnims_Shroomish[] = { sAnim_Shroomish };

static const union AnimCmd* const sAnims_Taillow[] = { sAnim_Taillow_WingDown_Left, // While gliding in
...
static const struct SpriteTemplate sSpriteTemplate_Shroomish = { .tileTag = GFXTAG_SHROOMISH_TAILLOW,
...
static const struct CompressedSpriteSheet sSpriteSheet_Shadow = { .data = sShadow_Gfx,
                                                                  .size = 0x180,
                                                                  .tag = GFXTAG_SHADOW };

static const union AffineAnimCmd sAffineAnim_Unused3[] = { AFFINEANIMCMD_FRAME(0x80, 0x80, 0, 0),
                                                           AFFINEANIMCMD_FRAME(2, 2, 0, 60), AFFINEANIMCMD_END };

static const union AffineAnimCmd sAffineAnim_TaillowShadow[] = {
    AFFINEANIMCMD_FRAME(0x100, 0x100, 0, 0), AFFINEANIMCMD_FRAME(-2, 0x0, 0, 15), AFFINEANIMCMD_FRAME(-1, -2, 0, 15),
    AFFINEANIMCMD_FRAME(-1, -5, 0, 24), AFFINEANIMCMD_END
};

static const union AffineAnimCmd* const sAffineAnims_Unused3[] = { sAffineAnim_Unused3 };

static const union AffineAnimCmd* const sAffineAnims_TaillowShadow[] = { sAffineAnim_TaillowShadow };

static const union AffineAnimCmd sAffineAnim_Unused4[] = { AFFINEANIMCMD_FRAME(0x100, 0x100, 0, 0), AFFINEANIMCMD_END };
    ...
    for (i = 0; i < NUM_BOARD_COLORS; i++) {
        for (j = 0; j < NUM_BOARD_POKES; j++) {
            ...
            spriteId = sRoulette->spriteIds[(i * NUM_BOARD_POKES) + SPR_WHEEL_ICONS + j] =
                CreateWheelIconSprite(&sSpriteTemplates_WheelIcons[i * NUM_BOARD_POKES + j], 40, &angle);
            gSprites[spriteId].animPaused = TRUE;
            gSprites[spriteId].affineAnimPaused = TRUE;
        }
    }
...
//   e.g. if the ball is screen right (90), but wheel is upside down (180), sBallWheelAngle is 270 (because the ball is
//   wheel left)
...
static void CreateShroomishSprite(struct Sprite* ball) {
    ...
    sRoulette->spriteIds[SPR_CLEAR_MON_SHADOW_1] =
        CreateSprite(&sSpriteTemplate_ShroomishShadow[0], coords[ball->sStuckOnWheelLeft][0],
                     coords[ball->sStuckOnWheelLeft][1], 59);
...
static void CreateTaillowSprite(struct Sprite* ball) {
    ...
    t = ball->data[7] - 2;
    sRoulette->spriteIds[SPR_CLEAR_MON] = CreateSprite(&sSpriteTemplate_Taillow, coords[ball->sStuckOnWheelLeft][0],
                                                       coords[ball->sStuckOnWheelLeft][1], 50);
    StartSpriteAnim(&gSprites[sRoulette->spriteIds[SPR_CLEAR_MON]], ball->sStuckOnWheelLeft);
    sRoulette->spriteIds[SPR_CLEAR_MON_SHADOW_1] = CreateSprite(
        &sSpriteTemplate_TaillowShadow, coords[ball->sStuckOnWheelLeft][0], coords[ball->sStuckOnWheelLeft][1], 51);
...
static void SetBallStuck(struct Sprite* sprite) {
    ...
    for (i = slotsToSkip; i < maxSlotToCheck; i++) {
        if (!(sRoulette->hitFlags & sRouletteSlots[slotId].flag)) {
            ...
            if (betSlotId == 0 && (sRouletteSlots[slotId].flag &
                                   sGridSelections[sRoulette->betSelection[sRoulette->curBallNum]].inSelectionFlags))
                betSlotId = i;
        }
...
static const u16 sShroomishShadowAlphas[] = {
    0x907, 0x808, 0x709, 0x60A, 0x50B, 0x40C, 0x30D, 0x20E, 0x10F, 0x010,
};
...
static void SpriteCB_Taillow_FlyIn(struct Sprite* sprite) {
    ...
    s8 yMoveOffsets[][2] = {
        { 2, 0 }, { 2, 0 }, { 2, -1 }, { 2, -1 }, { 2, -1 }, { 2, -1 }, { 2, -2 }, { 2, -2 },
    };
