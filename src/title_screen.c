...
const u16 gIntroWaterDropAlphaBlend[] = {
    BLDALPHA_BLEND(16, 0),  BLDALPHA_BLEND(16, 1),  BLDALPHA_BLEND(16, 2),
    BLDALPHA_BLEND(16, 3),  BLDALPHA_BLEND(16, 4),  BLDALPHA_BLEND(16, 5),
    BLDALPHA_BLEND(16, 6),  BLDALPHA_BLEND(16, 7),  BLDALPHA_BLEND(16, 8),
    BLDALPHA_BLEND(16, 9),  BLDALPHA_BLEND(16, 10), BLDALPHA_BLEND(16, 11),
    BLDALPHA_BLEND(16, 12), BLDALPHA_BLEND(16, 13), BLDALPHA_BLEND(16, 14),
    BLDALPHA_BLEND(16, 15), BLDALPHA_BLEND(15, 16), BLDALPHA_BLEND(14, 16),
    BLDALPHA_BLEND(13, 16), BLDALPHA_BLEND(12, 16), BLDALPHA_BLEND(11, 16),
    BLDALPHA_BLEND(10, 16), BLDALPHA_BLEND(9, 16),  BLDALPHA_BLEND(8, 16),
    BLDALPHA_BLEND(7, 16),  BLDALPHA_BLEND(6, 16),  BLDALPHA_BLEND(5, 16),
    BLDALPHA_BLEND(4, 16),  BLDALPHA_BLEND(3, 16),  BLDALPHA_BLEND(2, 16),
    BLDALPHA_BLEND(1, 16),  BLDALPHA_BLEND(0, 16),  [32 ... 63] = BLDALPHA_BLEND(0, 16)
};
...
static const struct CompressedSpriteSheet sSpriteSheet_EmeraldVersion[] = {
    { .data = gTitleScreenEmeraldVersionGfx, .size = 0x1000, .tag = 1000 },
    {},
};
...
static const union AnimCmd* const sStartCopyrightBannerAnimTable[] = {
    sCopyrightBannerAnim0, sCopyrightBannerAnim1, sCopyrightBannerAnim2, sCopyrightBannerAnim3, sCopyrightBannerAnim4,
    sCopyrightBannerAnim5, sCopyrightBannerAnim6, sCopyrightBannerAnim7, sCopyrightBannerAnim8, sCopyrightBannerAnim9,
};
...
static const struct CompressedSpriteSheet sSpriteSheet_PressStart[] = {
    { .data = gTitleScreenPressStartGfx, .size = 0x520, .tag = 1001 },
    {},
};

static const struct SpritePalette sSpritePalette_PressStart[] = {
    { .data = gTitleScreenPressStartPal, .tag = 1001 },
    {},
};
...
static const struct CompressedSpriteSheet sPokemonLogoShineSpriteSheet[] = {
    { .data = sTitleScreenLogoShineGfx, .size = 0x800, .tag = 1002 },
    {},
};
...
static void SpriteCB_PokemonLogoShine(struct Sprite* sprite) {
    if (sprite->pos1.x < 272) {
        if (sprite->data[0]) // Flash background
        {
            ...
            if (sprite->pos1.x == DISPLAY_WIDTH / 2 + 12 || sprite->pos1.x == DISPLAY_WIDTH / 2 + 16 ||
                sprite->pos1.x == DISPLAY_WIDTH / 2 + 20 || sprite->pos1.x == DISPLAY_WIDTH / 2 + 24)
    ...
    switch (gMain.state) {
        ...
        case 4:
            ...
            SetGpuReg(REG_OFFSET_BG0CNT,
                      BGCNT_PRIORITY(3) | BGCNT_CHARBASE(2) | BGCNT_SCREENBASE(26) | BGCNT_16COLOR | BGCNT_TXT256x256);
            SetGpuReg(REG_OFFSET_BG1CNT,
                      BGCNT_PRIORITY(2) | BGCNT_CHARBASE(3) | BGCNT_SCREENBASE(27) | BGCNT_16COLOR | BGCNT_TXT256x256);
            SetGpuReg(REG_OFFSET_BG2CNT,
                      BGCNT_PRIORITY(1) | BGCNT_CHARBASE(0) | BGCNT_SCREENBASE(9) | BGCNT_256COLOR | BGCNT_AFF256x256);
...
static void Task_TitleScreenPhase2(u8 taskId) {
    ...
    if (gTasks[taskId].tCounter != 0) {
        ...
        SetGpuReg(REG_OFFSET_BLDY, 0);
        SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_MODE_1 | DISPCNT_OBJ_1D_MAP | DISPCNT_BG0_ON | DISPCNT_BG1_ON |
...
static void Task_TitleScreenPhase3(u8 taskId) {
    ...
    } else if ((gMain.heldKeys & RESET_RTC_BUTTON_COMBO) == RESET_RTC_BUTTON_COMBO && CanResetRTC() == TRUE) {
        FadeOutBGM(4);
