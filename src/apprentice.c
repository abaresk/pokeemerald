 ...
 * They ask the player a series of questions once per day, and eventually depart the lobby to be replaced by a new
 * Apprentice
 *
 * ## Initial Questions
 * The first question they always ask is a request to be taught, which cannot be rejected
 * The second question (which follows immediately after) is whether they should participate in Battle Tower Lv 50 or
 * Open Lv After these opening questions they always ask the player to choose between 2 mons, which they repeat 3 times
 *
 * ## Random Questions
 * After choosing 3 mons for them, the Apprentice will randomly ask between 1 and 8 questions of 4 different types, as
 * follows
 ...
 * Up to 4 old Apprentices are saved and can be encountered (or partnered with) during challenges of the mode they were
 * told to battle in They can also be record mixed to and from other Emerald games Old/record mixed Apprentices are
 * stored in struct Apprentice apprentices of SaveBlock2 and the current Apprentice is stored in struct
...
void BufferApprenticeChallengeText(u8 saveApprenticeId) {
    ...
    ConvertIntToDecimalStringN(gStringVar2, gSaveBlock2Ptr->apprentices[saveApprenticeId].number,
                               STR_CONV_MODE_RIGHT_ALIGN, i);
...
#define APPRENTICE_SPECIES_ID(speciesArrId, monId)                                                                 \
    speciesArrId = (PLAYER_APPRENTICE.speciesIds[monId] >> (((PLAYER_APPRENTICE.party >> monId) & 1) << 2)) & 0xF; \
    do {                                                                                                           \
    } while (0)
...
static u16 GetDefaultMove(u8 monId, u8 speciesArrayId, u8 moveSlot) {
    ...
    numQuestions = 0;
    for (i = 0; i < APPRENTICE_MAX_QUESTIONS && PLAYER_APPRENTICE.questions[i].questionId != QUESTION_ID_WIN_SPEECH;
         i++)
        numQuestions++;
    ...
    for (i = 0; i < numQuestions && i < CURRENT_QUESTION_NUM; i++) {
        if (PLAYER_APPRENTICE.questions[i].questionId == QUESTION_ID_WHICH_MOVE &&
            PLAYER_APPRENTICE.questions[i].monId == monId && PLAYER_APPRENTICE.questions[i].suggestedChange) {
...
static void InitQuestionData(void) {
    ...
    if (gSpecialVar_0x8005 == APPRENTICE_QUESTION_WHICH_MON) {
        ...
        if (PLAYER_APPRENTICE.questionsAnswered >= NUM_WHICH_MON_QUESTIONS &&
            ...
            gApprenticeQuestionData->moveId1 =
                GetDefaultMove(count, id1, PLAYER_APPRENTICE.questions[CURRENT_QUESTION_NUM].moveSlot);
...
static void ApprenticeBufferString(void) {
    ...
    switch (gSpecialVar_0x8006) {
        ...
        case APPRENTICE_BUFF_NAME:
            TVShowConvertInternationalString(text, GetApprenticeNameInLanguage(PLAYER_APPRENTICE.id, LANGUAGE_ENGLISH),
                                             LANGUAGE_ENGLISH);
...
static void TrySetApprenticeHeldItem(void) {
    ...
    for (count = 0, j = 0;
         j < APPRENTICE_MAX_QUESTIONS && PLAYER_APPRENTICE.questions[j].questionId != QUESTION_ID_WIN_SPEECH;
         count++, j++)
        ;
