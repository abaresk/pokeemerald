...
const struct BattleAnimBackground gBattleAnimBackgroundTable[] = {
    ...
    [BG_GHOST] = { gBattleAnimBgImage_Ghost,
        gBattleAnimBgPalette_Ghost,
        gBattleAnimBgTilemap_Ghost },
    [BG_PSYCHIC] = { gBattleAnimBgImage_Psychic,
        gBattleAnimBgPalette_Psychic,
    ...
    [BG_DRILL] = { gBattleAnimBgImage_Drill,
        gBattleAnimBgPalette_Drill,
        gBattleAnimBgTilemap_Drill },
    [BG_DRILL_CONTESTS] = { gBattleAnimBgImage_Drill,
        gBattleAnimBgPalette_Drill,
    ...
    [BG_THUNDER] = { gBattleAnimBgImage_Thunder,
        gBattleAnimBgPalette_Thunder,
    ...
    [BG_COSMIC] = { gBattleAnimBgImage_Cosmic,
        gBattleAnimBgPalette_Cosmic,
        gBattleAnimBgTilemap_Cosmic },
    [BG_IN_AIR] = { gBattleAnimBgImage_InAir,
        gBattleAnimBgPalette_InAir,
        gBattleAnimBgTilemap_InAir },
    [BG_SKY] = { gBattleAnimBgImage_Drill, gBattleAnimBgPalette_Sky, gBattleAnimBgTilemap_Drill },
    [BG_SKY_CONTESTS] = { gBattleAnimBgImage_Drill,
        gBattleAnimBgPalette_Sky,
        gBattleAnimBgTilemap_DrillContests },
    [BG_AURORA] = { gBattleAnimBgImage_Aurora,
        gBattleAnimBgPalette_Aurora,
        gBattleAnimBgTilemap_Aurora },
    [BG_FISSURE] = { gBattleAnimBgImage_Fissure,
        gBattleAnimBgPalette_Fissure,
        gBattleAnimBgTilemap_Fissure },
    [BG_BUG_OPPONENT] = { gBattleAnimBgImage_Highspeed,
        gBattleAnimBgPalette_Bug,
        gBattleAnimBgTilemap_HighspeedOpponent },
    [BG_BUG_PLAYER] = { gBattleAnimBgImage_Highspeed,
        gBattleAnimBgPalette_Bug,
        gBattleAnimBgTilemap_HighspeedPlayer },
    [BG_SOLARBEAM_OPPONENT] = { gBattleAnimBgImage_Impact,
        gBattleAnimBgPalette_Solarbeam,
        gBattleAnimBgTilemap_ImpactOpponent },
    [BG_SOLARBEAM_PLAYER] = { gBattleAnimBgImage_Impact,
        gBattleAnimBgPalette_Solarbeam,
        gBattleAnimBgTilemap_ImpactPlayer },
    [BG_SOLARBEAM_CONTESTS] = { gBattleAnimBgImage_Impact,
        gBattleAnimBgPalette_Solarbeam,
        gBattleAnimBgTilemap_ImpactContests },
};

static void (*const sScriptCmdTable[])(void) = { ScriptCmd_loadspritegfx,
    ...
    if (!IsContest())
    {
        ...
        for (i = 0; i < MAX_BATTLERS_COUNT; i++)
        {
            if (GetBattlerSide(i) != B_SIDE_PLAYER)
                gAnimBattlerSpecies[i] =
                    GetMonData(&gEnemyParty[gBattlerPartyIndexes[i]], MON_DATA_SPECIES);
            else
                gAnimBattlerSpecies[i] =
                    GetMonData(&gPlayerParty[gBattlerPartyIndexes[i]], MON_DATA_SPECIES);
        }
    }
    else
    {
...
static void ScriptCmd_end(void)
{
    ...
    if (gAnimVisualTaskCount != 0 || gAnimSoundTaskCount != 0 || sMonAnimTaskIdArray[0] != 0xFF ||
        sMonAnimTaskIdArray[1] != 0xFF)
    {
...
bool8 IsBattlerSpriteVisible(u8 battlerId)
{
    ...
    if (!gBattleSpritesDataPtr->battlerData[battlerId].invisible ||
...
void MoveBattlerSpriteToBG(u8 battlerId, bool8 toBG_2, bool8 setSpriteInvisible)
{
    ...
    if (!toBG_2)
    {
        ...
        gBattle_BG1_X =
            -(gSprites[battlerSpriteId].pos1.x + gSprites[battlerSpriteId].pos2.x) + 0x20;
        if (IsContest() && IsSpeciesNotUnown(gContestResources->moveAnim->species))
            gBattle_BG1_X--;

        gBattle_BG1_Y =
            -(gSprites[battlerSpriteId].pos1.y + gSprites[battlerSpriteId].pos2.y) + 0x20;
        ...
        CpuCopy32(&gPlttBufferUnfaded[0x100 + battlerId * 16],
            (void *)(BG_PLTT + animBg.paletteId * 32),
            0x20);
    ...
    else
    {
        ...
        sub_8118FBC(2,
            ...
            animBg.paletteId,
            animBg.bgTiles + 0x1000,
            animBg.bgTilemap + 0x400,
            animBg.tilesOffset);
    }
}
...
// Uses of this function that rely on a TRUE return are expecting inBattle to not be ticked as
// defined in contest behavior. As a result, if misused, this function cannot reliably discern
// between field and contest status and could result in undefined behavior.
...
static void LoadMoveBg(u16 bgId)
{
    if (IsContest())
    {
        ...
        LoadCompressedPalette(
            gBattleAnimBackgroundTable[bgId].palette, GetBattleBgPaletteNum() * 16, 32);
    }
    else
    {
...
{
    if (!IsContest() &&
        gBattleSpritesDataPtr->healthBoxesData[gBattleAnimAttacker].statusAnimActive)
    {
...
static void ScriptCmd_monbgprio_28(void)
{
    ...
    battlerPosition = GetBattlerPosition(battlerId);
    if (!IsContest() &&
        (battlerPosition == B_POSITION_PLAYER_LEFT || battlerPosition == B_POSITION_OPPONENT_RIGHT))
    {
...
static void ScriptCmd_monbgprio_2A(void)
{
    ...
    if (GetBattlerSide(gBattleAnimAttacker) != GetBattlerSide(gBattleAnimTarget))
    {
        ...
        battlerPosition = GetBattlerPosition(battlerId);
        if (!IsContest() && (battlerPosition == B_POSITION_PLAYER_LEFT ||
