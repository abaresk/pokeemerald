...
// Save status
enum { SAVE_IN_PROGRESS, SAVE_SUCCESS, SAVE_CANCELED, SAVE_ERROR };
...
static const u8* const sPyramindFloorNames[] = { gText_Floor1, gText_Floor2, gText_Floor3, gText_Floor4,
...
static const struct WindowTemplate sUnknown_085105AC[] = { { 0, 2, 0xF, 0x1A, 4, 0xF, 0x194 }, DUMMY_WIN_TEMPLATE };
...
static bool32 PrintStartMenuActions(s8* pIndex, u32 count) {
    ...
    do {
        if (sStartMenuItems[sCurrentStartMenuActions[index]].func.u8_void == StartMenuPlayerNameCallback) {
            PrintPlayerNameOnWindow(GetStartMenuWindowId(), sStartMenuItems[sCurrentStartMenuActions[index]].text, 8,
                                    (index << 4) + 9);
        } else {
...
static bool32 InitStartMenuStep(void) {
    ...
    switch (state) {
        ...
        case 5:
            sStartMenuCursorPos =
                sub_81983AC(GetStartMenuWindowId(), 1, 0, 9, 16, sNumStartMenuActions, sStartMenuCursorPos);
    ...
    if (!gPaletteFade.active) {
        switch (*state) {
            case 0:
                ...
                AddTextPrinterParameterized2(0, 1, gText_SavingDontTurnOffPower, TEXT_SPEED_FF, NULL,
                                             TEXT_COLOR_DARK_GREY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GREY);
