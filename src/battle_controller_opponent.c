...
static void (*const sOpponentBufferCommands[CONTROLLER_CMDS_COUNT])(
    void) = { OpponentHandleGetMonData,
...
static void sub_805F2F0(void)
{
    ...
    if (!IsDoubleBattle()
        || ((IsDoubleBattle() && (gBattleTypeFlags & BATTLE_TYPE_MULTI))
    ...
    if (var)
    {
        if (var2 == TRUE)
        {
            if (var2 && gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].field_1_x1
        ...
        else if (gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].field_1_x1)
        {
            ...
            {
                if (gBattleSpritesDataPtr->healthBoxesData[gActiveBattler ^ BIT_FLANK].flag_x80 == 0
                    && gBattleSpritesDataPtr->healthBoxesData[gActiveBattler ^ BIT_FLANK].field_1_x1
                           == 0)
                {
                    FreeSpriteTilesByTag(0x27F9);
...
static void sub_805F560(void)
{
    ...
    if (sp && r10)
    {
        if (IsDoubleBattle() && !(gBattleTypeFlags & BATTLE_TYPE_MULTI))
        {
            ...
            SetBattlerShadowSpriteCallback(gActiveBattler ^ BIT_FLANK,
...
{
    struct BattlePokemon battleMon;
    u8 *src = (u8 *)&gEnemyParty[gBattlerPartyIndexes[gActiveBattler]]
    ...
    else
    {
            ...
            switch (chosenMoveId)
            {
                ...
                default:
                    ...
                    BtlController_EmitTwoReturnValues(
                        1, 10, (chosenMoveId) | (gBattlerTarget << 8));
                    break;
            }
            OpponentBufferExecCompleted();
        }
        else
        {
            ...
            if (gBattleMoves[move].target & (MOVE_TARGET_USER_OR_SELECTED | MOVE_TARGET_USER))
                ...
                BtlController_EmitTwoReturnValues(
                    1, 10, (chosenMoveId) | (GetBattlerAtPosition(Random() & 2) << 8));
            else
                BtlController_EmitTwoReturnValues(
                    1, 10, (chosenMoveId) | (GetBattlerAtPosition(B_POSITION_PLAYER_LEFT) << 8));
