...
static const union AnimCmd gUnknown_086104B4[] = {
    ANIMCMD_FRAME(16, 4), ANIMCMD_FRAME(0, 4),  ANIMCMD_FRAME(32, 4), ANIMCMD_FRAME(0, 4),  ANIMCMD_FRAME(16, 4),
    ANIMCMD_FRAME(0, 4),  ANIMCMD_FRAME(32, 4), ANIMCMD_FRAME(0, 4),  ANIMCMD_FRAME(0, 32), ANIMCMD_FRAME(16, 8),
    ANIMCMD_FRAME(0, 8),  ANIMCMD_FRAME(32, 8), ANIMCMD_FRAME(0, 8),  ANIMCMD_FRAME(16, 8), ANIMCMD_FRAME(0, 8),
    ANIMCMD_FRAME(32, 8), ANIMCMD_FRAME(0, 8),  ANIMCMD_END,
};
...
static const union AffineAnimCmd gUnknown_0861050C[] = {
    AFFINEANIMCMD_FRAME(5, 5, 0, 0),  AFFINEANIMCMD_FRAME(0, 0, 0, 1),   AFFINEANIMCMD_FRAME(16, 5, 0, 0),
    AFFINEANIMCMD_FRAME(0, 0, 0, 1),  AFFINEANIMCMD_FRAME(32, 5, 0, 0),  AFFINEANIMCMD_FRAME(0, 0, 0, 1),
    AFFINEANIMCMD_FRAME(64, 5, 0, 0), AFFINEANIMCMD_FRAME(0, 0, 0, 1),   AFFINEANIMCMD_FRAME(128, 5, 0, 0),
    AFFINEANIMCMD_FRAME(0, 0, 0, 1),  AFFINEANIMCMD_FRAME(256, 5, 0, 0), AFFINEANIMCMD_END,
};

static const union AffineAnimCmd gUnknown_0861056C[] = {
    AFFINEANIMCMD_FRAME(128, 5, 0, 0), AFFINEANIMCMD_FRAME(0, 0, 0, 1),
    AFFINEANIMCMD_FRAME(64, 5, 0, 0),  AFFINEANIMCMD_FRAME(0, 0, 0, 1),
    AFFINEANIMCMD_FRAME(32, 5, 0, 0),  AFFINEANIMCMD_FRAME(0, 0, 0, 1),
    AFFINEANIMCMD_FRAME(16, 5, 0, 0),  AFFINEANIMCMD_FRAME(0, 0, 0, 1),
    AFFINEANIMCMD_FRAME(5, 5, 0, 0),   AFFINEANIMCMD_END,
};
...
static const struct SwapActionIdAndFunc sSwap_EnemyScreenActions[] = {
    { 1, Swap_ActionMon },         { 1, Swap_ActionMon },    { 1, Swap_ActionMon },
    { 2, Swap_ActionPkmnForSwap }, { 3, Swap_ActionCancel },
};
...
static void CreateFrontierFactorySelectableMons(u8 firstMonId) {
    ...
    for (i = 0; i < SELECTABLE_MONS_COUNT; i++) {
        ...
        CreateMonWithEVSpreadNatureOTID(&sFactorySelectScreen->mons[i + firstMonId].monData,
                                        gFacilityTrainerMons[monId].species, level, gFacilityTrainerMons[monId].nature,
                                        ivs, gFacilityTrainerMons[monId].evSpread, otId);
        happiness = 0;
        for (j = 0; j < MAX_MON_MOVES; j++)
            SetMonMoveAvoidReturn(&sFactorySelectScreen->mons[i + firstMonId].monData,
                                  gFacilityTrainerMons[monId].moves[j], j);
        SetMonData(&sFactorySelectScreen->mons[i + firstMonId].monData, MON_DATA_FRIENDSHIP, &happiness);
        SetMonData(&sFactorySelectScreen->mons[i + firstMonId].monData, MON_DATA_HELD_ITEM,
    ...
    switch (task->data[0]) {
        case 0:
            ...
            SetGpuReg(REG_OFFSET_WINOUT,
                      WINOUT_WIN01_BG0 | WINOUT_WIN01_BG1 | WINOUT_WIN01_BG2 | WINOUT_WIN01_CLR | WINOUT_WIN01_OBJ);
            break;
        case 1:
            ShowBg(3);
            SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_TGT1_BG3 | BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_BG1 | BLDCNT_TGT2_OBJ);
...
static void Swap_InitAllSprites(void) {
    ...
    sFactorySwapScreen->cursorSpriteId = CreateSprite(
        &gUnknown_0861084C, gSprites[sFactorySwapScreen->ballSpriteIds[sFactorySwapScreen->cursorPos]].pos1.x, 88, 0);
...
static void Swap_ShowSummaryMonSprite(void) {
    ...
    sFactorySwapScreen->unk2C.field0 = CreateMonPicSprite_HandleDeoxys(
        species, personality, otId, TRUE, 88, 32, 15, 0xFFFF); // BUG: otId and personality should be switched.
