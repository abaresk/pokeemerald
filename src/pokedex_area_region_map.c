...
void LoadPokedexAreaMapGfx(const struct PokedexAreaMapTemplate* template) {
    ...
    if (mode == 0) {
        ...
        sub_8199D3C(DecompressAndCopyTileDataToVram(template->bg, sPokedexAreaMap_Tilemap, 0, 0, 1), template->offset,
                    32, 32, FALSE);
    } else {
        ...
        sub_8199D3C(DecompressAndCopyTileDataToVram(template->bg, sPokedexAreaMapAffine_Tilemap, 0, 0, 1),
                    template->offset, 64, 64, TRUE);
    }
