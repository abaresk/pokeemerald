...
static void (*const sMachBikeTransitions[])(u8) = {
    MachBikeTransition_FaceDirection, // Face vs Turn: Face has no anim while Turn does. Turn checks for collision
                                      // because if you turn right as opposed to face right, if there is a wall there,
                                      // turn will make a bonk sound effect while face will not.
    MachBikeTransition_TurnDirection,
    MachBikeTransition_TrySpeedUp,
    MachBikeTransition_TrySlowDown,
};

// bikeFrameCounter is input which is represented by sMachBikeSpeeds in order: 0 is normal speed (1 speed), 1 is fast
// speed (2 speed), 2 is fastest speed (4 speed)
...
static u8 (*const sAcroBikeInputHandlers[])(u8*, u16, u16) = {
    AcroBikeHandleInputNormal,   AcroBikeHandleInputTurning,       AcroBikeHandleInputWheelieStanding,
    AcroBikeHandleInputBunnyHop, AcroBikeHandleInputWheelieMoving, AcroBikeHandleInputSidewaysJump,
...
// this is a list of timers to compare against later, terminated with 0. the only timer being compared against is 4
// frames in this list.
static const u8 sAcroBikeJumpTimerList[] = { 4, 0 };

// this is a list of history inputs to do in order to do the check to retrieve a jump direction for acro bike. it seems
// to be an extensible list, so its possible that Game Freak may have intended for the Acro Bike to have more complex
// tricks at some point. The final list only has the acro jump.
static const struct BikeHistoryInputInfo sAcroBikeTricksList[] = {
    // the 0xF is a mask performed with each byte of the array in order to perform the check on only the last entry of
    // the history list, otherwise the check wouldn't work as there can be 0xF0 as opposed to 0x0F.
...
static u8 GetMachBikeTransition(u8* dirTraveling) {
    ...
    if (*dirTraveling == 0) {
        *dirTraveling =
            direction; // update the direction, since below we either faced a direction or we started moving.
    ...
    // we need to check if the last traveled direction changed from the new direction as well as ensuring that we dont
    // update the state while the player is moving: see the else check.
...
// the difference between face direction and turn direction is that one changes direction while the other does the
// animation of turning as well as changing direction.
...
static void MachBikeTransition_TrySpeedUp(u8 direction) {
    ...
    if (CanBikeFaceDirOnMetatile(direction, playerObjEvent->currentMetatileBehavior) == FALSE) {
        ...
        if (collision > 0 && collision < COLLISION_VERTICAL_RAIL) {
            ...
            // we did not hit anything that can slow us down, so perform the advancement callback depending on the
            // bikeFrameCounter and try to increase the mach bike's speed.
            sMachBikeSpeedCallbacks[gPlayerAvatar.bikeFrameCounter](direction);
            gPlayerAvatar.bikeSpeed =
                gPlayerAvatar.bikeFrameCounter +
                (gPlayerAvatar.bikeFrameCounter >> 1); // same as dividing by 2, but compiler is insistent on >> 1
...
static u8 AcroBikeHandleInputTurning(u8* newDirection, u16 newKeys, u16 heldKeys) {
    ...
    *newDirection = gPlayerAvatar.newDirBackup;
    ...
    if (*newDirection == AcroBike_GetJumpDirection()) {
        Bike_SetBikeStill(); // Bike_SetBikeStill sets speed to standing, but the next line immediately overrides it.
                             // could have just reset acroBikeState to 0 here instead of wasting a jump.
...
static void AcroBike_TryHistoryUpdate(u16 newKeys, u16 heldKeys) // newKeys is unused
{
    ...
    direction =
        heldKeys & (A_BUTTON | B_BUTTON | SELECT_BUTTON | START_BUTTON); // directions is reused for some reason.
...
static u8 AcroBike_GetJumpDirection(void) {
    ...
    for (i = 0; i < ARRAY_COUNT(sAcroBikeTricksList); i++) {
        ...
        if (dirHistory == historyInputInfo->dirHistoryMatch &&
            ...
            HasPlayerInputTakenLongerThanList(historyInputInfo->dirTimerHistoryList,
...
void Bike_UpdateBikeCounterSpeed(u8 counter) {
    ...
    gPlayerAvatar.bikeSpeed =
        gPlayerAvatar.bikeFrameCounter + (gPlayerAvatar.bikeFrameCounter >> 1); // lazy way of multiplying by 1.5.
}
...
s16 GetPlayerSpeed(void) {
    // because the player pressed a direction, it won't ever return a speed of 0 since this function returns the
