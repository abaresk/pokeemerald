...
static const u8 gUnknown_0858CF94[][4] = {
    { 1, 0, 3, 2 }, { 3, 0, 1, 2 }, { 2, 0, 3, 1 }, { 1, 3, 0, 2 }, { 2, 3, 0, 1 },
...
static void PrepareExchangePacket(void) {
    ...
    if (Link_AnyPartnersPlayingRubyOrSapphire()) {
        ...
        memcpy(&sSentRecord->emerald.battleTowerRecord, sBattleTowerSave,
...
static void ReceiveExchangePacket(u32 which) {
    if (Link_AnyPartnersPlayingRubyOrSapphire()) {
        ...
        ReceiveDaycareMailData(&sReceivedRecords->ruby.dayCareMail, sizeof(struct PlayerRecordsRS), which,
        ...
        ReceiveDaycareMailData(&sReceivedRecords->emerald.dayCareMail, sizeof(struct PlayerRecordsEmerald), which,
                               sReceivedRecords->emerald.tvShows);
        ReceiveBattleTowerData(&sReceivedRecords->emerald.battleTowerRecord, sizeof(struct PlayerRecordsEmerald),
                               which);
        ...
        ReceiveRankingHallRecords(&sReceivedRecords->emerald.hallRecords, sizeof(struct PlayerRecordsEmerald),
                                  (u8)which);
    }
}
...
static void Task_CopyReceiveBuffer(u8 taskId) {
    ...
    if (status == GetLinkPlayerCountAsBitFlags()) {
        ...
        for (i = 0; i < GetLinkPlayerCount(); i++) {
            ...
            if ((status >> i) & 1) {
                dest = LoadPtrFromTaskData((u16*)&task->data[5]) + task->data[i + 1] * BUFFER_CHUNK_SIZE +
                       sRecordStructSize * i;
...
static void ReceiveBattleTowerData(void* battleTowerRecord, size_t recordSize, u8 which) {
    ...
    ShufflePlayerIndices(mixIndices);
    if (Link_AnyPartnersPlayingRubyOrSapphire()) {
        if (RubyBattleTowerRecordToEmerald((void*)battleTowerRecord + recordSize * mixIndices[which],
        ...
        memcpy((void*)battleTowerRecord + recordSize * which, (void*)battleTowerRecord + recordSize * mixIndices[which],
...
static void sub_80E8578(struct RecordMixingHallRecords* dst, void* hallRecords, size_t recordSize, u32 arg3,
                        s32 linkPlayerCount) {
    s32 i, j, k, l;
    s32 var_68;
