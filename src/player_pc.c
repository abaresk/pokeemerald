...
static const struct MenuAction sPlayerPCMenuActions[] = { { gText_ItemStorage, PlayerPC_ItemStorage },
                                                          { gText_Mailbox, PlayerPC_Mailbox },
                                                          { gText_Decoration, PlayerPC_Decoration },
                                                          { gText_TurnOff, PlayerPC_TurnOff } };

static const u8 gBedroomPC_OptionOrder[] = { PLAYERPC_MENU_ITEMSTORAGE, PLAYERPC_MENU_MAILBOX, PLAYERPC_MENU_DECORATION,
                                             PLAYERPC_MENU_TURNOFF };

static const u8 gPlayerPC_OptionOrder[] = { PLAYERPC_MENU_ITEMSTORAGE, PLAYERPC_MENU_MAILBOX, PLAYERPC_MENU_TURNOFF };

static const struct MenuAction gPCText_ItemPCOptionsText[] = { { gText_WithdrawItem, ItemStorage_Withdraw },
...
const struct MenuAction gMailboxMailOptions[] = { { gText_Read, Mailbox_DoMailRead },
...
static const struct ListMenuTemplate gUnknown_085DFF44 = { .items = NULL,
...
static const struct WindowTemplate gUnknown_085DFF5C[5] = {
    { .bg = 0, .tilemapLeft = 16, .tilemapTop = 1, .width = 13, .height = 18, .paletteNum = 15, .baseBlock = 0x0001 },
    { .bg = 0, .tilemapLeft = 1, .tilemapTop = 13, .width = 13, .height = 6, .paletteNum = 15, .baseBlock = 0x00EB },
    { .bg = 0, .tilemapLeft = 1, .tilemapTop = 8, .width = 3, .height = 3, .paletteNum = 15, .baseBlock = 0x0153 },
    { .bg = 0, .tilemapLeft = 1, .tilemapTop = 1, .width = 13, .height = 2, .paletteNum = 15, .baseBlock = 0x0139 },
    { .bg = 0, .tilemapLeft = 8, .tilemapTop = 9, .width = 6, .height = 2, .paletteNum = 15, .baseBlock = 0x015C }
};

static const struct WindowTemplate gUnknown_085DFF84 = {
    .bg = 0, .tilemapLeft = 9, .tilemapTop = 7, .width = 5, .height = 4, .paletteNum = 15, .baseBlock = 0x0168
};
...
void NewGameInitPCItems(void) {
    ...
    for (i = 0, ClearItemSlots(gSaveBlock1Ptr->pcItems, ARRAY_COUNT(gSaveBlock1Ptr->pcItems));
         NEW_GAME_PC_ITEMS(i, PC_ITEM_ID) && NEW_GAME_PC_ITEMS(i, PC_QUANTITY) &&
...
static void PlayerPC_TurnOff(u8 taskId) {
    if (gPcItemMenuOptionsNum ==
        4) // if the option count is 4, we are at the bedroom PC, so do gender specific handling.
    {
...
static void Mailbox_DrawMailboxMenu(u8 taskId) {
    ...
    sub_81D1C84(1);
    AddTextPrinterParameterized(windowId, 1, gText_Mailbox, GetStringCenterAlignXOffset(1, gText_Mailbox, 0x40), 1, 0,
                                NULL);
    ScheduleBgCopyTilemapToVram(0);
...
static void Mailbox_PrintWhatToDoWithPlayerMailText(u8 taskId) {
    StringCopy(gStringVar1,
               gSaveBlock1Ptr->mail[playerPCItemPageInfo.itemsAbove + 6 + playerPCItemPageInfo.cursorPos].playerName);
...
static void Mailbox_FadeAndReadMail(u8 taskId) {
    if (!gPaletteFade.active) {
        sub_81D1EC0();
        CleanupOverworldWindowsAndTilemaps();
        ReadMail(&(gSaveBlock1Ptr->mail[playerPCItemPageInfo.itemsAbove + 6 + playerPCItemPageInfo.cursorPos]),
                 Mailbox_ReturnToFieldFromReadMail, TRUE);
        DestroyTask(taskId);
    }
}
...
static void Mailbox_DoMailMoveToBag(u8 taskId) {
    ...
    if (!AddBagItem(mailStruct->itemId, 1)) {
        ...
        if (playerPCItemPageInfo.count < (playerPCItemPageInfo.pageItems + playerPCItemPageInfo.itemsAbove) &&
            playerPCItemPageInfo.itemsAbove != 0)
...
static void Mailbox_UpdateMailListAfterDeposit(void) {
    ...
    playerPCItemPageInfo.count = GetMailboxMailCount();
    Mailbox_UpdateMailList();
    if (prevCount != playerPCItemPageInfo.count &&
        (playerPCItemPageInfo.count < (playerPCItemPageInfo.pageItems + playerPCItemPageInfo.itemsAbove)) &&
        playerPCItemPageInfo.itemsAbove != 0)
        playerPCItemPageInfo.itemsAbove--;
    ItemStorage_SetItemAndMailCount(taskId);
    LoadMessageBoxAndBorderGfx();
...
static void fish4_goto_x5_or_x6(u8 windowId, s32 id, u8 yOffset) {
    if (id != -2) {
        ...
        AddTextPrinterParameterized(windowId, 7, gStringVar4, GetStringRightAlignXOffset(7, gStringVar4, 104), yOffset,
                                    0xFF, NULL);
    }
}
...
static void ItemStorage_StartScrollIndicator(void) {
    if (playerPCItemPageInfo.scrollIndicatorId == 0xFF)
        playerPCItemPageInfo.scrollIndicatorId = AddScrollIndicatorArrowPairParameterized(
            SCROLL_ARROW_UP, 0xB0, 0xC, 0x94, playerPCItemPageInfo.count - playerPCItemPageInfo.pageItems, 0x13F8,
            0x13F8, &(playerPCItemPageInfo.itemsAbove));
}
...
static void sub_816BFE0(u8 y, u8 b, u8 speed) {
    ...
    if (b == 0xFF)
        FillWindowPixelRect(windowId, PIXEL_FILL(1), 0, y, GetMenuCursorDimensionByFont(1, 0),
...
static void sub_816C140(void) {
    sub_812225C(&(playerPCItemPageInfo.itemsAbove), &(playerPCItemPageInfo.cursorPos), playerPCItemPageInfo.pageItems,
                playerPCItemPageInfo.count);
}
...
static void ItemStorage_HandleRemoveItem(u8 taskId) {
    ...
    if (gMain.newKeys & (A_BUTTON | B_BUTTON)) {
        ...
        ItemStorage_RefreshListMenu();
        data[5] =
            ListMenuInit(&gMultiuseListMenuTemplate, playerPCItemPageInfo.itemsAbove, playerPCItemPageInfo.cursorPos);
        ItemStorage_StartScrollIndicatorAndProcessInput(taskId);
    }
}
