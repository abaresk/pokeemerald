...
static s32 ListMenuHandler_AllItemsAvailable(u8*, u8*, u8*, const struct WindowTemplate*,
                                             const struct ListMenuTemplate*);
static s32 TradeBoardMenuHandler(u8*, u8*, u8*, u8*, const struct WindowTemplate*, const struct ListMenuTemplate*,
...
static void PrintNumPlayersWaitingForMsg(u8 windowId, u8 capacityCode, u8 stringId) {
    ...
    switch (capacityCode << 8) {
        case LINK_GROUP_CAPACITY(0, 2):
            UR_AddTextPrinterParameterized(windowId, 1, sPlayersNeededOrModeTexts[0][stringId - 1], 0, 1,
                                           UR_COLOR_DKE_WHT_LTE);
            break;
        case LINK_GROUP_CAPACITY(0, 4):
            UR_AddTextPrinterParameterized(windowId, 1, sPlayersNeededOrModeTexts[1][stringId - 1], 0, 1,
                                           UR_COLOR_DKE_WHT_LTE);
            break;
        case LINK_GROUP_CAPACITY(2, 5):
            UR_AddTextPrinterParameterized(windowId, 1, sPlayersNeededOrModeTexts[2][stringId - 1], 0, 1,
                                           UR_COLOR_DKE_WHT_LTE);
            break;
        case LINK_GROUP_CAPACITY(3, 5):
            UR_AddTextPrinterParameterized(windowId, 1, sPlayersNeededOrModeTexts[3][stringId - 1], 0, 1,
                                           UR_COLOR_DKE_WHT_LTE);
            break;
        case LINK_GROUP_CAPACITY(2, 4):
            UR_AddTextPrinterParameterized(windowId, 1, sPlayersNeededOrModeTexts[4][stringId - 1], 0, 1,
                                           UR_COLOR_DKE_WHT_LTE);
            break;
    }
    ...
    switch (data->state) {
        ...
        case LL_STATE_AWAIT_PLAYERS:
            ...
            if (GROUP_MIN(sPlayerActivityGroupSize) != 0 &&
                ...
                GROUP_MAX(sPlayerActivityGroupSize) != 0 && sub_8012240() && gMain.newKeys & START_BUTTON) {
            ...
            if (val == 1) // Send complete
            {
                if (data->joinRequestAnswer == RFU_STATUS_JOIN_GROUP_OK) {
                    ...
                    if (data->playerCount == GROUP_MAX(sPlayerActivityGroupSize)) {
                        ...
                        LinkRfu_StopManagerAndFinalizeSlots();
                        PrintNumPlayersWaitingForMsg(data->nPlayerModeWindowId, sPlayerActivityGroupSize,
                                                     data->playerCount);
                    } else {
                        data->state = LL_STATE_GET_AWAITING_PLAYERS_TEXT;
                    }
        ...
        case LL_STATE_CONFIRMED_MEMBERS:
            if (!Leader_SetStateIfMemberListChanged(data, LL_STATE_ACCEPT_NEW_MEMBER_PROMPT,
...
static void Task_StartActivity(u8 taskId) {
    ...
    switch (gPlayerCurrActivity) {
        ...
        case ACTIVITY_BATTLE_SINGLE:
            ...
            SavePlayerParty();
            LoadPlayerBag();
            WarpForCableClubActivity(MAP_GROUP(BATTLE_COLOSSEUM_2P), MAP_NUM(BATTLE_COLOSSEUM_2P), 6, 8,
                                     USING_SINGLE_BATTLE);
        ...
        case ACTIVITY_BATTLE_MULTI:
            ...
            LoadPlayerBag();
            CreateTrainerCardInBuffer(gBlockSendBuffer, TRUE);
            WarpForCableClubActivity(MAP_GROUP(BATTLE_COLOSSEUM_4P), MAP_NUM(BATTLE_COLOSSEUM_4P), 5, 8,
                                     USING_MULTI_BATTLE);
            SetMainCallback2(CB2_TransitionToCableClub);
            break;
        case ACTIVITY_TRADE:
...
static void Task_CardOrNewsOverWireless(u8 taskId) {
    ...
    switch (data->state) {
        ...
        case 2:
            ...
            ClearUnkStruct_x20Array(data->field_0->arr, 16);
            data->listenTaskId =
                CreateTask_ListenForPartnersWithSerial7F7D(data->field_4, data->isWonderNews + LINK_GROUP_WONDER_CARD);
                    ...
                    if (data->refreshTimer > 120) {
                        if (data->field_0->arr[0].groupScheduledAnim == UNION_ROOM_SPAWN_IN &&
                            ...
                            if (HasWonderCardOrNewsByLinkGroup(&data->field_0->arr[0].gname_uname.gname,
                                ...
                                CreateTask_RfuReconnectWithParent(
                                    data->field_0->arr[0].gname_uname.playerName,
                                    ReadAsU16(data->field_0->arr[0].gname_uname.gname.unk_00.playerTrainerId));
                                PlaySE(SE_POKENAV_ON);
                                data->state = 4;
                            } else {
                                PlaySE(SE_BOO);
...
static void Task_RunUnionRoom(u8 taskId) {
    ...
    switch (uroom->state) {
        case UR_STATE_INIT:
            ...
            uroom->searchTaskId =
                CreateTask_SearchForChildOrParent(uroom->field_C, uroom->field_4, LINK_GROUP_UNION_ROOM_RESUME);
        ...
        case UR_STATE_INIT_LINK:
            ...
            SetTradeBoardRegisteredMonInfo(sUnionRoomTrade.type, sUnionRoomTrade.playerSpecies,
                ...
                switch (sUnionRoomTrade.state) {
                    case URTRADE_STATE_REGISTERING:
                        ...
                        if (id >= PARTY_SIZE) {
                            ...
                            ScheduleFieldMessageWithFollowupState(UR_STATE_REGISTER_REQUEST_TYPE,
                ...
                switch (HandlePlayerListUpdate()) {
                    ...
                    case 4:
                        ...
                        UpdateGameData_SetActivity(ACTIVITY_NPCTALK | IN_UNION_ROOM,
                                                   GetActivePartnerSpriteGenderParam(uroom), FALSE);
                        break;
                }
            ...
            switch (UnionRoomGetPlayerInteractionResponse(uroom->field_0, taskData[0], taskData[1], playerGender)) {
                ...
                case 1: // Link communicating
                    sub_8012188(uroom->field_0->arr[taskData[1]].gname_uname.playerName,
                                &uroom->field_0->arr[taskData[1]].gname_uname.gname, gPlayerCurrActivity);
            ...
            switch (RfuGetStatus()) {
                ...
                case RFU_STATUS_CONNECTION_ERROR:
                    if (IsUnionRoomListenTaskActive() == TRUE)
                        ...
                        ScheduleFieldMessageWithFollowupState(UR_STATE_CANCEL_ACTIVITY_LINK_ERROR,
        ...
        case UR_STATE_COMMUNICATING_WAIT_FOR_DATA:
            if (!FuncIsActiveTask(Task_ExchangeCards)) {
                if (gPlayerCurrActivity == (ACTIVITY_TRADE | IN_UNION_ROOM))
                    ScheduleFieldMessageWithFollowupState(UR_STATE_SEND_TRADE_REQUST,
        ...
        case UR_STATE_DO_SOMETHING_PROMPT:
            ...
            playerGender = GetUnionRoomPlayerGender(taskData[1], uroom->field_0);
            ScheduleFieldMessageWithFollowupState(UR_STATE_HANDLE_DO_SOMETHING_PROMPT_INPUT,
                ...
                if (!gReceivedRemoteLinkPlayers) {
                    ...
                    if (input == -2 || input == IN_UNION_ROOM) {
                        ...
                        if (gPlayerCurrActivity == (ACTIVITY_BATTLE_SINGLE | IN_UNION_ROOM) &&
                            ...
                            ScheduleFieldMessageWithFollowupState(UR_STATE_DO_SOMETHING_PROMPT,
        ...
        case UR_STATE_REGISTER_PROMPT_HANDLE_INPUT:
            input = ListMenuHandler_AllItemsAvailable(
                &uroom->textState, &uroom->tradeBoardSelectWindowId, &uroom->tradeBoardDetailsWindowId,
                &sWindowTemplate_RegisterForTrade, &sListMenuTemplate_RegisterForTrade);
            if (input != -1) {
                if (input == -2 || input == 3) {
                    uroom->state = UR_STATE_MAIN;
                    HandleCancelActivity(TRUE);
                    ...
                    switch (input) {
                        case 1: // REGISTER
                            ScheduleFieldMessageWithFollowupState(UR_STATE_REGISTER_SELECT_MON_FADE,
                                                                  sText_WhichMonWillYouOffer);
                            break;
                        case 2: // INFO
                            ScheduleFieldMessageWithFollowupState(UR_STATE_REGISTER_PROMPT_HANDLE_INPUT,
        ...
        case UR_STATE_REGISTER_REQUEST_TYPE:
            input = ListMenuHandler_AllItemsAvailable(&uroom->textState, &uroom->tradeBoardSelectWindowId,
                                                      &uroom->tradeBoardDetailsWindowId, &gUnknown_082F0294,
        ...
        case UR_STATE_TRADING_BOARD_HANDLE_INPUT:
            input = TradeBoardMenuHandler(&uroom->textState, &uroom->tradeBoardSelectWindowId,
                                          &uroom->tradeBoardListMenuId, &uroom->tradeBoardDetailsWindowId,
                                          &gUnknown_082F034C, &sTradeBoardListMenuTemplate, uroom->field_0);
            if (input != -1) {
                switch (input) {
                    case -2:
                    case 8: // EXIT
                    ...
                    default:
                        UR_ClearBg0();
                        switch (
                            IsRequestedTypeOrEggInPlayerParty(uroom->field_0->arr[input].gname_uname.gname.type,
                                                              uroom->field_0->arr[input].gname_uname.gname.species)) {
                            case UR_TRADE_MATCH:
                                IntlConvPartnerUname7(gStringVar1, &uroom->field_0->arr[input]);
                                ScheduleFieldMessageWithFollowupState(UR_STATE_TRADE_PROMPT,
                            ...
                            case UR_TRADE_NOTYPE:
                                ...
                                ScheduleFieldMessageWithFollowupState(UR_STATE_TRADING_BOARD_LOAD,
        ...
        case UR_STATE_TRADE_SELECT_MON:
            if (PrintOnTextbox(&uroom->textState, sText_WhichMonWillYouOffer)) {
                ...
                memcpy(&gPartnerTgtGnameSub, &uroom->field_0->arr[taskData[1]].gname_uname.gname.unk_00,
...
static bool32 UnionRoom_HandleContactFromOtherPlayer(struct WirelessLink_URoom* uroom) {
    if (uroom->recvActivityRequest[0] != 0) {
        s32 id = GetChatLeaderActionRequestMessage(gStringVar4, gLinkPlayers[1].gender, &uroom->recvActivityRequest[0],
                                                   uroom);
        if (id == 0) // Error
        {
            return TRUE;
                        ...
                        for (i = 0; i < PLAYER_NAME_LENGTH + 1; i++) {
                            if (data->field_0->arr[i].groupScheduledAnim == UNION_ROOM_SPAWN_IN) {
                                ...
                                if (PlayerHasMetTrainerBefore(
                                        ReadAsU16(data->field_0->arr[i].gname_uname.gname.unk_00.playerTrainerId),
                                        text)) {
        ...
        if (data->field_0->arr[j].groupScheduledAnim != UNION_ROOM_SPAWN_NONE) {
            ...
            if (i != 0xFF) {
                if (data->field_0->arr[j].groupScheduledAnim == UNION_ROOM_SPAWN_IN) {
                    if (AreUnionRoomPlayerGnamesDifferent(&data->field_0->arr[j].gname_uname,
                                                          &data->field_4->arr[i].gname_uname)) {
...
static u8 CreateTask_SearchForChildOrParent(struct UnkStruct_Main4* main4_parent, struct UnkStruct_Main4* main4_child,
                                            u32 linkGroup) {
...
static s32 ListMenuHandler_AllItemsAvailable(u8* state, u8* windowId, u8* listMenuId,
...
static s32 TradeBoardMenuHandler(u8* state, u8* windowId, u8* listMenuId, u8* tradeBoardWindowId,
                                 const struct WindowTemplate* winTemplate, const struct ListMenuTemplate* menuTemplate,
...
static bool32 AreUnionRoomPlayerGnamesDifferent(struct WirelessGnameUnamePair* pair1,
                                                struct WirelessGnameUnamePair* pair2) {
    s32 i;
...
static void PrintUnionRoomGroupOnWindow(u8 windowId, u8 x, u8 y, struct UnkStruct_x20* group, u8 colorIdx, u8 id) {
    ...
    if (group->groupScheduledAnim == UNION_ROOM_SPAWN_IN && !(activity & IN_UNION_ROOM)) {
        ...
        UR_AddTextPrinterParameterized(windowId, 1, gStringVar4, GetStringRightAlignXOffset(1, gStringVar4, 0x88), y,
                                       colorIdx);
    }
}

static void PrintGroupMemberCandidateOnWindowWithColor(u8 windowId, u8 x, u8 y, struct UnkStruct_x20* group,
                                                       u8 colorIdx, u8 id) {
    u8 trainerId[6];
    ...
    if (group->groupScheduledAnim == UNION_ROOM_SPAWN_IN) {
        ...
        StringCopy(gStringVar4, sText_ID);
        StringAppend(gStringVar4, trainerId);
        UR_AddTextPrinterParameterized(windowId, 1, gStringVar4, GetStringRightAlignXOffset(1, gStringVar4, 0x68), y,
                                       colorIdx);
    }
}
...
static s32 UnionRoomGetPlayerInteractionResponse(struct UnkStruct_Main0* main0, bool8 overrideGender, u8 playerIdx,
                                                 u32 playerGender) {
    bool32 metBefore;
        ...
        switch (r5->gname_uname.gname.activity & 0x3F) {
            case ACTIVITY_BATTLE_SINGLE:
                StringExpandPlaceholders(
                    gStringVar4, sBattleReactionTexts[playerGender][Random() % ARRAY_COUNT(sBattleReactionTexts[0])]);
                break;
            case ACTIVITY_TRADE:
            ...
            case ACTIVITY_CHAT:
                StringExpandPlaceholders(
                    gStringVar4, sChatReactionTexts[playerGender][Random() % ARRAY_COUNT(sChatReactionTexts[0])]);
                break;
            case ACTIVITY_CARD:
                StringExpandPlaceholders(
                    gStringVar4,
                    sTrainerCardReactionTexts[playerGender][Random() % ARRAY_COUNT(sTrainerCardReactionTexts[0])]);
                break;
            default:
...
static s32 GetChatLeaderActionRequestMessage(u8* dst, u32 gender, u16* activityData, struct WirelessLink_URoom* uroom) {
    ...
    switch (activityData[0]) {
        ...
        case ACTIVITY_CHAT | IN_UNION_ROOM:
            ...
            break;
        case ACTIVITY_TRADE | IN_UNION_ROOM:
            ConvertIntToDecimalStringN(uroom->activityRequestStrbufs[0], sUnionRoomTrade.playerLevel,
            ...
            for (i = 0; i < RFU_CHILD_MAX; i++) {
                if (gRfuLinkStatus->partner[i].serialNo == 2) {
                    ConvertIntToDecimalStringN(uroom->activityRequestStrbufs[2], activityData[2],
                                               STR_CONV_MODE_LEFT_ALIGN, 3);
...
static void ViewURoomPartnerTrainerCard(u8* unused, struct WirelessLink_URoom* data, bool8 isParent) {
    ...
    ConvertIntToDecimalStringN(data->trainerCardStrBuffer[2], trainerCard->caughtMonsCount, STR_CONV_MODE_LEFT_ALIGN,
                               3);
    ...
    ConvertIntToDecimalStringN(data->trainerCardStrBuffer[4], trainerCard->playTimeMinutes, STR_CONV_MODE_LEADING_ZEROS,
                               2);
