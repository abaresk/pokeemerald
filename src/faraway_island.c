...
// When the player enters Faraway Island interior it begins a "hide and seek" minigame where Mew disappears into the
// grass This function returns the direction Mew will take a step, and is run every time the player takes a step
...
void SetMewAboveGrass(void) {
    ...
    if (gSpecialVar_0x8004 == 1) {
        ...
        sGrassSpriteId = CreateSpriteAtEnd(gFieldEffectObjectTemplatePointers[FLDEFFOBJ_LONG_GRASS], x, y,
                                           gSprites[mew->spriteId].subpriority - 1);
