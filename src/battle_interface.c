...
static u8 *AddTextPrinterAndCreateWindowOnHealthbox(
    const u8 *str, u32 x, u32 y, u32 bgColor, u32 *windowId);
...
static s32 CalcNewBarValue(
    s32 maxValue, s32 currValue, s32 receivedValue, s32 *arg3, u8 arg4, u16 arg5);
static u8 GetScaledExpFraction(s32 currValue, s32 receivedValue, s32 maxValue, u8 scale);
static void MoveBattleBarGraphically(u8 battlerId, u8 whichBar);
static u8 CalcBarFilledPixels(
    s32 maxValue, s32 oldValue, s32 receivedValue, s32 *currValue, u8 *arg4, u8 scale);
...
static const struct SpriteTemplate sHealthboxSafariSpriteTemplate = { .tileTag =
...
static const struct Subsprite sUnknown_0832C220[] = { { .x = 240,
                                                          .y = 0,
                                                          .shape = SPRITE_SHAPE(64x32),
                                                          .size = SPRITE_SIZE(64x32),
                                                          .tileOffset = 0,
...
static const struct Subsprite sUnknown_0832C234[] = { { .x = 240,
                                                          .y = 0,
                                                          .shape = SPRITE_SHAPE(64x32),
                                                          .size = SPRITE_SIZE(64x32),
                                                          .tileOffset = 64,
...
static const struct Subsprite sUnknown_0832C248[] = { { .x = 240,
                                                          .y = 0,
                                                          .shape = SPRITE_SHAPE(64x32),
                                                          .size = SPRITE_SIZE(64x32),
                                                          .tileOffset = 0,
...
static const struct Subsprite sUnknown_0832C250[] = { { .x = 240,
                                                          .y = 0,
                                                          .shape = SPRITE_SHAPE(64x32),
                                                          .size = SPRITE_SIZE(64x32),
                                                          .tileOffset = 0,
...
static const struct Subsprite sUnknown_0832C258[] = { { .x = 240,
                                                          .y = 0,
                                                          .shape = SPRITE_SHAPE(32x8),
                                                          .size = SPRITE_SIZE(32x8),
                                                          .tileOffset = 0,
...
static const struct Subsprite sUnknown_0832C260[] = { { .x = 240,
                                                          .y = 0,
                                                          .shape = SPRITE_SHAPE(32x8),
                                                          .size = SPRITE_SIZE(32x8),
                                                          .tileOffset = 0,
...
static const struct SubspriteTable sUnknown_0832C26C[] = { { ARRAY_COUNT(sUnknown_0832C220),
                                                               sUnknown_0832C220 },
...
static const struct SubspriteTable sUnknown_0832C28C[] = { { ARRAY_COUNT(sUnknown_0832C258),
                                                               sUnknown_0832C258 },
    { ARRAY_COUNT(sUnknown_0832C260), sUnknown_0832C260 } };

static const struct Subsprite sStatusSummaryBar_Subsprites_0[] = { { .x = 160,
                                                                       .y = 0,
                                                                       .shape = SPRITE_SHAPE(32x8),
                                                                       .size = SPRITE_SIZE(32x8),
                                                                       .tileOffset = 0,
...
static const struct Subsprite sUnknown_0832C2AC[] = { { .x = 160,
                                                          .y = 0,
                                                          .shape = SPRITE_SHAPE(32x8),
                                                          .size = SPRITE_SIZE(32x8),
                                                          .tileOffset = 0,
...
static const struct SubspriteTable sUnknown_0832C2CC[] = { { ARRAY_COUNT(sUnknown_0832C2AC),
    sUnknown_0832C2AC } };
...
// There's also the third sprite under name of healthbarSprite that refers to the healthbar visible
// on the healtbox.
...
u8 CreateBattlerHealthboxSprites(u8 battlerId)
{
    ...
    if (!IsDoubleBattle())
    {
        if (GetBattlerSide(battlerId) == B_SIDE_PLAYER)
        {
            ...
            healthboxRightSpriteId =
                CreateSpriteAtEnd(&sHealthboxPlayerSpriteTemplates[0], 240, 160, 1);
        ...
        else
        {
            healthboxLeftSpriteId =
                CreateSprite(&sHealthboxOpponentSpriteTemplates[0], 240, 160, 1);
            healthboxRightSpriteId =
                CreateSpriteAtEnd(&sHealthboxOpponentSpriteTemplates[0], 240, 160, 1);
    ...
    else
    {
        if (GetBattlerSide(battlerId) == B_SIDE_PLAYER)
        {
            healthboxLeftSpriteId = CreateSprite(
                &sHealthboxPlayerSpriteTemplates[GetBattlerPosition(battlerId) / 2], 240, 160, 1);
        ...
        {
            healthboxLeftSpriteId = CreateSprite(
                &sHealthboxOpponentSpriteTemplates[GetBattlerPosition(battlerId) / 2], 240, 160, 1);
            healthboxRightSpriteId = CreateSpriteAtEnd(
                &sHealthboxOpponentSpriteTemplates[GetBattlerPosition(battlerId) / 2], 240, 160, 1);
    ...
    healthbarSpriteId =
        CreateSpriteAtEnd(&sHealthbarSpriteTemplates[gBattlerPositions[battlerId]], 140, 60, 0);
    ...
    CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_1),
        (void *)(OBJ_VRAM0 + healthBarSpritePtr->oam.tileNum * TILE_SIZE_4BPP),
        64);
...
void SetBattleBarStruct(
    u8 battlerId, u8 healthboxSpriteId, s32 maxVal, s32 oldVal, s32 receivedValue)
{
        ...
        else
        {
            ...
            for (i = 0; i < 3; i++)
            {
                CpuCopy32(&gMonSpritesGfxPtr->barFontGfx[i * 64 + 32],
                    (void *)((OBJ_VRAM0) +
                             TILE_SIZE_4BPP * (gSprites[healthboxSpriteId].oam.tileNum + var + i)),
                    0x20);
            }
        }
    }
...
static void UpdateHpTextInHealthboxInDoubles(u8 healthboxSpriteId, s16 value, u8 maxOrCurrent)
{
    ...
    if (GetBattlerSide(gSprites[healthboxSpriteId].hMain_Battler) == B_SIDE_PLAYER)
    {
        ...
        {
            spriteTileNum =
                gSprites[gSprites[healthboxSpriteId].data[5]].oam.tileNum * TILE_SIZE_4BPP;
            ...
            if (maxOrCurrent != HP_CURRENT) // doubles, max hp
            {
                ...
                HpTextIntoHealthboxObject(
                    (void *)(OBJ_VRAM0) + spriteTileNum + 0xC0, windowTileData, 2);
                RemoveWindowOnHealthbox(windowId);
                CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_116),
                    (void *)(OBJ_VRAM0 + 0x680) +
                        (gSprites[healthboxSpriteId].oam.tileNum * TILE_SIZE_4BPP),
                    0x20);
            }
            else
            {
                ...
                HpTextIntoHealthboxObject(
                    (void *)(OBJ_VRAM0 + 0x60) + spriteTileNum, windowTileData, 3);
                RemoveWindowOnHealthbox(windowId);
            }
        }
    }
    else
    {
        ...
        if (gBattleSpritesDataPtr->battlerData[battlerId]
                .hpNumbersNoBars) // don't print text if only bars are visible
        {
            u8 var = 4;
            u8 r7;
            ...
            if (maxOrCurrent == HP_CURRENT)
            {
                CpuCopy32(&gMonSpritesGfxPtr->barFontGfx[224],
                    ...
                    0x20);
                CpuFill32(
                    0, (void *)((OBJ_VRAM0) + (gSprites[r7].oam.tileNum * TILE_SIZE_4BPP)), 0x20);
            }
            else
            {
                if (GetBattlerSide(battlerId) ==
                    B_SIDE_PLAYER) // Impossible to reach part, because the battlerId is from the
                                   // opponent's side.
                {
                    CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_116),
                        (void *)(OBJ_VRAM0) +
                            ((gSprites[healthboxSpriteId].oam.tileNum + 52) * TILE_SIZE_4BPP),
                        0x20);
                }
            }
        }
...
static void PrintSafariMonInfo(u8 healthboxSpriteId, struct Pokemon *mon)
{
    ...
    barFontGfx =
        &gMonSpritesGfxPtr
             ->barFontGfx[0x520 +
                          (GetBattlerPosition(gSprites[healthboxSpriteId].hMain_Battler) * 384)];
    var = 5;
    ...
    {
        spriteTileNum =
            (gSprites[healthboxSpriteId].oam.tileNum + (j - (j / 8 * 8)) + (j / 8 * 64)) *
            TILE_SIZE_4BPP;
        CpuCopy32(barFontGfx, (void *)(OBJ_VRAM0) + (spriteTileNum), 0x20);
        barFontGfx += 0x20;

        spriteTileNum =
            (8 + gSprites[healthboxSpriteId].oam.tileNum + (j - (j / 8 * 8)) + (j / 8 * 64)) *
            TILE_SIZE_4BPP;
        CpuCopy32(barFontGfx, (void *)(OBJ_VRAM0) + (spriteTileNum), 0x20);
        barFontGfx += 0x20;
    }
    ...
    ConvertIntToDecimalStringN(
        text + 6, gBattleStruct->safariCatchFactor, STR_CONV_MODE_RIGHT_ALIGN, 2);
    ConvertIntToDecimalStringN(
        text + 9, gBattleStruct->safariEscapeFactor, STR_CONV_MODE_RIGHT_ALIGN, 2);
    text[5] = CHAR_SPACE;
    text[8] = CHAR_SLASH;
    RenderTextFont9(gMonSpritesGfxPtr->barFontGfx, 9, text);
    ...
    for (j = 0; j < 5; j++)
    {
        if (j <= 1)
        {
            CpuCopy32(&gMonSpritesGfxPtr->barFontGfx[0x40 * j + 0x20],
                (void *)(OBJ_VRAM0) +
                    (gSprites[healthBarSpriteId].oam.tileNum + 2 + j) * TILE_SIZE_4BPP,
                32);
        }
        else
        {
            CpuCopy32(&gMonSpritesGfxPtr->barFontGfx[0x40 * j + 0x20],
                (void *)(OBJ_VRAM0 + 0xC0) +
                    (j + gSprites[healthBarSpriteId].oam.tileNum) * TILE_SIZE_4BPP,
                32);
        }
    }
}
            ...
            (IsDoubleBattle() || GetBattlerSide(i) != B_SIDE_PLAYER))
        {
            ...
            if (GetBattlerSide(i) == B_SIDE_PLAYER)
            {
                ...
                if (noBars == TRUE) // bars to text
                {
                    ...
                    CpuFill32(0,
                        (void *)(OBJ_VRAM0 +
                                 gSprites[healthBarSpriteId].oam.tileNum * TILE_SIZE_4BPP),
                        0x100);
                    UpdateHpTextInHealthboxInDoubles(gHealthboxSpriteIds[i],
                        GetMonData(&gPlayerParty[gBattlerPartyIndexes[i]], MON_DATA_HP),
                        HP_CURRENT);
                    UpdateHpTextInHealthboxInDoubles(gHealthboxSpriteIds[i],
                        GetMonData(&gPlayerParty[gBattlerPartyIndexes[i]], MON_DATA_MAX_HP),
                        HP_MAX);
                }
                else // text to bars
                {
                    ...
                    UpdateHealthboxAttribute(gHealthboxSpriteIds[i],
                        &gPlayerParty[gBattlerPartyIndexes[i]],
                        HEALTHBOX_HEALTH_BAR);
                    CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_117),
                        (void *)(OBJ_VRAM0 + 0x680 +
                                 gSprites[gHealthboxSpriteIds[i]].oam.tileNum * TILE_SIZE_4BPP),
                        32);
                }
            }
            else
            {
                if (noBars == TRUE) // bars to text
                {
                    if (gBattleTypeFlags & BATTLE_TYPE_SAFARI)
                    {
                        ...
                        PrintSafariMonInfo(
                            gHealthboxSpriteIds[i], &gEnemyParty[gBattlerPartyIndexes[i]]);
                    }
                    else
                    {
                        healthBarSpriteId =
                            gSprites[gHealthboxSpriteIds[i]].hMain_HealthBarSpriteId;

                        CpuFill32(0,
                            (void *)(OBJ_VRAM0 + gSprites[healthBarSpriteId].oam.tileNum * 32),
                            0x100);
                        UpdateHpTextInHealthboxInDoubles(gHealthboxSpriteIds[i],
                            GetMonData(&gEnemyParty[gBattlerPartyIndexes[i]], MON_DATA_HP),
                            HP_CURRENT);
                        UpdateHpTextInHealthboxInDoubles(gHealthboxSpriteIds[i],
                            GetMonData(&gEnemyParty[gBattlerPartyIndexes[i]], MON_DATA_MAX_HP),
                            HP_MAX);
                    }
                }
                else // text to bars
                {
                    UpdateStatusIconInHealthbox(gHealthboxSpriteIds[i]);
                    UpdateHealthboxAttribute(gHealthboxSpriteIds[i],
                        &gEnemyParty[gBattlerPartyIndexes[i]],
                        HEALTHBOX_HEALTH_BAR);
                    if (gBattleTypeFlags & BATTLE_TYPE_SAFARI)
                        UpdateHealthboxAttribute(gHealthboxSpriteIds[i],
                            &gEnemyParty[gBattlerPartyIndexes[i]],
...
u8 CreatePartyStatusSummarySprites(
    u8 battlerId, struct HpAndStatus *partyInfo, u8 arg2, bool8 isBattleStart)
{
    bool8 isOpponent;
    ...
    summaryBarSpriteId =
        CreateSprite(&sStatusSummaryBarSpriteTemplates[isOpponent], bar_X, bar_Y, 10);
    ...
    {
        ballIconSpritesIds[i] =
            CreateSpriteAtEnd(&sStatusSummaryBallsSpriteTemplates[isOpponent], bar_X, bar_Y - 4, 9);
            ...
            for (i = 0, var = 5, j = 0; j < PARTY_SIZE; j++)
            {
                ...
                else if (partyInfo[j].hp == 0) // fainted mon
                {
                    gSprites[ballIconSpritesIds[i]].oam.tileNum += 3;
                         ...
                         gBattleStruct->arenaLostPlayerMons & gBitTable[j])
                {
                    gSprites[ballIconSpritesIds[i]].oam.tileNum += 3;
                }
...
static void sub_8073E08(u8 taskId)
{
    if ((gTasks[taskId].data[11]++ % 2) == 0)
    {
        ...
        SetGpuReg(REG_OFFSET_BLDALPHA,
            BLDALPHA_BLEND(gTasks[taskId].data[15], 16 - gTasks[taskId].data[15]));
    }
    if (gTasks[taskId].tData15 == 0)
...
static void UpdateNickInHealthbox(u8 healthboxSpriteId, struct Pokemon *mon)
{
    ...
    default:
        ...
        windowTileData =
            AddTextPrinterAndCreateWindowOnHealthbox(gDisplayedStringBattle, 0, 3, 2, &windowId);
        break;
    case MON_MALE:
...
static void TryAddPokeballIconToHealthbox(u8 healthboxSpriteId, bool8 noStatus)
{
    ...
    if (GetBattlerSide(battlerId) == B_SIDE_PLAYER)
        return;
    if (!GetSetPokedexFlag(SpeciesToNationalPokedexNum(GetMonData(
                               &gEnemyParty[gBattlerPartyIndexes[battlerId]], MON_DATA_SPECIES)),
            FLAG_GET_CAUGHT))
        return;
    ...
    if (noStatus)
        CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_70),
            (void *)(OBJ_VRAM0 + (gSprites[healthBarSpriteId].oam.tileNum + 8) * TILE_SIZE_4BPP),
            32);
    else
        CpuFill32(0,
            (void *)(OBJ_VRAM0 + (gSprites[healthBarSpriteId].oam.tileNum + 8) * TILE_SIZE_4BPP),
...
static void UpdateStatusIconInHealthbox(u8 healthboxSpriteId)
{
    ...
    {
        statusGfxPtr = GetHealthboxElementGfxPtr(
            GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_SLP_BATTLER0, battlerId));
        statusPalId = PAL_STATUS_SLP;
    }
    else if (status & STATUS1_PSN_ANY)
    {
        statusGfxPtr = GetHealthboxElementGfxPtr(
            GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_PSN_BATTLER0, battlerId));
        statusPalId = PAL_STATUS_PSN;
    }
    else if (status & STATUS1_BURN)
    {
        statusGfxPtr = GetHealthboxElementGfxPtr(
            GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_BRN_BATTLER0, battlerId));
        statusPalId = PAL_STATUS_BRN;
    }
    else if (status & STATUS1_FREEZE)
    {
        statusGfxPtr = GetHealthboxElementGfxPtr(
            GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_FRZ_BATTLER0, battlerId));
        statusPalId = PAL_STATUS_FRZ;
    }
    else if (status & STATUS1_PARALYSIS)
    {
        statusGfxPtr = GetHealthboxElementGfxPtr(
            GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_PRZ_BATTLER0, battlerId));
    ...
    else
    {
        ...
        for (i = 0; i < 3; i++)
            CpuCopy32(statusGfxPtr,
                (void *)(OBJ_VRAM0 + (gSprites[healthboxSpriteId].oam.tileNum + tileNumAdder + i) *
                                         TILE_SIZE_4BPP),
                32);
    ...
    CpuCopy32(statusGfxPtr,
        (void *)(OBJ_VRAM0 +
                 (gSprites[healthboxSpriteId].oam.tileNum + tileNumAdder) * TILE_SIZE_4BPP),
        96);
    if (IsDoubleBattle() == TRUE || GetBattlerSide(battlerId) == B_SIDE_OPPONENT)
    {
        if (!gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars)
        {
            ...
            CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_65),
                (void *)(OBJ_VRAM0 +
                         (gSprites[healthBarSpriteId].oam.tileNum + 1) * TILE_SIZE_4BPP),
                32);
        }
    }
...
static void UpdateSafariBallsTextOnHealthbox(u8 healthboxSpriteId)
{
    ...
    windowTileData =
        AddTextPrinterAndCreateWindowOnHealthbox(gText_SafariBalls, 0, 3, 2, &windowId);
    spriteTileNum = gSprites[healthboxSpriteId].oam.tileNum * TILE_SIZE_4BPP;
...
static void UpdateLeftNoOfBallsTextOnHealthbox(u8 healthboxSpriteId)
{
    ...
    SafariTextIntoHealthboxObject(
        (void *)(OBJ_VRAM0 + 0xA00) + spriteTileNum, windowTileData + 0x40, 4);
    RemoveWindowOnHealthbox(windowId);
}
...
void UpdateHealthboxAttribute(u8 healthboxSpriteId, struct Pokemon *mon, u8 elementId)
{
    ...
    if (GetBattlerSide(gSprites[healthboxSpriteId].hMain_Battler) == B_SIDE_PLAYER)
    {
        ...
        if (!isDoubles && (elementId == HEALTHBOX_EXP_BAR || elementId == HEALTHBOX_ALL))
        {
            ...
            maxExpBarValue =
                gExperienceTables[gBaseStats[species].growthRate][level + 1] - currLevelExp;
            SetBattleBarStruct(
...
s32 MoveBattleBar(u8 battlerId, u8 healthboxSpriteId, u8 whichBar, u8 unused)
{
    ...
    {
        u16 expFraction =
            GetScaledExpFraction(gBattleSpritesDataPtr->battleBars[battlerId].oldValue,
                gBattleSpritesDataPtr->battleBars[battlerId].receivedValue,
...
static void MoveBattleBarGraphically(u8 battlerId, u8 whichBar)
{
    ...
    case HEALTH_BAR:
        ...
        {
            u8 healthbarSpriteId =
                gSprites[gBattleSpritesDataPtr->battleBars[battlerId].healthboxSpriteId]
                    .hMain_HealthBarSpriteId;
            if (i < 2)
                CpuCopy32(GetHealthboxElementGfxPtr(barElementId) + array[i] * 32,
                    (void *)(OBJ_VRAM0 +
                             (gSprites[healthbarSpriteId].oam.tileNum + 2 + i) * TILE_SIZE_4BPP),
                    32);
            else
                CpuCopy32(GetHealthboxElementGfxPtr(barElementId) + array[i] * 32,
                    (void *)(OBJ_VRAM0 + 64 +
                             (i + gSprites[healthbarSpriteId].oam.tileNum) * TILE_SIZE_4BPP),
                    32);
        }
        break;
    case EXP_BAR:
        ...
        for (i = 0; i < 8; i++)
        {
            if (i < 4)
                CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_12) + array[i] * 32,
                    (void *)(OBJ_VRAM0 + (gSprites[gBattleSpritesDataPtr->battleBars[battlerId]
                                                       .healthboxSpriteId]
                                                 .oam.tileNum +
                                             0x24 + i) *
                                             TILE_SIZE_4BPP),
                ...
                CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_12) + array[i] * 32,
                    (void *)(OBJ_VRAM0 + 0xB80 +
                             (i + gSprites[gBattleSpritesDataPtr->battleBars[battlerId]
                                               .healthboxSpriteId]
                                      .oam.tileNum) *
                                 TILE_SIZE_4BPP),
                    32);
...
static s32 CalcNewBarValue(
    s32 maxValue, s32 oldValue, s32 receivedValue, s32 *currValue, u8 scale, u16 toAdd)
{
    s32 ret, newValue;
    scale *= 8;
