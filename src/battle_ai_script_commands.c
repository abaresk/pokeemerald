...
void BattleAI_SetupAIData(u8 defaultScoreMoves)
{
    ...
    else if (gBattleTypeFlags & (BATTLE_TYPE_FRONTIER | BATTLE_TYPE_EREADER_TRAINER |
...
static void BattleAI_DoAIProcessing(void)
{
    while (AI_THINKING_STRUCT->aiState != AIState_FinishedProcessing)
    {
        ...
        case AIState_SettingUp:
            gAIScriptPtr =
                gBattleAI_ScriptsTable[AI_THINKING_STRUCT->aiLogicId]; // set AI ptr to logic ID.
...
static void RecordLastUsedMoveByTarget(void)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (BATTLE_HISTORY->usedMoves[gBattlerTarget].moves[i] !=
                gLastMoves[gBattlerTarget] // HACK: This redundant condition is a hack to make the
                                           // asm match.
    ...
    if (gAIScriptPtr[1] == AI_TARGET || gAIScriptPtr[1] == AI_TARGET_PARTNER)
    {
        ...
        else if (gBaseStats[gBattleMons[battlerId].species].abilities[0] != ABILITY_NONE)
        {
            if (gBaseStats[gBattleMons[battlerId].species].abilities[1] != ABILITY_NONE)
            {
                ...
                if (gBaseStats[gBattleMons[battlerId].species].abilities[0] !=
