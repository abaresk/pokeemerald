...
static const u16 sWirelessLinkIconPalette[] =
    INCBIN_U16("graphics/interface/wireless_link_icon.gbapal");
static const u32 sWirelessLinkIconPic[] =
    INCBIN_U32("graphics/interface/wireless_link_icon.4bpp.lz");

// Most of the below two tables won't make sense with ASCII encoding.
static const u8 sWireless_ASCIItoRSETable[256] = { EOS,
    0x95,
    0x96,
    0x97,
    0x98,
...
static const u8 sWireless_RSEtoASCIITable[256] = { [CHAR_SPACE] = ' ',
    0x86,
    0x87,
    0x88,
    0x89,
...
static const union AnimCmd sWirelessStatusIndicator_3Bars[] = { ANIMCMD_FRAME(4, 5),
    ANIMCMD_FRAME(8, 5),
    ANIMCMD_FRAME(12, 5),
    ANIMCMD_FRAME(16, 10),
    ANIMCMD_FRAME(12, 5),
...
static const union AnimCmd sWirelessStatusIndicator_2Bars[] = { ANIMCMD_FRAME(4, 5),
    ANIMCMD_FRAME(8, 5),
    ANIMCMD_FRAME(12, 10),
    ANIMCMD_FRAME(8, 5),
    ANIMCMD_JUMP(0) };
...
static const union AnimCmd sWirelessStatusIndicator_1Bar[] = {
    ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(8, 5), ANIMCMD_JUMP(0)
};

static const union AnimCmd sWirelessStatusIndicator_Searching[] = {
    ANIMCMD_FRAME(4, 10), ANIMCMD_FRAME(20, 10), ANIMCMD_JUMP(0)
};

static const union AnimCmd sWirelessStatusIndicator_Error[] = {
    ANIMCMD_FRAME(24, 10), ANIMCMD_FRAME(4, 10), ANIMCMD_JUMP(0)
};
...
static const struct SpriteTemplate sWirelessStatusIndicatorSpriteTemplate = { .tileTag = 0xD431,
    .paletteTag = 0xD432,
...
bool8 LinkRfu_GetNameIfCompatible(struct GFtgtGname *buff1, u8 *buff2, u8 idx)
{
    ...
    if (lman.parent_child == MODE_PARENT)
    {
        ...
        if (IsRfuSerialNumberValid(gRfuLinkStatus->partner[idx].serialNo) &&
            ((gRfuLinkStatus->getNameFlag >> idx) & 1))
        {
...
void CreateWirelessStatusIndicatorSprite(u8 x, u8 y)
{
    ...
    if (gRfuLinkStatus->parentChild == MODE_PARENT)
    {
        ...
        gSprites[sprId].sTileStart =
            GetSpriteTileStartByTag(sWirelessStatusIndicatorSpriteSheet.tag);
    ...
    else
    {
        gWirelessStatusIndicatorSpriteId =
            CreateSprite(&sWirelessStatusIndicatorSpriteTemplate, x, y, 0);
        gSprites[gWirelessStatusIndicatorSpriteId].sValidator = STATUS_INDICATOR_ACTIVE;
        gSprites[gWirelessStatusIndicatorSpriteId].sTileStart =
            GetSpriteTileStartByTag(sWirelessStatusIndicatorSpriteSheet.tag);
...
{
    if (gWirelessStatusIndicatorSpriteId != 0xFF &&
        gSprites[gWirelessStatusIndicatorSpriteId].sValidator == STATUS_INDICATOR_ACTIVE)
    {
        ...
        if (sprite->anims[sprite->sCurrAnimNum][sprite->sFrameIdx].frame.duration <
            sprite->sFrameDelay)
        {
            sprite->sFrameIdx++;
            sprite->sFrameDelay = 0;
        ...
        gMain.oamBuffer[125].tileNum =
            sprite->sTileStart +
            sprite->anims[sprite->sCurrAnimNum][sprite->sFrameIdx].frame.imageValue;
...
// Save the currently connected players into the trainer records, shifting all previous records
// down.
void RecordMixTrainerNames(void)
{
    if (gWirelessCommType != 0)
    {
        ...
        struct TrainerNameRecord *newRecords = calloc(
            ARRAY_COUNT(gSaveBlock1Ptr->trainerNameRecords), sizeof(struct TrainerNameRecord));
        ...
        for (i = 0; i < GetLinkPlayerCount(); i++)
        {
            ...
            {
                if ((u16)gLinkPlayers[i].trainerId ==
                        gSaveBlock1Ptr->trainerNameRecords[j].trainerId &&
                    StringCompare(gLinkPlayers[i].name,
                        gSaveBlock1Ptr->trainerNameRecords[j].trainerName) == 0)
                {
        ...
        for (i = 0; i < GetLinkPlayerCount(); i++)
        {
            ...
            {
                CopyTrainerRecord(
                    &newRecords[nextSpace], (u16)gLinkPlayers[i].trainerId, gLinkPlayers[i].name);

                // If we already had a record for this trainer, wipe it so that the next step
                // doesn't duplicate it.
                if (connectedTrainerRecordIndices[i] >= 0)
                {
                    memset(gSaveBlock1Ptr->trainerNameRecords[connectedTrainerRecordIndices[i]]
                               .trainerName,
                        0,
                        8);
                }
        ...
        // Copy all non-empty records to the new list, in the order they appear on the old list. If
        // the list is full, the last (oldest) records will be dropped.
        for (i = 0; i < (int)ARRAY_COUNT(gSaveBlock1Ptr->trainerNameRecords); i++)
        {
            ...
            {
                CopyTrainerRecord(&newRecords[nextSpace],
        ...
        memcpy(gSaveBlock1Ptr->trainerNameRecords,
            newRecords,
            sizeof(gSaveBlock1Ptr->trainerNameRecords));
        free(newRecords);
    }
}
...
bool32 PlayerHasMetTrainerBefore(u16 id, u8 *name)
{
    ...
    for (i = 0; i < (int)ARRAY_COUNT(gSaveBlock1Ptr->trainerNameRecords); i++)
    {
        if (StringCompare(gSaveBlock1Ptr->trainerNameRecords[i].trainerName, name) == 0 &&
