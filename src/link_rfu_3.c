...
static const union AnimCmd sWirelessStatusIndicator_3Bars[] = { ANIMCMD_FRAME(4, 5),  ANIMCMD_FRAME(8, 5),
                                                                ANIMCMD_FRAME(12, 5), ANIMCMD_FRAME(16, 10),
                                                                ANIMCMD_FRAME(12, 5), ANIMCMD_FRAME(8, 5),
                                                                ANIMCMD_JUMP(0) };

static const union AnimCmd sWirelessStatusIndicator_2Bars[] = { ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(8, 5),
                                                                ANIMCMD_FRAME(12, 10), ANIMCMD_FRAME(8, 5),
                                                                ANIMCMD_JUMP(0) };

static const union AnimCmd sWirelessStatusIndicator_1Bar[] = { ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(8, 5),
                                                               ANIMCMD_JUMP(0) };

static const union AnimCmd sWirelessStatusIndicator_Searching[] = { ANIMCMD_FRAME(4, 10), ANIMCMD_FRAME(20, 10),
                                                                    ANIMCMD_JUMP(0) };

static const union AnimCmd sWirelessStatusIndicator_Error[] = { ANIMCMD_FRAME(24, 10), ANIMCMD_FRAME(4, 10),
                                                                ANIMCMD_JUMP(0) };
...
static const struct CompressedSpriteSheet sWirelessStatusIndicatorSpriteSheet = { sWirelessLinkIconPic, 0x0380,
                                                                                  0xD431 };

static const struct SpritePalette sWirelessStatusIndicatorSpritePalette = { sWirelessLinkIconPalette, 0xD432 };
...
void UpdateWirelessStatusIndicatorSprite(void) {
    if (gWirelessStatusIndicatorSpriteId != 0xFF &&
        ...
        gMain.oamBuffer[125].tileNum =
            sprite->sTileStart + sprite->anims[sprite->sCurrAnimNum][sprite->sFrameIdx].frame.imageValue;
...
void RecordMixTrainerNames(void) {
    if (gWirelessCommType != 0) {
        ...
        struct TrainerNameRecord* newRecords =
            calloc(ARRAY_COUNT(gSaveBlock1Ptr->trainerNameRecords), sizeof(struct TrainerNameRecord));

        // Check if we already have a record saved for connected trainers.
        for (i = 0; i < GetLinkPlayerCount(); i++) {
            connectedTrainerRecordIndices[i] = -1;
            for (j = 0; j < (int)ARRAY_COUNT(gSaveBlock1Ptr->trainerNameRecords); j++) {
                if ((u16)gLinkPlayers[i].trainerId == gSaveBlock1Ptr->trainerNameRecords[j].trainerId &&
                    StringCompare(gLinkPlayers[i].name, gSaveBlock1Ptr->trainerNameRecords[j].trainerName) == 0) {
        ...
        for (i = 0; i < (int)ARRAY_COUNT(gSaveBlock1Ptr->trainerNameRecords); i++) {
            if (NameIsNotEmpty(gSaveBlock1Ptr->trainerNameRecords[i].trainerName)) {
                CopyTrainerRecord(&newRecords[nextSpace], gSaveBlock1Ptr->trainerNameRecords[i].trainerId,
