...
static const struct MenuAction sShopMenuActions_BuyQuit[] = {
    { gText_ShopBuy, { .void_u8 = Task_HandleShopMenuBuy } }, { gText_ShopQuit, { .void_u8 = Task_HandleShopMenuQuit } }
};
...
static u8 CreateShopMenu(u8 martType) {
    ...
    if (martType == MART_TYPE_NORMAL) {
        ...
        winTemplate.width =
            GetMaxWidthInMenuTable(sShopMenuActions_BuySellQuit, ARRAY_COUNT(sShopMenuActions_BuySellQuit));
...
static void BuyMenuPrintPriceInList(u8 windowId, s32 item, u8 y) {
    ...
    if (item != -2) {
        if (gMartInfo.martType == MART_TYPE_NORMAL) {
            ConvertIntToDecimalStringN(gStringVar1, ItemId_GetPrice(item) >> GetPriceReduction(POKENEWS_SLATEPORT),
...
static void BuyMenuAddScrollIndicatorArrows(void) {
    if (gShopDataPtr->scrollIndicatorsTaskId == 0xFF && gMartInfo.itemCount + 1 > 8) {
        gShopDataPtr->scrollIndicatorsTaskId = AddScrollIndicatorArrowPairParameterized(
            SCROLL_ARROW_UP, 0xAC, 0xC, 0x94, gMartInfo.itemCount - 7, 2100, 2100, &gShopDataPtr->scrollOffset);
    }
}
    ...
    for (j = 0; j < 10; j++) {
        for (i = 0; i < 15; i++) {
            ...
            if (metatile < NUM_METATILES_IN_PRIMARY) {
                BuyMenuDrawMapMetatile(i, j, (u16*)mapLayout->primaryTileset->metatiles + metatile * 8,
                                       metatileLayerType);
            } else {
                BuyMenuDrawMapMetatile(
                    i, j, (u16*)mapLayout->secondaryTileset->metatiles + ((metatile - NUM_METATILES_IN_PRIMARY) * 8),
                    metatileLayerType);
            }
        }
    }
...
static void BuyMenuDrawObjectEvents(void) {
    ...
    for (i = 0; i < OBJECT_EVENTS_COUNT; i++) {
        ...
        graphicsInfo =
            GetObjectEventGraphicsInfo(gObjectEvents[gShopDataPtr->viewportObjects[i][OBJ_EVENT_ID]].graphicsId);

        spriteId = AddPseudoObjectEvent(
            gObjectEvents[gShopDataPtr->viewportObjects[i][OBJ_EVENT_ID]].graphicsId, SpriteCallbackDummy,
            (u16)gShopDataPtr->viewportObjects[i][X_COORD] * 16 + 8,
            (u16)gShopDataPtr->viewportObjects[i][Y_COORD] * 16 + 48 - graphicsInfo->height / 2, 2);
