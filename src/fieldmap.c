...
static void FillSouthConnection(struct MapHeader const* mapHeader, struct MapHeader const* connectedMapHeader,
                                s32 offset);
static void FillNorthConnection(struct MapHeader const* mapHeader, struct MapHeader const* connectedMapHeader,
                                s32 offset);
static void FillWestConnection(struct MapHeader const* mapHeader, struct MapHeader const* connectedMapHeader,
                               s32 offset);
static void FillEastConnection(struct MapHeader const* mapHeader, struct MapHeader const* connectedMapHeader,
                               s32 offset);
...
static void sub_8087F54(int x, int y, struct MapHeader const* connectedMapHeader, int x2, int y2, int width,
                        int height) {
    int i;
    u16* src;
    u16* dest;
...
static void FillSouthConnection(struct MapHeader const* mapHeader, struct MapHeader const* connectedMapHeader,
    ...
    if (connectedMapHeader) {
        ...
        sub_8087F54(x, y, connectedMapHeader, x2, /*y2*/ 0, width, /*height*/ 7);
    }
}
...
static void FillNorthConnection(struct MapHeader const* mapHeader, struct MapHeader const* connectedMapHeader,
    ...
    if (connectedMapHeader) {
        ...
        sub_8087F54(x, /*y*/ 0, connectedMapHeader, x2, y2, width, /*height*/ 7);
    }
}
...
static void FillWestConnection(struct MapHeader const* mapHeader, struct MapHeader const* connectedMapHeader,
    ...
    if (connectedMapHeader) {
        ...
        sub_8087F54(
            /*x*/ 0, y, connectedMapHeader, x2, y2,
            /*width*/ 7, height);
    }
}
...
static void FillEastConnection(struct MapHeader const* mapHeader, struct MapHeader const* connectedMapHeader,
    ...
    if (connectedMapHeader) {
        ...
        sub_8087F54(x, y, connectedMapHeader,
                    /*x2*/ 0, y2,
                    /*width*/ 8, height);
    }
}
...
void MapGridSetMetatileIdAt(int x, int y, u16 metatile) {
    ...
    if (x >= 0 && x < gBackupMapLayout.width && y >= 0 && y < gBackupMapLayout.height) {
        ...
        gBackupMapLayout.map[i] =
            (gBackupMapLayout.map[i] & METATILE_ELEVATION_MASK) | (metatile & ~METATILE_ELEVATION_MASK);
    }
}
...
void CopySecondaryTilesetToVramUsingHeap(struct MapLayout const* mapLayout) {
    CopyTilesetToVramUsingHeap(mapLayout->secondaryTileset, NUM_TILES_TOTAL - NUM_TILES_IN_PRIMARY,
...
void LoadSecondaryTilesetPalette(struct MapLayout const* mapLayout) {
    LoadTilesetPalette(mapLayout->secondaryTileset, NUM_PALS_IN_PRIMARY * 16,
                       (NUM_PALS_TOTAL - NUM_PALS_IN_PRIMARY) * 16 * 2);
}
