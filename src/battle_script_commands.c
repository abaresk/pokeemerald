...
#define DEFENDER_IS_PROTECTED \
    ((gProtectStructs[gBattlerTarget].protected) && (gBattleMoves[gCurrentMove].flags & FLAG_PROTECT_AFFECTED))
...
// unused
static const u8 sRubyLevelUpStatBoxStats[] = { MON_DATA_MAX_HP, MON_DATA_SPATK, MON_DATA_ATK,
...
static const struct SpriteTemplate sSpriteTemplate_MonIconOnLvlUpBox = { .tileTag = MON_ICON_LVLUP_BOX_TAG,
...
static const u16 sMovesForbiddenToCopy[] = {
    ...
    MOVE_HELPING_HAND, MOVE_COVET,       MOVE_TRICK,   MOVE_FOCUS_PUNCH, METRONOME_FORBIDDEN_END
};
...
static const u16 sNaturePowerMoves[] = {
    ...
    [BATTLE_TERRAIN_SAND] = MOVE_EARTHQUAKE,     [BATTLE_TERRAIN_UNDERWATER] = MOVE_HYDRO_PUMP,
    [BATTLE_TERRAIN_WATER] = MOVE_SURF,          [BATTLE_TERRAIN_POND] = MOVE_BUBBLE_BEAM,
    [BATTLE_TERRAIN_MOUNTAIN] = MOVE_ROCK_SLIDE, [BATTLE_TERRAIN_CAVE] = MOVE_SHADOW_BALL,
    [BATTLE_TERRAIN_BUILDING] = MOVE_SWIFT,      [BATTLE_TERRAIN_PLAIN] = MOVE_SWIFT
};
...
static const u16 sPickupItems[] = {
    ...
    ITEM_X_ATTACK, ITEM_FULL_HEAL, ITEM_ULTRA_BALL,   ITEM_HYPER_POTION, ITEM_RARE_CANDY, ITEM_PROTEIN,
    ITEM_REVIVE,   ITEM_HP_UP,     ITEM_FULL_RESTORE, ITEM_MAX_REVIVE,   ITEM_PP_UP,      ITEM_MAX_ELIXIR,
};

static const u16 sRarePickupItems[] = {
    ITEM_HYPER_POTION, ITEM_NUGGET, ITEM_KINGS_ROCK,       ITEM_FULL_RESTORE, ITEM_ETHER,           ITEM_WHITE_HERB,
    ITEM_TM44_REST,    ITEM_ELIXIR, ITEM_TM01_FOCUS_PUNCH, ITEM_LEFTOVERS,    ITEM_TM26_EARTHQUAKE,
};

static const u8 sPickupProbabilities[] = { 30, 40, 50, 60, 70, 80, 90, 94, 98 };

static const u8 sTerrainToType[] = {
    [BATTLE_TERRAIN_GRASS] = TYPE_GRASS,     [BATTLE_TERRAIN_LONG_GRASS] = TYPE_GRASS,
    [BATTLE_TERRAIN_SAND] = TYPE_GROUND,     [BATTLE_TERRAIN_UNDERWATER] = TYPE_WATER,
    [BATTLE_TERRAIN_WATER] = TYPE_WATER,     [BATTLE_TERRAIN_POND] = TYPE_WATER,
    [BATTLE_TERRAIN_MOUNTAIN] = TYPE_ROCK,   [BATTLE_TERRAIN_CAVE] = TYPE_ROCK,
    [BATTLE_TERRAIN_BUILDING] = TYPE_NORMAL, [BATTLE_TERRAIN_PLAIN] = TYPE_NORMAL,
};
...
static const u8 sBattlePalaceNatureToFlavorTextId[NUM_NATURES] = {
    [NATURE_HARDY] = 3,  [NATURE_LONELY] = 0, [NATURE_BRAVE] = 1,   [NATURE_ADAMANT] = 0, [NATURE_NAUGHTY] = 0,
    [NATURE_BOLD] = 1,   [NATURE_DOCILE] = 3, [NATURE_RELAXED] = 0, [NATURE_IMPISH] = 1,  [NATURE_LAX] = 2,
    [NATURE_TIMID] = 2,  [NATURE_HASTY] = 0,  [NATURE_SERIOUS] = 3, [NATURE_JOLLY] = 1,   [NATURE_NAIVE] = 3,
    [NATURE_MODEST] = 1, [NATURE_MILD] = 2,   [NATURE_QUIET] = 3,   [NATURE_BASHFUL] = 3, [NATURE_RASH] = 2,
    [NATURE_CALM] = 1,   [NATURE_GENTLE] = 0, [NATURE_SASSY] = 2,   [NATURE_CAREFUL] = 2, [NATURE_QUIRKY] = 3,
};
...
static void Cmd_ppreduce(void) {
    ...
    if (!gSpecialStatuses[gBattlerAttacker].ppNotAffectedByPressure) {
        switch (gBattleMoves[gCurrentMove].target) {
            case MOVE_TARGET_FOES_AND_ALLY:
                ppToDeduct +=
                    AbilityBattleEffects(ABILITYEFFECT_COUNT_ON_FIELD, gBattlerAttacker, ABILITY_PRESSURE, 0, 0);
                break;
            case MOVE_TARGET_BOTH:
            case MOVE_TARGET_OPPONENTS_FIELD:
                ppToDeduct +=
                    AbilityBattleEffects(ABILITYEFFECT_COUNT_OTHER_SIDE, gBattlerAttacker, ABILITY_PRESSURE, 0, 0);
                break;
            default:
    ...
    if (!(gHitMarker & (HITMARKER_NO_PPDEDUCT | HITMARKER_NO_ATTACKSTRING)) &&
        ...
        if (!(gBattleMons[gBattlerAttacker].status2 & STATUS2_TRANSFORMED) &&
            ...
            BtlController_EmitSetMonData(0, REQUEST_PPMOVE1_BATTLE + gCurrMovePos, 0, 1,
                                         &gBattleMons[gBattlerAttacker].pp[gCurrMovePos]);
...
static void Cmd_critcalc(void) {
    ...
    critChance =
        2 * ((gBattleMons[gBattlerAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0) +
        (gBattleMoves[gCurrentMove].effect == EFFECT_HIGH_CRITICAL) +
        (gBattleMoves[gCurrentMove].effect == EFFECT_SKY_ATTACK) +
        (gBattleMoves[gCurrentMove].effect == EFFECT_BLAZE_KICK) +
        (gBattleMoves[gCurrentMove].effect == EFFECT_POISON_TAIL) + (holdEffect == HOLD_EFFECT_SCOPE_LENS) +
...
static void Cmd_damagecalc(void) {
    ...
    gBattleMoveDamage =
        CalculateBaseDamage(&gBattleMons[gBattlerAttacker], &gBattleMons[gBattlerTarget], gCurrentMove, sideStatus,
                            gDynamicBasePower, gBattleStruct->dynamicMoveType, gBattlerAttacker, gBattlerTarget);
...
void AI_CalcDmg(u8 attacker, u8 defender) {
    ...
    gBattleMoveDamage = CalculateBaseDamage(&gBattleMons[attacker], &gBattleMons[defender], gCurrentMove, sideStatus,
                                            gDynamicBasePower, gBattleStruct->dynamicMoveType, attacker, defender);
    gDynamicBasePower = 0;
    gBattleMoveDamage = gBattleMoveDamage * gCritMultiplier * gBattleScripting.dmgMultiplier;
...
static void ModulateDmgByType2(u8 multiplier, u16 move,
                               u8* flags) // same as ModulateDmgByType except different arguments
{
    gBattleMoveDamage = gBattleMoveDamage * multiplier / 10;
    if (gBattleMoveDamage == 0 && multiplier != 0)
        gBattleMoveDamage = 1;
...
static void
Cmd_adjustnormaldamage2(void) // The same as adjustnormaldamage except it doesn't check for false swipe move effect.
{
    u8 holdEffect, param;
...
static void Cmd_attackanimation(void) {
    ...
    if ((gHitMarker & HITMARKER_NO_ANIMATIONS) && (gCurrentMove != MOVE_TRANSFORM && gCurrentMove != MOVE_SUBSTITUTE)) {
        ...
        if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)) {
            ...
            BtlController_EmitMoveAnimation(0, gCurrentMove, gBattleScripting.animTurn, gBattleMovePower,
                                            gBattleMoveDamage, gBattleMons[gBattlerAttacker].friendship,
...
static void Cmd_healthbarupdate(void) {
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)) {
        ...
        if (gBattleMons[gActiveBattler].status2 & STATUS2_SUBSTITUTE && gDisableStructs[gActiveBattler].substituteHP &&
...
void SetMoveEffect(bool8 primary, u8 certain) {
    ...
    if (gSideStatuses[GET_BATTLER_SIDE(gEffectBattler)] & SIDE_STATUS_SAFEGUARD &&
        !(gHitMarker & HITMARKER_IGNORE_SAFEGUARD) && !primary && gBattleCommunication[MOVE_EFFECT_BYTE] <= 7)
        INCREMENT_RESET_RETURN

    if (gBattleMons[gEffectBattler].hp == 0 && gBattleCommunication[MOVE_EFFECT_BYTE] != MOVE_EFFECT_PAYDAY &&
        ...
        switch (sStatusFlagsForMoveEffects[gBattleCommunication[MOVE_EFFECT_BYTE]]) {
            ...
            case STATUS1_POISON:
                ...
                if ((IS_BATTLER_OF_TYPE(gEffectBattler, TYPE_POISON) ||
                    ...
                    (gHitMarker & HITMARKER_IGNORE_SAFEGUARD) && (primary == TRUE || certain == MOVE_EFFECT_CERTAIN)) {
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
                    gBattlescriptCurrInstr = BattleScript_PSNPrevention;
            ...
            switch (gBattleCommunication[MOVE_EFFECT_BYTE]) {
                ...
                case MOVE_EFFECT_EVS_PLUS_1:
                    if (ChangeStatBuffs(SET_STAT_BUFF_VALUE(1),
                        ...
                        gBattleScripting.animArg1 =
                            gBattleCommunication[MOVE_EFFECT_BYTE] & ~(MOVE_EFFECT_AFFECTS_USER | MOVE_EFFECT_CERTAIN);
                ...
                case MOVE_EFFECT_STEAL_ITEM: {
                    ...
                    if (GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT &&
                        ...
                        gActiveBattler = gBattlerTarget;
                        BtlController_EmitSetMonData(0, REQUEST_HELDITEM_BATTLE, 0, 2,
    ...
    if (gBattlescriptCurrInstr[2] != 0) {
        ...
        if (!(gAbsentBattlerFlags & gBitTable[gActiveBattler]) && gBattleMons[gActiveBattler].hp == 0) {
            ...
            if ((gStatuses3[gBattlerTarget] & STATUS3_GRUDGE) && !(gHitMarker & HITMARKER_GRUDGE) &&
                ...
                gActiveBattler = gBattlerAttacker;
                BtlController_EmitSetMonData(0, moveIndex + REQUEST_PPMOVE1_BATTLE, 0, 1,
    ...
    switch (gBattleScripting.getexpState) {
        case 0: // check if should receive exp at all
            if (GetBattlerSide(gBattlerFainted) != B_SIDE_OPPONENT ||
                (gBattleTypeFlags &
                 (BATTLE_TYPE_LINK | BATTLE_TYPE_x2000000 | BATTLE_TYPE_TRAINER_HILL | BATTLE_TYPE_FRONTIER |
        ...
        case 1: // calculate experience points to redistribute
        {
            ...
            calculatedExp =
                gBaseStats[gBattleMons[gBattlerFainted].species].expYield * gBattleMons[gBattlerFainted].level / 7;
        ...
        case 4: // lvl up if necessary
            if (gBattleControllerExecFlags == 0) {
                ...
                if (gBattleBufferB[gActiveBattler][0] == CONTROLLER_TWORETURNVALUES &&
                    ...
                    PREPARE_MON_NICK_WITH_PREFIX_BUFFER(gBattleTextBuff1, gActiveBattler,
                                                        gBattleStruct->expGetterMonId);
                    PREPARE_BYTE_NUMBER_BUFFER(
                        gBattleTextBuff2, 3, GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL));
                    ...
                    if (gBattlerPartyIndexes[0] == gBattleStruct->expGetterMonId && gBattleMons[0].hp) {
                        ...
                        gBattleMons[0].spAttack =
                            GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPATK);
                        gBattleMons[0].spDefense =
                            GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPDEF);
                    }
                    // What is else if?
...
static void Cmd_playanimation(void) {
    ...
    if (gBattlescriptCurrInstr[2] == B_ANIM_STATS_CHANGE || gBattlescriptCurrInstr[2] == B_ANIM_SNATCH_MOVE ||
    ...
    if (*animationIdPtr == B_ANIM_STATS_CHANGE || *animationIdPtr == B_ANIM_SNATCH_MOVE ||
        *animationIdPtr == B_ANIM_SUBSTITUTE_FADE) {
        ...
        gBattlescriptCurrInstr += 10;
               ...
               *animationIdPtr == B_ANIM_SANDSTORM_CONTINUES || *animationIdPtr == B_ANIM_HAIL_CONTINUES) {
        BtlController_EmitBattleAnimation(0, *animationIdPtr, *argumentPtr);
        MarkBattlerForControllerExec(gActiveBattler);
        gBattlescriptCurrInstr += 10;
    } else if (gStatuses3[gActiveBattler] & STATUS3_SEMI_INVULNERABLE) {
    ...
    do {
        switch (gBattleScripting.moveendState) {
            case MOVEEND_RAGE: // rage check
                if (gBattleMons[gBattlerTarget].status2 & STATUS2_RAGE && gBattleMons[gBattlerTarget].hp != 0 &&
            ...
            case MOVEEND_DEFROST: // defrosting check
                if (gBattleMons[gBattlerTarget].status1 & STATUS1_FREEZE && gBattleMons[gBattlerTarget].hp != 0 &&
                    gBattlerAttacker != gBattlerTarget && gSpecialStatuses[gBattlerTarget].specialDmg &&
...
static void Cmd_typecalc2(void) {
    ...
    if (gBattleMons[gBattlerTarget].ability == ABILITY_WONDER_GUARD && !(flags & MOVE_RESULT_NO_EFFECT) &&
        AttacksThisTurn(gBattlerAttacker, gCurrentMove) == 2 &&
        (!(flags & MOVE_RESULT_SUPER_EFFECTIVE) ||
         ((flags & (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE)) ==
          (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE))) &&
...
static void Cmd_switchinanim(void) {
    ...
    if (GetBattlerSide(gActiveBattler) == B_SIDE_OPPONENT &&
        !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_x2000000 |
...
static void sub_804CF10(u8 slotId) {
    ...
    *(gBattleStruct->monToSwitchIntoId + gActiveBattler) = PARTY_SIZE;
    ...
    BtlController_EmitChoosePokemon(0, PARTY_ACTION_SEND_OUT, slotId, ABILITY_NONE,
    ...
    if (gBattlescriptCurrInstr[1] == 5) {
        ...
        if (gSpecialStatuses[battlerId].flag40) {
            ...
            *(gBattleStruct->monToSwitchIntoId + gActiveBattler) = 6;
            ...
            BtlController_EmitChoosePokemon(0, hitmarkerFaintBits,
                                            *(gBattleStruct->monToSwitchIntoId + (gActiveBattler ^ 2)), ABILITY_NONE,
                                            gBattleStruct->field_60[gActiveBattler]);
            MarkBattlerForControllerExec(gActiveBattler);
...
static void Cmd_switchineffects(void) {
    ...
    if (!(gSideStatuses[GetBattlerSide(gActiveBattler)] & SIDE_STATUS_SPIKES_DAMAGED) &&
        ...
        // The truant counter is not updated in the case where a mon switches in after a lost judgement in the battle
        // arena.
    ...
    switch (gBattleScripting.learnMoveState) {
        ...
        case 2:
            if (!gPaletteFade.active) {
                ...
                ShowSelectMovePokemonSummaryScreen(gPlayerParty, gBattleStruct->expGetterMonId, gPlayerPartyCount - 1,
                                                   ReshowBattleScreenAfterMenu, gMoveToLearn);
                gBattleScripting.learnMoveState++;
            }
            break;
        case 3:
        ...
        case 4:
            if (!gPaletteFade.active && gMain.callback2 == BattleMainCB2) {
                ...
                if (movePosition == MAX_MON_MOVES) {
                    ...
                    u16 moveId =
                        GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_MOVE1 + movePosition);
                    if (IsHMMove2(moveId)) {
...
static void Cmd_statusanimation(void) {
    if (gBattleControllerExecFlags == 0) {
        ...
        if (!(gStatuses3[gActiveBattler] & STATUS3_SEMI_INVULNERABLE) &&
            gDisableStructs[gActiveBattler].substituteHP == 0 && !(gHitMarker & HITMARKER_NO_ANIMATIONS)) {
...
static void DrawLevelUpWindow1(void) {
    ...
    DrawLevelUpWindowPg1(0xD, gBattleResources->beforeLvlUp->stats, currStats, TEXT_DYNAMIC_COLOR_5,
                         TEXT_DYNAMIC_COLOR_4, TEXT_DYNAMIC_COLOR_6);
}
...
static void Cmd_useitemonopponent(void) {
    ...
    PokemonUseItemEffects(&gEnemyParty[gBattlerPartyIndexes[gBattlerAttacker]], gLastUsedItem,
                          gBattlerPartyIndexes[gBattlerAttacker], 0, 1);
    gBattlescriptCurrInstr += 1;
}
    ...
    switch (gBattlescriptCurrInstr[2]) {
        ...
        case 7:
            if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_DOUBLE)) &&
                gBattleTypeFlags & BATTLE_TYPE_TRAINER && gBattleMons[0].hp != 0 && gBattleMons[1].hp != 0) {
                gHitMarker &= ~(HITMARKER_x400000);
            }
            break;
        case VARIOUS_PALACE_FLAVOR_TEXT:
            ...
            if (!(gBattleStruct->palaceFlags & gBitTable[gActiveBattler]) &&
                ...
                gBattleStruct->palaceFlags |= gBitTable[gActiveBattler];
                gBattleCommunication[0] = TRUE;
                gBattleCommunication[MULTISTRING_CHOOSER] = sBattlePalaceNatureToFlavorTextId[GetNatureFromPersonality(
                    gBattleMons[gActiveBattler].personality)];
            }
            break;
    ...
    if (gDisableStructs[gBattlerAttacker].stockpileCounter == 0) {
        ...
        if (gBattleCommunication[6] != 1) {
            gBattleMoveDamage = CalculateBaseDamage(&gBattleMons[gBattlerAttacker], &gBattleMons[gBattlerTarget],
                                                    gCurrentMove, gSideStatuses[GET_BATTLER_SIDE(gBattlerTarget)], 0, 0,
                                                    gBattlerAttacker, gBattlerTarget) *
                                gDisableStructs[gBattlerAttacker].stockpileCounter;
...
static void Cmd_stockpiletohpheal(void) {
    ...
    if (gDisableStructs[gBattlerAttacker].stockpileCounter == 0) {
        ...
        gBattleMoveDamage =
            gBattleMons[gBattlerAttacker].maxHP / (1 << (3 - gDisableStructs[gBattlerAttacker].stockpileCounter));
...
static void Cmd_statbuffchange(void) {
    ...
    if (ChangeStatBuffs(gBattleScripting.statChanger & 0xF0, GET_STAT_BUFF_ID(gBattleScripting.statChanger),
                        gBattlescriptCurrInstr[1], jumpPtr) == STAT_BUFF_WORKED)
        gBattlescriptCurrInstr += 6;
}
...
static bool8 TryDoForceSwitchOut(void) {
    if (gBattleMons[gBattlerAttacker].level >= gBattleMons[gBattlerTarget].level) {
        *(gBattleStruct->field_58 + gBattlerTarget) = gBattlerPartyIndexes[gBattlerTarget];
        ...
        if ((u32)((random * (gBattleMons[gBattlerAttacker].level + gBattleMons[gBattlerTarget].level) >> 8) + 1) <=
...
static void Cmd_weatherdamage(void) {
    if (WEATHER_HAS_EFFECT) {
        if (gBattleWeather & WEATHER_SANDSTORM_ANY) {
            if (gBattleMons[gBattlerAttacker].type1 != TYPE_ROCK && gBattleMons[gBattlerAttacker].type1 != TYPE_STEEL &&
                gBattleMons[gBattlerAttacker].type1 != TYPE_GROUND &&
                gBattleMons[gBattlerAttacker].type2 != TYPE_ROCK && gBattleMons[gBattlerAttacker].type2 != TYPE_STEEL &&
...
static void Cmd_updatestatusicon(void) {
    ...
    if (gBattlescriptCurrInstr[1] != BS_ATTACKER_WITH_PARTNER) {
        ...
        gActiveBattler = gBattlerAttacker;
        if (!(gAbsentBattlerFlags & gBitTable[gActiveBattler])) {
            BtlController_EmitStatusIconUpdate(0, gBattleMons[gActiveBattler].status1,
...
static void Cmd_setsubstitute(void) {
    ...
    if (gBattleMons[gBattlerAttacker].hp <= hp) {
        ...
        gBattleMoveDamage =
            gBattleMons[gBattlerAttacker].maxHP / 4; // one bit value will only work for pokemon which max hp can go to
...
static void Cmd_mimicattackcopy(void) {
    ...
    if (IsMoveUncopyableByMimic(gLastMoves[gBattlerTarget]) ||
        gBattleMons[gBattlerAttacker].status2 & STATUS2_TRANSFORMED || gLastMoves[gBattlerTarget] == 0 ||
...
static void Cmd_disablelastusedattack(void) {
    ...
    if (gDisableStructs[gBattlerTarget].disabledMove == 0 && i != MAX_MON_MOVES &&
        ...
        gDisableStructs[gBattlerTarget].disableTimerStartValue =
            gDisableStructs[gBattlerTarget].disableTimer; // used to save the random amount of turns?
        gBattlescriptCurrInstr += 5;
    } else {
        gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
    }
        ...
        for (j = 0, rands = 0; rands < sizeof(gTypeEffectiveness); j += 3, rands += 3) {
            switch (TYPE_EFFECT_ATK_TYPE(j)) {
                ...
                case TYPE_FORESIGHT:
                    break;
                default:
                    if (TYPE_EFFECT_ATK_TYPE(j) == gLastHitByType[gBattlerAttacker] && TYPE_EFFECT_MULTIPLIER(j) <= 5 &&
                        !IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_EFFECT_DEF_TYPE(i))) {
...
static void Cmd_copymovepermanently(void) // sketch
{
    ...
    if (!(gBattleMons[gBattlerAttacker].status2 & STATUS2_TRANSFORMED) &&
        gLastPrintedMoves[gBattlerTarget] != MOVE_STRUGGLE && gLastPrintedMoves[gBattlerTarget] != 0 &&
        gLastPrintedMoves[gBattlerTarget] != 0xFFFF && gLastPrintedMoves[gBattlerTarget] != MOVE_SKETCH) {
        s32 i;
...
static bool8 IsTwoTurnsMove(u16 move) {
    if (gBattleMoves[move].effect == EFFECT_SKULL_BASH || gBattleMoves[move].effect == EFFECT_RAZOR_WIND ||
        gBattleMoves[move].effect == EFFECT_SKY_ATTACK || gBattleMoves[move].effect == EFFECT_SOLARBEAM ||
        gBattleMoves[move].effect == EFFECT_SEMI_INVULNERABLE || gBattleMoves[move].effect == EFFECT_BIDE)
        return TRUE;
    else
        return FALSE;
}
    ...
    if (gCurrentMove == MOVE_HEAL_BELL) {
        ...
        for (i = 0; i < PARTY_SIZE; i++) {
            ...
            if (species != SPECIES_NONE && species != SPECIES_EGG) {
                ...
                else if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE && gBattlerPartyIndexes[gActiveBattler] == i &&
                         !(gAbsentBattlerFlags & gBitTable[gActiveBattler]))
...
static void Cmd_hiddenpowercalc(void) {
    ...
    powerBits =
        ((gBattleMons[gBattlerAttacker].hpIV & 2) >> 1) | ((gBattleMons[gBattlerAttacker].attackIV & 2) << 0) |
        ((gBattleMons[gBattlerAttacker].defenseIV & 2) << 1) | ((gBattleMons[gBattlerAttacker].speedIV & 2) << 2) |
        ((gBattleMons[gBattlerAttacker].spAttackIV & 2) << 3) | ((gBattleMons[gBattlerAttacker].spDefenseIV & 2) << 4);

    typeBits =
        ((gBattleMons[gBattlerAttacker].hpIV & 1) << 0) | ((gBattleMons[gBattlerAttacker].attackIV & 1) << 1) |
        ((gBattleMons[gBattlerAttacker].defenseIV & 1) << 2) | ((gBattleMons[gBattlerAttacker].speedIV & 1) << 3) |
        ((gBattleMons[gBattlerAttacker].spAttackIV & 1) << 4) | ((gBattleMons[gBattlerAttacker].spDefenseIV & 1) << 5);
...
static void Cmd_trysetfutureattack(void) {
    if (gWishFutureKnock.futureSightCounter[gBattlerTarget] != 0) {
        ...
        gWishFutureKnock.futureSightDmg[gBattlerTarget] = CalculateBaseDamage(
            &gBattleMons[gBattlerAttacker], &gBattleMons[gBattlerTarget], gCurrentMove,
            gSideStatuses[GET_BATTLER_SIDE(gBattlerTarget)], 0, 0, gBattlerAttacker, gBattlerTarget);
...
static void Cmd_jumpifattackandspecialattackcannotfall(void) // memento
{
    if (gBattleMons[gBattlerTarget].statStages[STAT_ATK] == MIN_STAT_STAGE &&
        gBattleMons[gBattlerTarget].statStages[STAT_SPATK] == MIN_STAT_STAGE && gBattleCommunication[6] != 1) {
        gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
    } else {
        gActiveBattler = gBattlerAttacker;
        gBattleMoveDamage = gBattleMons[gActiveBattler].hp;
...
static void Cmd_tryswapitems(void) // trick
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_TRAINER_HILL ||
        (GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT &&
         !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_FRONTIER |
        ...
        if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_FRONTIER |
                                  BATTLE_TYPE_SECRET_BASE | BATTLE_TYPE_x2000000)) &&
        ...
        // or if either of them is an enigma berry or a mail
                 ...
                 gBattleMons[gBattlerTarget].item == ITEM_ENIGMA_BERRY ||
                 IS_ITEM_MAIL(gBattleMons[gBattlerAttacker].item) || IS_ITEM_MAIL(gBattleMons[gBattlerTarget].item)) {
            gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
        }
...
static void Cmd_tryimprison(void) {
    if ((gStatuses3[gBattlerAttacker] & STATUS3_IMPRISONED_OTHERS)) {
        ...
        if (battlerId == gBattlersCount) // In Generation 3 games, Imprison fails if the user doesn't share any moves
                                         // with any of the foes
            gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
    }
}
...
static void Cmd_weightdamagecalculation(void) {
    ...
    for (i = 0; sWeightToDamageTable[i] != 0xFFFF; i += 2) {
        if (sWeightToDamageTable[i] >
            GetPokedexHeightWeight(SpeciesToNationalPokedexNum(gBattleMons[gBattlerTarget].species), 1))
            break;
    }
...
static void Cmd_switchoutabilities(void) {
    ...
    switch (gBattleMons[gActiveBattler].ability) {
        case ABILITY_NATURAL_CURE:
            gBattleMons[gActiveBattler].status1 = 0;
            BtlController_EmitSetMonData(0, REQUEST_STATUS_BATTLE,
                                         gBitTable[*(gBattleStruct->field_58 + gActiveBattler)], 4,
                                         &gBattleMons[gActiveBattler].status1);
            MarkBattlerForControllerExec(gActiveBattler);
            break;
    }
...
static void Cmd_pickup(void) {
    ...
    if (InBattlePike()) {
        ...
        for (i = 0; i < PARTY_SIZE; i++) {
            ...
            if (ability == ABILITY_PICKUP && species != 0 && species != SPECIES_EGG && heldItem == ITEM_NONE &&
                (Random() % 10) == 0) {
        ...
        if (gLastUsedItem > ITEM_SAFARI_BALL) {
            switch (gLastUsedItem) {
                ...
                case ITEM_REPEAT_BALL:
                    if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(gBattleMons[gBattlerTarget].species),
                                          FLAG_GET_CAUGHT))
                        ballMultiplier = 30;
                    else
                        ballMultiplier = 10;
...
static void Cmd_givecaughtmon(void) {
    ...
    gBattleResults.caughtMonBall =
        GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattlerAttacker ^ BIT_SIDE]], MON_DATA_POKEBALL, NULL);
        ...
        case 1:
            if (!gPaletteFade.active) {
                ...
                gBattleCommunication[TASK_ID] =
                    DisplayCaughtMonDexPage(SpeciesToNationalPokedexNum(species), gBattleMons[gBattlerTarget].otId,
    ...
    switch (gBattleCommunication[MULTIUSE_STATE]) {
        ...
        case 2:
            if (!gPaletteFade.active) {
                ...
                DoNamingScreen(
                    NAMING_SCREEN_CAUGHT_MON, gBattleStruct->caughtMonNick,
                    ...
                    GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattlerAttacker ^ BIT_SIDE]], MON_DATA_PERSONALITY,
                               NULL),
                    BattleMainCB2);
        ...
        case 3:
            if (gMain.callback2 == BattleMainCB2 && !gPaletteFade.active) {
                SetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattlerAttacker ^ BIT_SIDE]], MON_DATA_NICKNAME,
                           gBattleStruct->caughtMonNick);
                gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
            }
            break;
