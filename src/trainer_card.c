...
static const u16* const sHoennTrainerCardStarPals[] = {
    gHoennTrainerCard0Star_Pal, sHoennTrainerCard1Star_Pal, sHoennTrainerCard2Star_Pal,
    sHoennTrainerCard3Star_Pal, sHoennTrainerCard4Star_Pal,
};

static const u16* const sKantoTrainerCardStarPals[] = {
    gKantoTrainerCard0Star_Pal, sKantoTrainerCard1Star_Pal, sKantoTrainerCard2Star_Pal,
    sKantoTrainerCard3Star_Pal, sKantoTrainerCard4Star_Pal,
};
...
static const u8 sTimeColonInvisibleTextColors[6] = { TEXT_COLOR_TRANSPARENT, TEXT_COLOR_TRANSPARENT,
...
static bool8 (*const sTrainerCardFlipTasks[])(struct Task*) = {
    ...
    Task_SetCardFlipped, Task_AnimateCardFlipUp,   Task_EndCardFlip,
};
...
static void PrintPokedexOnCard(void) {
    ...
    if (FlagGet(FLAG_SYS_POKEDEX_GET)) {
        ...
        StringCopy(
            ConvertIntToDecimalStringN(gStringVar4, sData->trainerCard.caughtMonsCount, STR_CONV_MODE_LEFT_ALIGN, 3),
            gText_EmptyString6);
        if (!sData->isHoenn) {
            xOffset = GetStringRightAlignXOffset(1, gStringVar4, 144);
            top = 72;
...
static void PrintTimeOnCard(void) {
    ...
    x += 18;
    AddTextPrinterParameterized3(1, 1, x, y, sTimeColonTextColors[sData->timeColonInvisible], TEXT_SPEED_FF,
                                 gText_Colon2);
    x += width;
...
static void PrintProfilePhraseOnCard(void) {
    ...
    if (sData->isLink) {
        AddTextPrinterParameterized3(1, 1, 8, yOffsetsLine1[sData->isHoenn], sTrainerCardTextColors, TEXT_SPEED_FF,
                                     sData->easyChatProfile[0]);
        AddTextPrinterParameterized3(1, 1, GetStringWidth(1, sData->easyChatProfile[0], 0) + 14,
                                     yOffsetsLine1[sData->isHoenn], sTrainerCardTextColors, TEXT_SPEED_FF,
                                     sData->easyChatProfile[1]);
        AddTextPrinterParameterized3(1, 1, 8, yOffsetsLine2[sData->isHoenn], sTrainerCardTextColors, TEXT_SPEED_FF,
                                     sData->easyChatProfile[2]);
        AddTextPrinterParameterized3(1, 1, GetStringWidth(1, sData->easyChatProfile[2], 0) + 14,
                                     yOffsetsLine2[sData->isHoenn], sTrainerCardTextColors, TEXT_SPEED_FF,
...
static void PrintNameOnCardBack(void) {
    if (!sData->isHoenn)
        ...
        AddTextPrinterParameterized3(1, 1, GetStringRightAlignXOffset(1, sData->textPlayersCard, 216), 9,
                                     sTrainerCardTextColors, TEXT_SPEED_FF, sData->textPlayersCard);
}
...
static void PrintStatOnBackOfCard(u8 top, const u8* statName, u8* stat, const u8* color) {
    ...
    AddTextPrinterParameterized3(1, 1, xOffsets[sData->isHoenn], top * 16 + 33, sTrainerCardTextColors, TEXT_SPEED_FF,
                                 statName);
    AddTextPrinterParameterized3(1, 1, GetStringRightAlignXOffset(1, stat, widths[sData->isHoenn]), top * 16 + 33,
                                 color, TEXT_SPEED_FF, stat);
}
...
static void BufferLinkBattleResults(void) {
    if (sData->hasLinkResults) {
        ...
        ConvertIntToDecimalStringN(sData->textLinkBattleWins, sData->trainerCard.linkBattleWins,
                                   STR_CONV_MODE_LEFT_ALIGN, 4);
        ConvertIntToDecimalStringN(sData->textLinkBattleLosses, sData->trainerCard.linkBattleLosses,
                                   STR_CONV_MODE_LEFT_ALIGN, 4);
    }
}
...
static void BufferNumTrades(void) {
    if (sData->hasTrades)
        ConvertIntToDecimalStringN(sData->textNumTrades, sData->trainerCard.pokemonTrades, STR_CONV_MODE_RIGHT_ALIGN,
                                   5);
}
...
static void BufferUnionRoomStats(void) {
    if (sData->cardType == CARD_TYPE_FRLG && sData->trainerCard.unionRoomNum)
        ConvertIntToDecimalStringN(sData->textUnionRoomStats, sData->trainerCard.unionRoomNum,
                                   STR_CONV_MODE_RIGHT_ALIGN, 5);
}
...
static void BufferLinkContestNum(void) {
    if (sData->cardType != CARD_TYPE_FRLG && sData->trainerCard.contestsWithFriends)
        ConvertIntToDecimalStringN(sData->textNumLinkContests, sData->trainerCard.contestsWithFriends,
                                   STR_CONV_MODE_RIGHT_ALIGN, 5);
}
    ...
    switch (sData->cardType) {
        case CARD_TYPE_RS:
            if (sData->hasBattleTowerWins) {
                ConvertIntToDecimalStringN(gStringVar1, sData->trainerCard.battleTowerWins, STR_CONV_MODE_RIGHT_ALIGN,
                                           4);
                ConvertIntToDecimalStringN(gStringVar2, sData->trainerCard.battleTowerStraightWins,
...
static void CreateTrainerCardTrainerPic(void) {
    if (InUnionRoom() == TRUE && gReceivedRemoteLinkPlayers == 1) {
        CreateTrainerCardTrainerPicSprite(FacilityClassToPicIndex(sData->trainerCard.facilityClass), TRUE,
                                          sTrainerPicOffset[sData->isHoenn][sData->trainerCard.gender][0],
                                          sTrainerPicOffset[sData->isHoenn][sData->trainerCard.gender][1], 8, 2);
    } else {
        CreateTrainerCardTrainerPicSprite(
            FacilityClassToPicIndex(sTrainerPicFacilityClass[sData->cardType][sData->trainerCard.gender]), TRUE,
            sTrainerPicOffset[sData->isHoenn][sData->trainerCard.gender][0],
            sTrainerPicOffset[sData->isHoenn][sData->trainerCard.gender][1], 8, 2);
    }
}
