...
static const u16 sRegionMapPlayerIcon_BrendanPal[] =
    INCBIN_U16("graphics/pokenav/brendan_icon.gbapal");
...
static const u8 sRegionMap_MapSectionLayout[] =
    INCBIN_U8("graphics/pokenav/region_map_section_layout.bin");

#include "data/region_map/region_map_entries.h"

static const u16 sRegionMap_SpecialPlaceLocations[][2] = { { MAPSEC_UNDERWATER_TERRA_CAVE,
                                                               MAPSEC_ROUTE_105 },
...
static const u16 sMarineCaveMapSecIds[] = {
    MAPSEC_MARINE_CAVE, MAPSEC_UNDERWATER_MARINE_CAVE, MAPSEC_UNDERWATER_MARINE_CAVE
};
...
static const u8 sMapSecAquaHideoutOld[] = { MAPSEC_AQUA_HIDEOUT_OLD };
...
static const union AnimCmd sRegionMapCursorAnim2[] = { ANIMCMD_FRAME(0, 10),
    ANIMCMD_FRAME(16, 10),
    ANIMCMD_FRAME(32, 10),
    ANIMCMD_FRAME(16, 10),
    ANIMCMD_JUMP(0) };

static const union AnimCmd *const sRegionMapCursorAnimTable[] = { sRegionMapCursorAnim1,
    sRegionMapCursorAnim2 };

static const struct SpritePalette sRegionMapCursorSpritePalette = { .data = sRegionMapCursorPal,
    .tag = 0 };

static const struct SpriteTemplate sRegionMapCursorSpriteTemplate = { .tileTag = 0,
    .paletteTag = 0,
...
static const struct OamData sRegionMapPlayerIconOam = {
    .shape = SPRITE_SHAPE(16x16), .size = SPRITE_SIZE(16x16), .priority = 2
};

static const union AnimCmd sRegionMapPlayerIconAnim1[] = { ANIMCMD_FRAME(0, 5), ANIMCMD_END };

static const union AnimCmd *const sRegionMapPlayerIconAnimTable[] = { sRegionMapPlayerIconAnim1 };
...
static const u8 sMapHealLocations[][3] = { { MAP_GROUP(LITTLEROOT_TOWN),
                                               MAP_NUM(LITTLEROOT_TOWN),
...
static const u8 *const sEverGrandeCityNames[] = { gText_PokemonLeague, gText_PokemonCenter };

static const struct MultiNameFlyDest sMultiNameFlyDestinations[] = { { .name = sEverGrandeCityNames,
    .mapSecId = MAPSEC_EVER_GRANDE_CITY,
    .flag = FLAG_LANDMARK_POKEMON_LEAGUE } };

static const struct BgTemplate sFlyMapBgTemplates[] = { { .bg = 0,
                                                            .charBaseIndex = 0,
                                                            .mapBaseIndex = 31,
                                                            .screenSize = 0,
                                                            .paletteMode = 0,
...
static const struct WindowTemplate sFlyMapWindowTemplates[] = { { .bg = 0,
                                                                    .tilemapLeft = 17,
                                                                    .tilemapTop = 17,
                                                                    .width = 12,
                                                                    .height = 2,
...
static const struct SpritePalette sFlyTargetIconsSpritePalette = { .data = sFlyTargetIcons_Pal,
    .tag = 2 };

static const u16 sRedOutlineFlyDestinations[][2] = {
    { FLAG_LANDMARK_BATTLE_FRONTIER, MAPSEC_BATTLE_FRONTIER }, { -1, MAPSEC_NONE }
};
...
static const union AnimCmd sFlyDestIcon_Anim_8x8CanFly[] = { ANIMCMD_FRAME(0, 5), ANIMCMD_END };

static const union AnimCmd sFlyDestIcon_Anim_16x8CanFly[] = { ANIMCMD_FRAME(1, 5), ANIMCMD_END };

static const union AnimCmd sFlyDestIcon_Anim_8x16CanFly[] = { ANIMCMD_FRAME(3, 5), ANIMCMD_END };

static const union AnimCmd sFlyDestIcon_Anim_8x8CantFly[] = { ANIMCMD_FRAME(5, 5), ANIMCMD_END };

static const union AnimCmd sFlyDestIcon_Anim_16x8CantFly[] = { ANIMCMD_FRAME(6, 5), ANIMCMD_END };

static const union AnimCmd sFlyDestIcon_Anim_8x16CantFly[] = { ANIMCMD_FRAME(8, 5), ANIMCMD_END };

// Only used by Battle Frontier
static const union AnimCmd sFlyDestIcon_Anim_RedOutline[] = { ANIMCMD_FRAME(10, 5), ANIMCMD_END };

static const union AnimCmd *const sFlyDestIcon_Anims[] = { [SPRITE_SHAPE(8x8)] =
...
static const struct SpriteTemplate sFlyDestIconSpriteTemplate = { .tileTag = 2,
    .paletteTag = 2,
...
void InitRegionMapData(struct RegionMap *regionMap, const struct BgTemplate *template, bool8 zoomed)
{
    ...
    gRegionMap->inputCallback =
        zoomed == TRUE ? ProcessRegionMapInput_Zoomed : ProcessRegionMapInput_Full;
    if (template != NULL)
    {
            ...
            if (gRegionMap->bgManaged)
            {
                if (!FreeTempTileDataBuffersIfPossible())
                    DecompressAndCopyTileDataToVram(
                        gRegionMap->bgNum, sRegionMapBg_TilemapLZ, 0, 0, 1);
            }
            else
            {
        ...
        case 6:
            ...
            else
            {
                ...
                CalcZoomScrollParams(
                    gRegionMap->scrollX, gRegionMap->scrollY, 0x38, 0x48, 0x80, 0x80, 0);
            }
            break;
        case 7:
...
bool8 UpdateRegionMapZoom(void)
{
    ...
    if (gRegionMap->unk_06e == 16)
    {
        ...
        gRegionMap->inputCallback = (gRegionMap->zoomed == FALSE) ? ProcessRegionMapInput_Full
    ...
    CalcZoomScrollParams(gRegionMap->scrollX,
        gRegionMap->scrollY,
        0x38,
        0x48,
        gRegionMap->unk_04c >> 8,
        gRegionMap->unk_04c >> 8,
        0);
    return retVal;
}
...
static void InitMapBasedOnPlayerLocation(void)
{
    ...
    switch (
        GetMapTypeByGroupAndId(gSaveBlock1Ptr->location.mapGroup, gSaveBlock1Ptr->location.mapNum))
    {
        default:
        case MAP_TYPE_TOWN:
        case MAP_TYPE_CITY:
        ...
        case MAP_TYPE_OCEAN_ROUTE:
            ...
            if (gRegionMap->mapSecId == MAPSEC_UNDERWATER_128
                || gRegionMap->mapSecId == MAPSEC_UNDERWATER_MARINE_CAVE)
        ...
        case MAP_TYPE_SECRET_BASE:
            mapHeader = Overworld_GetMapHeaderByGroupAndId(
                (u16)gSaveBlock1Ptr->dynamicWarp.mapGroup, (u16)gSaveBlock1Ptr->dynamicWarp.mapNum);
            gRegionMap->mapSecId = mapHeader->regionMapSectionId;
            gRegionMap->playerIsInCave = TRUE;
            mapWidth = mapHeader->mapLayout->width;
            mapHeight = mapHeader->mapLayout->height;
...
static void RegionMap_InitializeStateBasedOnSSTidalLocation(void)
{
    ...
    switch (GetSSTidalLocation(&mapGroup, &mapNum, &xOnMap, &yOnMap))
    {
        ...
        case SS_TIDAL_LOCATION_CURRENTS:
            ...
            gRegionMap->mapSecId = mapHeader->regionMapSectionId;
            dimensionScale =
                mapHeader->mapLayout->width / gRegionMapEntries[gRegionMap->mapSecId].width;
            if (dimensionScale == 0)
                dimensionScale = 1;
            ...
            dimensionScale =
                mapHeader->mapLayout->height / gRegionMapEntries[gRegionMap->mapSecId].height;
            if (dimensionScale == 0)
                dimensionScale = 1;
...
static u8 GetMapsecType(u16 mapSecId)
{
    switch (mapSecId)
    {
        ...
        case MAPSEC_LITTLEROOT_TOWN:
            return FlagGet(FLAG_VISITED_LITTLEROOT_TOWN) ? MAPSECTYPE_CITY_CANFLY
                                                         : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_OLDALE_TOWN:
            return FlagGet(FLAG_VISITED_OLDALE_TOWN) ? MAPSECTYPE_CITY_CANFLY
                                                     : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_DEWFORD_TOWN:
            return FlagGet(FLAG_VISITED_DEWFORD_TOWN) ? MAPSECTYPE_CITY_CANFLY
                                                      : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_LAVARIDGE_TOWN:
            return FlagGet(FLAG_VISITED_LAVARIDGE_TOWN) ? MAPSECTYPE_CITY_CANFLY
                                                        : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_FALLARBOR_TOWN:
            return FlagGet(FLAG_VISITED_FALLARBOR_TOWN) ? MAPSECTYPE_CITY_CANFLY
                                                        : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_VERDANTURF_TOWN:
            return FlagGet(FLAG_VISITED_VERDANTURF_TOWN) ? MAPSECTYPE_CITY_CANFLY
                                                         : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_PACIFIDLOG_TOWN:
            return FlagGet(FLAG_VISITED_PACIFIDLOG_TOWN) ? MAPSECTYPE_CITY_CANFLY
                                                         : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_PETALBURG_CITY:
            return FlagGet(FLAG_VISITED_PETALBURG_CITY) ? MAPSECTYPE_CITY_CANFLY
                                                        : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_SLATEPORT_CITY:
            return FlagGet(FLAG_VISITED_SLATEPORT_CITY) ? MAPSECTYPE_CITY_CANFLY
                                                        : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_MAUVILLE_CITY:
            return FlagGet(FLAG_VISITED_MAUVILLE_CITY) ? MAPSECTYPE_CITY_CANFLY
                                                       : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_RUSTBORO_CITY:
            return FlagGet(FLAG_VISITED_RUSTBORO_CITY) ? MAPSECTYPE_CITY_CANFLY
                                                       : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_FORTREE_CITY:
            return FlagGet(FLAG_VISITED_FORTREE_CITY) ? MAPSECTYPE_CITY_CANFLY
                                                      : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_LILYCOVE_CITY:
            return FlagGet(FLAG_VISITED_LILYCOVE_CITY) ? MAPSECTYPE_CITY_CANFLY
                                                       : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_MOSSDEEP_CITY:
            return FlagGet(FLAG_VISITED_MOSSDEEP_CITY) ? MAPSECTYPE_CITY_CANFLY
                                                       : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_SOOTOPOLIS_CITY:
            return FlagGet(FLAG_VISITED_SOOTOPOLIS_CITY) ? MAPSECTYPE_CITY_CANFLY
                                                         : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_EVER_GRANDE_CITY:
            return FlagGet(FLAG_VISITED_EVER_GRANDE_CITY) ? MAPSECTYPE_CITY_CANFLY
                                                          : MAPSECTYPE_CITY_CANTFLY;
        case MAPSEC_BATTLE_FRONTIER:
            return FlagGet(FLAG_LANDMARK_BATTLE_FRONTIER) ? MAPSECTYPE_BATTLE_FRONTIER
                                                          : MAPSECTYPE_NONE;
...
// Probably meant to be an "IsPlayerInIndoorDungeon" function, but in practice it only has the one
// mapsec Additionally, because the mapsec doesnt exist in Emerald, this function always returns
// FALSE
static bool32 IsPlayerInAquaHideout(u8 mapSecId)
{
    u32 i;
...
void CreateRegionMapPlayerIcon(u16 tileTag, u16 paletteTag)
{
    ...
    struct SpriteTemplate template = { tileTag,
        paletteTag,
...
static void SpriteCB_PlayerIconMapZoomed(struct Sprite *sprite)
{
    ...
    if (sprite->data[0] < -8 || sprite->data[0] > 0xa8 || sprite->data[1] < -8
        || sprite->data[1] > 0xf8)
    {
        sprite->data[2] = FALSE;
    }
        ...
        && sFlyMap->regionMap.mapSecType <= MAPSECTYPE_BATTLE_FRONTIER)
    {
        ...
        for (i = 0; i < ARRAY_COUNT(sMultiNameFlyDestinations); i++)
        {
            if (sFlyMap->regionMap.mapSecId == sMultiNameFlyDestinations[i].mapSecId)
            {
                ...
                {
                    StringLength(
                        sMultiNameFlyDestinations[i].name[sFlyMap->regionMap.posWithinMapSec]);
                    namePrinted = TRUE;
                    ...
                    AddTextPrinterParameterized(
                        1, 1, name, GetStringRightAlignXOffset(1, name, 96), 17, 0, NULL);
                    ScheduleBgCopyTilemapToVram(0);
                    gUnknown_03001180 = TRUE;
                }
                break;
...
// Flickers fly destination icon color (by hiding the fly icon sprite) if the cursor is currently on
// it
static void SpriteCB_FlyDestIcon(struct Sprite *sprite)
{
                        ...
                        default:
                            if (sMapHealLocations[sFlyMap->regionMap.mapSecId][2] != 0)
                                ...
                                SetWarpDestinationToMapWarp(
                                    sMapHealLocations[sFlyMap->regionMap.mapSecId][0],
                                    sMapHealLocations[sFlyMap->regionMap.mapSecId][1],
                                    -1);
                            break;
                    }
