...
// GROUPTYPE_TOTAL have no unique group and are simply counted in the total of "people
// communicating" UB: GROUPTYPE_NONE (-1) can potentially be used as an index into a u8[4] in
...
static void PrintHeaderTexts(void)
{
    ...
    WCSS_AddTextPrinterParameterized(0,
        1,
        sHeaderTexts[0],
        GetStringCenterAlignXOffset(1, sHeaderTexts[0], 0xC0),
        6,
        COLORMODE_GREEN);
    for (i = 0; i < (int)ARRAY_COUNT(*sHeaderTexts) - 1; i++)
    {
        WCSS_AddTextPrinterParameterized(
            1, 1, sHeaderTexts[i + 1], 0, 30 * i + 8, COLORMODE_WHITE_LGRAY);
    }
...
static void Task_WirelessCommunicationScreen(u8 taskId)
{
    ...
    switch (gTasks[taskId].tState)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            if (UpdateCommunicationCounts(sStatusScreen->groupCounts,
                    sStatusScreen->prevGroupCounts,
                    sStatusScreen->activities,
                    sStatusScreen->rfuTaskId))
            {
                ...
                for (i = 0; i < NUM_GROUPTYPES; i++)
                {
                    ConvertIntToDecimalStringN(
                        gStringVar4, sStatusScreen->groupCounts[i], STR_CONV_MODE_RIGHT_ALIGN, 2);
                    if (i != GROUPTYPE_TOTAL)
                        WCSS_AddTextPrinterParameterized(
                            2, 1, gStringVar4, 12, 30 * i + 8, COLORMODE_WHITE_LGRAY);
                    else
...
static void WCSS_AddTextPrinterParameterized(
    u8 windowId, u8 fontId, const u8 *str, u8 x, u8 y, u8 mode)
{
    u8 color[3];
