...
static union PokemonSubstruct *GetSubstruct(
    struct BoxPokemon *boxMon, u32 personality, u8 substructType);
...
const u16 gSpeciesToHoennPokedexNum[] = // Assigns all species to the Hoenn Dex Index (Summary No.
                                        // for Hoenn Dex)
    {
...
const u16 gSpeciesToNationalPokedexNum[] = // Assigns all species to the National Dex Index (Summary
                                           // No. for National Dex)
    {
...
// SPECIES_NONE are ignored in the following two tables, so decrement before accessing these arrays
...
const u16 gLinkPlayerFacilityClasses[NUM_MALE_LINK_FACILITY_CLASSES
                                     + NUM_FEMALE_LINK_FACILITY_CLASSES] = {
    // Male classes
...
static const u8 sGetMonDataEVConstants[] = { MON_DATA_HP_EV,
    MON_DATA_ATK_EV,
    MON_DATA_DEF_EV,
    MON_DATA_SPEED_EV,
    MON_DATA_SPDEF_EV,
...
static const u16 sHMMoves[] = { MOVE_CUT,
    MOVE_FLY,
    MOVE_SURF,
    MOVE_STRENGTH,
    MOVE_FLASH,
...
void CreateApprenticeMon(struct Pokemon *mon, const struct Apprentice *src, u8 monId)
{
    ...
    u32 personality =
        ((gApprentices[src->id].otId >> 8) | ((gApprentices[src->id].otId & 0xFF) << 8))
    ...
    u32 fixedOtId)
{
    ...
    CreateMon(
        mon, species, level, fixedIV, hasFixedPersonality, fixedPersonality, otIdType, fixedOtId);
...
u16 MonTryLearningNewMove(struct Pokemon *mon, bool8 firstMove)
{
    ...
    if (firstMove)
    {
        ...
        while (
            (gLevelUpLearnsets[species][sLearningMoveTableID] & LEVEL_UP_MOVE_LV) != (level << 9))
        {
...
u32 GetBoxMonData(struct BoxPokemon *boxMon, s32 field, u8 *data)
{
    ...
    switch (field)
    {
        ...
        case MON_DATA_NICKNAME:
        {
            ...
            {
                for (retVal = 0; retVal < POKEMON_NAME_LENGTH;
        ...
        case MON_DATA_IVS:
            retVal = substruct3->hpIV | (substruct3->attackIV << 5) | (substruct3->defenseIV << 10)
                     | (substruct3->speedIV << 15) | (substruct3->spAttackIV << 20)
        ...
        case MON_DATA_KNOWN_MOVES:
            if (substruct0->species && !substruct3->isEgg)
            {
                ...
                {
                    u16 move = moves[i];
                    if (substruct1->moves[0] == move || substruct1->moves[1] == move
                        || substruct1->moves[2] == move || substruct1->moves[3] == move)
                        retVal |= gBitTable[i];
                    i++;
                }
            }
        ...
        case MON_DATA_RIBBONS:
            ...
            if (substruct0->species && !substruct3->isEgg)
            {
                retVal = substruct3->championRibbon | (substruct3->coolRibbon << 1)
                         | (substruct3->beautyRibbon << 4) | (substruct3->cuteRibbon << 7)
                         | (substruct3->smartRibbon << 10) | (substruct3->toughRibbon << 13)
                         | (substruct3->winningRibbon << 16) | (substruct3->victoryRibbon << 17)
                         | (substruct3->artistRibbon << 18) | (substruct3->effortRibbon << 19)
                         | (substruct3->giftRibbon1 << 20) | (substruct3->giftRibbon2 << 21)
                         | (substruct3->giftRibbon3 << 22) | (substruct3->giftRibbon4 << 23)
                         | (substruct3->giftRibbon5 << 24) | (substruct3->giftRibbon6 << 25)
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        if (gBattleResources->secretBase->party.species[i])
        {
            ...
            for (j = 0; j < MAX_MON_MOVES; j++)
            {
                ...
                SetMonData(&gEnemyParty[i],
                    MON_DATA_PP1 + j,
                    &gBattleMoves[gBattleResources->secretBase->party.moves[i * MAX_MON_MOVES + j]]
                         .pp);
            }
        }
    }
...
u8 CalculatePPWithBonus(u16 move, u8 ppBonuses, u8 moveIndex)
{
    ...
    return basePP
           + ((basePP * 20 * ((gPPUpGetMask[moveIndex] & ppBonuses) >> (2 * moveIndex))) / 100);
}
...
void CopyPlayerPartyMonToBattleData(u8 battlerId, u8 partyIndex)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        gBattleMons[battlerId].moves[i] =
            GetMonData(&gPlayerParty[partyIndex], MON_DATA_MOVE1 + i, NULL);
        gBattleMons[battlerId].pp[i] =
            GetMonData(&gPlayerParty[partyIndex], MON_DATA_PP1 + i, NULL);
    }

    gBattleMons[battlerId].ppBonuses =
        GetMonData(&gPlayerParty[partyIndex], MON_DATA_PP_BONUSES, NULL);
    gBattleMons[battlerId].friendship =
        GetMonData(&gPlayerParty[partyIndex], MON_DATA_FRIENDSHIP, NULL);
    ...
    gBattleMons[battlerId].spAttackIV =
        GetMonData(&gPlayerParty[partyIndex], MON_DATA_SPATK_IV, NULL);
    gBattleMons[battlerId].spDefenseIV =
        GetMonData(&gPlayerParty[partyIndex], MON_DATA_SPDEF_IV, NULL);
    gBattleMons[battlerId].personality =
        GetMonData(&gPlayerParty[partyIndex], MON_DATA_PERSONALITY, NULL);
    ...
    gBattleMons[battlerId].abilityNum =
        GetMonData(&gPlayerParty[partyIndex], MON_DATA_ABILITY_NUM, NULL);
...
bool8 PokemonUseItemEffects(struct Pokemon *mon, u16 item, u8 partyIndex, u8 moveIndex, u8 e)
{
    ...
    for (cmdIndex = 0; cmdIndex < 6; cmdIndex++)
    {
        switch (cmdIndex)
        {
            ...
            case 0:
                if ((itemEffect[cmdIndex] & ITEM0_INFATUATION) && gMain.inBattle
                    && battlerId != MAX_BATTLERS_COUNT
                    ...
                    && gBattleMons[gActiveBattler].statStages[STAT_ATK] < MAX_STAT_STAGE)
                {
                    gBattleMons[gActiveBattler].statStages[STAT_ATK] +=
                        itemEffect[cmdIndex] & ITEM0_X_ATTACK;
            ...
            case 1:
                ...
                {
                    gBattleMons[gActiveBattler].statStages[STAT_DEF] +=
                        (itemEffect[cmdIndex] & ITEM1_X_DEFEND) >> 4;
                    ...
                    && gBattleMons[gActiveBattler].statStages[STAT_SPEED] < MAX_STAT_STAGE)
                {
                    gBattleMons[gActiveBattler].statStages[STAT_SPEED] +=
                        itemEffect[cmdIndex] & ITEM1_X_SPEED;
            ...
            case 2:
                ...
                {
                    gBattleMons[gActiveBattler].statStages[STAT_ACC] +=
                        (itemEffect[cmdIndex] & ITEM2_X_ACCURACY) >> 4;
                    ...
                    && gBattleMons[gActiveBattler].statStages[STAT_SPATK] < MAX_STAT_STAGE)
                {
                    gBattleMons[gActiveBattler].statStages[STAT_SPATK] +=
                        itemEffect[cmdIndex] & ITEM2_X_SPATK;
            ...
            case 3:
                ...
                {
                    dataUnsigned =
                        gExperienceTables[gBaseStats[GetMonData(mon, MON_DATA_SPECIES, NULL)]
                                              .growthRate]
                ...
                if ((itemEffect[cmdIndex] & ITEM3_POISON)
                    && HealStatusConditions(
                           mon, partyIndex, STATUS1_PSN_ANY | STATUS1_TOXIC_COUNTER, battlerId)
                           == 0)
                    retVal = FALSE;
            ...
            case 4:
                ...
                if (r10 & ITEM4_PP_UP)
                {
                    r10 &= ~ITEM4_PP_UP;
                    dataUnsigned =
                        (GetMonData(mon, MON_DATA_PP_BONUSES, NULL) & gPPUpGetMask[moveIndex])
                        >> (moveIndex * 2);
                    ...
                    {
                        dataUnsigned =
                            GetMonData(mon, MON_DATA_PP_BONUSES, NULL) + gPPUpAddMask[moveIndex];
                        ...
                        dataUnsigned =
                            ...
                            - var_38;
                        dataUnsigned =
                            GetMonData(mon, MON_DATA_PP1 + moveIndex, NULL) + dataUnsigned;
                    ...
                    if (r10 & 1)
                    {
                        switch (var_38)
                        {
                            ...
                            case 2:
                                ...
                                if (r10 & 0x10)
                                {
                                    ...
                                    if (gMain.inBattle)
                                    {
                                        if (battlerId != 4)
                                        {
                                            ...
                                            CopyPlayerPartyMonToBattleData(battlerId,
                                ...
                                if (GetMonData(mon, MON_DATA_MAX_HP, NULL)
                                    != GetMonData(mon, MON_DATA_HP, NULL))
                                {
                                    if (e == 0)
                                    {
                                        dataUnsigned =
                                            GetMonData(mon, MON_DATA_HP, NULL) + dataUnsigned;
                            ...
                            case 3:
                                                ...
                                                r5))
                                        {
                                            ...
                                            if (gMain.inBattle && battlerId != MAX_BATTLERS_COUNT
                                                && !(gBattleMons[battlerId].status2
                                                     & STATUS2_TRANSFORMED)
                                                && !(gDisableStructs[battlerId].mimickedMoves
                                                     & gBitTable[r5]))
                                ...
                                else
                                {
                                            ...
                                            moveIndex))
                                    {
                                        ...
                                        {
                                            moveId =
                                                GetMonData(mon, MON_DATA_MOVE1 + moveIndex, NULL);
                                            dataUnsigned = CalculatePPWithBonus(moveId,
                                                GetMonData(mon, MON_DATA_PP_BONUSES, NULL),
                                                moveIndex);
                                        }
                    ...
                    if (r10 & 1)
                    {
                        switch (var_38)
                        {
                            ...
                            case 3:
                                ...
                                r5 = itemEffect[var_3C];
                                dataSigned =
                                    GetMonData(mon, sGetMonDataEVConstants[var_38 + 2], NULL);
                                r2 = r5;
                                if (r2 > 0)
                                {
                                    if (evCount >= MAX_TOTAL_EVS)
                            ...
                            case 4:
                                dataUnsigned = (GetMonData(mon, MON_DATA_PP_BONUSES, NULL)
                                                   & gPPUpGetMask[moveIndex])
                                               >> (moveIndex * 2);
                                r5 = CalculatePPWithBonus(
                                    GetMonData(mon, MON_DATA_MOVE1 + moveIndex, NULL),
                                    GetMonData(mon, MON_DATA_PP_BONUSES, NULL),
                                    moveIndex);
                            ...
                            case 5:
                                if (GetMonData(mon, MON_DATA_FRIENDSHIP, NULL) < 100
                                    && (retVal == 0 || var_28 != 0) && !ShouldSkipFriendshipChange()
                                    && var_34 == 0)
                                {
                                    ...
                                    if (var_34 > 0)
                                    {
                                        if (GetMonData(mon, MON_DATA_POKEBALL, NULL)
                                            == ITEM_LUXURY_BALL)
                                            friendship++;
            ...
            for (i = 0; i < EVOS_PER_MON; i++)
            {
                switch (gEvolutionTable[species][i].method)
                {
                    ...
                    case EVO_LEVEL_ATK_EQ_DEF:
                        if (gEvolutionTable[species][i].param <= level)
                            if (GetMonData(mon, MON_DATA_ATK, 0)
                                == GetMonData(mon, MON_DATA_DEF, 0))
                                targetSpecies = gEvolutionTable[species][i].targetSpecies;
                        break;
...
{
    if (species == SPECIES_SPINDA && dest != gMonSpritesGfxPtr->sprites[0]
...
u16 ModifyStatByNature(u8 nature, u16 n, u8 statIndex)
{
    ...
    switch (gNatureStatTable[nature][statIndex - 1])
    {
        case 1:
            return (u16)(n * 110)
                   / 100; // NOTE: will overflow for n > 595 because the intermediate value is cast
                          // to u16 before the division. Fix by removing (u16) cast
        case -1:
            ...
            if (GetMonData(&party[i], MON_DATA_SPECIES, 0))
            {
                ...
                if (pokerus)
                {
                    if (pokerus & 0xF)
                    {
                        ...
                        if (i != (PARTY_SIZE - 1)
                            && !(GetMonData(&party[i + 1], MON_DATA_POKERUS, 0) & 0xF0))
                        {
                            SetMonData(&party[i + 1], MON_DATA_POKERUS, &curPokerus);
                            i++;
                        }
...
bool8 TryIncrementMonLevel(struct Pokemon *mon)
{
    ...
    if (nextLevel > MAX_LEVEL
        || expPoints < gExperienceTables[gBaseStats[species].growthRate][nextLevel])
    {
        return FALSE;
    }
    else
...
u8 GetMoveRelearnerMoves(struct Pokemon *mon, u16 *moves)
{
    ...
    for (i = 0; i < 20; i++)
    {
        ...
        if (moveLevel <= (level << 9))
        {
            for (j = 0; j < MAX_MON_MOVES
                        && learnedMoves[j] != (gLevelUpLearnsets[species][i] & LEVEL_UP_MOVE_ID);
                 j++)
                ;
            ...
            {
                for (k = 0;
                     k < numMoves && moves[k] != (gLevelUpLearnsets[species][i] & LEVEL_UP_MOVE_ID);
                     k++)
                    ;
        ...
        switch (trainerClass)
        {
            ...
            case TRAINER_CLASS_PKMN_TRAINER_3:
                if (gBattleTypeFlags & BATTLE_TYPE_FRONTIER)
                    return MUS_BATTLE35;
                if (!StringCompare(
                        gTrainers[gTrainerBattleOpponent_A].trainerName, gText_BattleWallyName))
                    return MUS_BATTLE20;
                return MUS_BATTLE35;
...
void SetMonPreventsSwitchingString(void)
{
    ...
    PREPARE_MON_NICK_WITH_PREFIX_BUFFER(gBattleTextBuff2,
        gBattlerInMenuId,
...
{
    if (!(gBattleTypeFlags
            & (BATTLE_TYPE_LEGENDARY | BATTLE_TYPE_TRAINER | BATTLE_TYPE_PYRAMID
                | BATTLE_TYPE_PIKE)))
    {
        u16 rnd = Random() % 100;
...
#define READ_PTR_FROM_TASK(taskId, dataId) \
    (void *)((                             \
        (u16)(gTasks[taskId].data[dataId]) | ((u16)(gTasks[taskId].data[dataId + 1]) << 0x10)))
...
static void sub_806F160(struct Unknown_806F160_Struct *structPtr)
{
    ...
    for (i = 0; i < FORCE_SIGNED(structPtr->field_0_0); i++)
    {
        ...
        for (j = 0; j < structPtr->field_1; j++)
        {
            ...
            asm("");
#endif
            structPtr->frameImages[i * structPtr->field_1 + j].data =
                &structPtr->byteArrays[i][j * 0x800];
        }
...
static void sub_806F1FC(struct Unknown_806F160_Struct *structPtr)
{
    ...
    for (i = 0; i < FORCE_SIGNED(structPtr->field_0_0); i++)
    {
        ...
        for (j = 0; j < structPtr->field_1; j++)
        {
            structPtr->frameImages[i * structPtr->field_0_0 + j].data =
                &structPtr->byteArrays[i][j * 0x800];
        }
...
struct Unknown_806F160_Struct *sub_806F2AC(u8 id, u8 arg1)
{
    ...
    structPtr->frameImages =
        AllocZeroed(sizeof(struct SpriteFrameImage) * structPtr->field_0_0 * structPtr->field_1);
