        ...
        case 3:
                ...
                GetMonData(mon, MON_DATA_LEVEL, NULL) != MAX_LEVEL)
            {
                dataUnsigned =
                    gExperienceTables[gBaseStats[GetMonData(mon, MON_DATA_SPECIES, NULL)]
                                          .growthRate][GetMonData(mon, MON_DATA_LEVEL, NULL) + 1];
            ...
            if ((itemEffect[cmdIndex] & ITEM3_POISON) &&
                HealStatusConditions(
                    mon, partyIndex, STATUS1_PSN_ANY | STATUS1_TOXIC_COUNTER, battlerId) == 0)
                retVal = FALSE;
...
u16 ModifyStatByNature(u8 nature, u16 n, u8 statIndex)
{
    ...
    case 1:
        return (u16)(n * 110) /
               100; // NOTE: will overflow for n > 595 because the intermediate value is cast to u16
                    // before the division. Fix by removing (u16) cast
    case -1:
...
{
    if (!(gBattleTypeFlags &
            (BATTLE_TYPE_LEGENDARY | BATTLE_TYPE_TRAINER | BATTLE_TYPE_PYRAMID | BATTLE_TYPE_PIKE)))
    {
        u16 rnd = Random() % 100;
