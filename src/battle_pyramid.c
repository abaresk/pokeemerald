...
static bool8 TrySetPyramidObjectEventPositionInSquare(
    u8 arg0, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId);
static bool8 TrySetPyramidObjectEventPositionAtCoords(
    bool8 objType, u8 x, u8 y, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId);
...
static const u16 sLongStreakRewardItems[] = { ITEM_BRIGHT_POWDER,
    ITEM_WHITE_HERB,
    ITEM_QUICK_CLAW,
    ITEM_LEFTOVERS,
    ITEM_MENTAL_HERB,
...
static const u8 sPickupPercentages[PICKUP_ITEMS_PER_ROUND] = {
...
static void SetBattlePyramidPrize(void)
{
    ...
    if (gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvlMode] > 41)
        gSaveBlock2Ptr->frontier.pyramidPrize =
            sLongStreakRewardItems[Random() % ARRAY_COUNT(sLongStreakRewardItems)];
    else
        gSaveBlock2Ptr->frontier.pyramidPrize =
...
static void Task_SetPyramidFloorPalette(u8 taskId)
{
    if (gPaletteFade.active)
    {
        CpuCopy16(gUnknown_08D856C8[gSaveBlock2Ptr->frontier.curChallengeBattleNum],
            &gPlttBufferUnfaded[96],
            32);
        DestroyTask(taskId);
    }
}
        ...
        for (j = 0; j < FRONTIER_PARTY_SIZE; j++)
        {
                ...
                GetMonData(&gPlayerParty[j], MON_DATA_SPECIES, NULL))
            {
                for (k = 0; k < MAX_MON_MOVES; k++)
                {
                    ...
                    {
                        if (GetMonData(&gSaveBlock1Ptr->playerParty[partyIndex],
                                MON_DATA_MOVE1 + l,
                                NULL) == GetMonData(&gPlayerParty[j], MON_DATA_MOVE1 + k, NULL))
                            break;
                    }
                    if (l == MAX_MON_MOVES)
...
static u8 GetPostBattleDirectionHintTextIndex(
    int *hintType, u8 minDistanceForExitHint, u8 defaultHintType)
{
    int x, y;
    u8 textIndex = 0;
    ...
    for (y = 0; y < 32; map += 47, y++)
    {
        for (x = 0; x < 32; x++)
        {
            if ((map[x] & METATILE_ID_MASK) == FLOOR_EXIT_METATILE)
            {
                ...
                if (x >= minDistanceForExitHint || x <= -minDistanceForExitHint ||
...
bool8 GetBattlePyramidTrainerFlag(u8 eventId)
{
    return gSaveBlock2Ptr->frontier.pyramidTrainerFlags &
           gBitTable[gObjectEvents[eventId].localId - 1];
}
...
static void MarkPyramidTrainerAsBattled(u16 trainerId)
{
    ...
    gSaveBlock1Ptr->objectEventTemplates[gSpecialVar_LastTalked - 1].movementType =
...
static void SetPyramidObjectPositionsUniformly(u8 objType)
{
    ...
    for (i = 0; i < numObjects; i++)
    {
        ...
        } while (!(bits & 4) && TrySetPyramidObjectEventPositionInSquare(
                                    objType, floorLayoutOffsets, squareId, objectStartIndex + i));
        bits &= 1;
    }
    free(floorLayoutOffsets);
}
...
static bool8 SetPyramidObjectPositionsInAndNearSquare(u8 objType, u8 squareId)
{
    ...
    for (i = 0; i < numObjects; i++)
    {
        ...
        if (r7 & 1)
        {
            if (TrySetPyramidObjectEventPositionInSquare(objType,
                    floorLayoutOffsets,
                    ...
                    objectStartIndex + i))
            {
                do
                {
                    ...
                    if (sBorderedSquareIds[squareId][borderedIndex] == 0xFF || borderedIndex >= 4)
                        borderedIndex = 0;
                    r7 += 2;
                } while (r7 >> 1 != 4 && TrySetPyramidObjectEventPositionInSquare(objType,
                                             floorLayoutOffsets,
                                             sBorderedSquareIds[squareId][borderedIndex],
                                             objectStartIndex + i));
                numPlacedObjects++;
                ...
                objectStartIndex + i))
        {
            do
            {
                ...
                if (sBorderedSquareIds[squareId][borderOffset] == 0xFF || borderOffset >= 4)
                    borderOffset = 0;
                r8++;
            } while (r8 != 4 && TrySetPyramidObjectEventPositionInSquare(objType,
                                    floorLayoutOffsets,
                                    sBorderedSquareIds[squareId][borderOffset],
                                    objectStartIndex + i));
            numPlacedObjects++;
...
static bool8 TrySetPyramidObjectEventPositionInSquare(
    u8 objType, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId)
{
    int x, y;
        ...
        for (y = 7; y > -1; y--)
        {
            for (x = 7; x > -1; x--)
            {
                if (!TrySetPyramidObjectEventPositionAtCoords(
                        objType, x, y, floorLayoutOffsets, squareId, objectEventId))
                    return FALSE;
            }
        }
    }
...
static bool8 TrySetPyramidObjectEventPositionAtCoords(
    u8 objType, u8 x, u8 y, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId)
{
    int i, j;
    ...
    for (i = 0; i < mapHeader->events->objectEventCount; i++)
    {
        ...
        if (objType != OBJ_TRAINERS ||
            mapHeader->events->objectEvents[i].graphicsId == OBJ_EVENT_GFX_ITEM_BALL)
        {
        ...
        {
            if (floorEvents[j].x == x + ((squareId % 4) * 8) &&
                floorEvents[j].y == y + ((squareId / 4) * 8))
                break;
        }
...
{
    int i;
    int rand = (gSaveBlock2Ptr->frontier.pyramidRandoms[0]) |
               (gSaveBlock2Ptr->frontier.pyramidRandoms[1] << 16);
    u8 id = GetPyramidFloorTemplateId();
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        if (i == 7)
        {
            rand = (gSaveBlock2Ptr->frontier.pyramidRandoms[2]) |
                   (gSaveBlock2Ptr->frontier.pyramidRandoms[3] << 16);
            rand >>= 8;
        }
    }
}
