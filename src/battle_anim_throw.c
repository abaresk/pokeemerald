...
enum { BALL_ROLL_1, BALL_PIVOT_1, BALL_ROLL_2, BALL_PIVOT_2, BALL_ROLL_3, BALL_NEXT_MOVE, BALL_WAIT_NEXT_SHAKE };

enum { MON_SHRINK, MON_SHRINK_STEP, MON_SHRINK_INVISIBLE, MON_SHRINK_FREE };
...
const union AnimCmd gAnim_RegularBall[] = {
    ANIMCMD_FRAME(0, 1), ANIMCMD_FRAME(1, 1), ANIMCMD_FRAME(2, 1), ANIMCMD_FRAME(0, 1, .hFlip = TRUE),
    ANIMCMD_FRAME(2, 1), ANIMCMD_FRAME(1, 1), ANIMCMD_JUMP(0),
};
...
const union AnimCmd* const gAnims_BallParticles[] = {
    gAnim_RegularBall, gAnim_MasterBall,        gAnim_NetDiveBall,
    gAnim_NestBall,    gAnim_LuxuryPremierBall, gAnim_UltraRepeatTimerBall,
};

const u8 gBallParticleAnimNums[] = {
    [BALL_POKE] = 0, [BALL_GREAT] = 0, [BALL_SAFARI] = 0, [BALL_ULTRA] = 5, [BALL_MASTER] = 1, [BALL_NET] = 2,
    [BALL_DIVE] = 2, [BALL_NEST] = 3,  [BALL_REPEAT] = 5, [BALL_TIMER] = 5, [BALL_LUXURY] = 4, [BALL_PREMIER] = 4,
};

const TaskFunc gBallParticleAnimationFuncs[] = {
    PokeBallOpenParticleAnimation,  GreatBallOpenParticleAnimation,  SafariBallOpenParticleAnimation,
    UltraBallOpenParticleAnimation, MasterBallOpenParticleAnimation, SafariBallOpenParticleAnimation,
    DiveBallOpenParticleAnimation,  UltraBallOpenParticleAnimation,  RepeatBallOpenParticleAnimation,
    TimerBallOpenParticleAnimation, GreatBallOpenParticleAnimation,  PremierBallOpenParticleAnimation,
};
...
static void SpriteCB_Ball_MonShrink_Step(struct Sprite* sprite) {
    ...
    switch (gTasks[taskId].tState) {
        case MON_SHRINK:
            ...
            gMonShrinkDistance =
                (gSprites[spriteId].pos1.y + gSprites[spriteId].pos2.y) - (sprite->pos1.y + sprite->pos2.y);
...
static void SpriteCB_Ball_Release_Wait(struct Sprite* sprite) {
    ...
    if (gSprites[gBattlerSpriteIds[gBattleAnimTarget]].affineAnimEnded) {
        ...
        gSprites[gBattlerSpriteIds[gBattleAnimTarget]].pos2.y =
            gSprites[gBattlerSpriteIds[gBattleAnimTarget]].sOffsetY >> 8;
    }
    ...
    if (sprite->animEnded && released) {
        ...
        gSprites[gBattlerSpriteIds[gBattleAnimTarget]].invisible =
            gBattleSpritesDataPtr->animationData->wildMonInvisible;
        sprite->sFrame = 0;
        sprite->callback = DestroySpriteAfterOneFrame;
        gDoingBattleAnim = 0;
        UpdateOamPriorityInAllHealthboxes(1);
...
static void SpriteCB_PokeBlock_Throw(struct Sprite* sprite) {
    ...
    sprite->sDuration = 30;
    sprite->sTargetX =
        GetBattlerSpriteCoord(GetBattlerAtPosition(B_POSITION_OPPONENT_LEFT), BATTLER_COORD_X) + gBattleAnimArgs[2];
    sprite->sTargetY =
        GetBattlerSpriteCoord(GetBattlerAtPosition(B_POSITION_OPPONENT_LEFT), BATTLER_COORD_Y) + gBattleAnimArgs[3];
