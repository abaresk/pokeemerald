...
u8 rfu_LMAN_establishConnection(u8 parent_child, u16 connect_period, u16 name_accept_period,
...
static bool8 rfu_LMAN_linkWatcher(u16 REQ_commandID) {
    ...
    if (gRfuLinkStatus->parentChild == MODE_PARENT) {
        ...
        if (lman.linkRecoveryTimer.active) {
            bm_disconnectSlot = 0;
            for (i = 0; i < RFU_CHILD_MAX; i++) {
                if ((lman.linkRecoveryTimer.active) & (1 << i) && lman.linkRecoveryTimer.count[i] &&
                    --lman.linkRecoveryTimer.count[i] == 0) {
                    lman.linkRecoveryTimer.active &= ~(1 << i);
        ...
        if (lman.state != LMAN_STATE_READY) {
            ...
            switch (lman.state) {
                ...
                case LMAN_STATE_CONFIG_SYSTEM:
                    rfu_REQ_configSystem(lman.init_param->availSlot_flag, lman.init_param->maxMFrame,
                                         lman.init_param->MC_TimerCount);
                    break;
                case LMAN_STATE_CONFIG_GAME_DATA:
                    rfu_REQ_configGameData(lman.init_param->mboot_flag, lman.init_param->serialNo,
                                           (const u8*)lman.init_param->gameName, lman.init_param->userName);
                    break;
                case LMAN_STATE_START_SEARCH_CHILD:
...
static void rfu_LMAN_REQ_callback(u16 reqCommandId, u16 reqResult) {
    ...
    if (lman.active != 0) {
        ...
        lman.active = 1;
               ...
               (reqCommandId == ID_DATA_TX_REQ || reqCommandId == ID_DATA_RX_REQ || reqCommandId == ID_MS_CHANGE_REQ)) {
        rfu_REQ_RFUStatus();
        rfu_waitREQComplete();
...
static void rfu_LMAN_PARENT_checkRecvChildName(void) {
    ...
    if (lman.state == LMAN_STATE_START_SEARCH_CHILD || lman.state == LMAN_STATE_POLL_SEARCH_CHILD ||
        ...
        newSlot = ((gRfuLinkStatus->connSlotFlag ^ lman.connectSlot_flag_old) & gRfuLinkStatus->connSlotFlag) &
...
static u8 rfu_LMAN_setFastSearchParent(u8 enable_flag) {
    if (lman.state == LMAN_STATE_START_SEARCH_PARENT || lman.state == LMAN_STATE_POLL_SEARCH_PARENT ||
        lman.state == LMAN_STATE_END_SEARCH_PARENT) {
        lman.param[0] = 7;
        rfu_LMAN_occureCallback(LMAN_MSG_LMAN_API_ERROR_RETURN, 1);
