...
const union AnimCmd gFangAnimCmds[] = {
    ANIMCMD_FRAME(0, 8), ANIMCMD_FRAME(16, 16), ANIMCMD_FRAME(32, 4), ANIMCMD_FRAME(48, 4), ANIMCMD_END,
};
...
const union AffineAnimCmd gSwallowDeformMonAffineAnimCmds[] = {
    AFFINEANIMCMD_FRAME(0, 6, 0, 20), AFFINEANIMCMD_FRAME(0, 0, 0, 20),  AFFINEANIMCMD_FRAME(7, -30, 0, 6),
    AFFINEANIMCMD_FRAME(0, 0, 0, 20), AFFINEANIMCMD_FRAME(-2, 3, 0, 20), AFFINEANIMCMD_END,
};
...
const union AffineAnimCmd gDeepInhaleAffineAnimCmds[] = {
    AFFINEANIMCMD_FRAME(16, 0, 0, 4), AFFINEANIMCMD_FRAME(0, -3, 0, 16), AFFINEANIMCMD_FRAME(4, 0, 0, 4),
    AFFINEANIMCMD_FRAME(0, 0, 0, 24), AFFINEANIMCMD_FRAME(-5, 3, 0, 16), AFFINEANIMCMD_END,
};
...
const union AffineAnimCmd gSlackOffSquishAffineAnimCmds[] = {
    AFFINEANIMCMD_FRAME(0, 16, 0, 4), AFFINEANIMCMD_FRAME(-2, 0, 0, 8),  AFFINEANIMCMD_FRAME(0, 4, 0, 4),
    AFFINEANIMCMD_FRAME(0, 0, 0, 24), AFFINEANIMCMD_FRAME(1, -5, 0, 16), AFFINEANIMCMD_END,
};
...
void AnimTask_SmokescreenImpact(u8 taskId) {
    SmokescreenImpact(GetBattlerSpriteCoord(gBattleAnimTarget, 2) + 8, GetBattlerSpriteCoord(gBattleAnimTarget, 3) + 8,
                      0);
...
static void AnimSpotlight(struct Sprite* sprite) {
    SetGpuReg(REG_OFFSET_WINOUT,
              WINOUT_WIN01_BG_ALL | WINOUT_WIN01_OBJ | WINOUT_WIN01_CLR | WINOUT_WINOBJ_BG_ALL | WINOUT_WINOBJ_OBJ);
...
static void AnimSpotlight_Step2(struct Sprite* sprite) {
    SetGpuReg(REG_OFFSET_WINOUT, WINOUT_WIN01_BG_ALL | WINOUT_WIN01_OBJ | WINOUT_WIN01_CLR | WINOUT_WINOBJ_BG_ALL |
...
void AnimTask_DefenseCurlDeformMon(u8 taskId) {
    switch (gTasks[taskId].data[0]) {
        case 0:
            PrepareAffineAnimInTaskData(&gTasks[taskId], GetAnimBattlerSpriteId(ANIM_ATTACKER),
...
static void AnimWishStar_Step(struct Sprite* sprite) {
    ...
    if (++sprite->data[2] % 3 == 0) {
        CreateSpriteAndAnimate(&gMiniTwinklingStarSpriteTemplate, sprite->pos1.x + sprite->pos2.x,
                               sprite->pos1.y + sprite->pos2.y, sprite->subpriority + 1);
    }
...
void AnimTask_StatusClearedEffect(u8 taskId) {
    sub_8117854(taskId, 0, 0x1A0, gBattleAnimAttacker, gBattleAnimArgs[0], 10, 2, 30, gCureBubblesGfx,
                gCureBubblesTilemap, gCureBubblesPal);
}
...
static void AnimTask_GlareEyeDots_Step(u8 taskId) {
    ...
    switch (task->data[0]) {
        case 0:
            if (++task->data[1] > 3) {
                task->data[1] = 0;
                GetGlareEyeDotCoords(task->data[11], task->data[12], task->data[13], task->data[14], task->data[5],
                                     task->data[2], &x, &y);
...
void AnimTask_BarrageBall(u8 taskId) {
    ...
    task->data[15] = CreateSprite(&gBarrageBallSpriteTemplate, task->data[11], task->data[12],
...
static void AnimMeteorMashStar_Step(struct Sprite* sprite) {
    ...
    if (!(sprite->data[5] & 1)) {
        CreateSprite(&gMiniTwinklingStarSpriteTemplate, sprite->pos1.x + sprite->pos2.x,
    ...
    switch (gTasks[taskId].data[0]) {
        ...
        case 1:
            if (IsContest()) {
                ...
                if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_PLAYER) {
                    personality =
                        GetMonData(&gPlayerParty[gBattlerPartyIndexes[gBattleAnimAttacker]], MON_DATA_PERSONALITY);
                    otId = GetMonData(&gPlayerParty[gBattlerPartyIndexes[gBattleAnimAttacker]], MON_DATA_OT_ID);
                    if (gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker].transformSpecies == SPECIES_NONE)
                        species =
                            GetMonData(&gPlayerParty[gBattlerPartyIndexes[gBattleAnimAttacker]], MON_DATA_SPECIES);
                    else
                    ...
                    personality =
                        GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattleAnimAttacker]], MON_DATA_PERSONALITY);
            ...
            spriteId2 = sub_80A8394(species, isBackPic, 0, x, GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y),
                                    subpriority, personality, otId, gBattleAnimAttacker, 0);
            if (gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker].transformSpecies != SPECIES_NONE)
