...
static void ClearPulseBlendPalettesSettings(struct PulseBlendPalette* pulseBlendPalette) {
    ...
    if (!pulseBlendPalette->available && pulseBlendPalette->pulseBlendSettings.restorePaletteOnUnload) {
        for (i = pulseBlendPalette->pulseBlendSettings.paletteOffset;
             i < pulseBlendPalette->pulseBlendSettings.paletteOffset + pulseBlendPalette->pulseBlendSettings.numColors;
             i++)
            ...
            if ((!pulseBlendPalette->available && pulseBlendPalette->inUse) &&
                ...
                if (--pulseBlendPalette->delayCounter == 0xFF) {
                    ...
                    BlendPalette(pulseBlendPalette->pulseBlendSettings.paletteOffset,
                                 pulseBlendPalette->pulseBlendSettings.numColors, pulseBlendPalette->blendCoeff,
                    ...
                    switch (pulseBlendPalette->pulseBlendSettings.fadeType) {
                        case 0: // Fade all the way to the max blend amount, then wrap around
                            // BUG: This comparison will never be true for maxBlendCoeff values that are >= 8. This is
                            // because maxBlendCoeff is a signed 4-bit field, but blendCoeff is an unsigned 4-bit field.
                            // This code is never reached, anyway, so the bug is not observable in vanilla gameplay.
                    ...
                    case (MODERN ? -2 : 2): // Flip back and forth
                        ...
                        if (pulseBlendPalette->fadeDirection)
                            pulseBlendPalette->blendCoeff = 0;
                        else pulseBlendPalette->blendCoeff = pulseBlendPalette->pulseBlendSettings.maxBlendCoeff & 0xF;
