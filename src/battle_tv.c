...
// const rom data
static const u16 sVariableDmgMoves[] = { MOVE_COUNTER,
    MOVE_FISSURE,
    MOVE_BIDE,
    MOVE_MIRROR_COAT,
    MOVE_HORN_DRILL,
...
static const u16 sUnknown_0860A4E0[] = { 0x0001,
    0x0001,
    0x0001,
    0x0004,
    0x0001,
...
static const u16 sUnknown_0860A6A0[] = { 0x0091,
    0x0003,
    0x00fa,
    0x0003,
    0x00be,
    ...
    0xffff,
    0x0000 };
static const u16 sUnknown_0860A728[] = { 0x013b,
    0x0003,
    0x00ac,
    0x0003,
    0x0035,
...
static const u16 sUnknown_0860A788[] = { 0x0055,
    0x0003,
    0x0009,
    0x0003,
    0x00d1,
...
static const u16 *const sPointsArray[] = { sUnknown_0860A4E0,
    sUnknown_0860A68C,
    sUnknown_0860A692,
    sUnknown_0860A6A0,
    sUnknown_0860A728,
...
static const u16 sUnknown_0860A8A4[] = { STRINGID_PKMNPERISHCOUNTFELL,
...
void BattleTv_SetDataBasedOnString(u16 stringId)
{
    ...
    switch (stringId)
    {
        ...
        case STRINGID_NOTVERYEFFECTIVE:
            ...
            if (!(gBattleTypeFlags & BATTLE_TYPE_LINK)
                && GetMonData(defMon, MON_DATA_HP, NULL) != 0)
                TrySetBattleSeminarShow();
            break;
        ...
        case STRINGID_PKMNLOSTPPGRUDGE:
            if (tvPtr->side[defSide].grudgeMonId != 0)
            {
                AddMovePoints(2,
                    4,
                    defSide,
                    (tvPtr->side[defSide].grudgeMonId - 1) * 4
                        + tvPtr->side[defSide].grudgeMoveSlot);
            }
            break;
        ...
        case STRINGID_PKMNAFFLICTEDBYCURSE:
                ...
                && tvPtr->pos[atkSide][atkFlank].curseMonId != 0)
            {
                AddMovePoints(8,
                    0,
                    tvPtr->pos[atkSide][atkFlank].curseMonId - 1,
        ...
        case STRINGID_PKMNSEEDED:
            tvPtr->pos[defSide][defFlank].leechSeedMonId =
        ...
        case STRINGID_PKMNSAPPEDBYLEECHSEED:
            if (tvPtr->pos[atkSide][atkFlank].leechSeedMonId != 0)
            {
                AddMovePoints(8,
                    1,
                    tvPtr->pos[atkSide][atkFlank].leechSeedMonId - 1,
        ...
        case STRINGID_PKMNFELLINTONIGHTMARE:
            tvPtr->pos[defSide][defFlank].nightmareMonId =
                gBattlerPartyIndexes[gBattlerAttacker] + 1;
        ...
        case STRINGID_PKMNLOCKEDINNIGHTMARE:
                ...
                && tvPtr->pos[atkSide][atkFlank].nightmareMonId != 0)
            {
                AddMovePoints(8,
                    5,
                    tvPtr->pos[atkSide][atkFlank].nightmareMonId - 1,
        ...
        case STRINGID_PKMNHURTBY:
                ...
                && tvPtr->pos[atkSide][atkFlank].wrapMonId != 0)
            {
                AddMovePoints(8,
                    6,
                    tvPtr->pos[atkSide][atkFlank].wrapMonId - 1,
        ...
        case STRINGID_PKMNHURTBYBURN:
            if (GetMonData(atkMon, MON_DATA_HP, NULL) != 0)
            {
                if (tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].brnMonId != 0)
                    AddMovePoints(8,
                        4,
                        tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].brnMonId - 1,
        ...
        case STRINGID_PKMNHURTBYPOISON:
            if (GetMonData(atkMon, MON_DATA_HP, NULL) != 0)
            {
                if (tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].psnMonId != 0)
                    AddMovePoints(8,
                        2,
                        tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].psnMonId - 1,
                ...
                if (tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].badPsnMonId != 0)
                    AddMovePoints(8,
                        3,
                        tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].badPsnMonId - 1,
        ...
        case STRINGID_PKMNIMMOBILIZEDBYLOVE:
            if (tvPtr->pos[atkSide][atkFlank].attractMonId != 0)
                AddMovePoints(9,
                    0,
                    tvPtr->pos[atkSide][atkFlank].attractMonId - 1,
        ...
        case STRINGID_PKMNISPARALYZED:
            if (tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].prlzMonId != 0)
                AddMovePoints(9,
                    2,
                    tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].prlzMonId - 1,
        ...
        case STRINGID_PKMNFASTASLEEP:
            if (tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].slpMonId != 0
                ...
                AddMovePoints(9,
                    3,
                    tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].slpMonId - 1,
        ...
        case STRINGID_PKMNISFROZEN:
            if (tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].frzMonId != 0)
                AddMovePoints(9,
                    4,
                    tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].frzMonId - 1,
                    tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].frzMoveSlot);
            break;
        case STRINGID_PKMNWASCONFUSED:
            tvPtr->pos[effSide][effFlank].confusionMonId =
                gBattlerPartyIndexes[gBattlerAttacker] + 1;
        ...
        case STRINGID_ITHURTCONFUSION:
            if (tvPtr->pos[atkSide][atkFlank].confusionMonId != 0)
                AddMovePoints(9,
                    1,
                    tvPtr->pos[atkSide][atkFlank].confusionMonId - 1,
        ...
        case STRINGID_PKMNHURTBYSPIKES:
            ...
            {
                AddMovePoints(10,
                    scriptingSide ^ BIT_SIDE,
                    tvPtr->side[scriptingSide].spikesMonId - 1,
        ...
        case STRINGID_PKMNUSEDSAFEGUARD:
            if (tvPtr->side[defSide].safeguardMonId != 0)
                AddMovePoints(15,
                    0,
                    tvPtr->side[defSide].safeguardMonId - 1,
                    tvPtr->side[defSide].safeguardMoveSlot);
            break;
        ...
        case STRINGID_PKMNPROTECTEDBYMIST:
            if (tvPtr->side[defSide].mistMonId != 0)
                AddMovePoints(
                    16, 0, tvPtr->side[defSide].mistMonId - 1, tvPtr->side[defSide].mistMoveSlot);
            break;
...
void TryPutLinkBattleTvShowOnAir(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_MULTI)
    {
            ...
            || (playerBestMonId >= 3 && GetLinkTrainerFlankId(gBattleScripting.multiplayerId)))
        {
            ...
            PutBattleUpdateOnTheAir(sub_806EF84(j, gBattleScripting.multiplayerId),
                moveId,
                playerBestSpecies,
                opponentBestSpecies);
        }
    ...
    else
    {
        PutBattleUpdateOnTheAir(
            gBattleScripting.multiplayerId ^ 1, moveId, playerBestSpecies, opponentBestSpecies);
    }
}
    ...
    switch (caseId)
    {
        ...
        case 3 ... 7:
            ...
            do
            {
                ...
                {
                    movePoints
                        ->points[atkSide][gBattlerPartyIndexes[gBattlerAttacker] * 4 + arg2] +=
                        ptr[i + 1];
                    break;
                }
                i += 2;
        ...
        case 10:
            ...
            break;
        case 11:
            if (tvPtr->pos[defSide][0].waterSportMonId != -(tvPtr->pos[defSide][1].waterSportMonId)
                && arg1 == 10)
            {
            ...
            break;
        case 12:
            if (tvPtr->pos[defSide][0].mudSportMonId != -(tvPtr->pos[defSide][1].mudSportMonId)
                && arg1 == 13)
            {
            ...
            case 7:
                if (tvPtr->side[atkSide].spikesMonId != 0)
                {
                    AddMovePoints(19,
                        0,
                        atkSide ^ BIT_SIDE,
                        (tvPtr->side[atkSide].spikesMonId - 1) * 4
                            + tvPtr->side[atkSide].spikesMoveSlot);
                }
                break;
            case 8:
                if (tvPtr->side[atkSide].futureSightMonId != 0)
            ...
            case 14:
                if (targetFainted == TRUE)
                {
                    AddMovePoints(20,
                        0,
                        atkSide,
                        (gBattlerPartyIndexes[gBattlerAttacker]) * 4
                            + tvPtr->side[atkSide].usedMoveSlot);
                }
                break;
            case 15:
                break;
        ...
        if (tvPtr->side[defSide].faintCause == 7)
        {
            if (tvPtr->side[defSide].spikesMonId != 0)
            {
                AddMovePoints(19,
                    0,
                    defSide ^ BIT_SIDE,
                    (tvPtr->side[defSide].spikesMonId - 1) * 4
                        + tvPtr->side[defSide].spikesMoveSlot);
            }
        }
        else
        {
...
static void TrySetBattleSeminarShow(void)
{
    ...
    if (gBattleTypeFlags & (BATTLE_TYPE_DOUBLE | BATTLE_TYPE_LINK | BATTLE_TYPE_x2000000))
        ...
        return;
    else if (gBattleMoves[gBattleMons[gBattlerAttacker]
                              .moves[gMoveSelectionCursor[gBattlerAttacker]]]
                 .power
             == 0)
        return;
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
            ...
            && dmgByMove[i] > dmgByMove[gMoveSelectionCursor[gBattlerAttacker]])
        {
            ...
            opponentSpecies = GetMonData(
                &gEnemyParty[gBattlerPartyIndexes[gBattlerTarget]], MON_DATA_SPECIES, NULL);
            playerSpecies = GetMonData(
                &gPlayerParty[gBattlerPartyIndexes[gBattlerAttacker]], MON_DATA_SPECIES, NULL);
            sub_80EE35C(opponentSpecies,
                playerSpecies,
                gMoveSelectionCursor[gBattlerAttacker],
                gBattleMons[gBattlerAttacker].moves,
