...
static const u8 sAvailSlots[] = { [1] = AVAIL_SLOT1, [2] = AVAIL_SLOT2, [3] = AVAIL_SLOT3, [4] = AVAIL_SLOT4 };

static const u32 sAllBlocksReceived[] = { 0x000000, 0x000001, 0x000003, 0x000007, 0x00000f, 0x00001f, 0x00003f,
...
static const char sASCII_RfuCmds[][15] = { "RFU WAIT",   "RFU BOOT",     "RFU ERROR",   "RFU RESET",
                                           "RFU CONFIG", "RFU START",    "RFU SC POLL", "RFU SP POLL",
                                           "RFU START",  "RFU SEND ERR", "RFU CP POLL" };

static const char sASCII_RecoverCmds[][16] = { "              ", "RECOVER START ", "DISSCONECT    ", "RECOVER SUUSES",
                                               "RECOVER FAILED" };
...
static const char sASCII_ClockCmds[][12] = { "           ", "CLOCK DRIFT", "BUSY SEND  ", "CMD REJECT ",
                                             "CLOCK SLAVE" };
...
static void RfuFunc_SendKeysToRfu(void) {
    if (gReceivedRemoteLinkPlayers && gHeldKeyCodeToSend != LINK_KEY_CODE_NULL && gLinkTransferringData != TRUE) {
        sHeldKeyCount++;
...
static void SendNextBlock(void) {
    ...
    for (i = 0; i < CMD_LENGTH - 1; i++)
        gSendCmd[i + 1] =
            (src[(i << 1) + Rfu.sendBlock.next * 12 + 1] << 8) | src[(i << 1) + Rfu.sendBlock.next * 12 + 0];
    Rfu.sendBlock.next++;
...
static void RfuCheckErrorStatus(void) {
    if (Rfu.errorState == 1 && lman.childClockSlave_flag == 0) {
        ...
        BufferLinkErrorInfo((Rfu.linkmanMsg << 16) | (Rfu.unk_10 << 8) | Rfu.unk_12, Rfu.recvQueue.count,
                            Rfu.sendQueue.count, RfuGetStatus() == RFU_STATUS_CONNECTION_ERROR);
        Rfu.errorState = 2;
        CloseLink();
...
void sub_80110B8(u32 a0) {
    ...
    if (GetHostRFUtgtGname()->activity == (ACTIVITY_CHAT | IN_UNION_ROOM)) {
        ...
        for (i = 0; i < 4; i++) {
            if ((r8 >> i) & 1) {
                child_sprite_genders |= ((0x80 | ((gLinkPlayers[Rfu.linkPlayerIdx[i]].gender & 1) << 3) |
    ...
    if (GetHostRFUtgtGname()->activity == (ACTIVITY_CHAT | IN_UNION_ROOM)) {
        ...
        if (tradeGname->species == SPECIES_EGG) {
            if (partnerGname->species == tradeGname->species)
                ...
                return TRUE;
        } else if (partnerGname->species != tradeGname->species || partnerGname->level != tradeGname->level ||
...
static void sub_801209C(u8 taskId) {
    ...
    if (Rfu.parentId != 0 && lman.parent_child == 0) {
        ...
        if (id != 0xFF) {
            if (!ShouldRejectPartnerConnectionBasedOnActivity(gTasks[taskId].data[1],
                                                              (void*)gRfuLinkStatus->partner[id].gname)) {
                if (gRfuLinkStatus->partner[id].slot != 0xFF &&
