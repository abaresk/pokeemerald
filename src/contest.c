...
static void SetupContestGpuRegs(void)
{
    ...
    SetGpuReg(REG_OFFSET_WINOUT,
        WINOUT_WIN01_BG0 | WINOUT_WIN01_BG1 | WINOUT_WIN01_BG2 | WINOUT_WIN01_BG3 | WINOUT_WIN01_OBJ
            | WINOUT_WIN01_CLR | WINOUT_WINOBJ_BG0 | WINOUT_WINOBJ_BG1 | WINOUT_WINOBJ_BG2
            | WINOUT_WINOBJ_BG3 | WINOUT_WINOBJ_OBJ | WINOUT_WINOBJ_CLR);
...
static bool8 SetupContestGraphics(u8 *stateVar)
{
    ...
    switch (*stateVar)
    {
        ...
        case 5:
            ...
            CpuCopy32(gPlttBufferUnfaded + (5 + gContestPlayerMonIndex) * 16,
                tempPalette2,
                16 * sizeof(u16));
            CpuCopy32(tempPalette2, gPlttBufferUnfaded + 128, 16 * sizeof(u16));
            CpuCopy32(tempPalette1,
                gPlttBufferUnfaded + (5 + gContestPlayerMonIndex) * 16,
                16 * sizeof(u16));
            DmaCopy32Defvars(3,
                gPlttBufferUnfaded,
    ...
    switch (gTasks[taskId].tState)
    {
        ...
        case APPEALSTATE_UPDATE_OPPONENT_HEARTS:
            for (i = 0; gTasks[taskId].data[1] != gContestantTurnOrder[i]; i++)
                ;
            UpdateAppealHearts(eContestantStatus[i].appeal + eContestantStatus[i].jam,
                -eContestantStatus[i].jam,
                i);
        ...
        case APPEALSTATE_UPDATE_CROWD:
            ...
            {
                r3 = eContestExcitement.moveExcitement; // Can't get this to use local variable.
                                                        // Should be "moveExcitement"
    ...
    if (StringLength(nickname) < PLAYER_NAME_LENGTH - 1)
    {
        while (*nickname != EOS)
        {
            if ((*nickname >= CHAR_A && *nickname <= CHAR_z)
                ...
                || *nickname == CHAR_DBL_QUOT_LEFT) // Most likely a typo, CHAR_SGL_QUOT_RIGHT
