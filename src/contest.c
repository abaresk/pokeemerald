...
static const struct SpriteSheet sSpriteSheet_SliderHeart = { .data = gContestSliderHeart_Gfx,
                                                             .size = 0x20,
...
static const union AffineAnimCmd sAffineAnim_SliderHeart_SpinDisappear[] = { AFFINEANIMCMD_FRAME(256, 256, 0, 0),
                                                                             AFFINEANIMCMD_FRAME(-10, -10, -20, 20),
                                                                             AFFINEANIMCMD_END };

static const union AffineAnimCmd sAffineAnim_SliderHeart_SpinAppear[] = { AFFINEANIMCMD_FRAME(56, 56, 0, 0),
                                                                          AFFINEANIMCMD_FRAME(10, 10, 20, 20),
                                                                          AFFINEANIMCMD_END };
...
static const struct SpritePalette sSpritePalette_NextTurn = { .data = gContestPal, .tag = TAG_NEXT_TURN_PAL };
...
static const struct Subsprite sSubsprites_NextTurn[] = {
    { .x = -28, .y = -4, .shape = SPRITE_SHAPE(32x8), .size = SPRITE_SIZE(32x8), .tileOffset = 0, .priority = 0 },
    { .x = 4, .y = -4, .shape = SPRITE_SHAPE(32x8), .size = SPRITE_SIZE(32x8), .tileOffset = 4, .priority = 0 }
};

static const struct SubspriteTable sSubspriteTable_NextTurn[] = { { .subspriteCount = ARRAY_COUNT(sSubsprites_NextTurn),
                                                                    .subsprites = sSubsprites_NextTurn } };

// Unused
static const struct CompressedSpriteSheet sSpriteSheet_Faces = { .data = gContestFaces_Gfx,
                                                                 .size = 0x180,
                                                                 .tag = TAG_FACES_GFX };
...
static const struct CompressedSpriteSheet sSpriteSheet_ApplauseMeter = { .data = gContestApplauseGfx,
                                                                         .size = 0x400,
                                                                         .tag = TAG_APPLAUSE_METER };

static const struct SpritePalette sSpritePalette_ApplauseMeter = { .data = gContestPal, .tag = TAG_APPLAUSE_METER };
...
const struct CompressedSpriteSheet sSpriteSheet_Judge = { .data = gContestJudgeGfx, .size = 0x800, .tag = TAG_JUDGE };

static const struct CompressedSpriteSheet sSpriteSheet_JudgeSymbols = { .data = gContestJudgeSymbolsGfx,
                                                                        .size = 0x380,
...
const union AffineAnimCmd sAffineAnim_ContestantsTurnBlinkEffect_0[] = { AFFINEANIMCMD_FRAME(0x100, 0x100, 0, 0),
                                                                         AFFINEANIMCMD_END };

const union AffineAnimCmd sAffineAnim_ContestantsTurnBlinkEffect_1[] = {
    AFFINEANIMCMD_FRAME(3, 3, 0, 15), AFFINEANIMCMD_FRAME(-3, -3, 0, 15), AFFINEANIMCMD_FRAME(3, 3, 0, 15),
    AFFINEANIMCMD_FRAME(-3, -3, 0, 15), AFFINEANIMCMD_END
};
...
static void SetupContestGpuRegs(void) {
    ...
    SetGpuReg(REG_OFFSET_WININ, WININ_WIN0_BG0 | WININ_WIN0_BG1 | WININ_WIN0_BG2 | WININ_WIN0_BG3 | WININ_WIN0_OBJ |
    ...
    SetGpuRegBits(REG_OFFSET_DISPCNT, DISPCNT_BG0_ON | DISPCNT_BG1_ON | DISPCNT_BG2_ON | DISPCNT_BG3_ON |
...
void LoadContestBgAfterMoveAnim(void) {
    ...
    for (i = 0; i < CONTESTANT_COUNT; i++) {
        ...
        LoadPalette(eUnknownHeap1A004.cachedWindowPalettes[contestantWindowId], 16 * (5 + gContestantTurnOrder[i]),
...
static bool8 SetupContestGraphics(u8* stateVar) {
    ...
    switch (*stateVar) {
        ...
        case 4:
            ...
            DmaCopy32Defvars(3, gContestResources->contestBgTilemaps[2], eUnknownHeap1A004.savedJunk,
        ...
        case 5:
            ...
            DmaCopy32Defvars(3, gPlttBufferUnfaded, eUnknownHeap1A004.cachedWindowPalettes,
...
static void Task_SelectedMove(u8 taskId) {
    if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK) {
        ...
        SetTaskFuncWithFollowupFunc(taskId2, Task_LinkContest_CommunicateMoveSelections,
    ...
    switch (gTasks[taskId].tState) {
        ...
        case APPEALSTATE_SLIDE_MON_IN:
            ...
            spriteId = CreateContestantSprite(
                gContestMons[eContest.currentContestant].species, gContestMons[eContest.currentContestant].otId,
                gContestMons[eContest.currentContestant].personality, eContest.currentContestant);
        ...
        case APPEALSTATE_TRY_PRINT_SKIP_NEXT_TURN_MSG:
            if (gTasks[taskId].tCounter++ > 9) {
                gTasks[taskId].tCounter = 0;
                if (eContestantStatus[contestant].numTurnsSkipped != 0 || eContestantStatus[contestant].turnSkipped) {
                    ContestClearGeneralTextWindow();
                    StringCopy(gStringVar1, gContestMons[contestant].nickname);
                ...
                if (++gTasks[taskId].tCounter > 50) {
                    if (!eContestantStatus[contestant].hasJudgesAttention) {
                        UpdateAppealHearts(eContestantStatus[contestant].appeal,
                                           eContestantStatus[contestant].comboAppealBonus, contestant);
        ...
        case APPEALSTATE_UPDATE_HEARTS_FROM_REPEAT:
            if (!Contest_RunTextPrinters()) {
                UpdateAppealHearts(eContestantStatus[contestant].appeal, -eContestantStatus[contestant].repeatJam,
                                   contestant);
        ...
        case APPEALSTATE_UPDATE_CROWD:
            if (eContestExcitement.frozen && contestant != eContestExcitement.freezer) {
                ...
                r3 = eContestExcitement
                         .moveExcitement; // Can't get this to use local variable. Should be "moveExcitement"
                if (eContestantStatus[contestant].overrideCategoryExcitementMod) {
                    ...
                    StringCopy(
                        gStringVar3,
                        sContestConditions[gContestMoves[eContestantStatus[contestant].currMove].contestCategory]);
                }
                ...
                case 2:
                    if (!eContest.isShowingApplauseMeter) {
                        if (gTasks[taskId].data[11]++ > 29) {
                            gTasks[taskId].data[11] = 0;
                            UpdateAppealHearts(eContestantStatus[contestant].appeal,
                                               eContestExcitement.excitementAppealBonus, contestant);
...
static bool8 DrawStatusSymbol(u8 contestant) {
    ...
    if (eContestantStatus[contestant].resistant || eContestantStatus[contestant].immune ||
        eContestantStatus[contestant].jamSafetyCount != 0 || eContestantStatus[contestant].jamReduction != 0)
...
void SaveLinkContestResults(void) {
    if ((gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK)) {
        gSaveBlock2Ptr
            ->contestLinkResults[gSpecialVar_ContestCategory][gContestFinalStandings[gContestPlayerMonIndex]] =
            ((gSaveBlock2Ptr
                  ->contestLinkResults[gSpecialVar_ContestCategory][gContestFinalStandings[gContestPlayerMonIndex]] +
              1) > 9999)
                ? 9999
...
static void Task_UpdateAppealHearts(u8 taskId) {
    ...
    if (++gTasks[taskId].tDelayTimer > 14) {
        ...
        ContestBG_FillBoxWithTile(0, heartOffset, newNumHearts + 22,
                                  gContestantTurnOrder[contestant] * 5 + 2 + onSecondLine, 1, 1, 17);
        if (heartsDelta > 0) {
...
// Used to hide (or subsequently reshow) the bottom two slider hearts that get hidden by text windows by moving them
// offscreen
...
static void CreateNextTurnSprites(void) {
    ...
    for (i = 0; i < CONTESTANT_COUNT; i++) {
        ...
        eContestGfxState[i].nextTurnSpriteId =
            CreateSprite(&sSpriteTemplates_NextTurn[i], 204, sNextTurnSpriteYPositions[gContestantTurnOrder[i]], 0);
...
static void UpdateBlendTaskContestantData(u8 contestant) {
    ...
    DmaCopy16Defvars(3, gPlttBufferUnfaded + palOffset1 * 16 + 10, gPlttBufferFaded + palOffset1 * 16 + 10, 2);
...
static u8 CreateContestantBoxBlinkSprites(u8 contestant) {
    ...
    CopySpriteTiles(0, 3, (void*)VRAM, (u16*)(BG_SCREEN_ADDR(28) + gContestantTurnOrder[contestant] * 5 * 64 + 0x26),
                    gContestResources->boxBlinkTiles1);

    CopySpriteTiles(0, 3, (void*)VRAM, (u16*)(BG_SCREEN_ADDR(28) + gContestantTurnOrder[contestant] * 5 * 64 + 0x36),
    ...
    RequestDma3Copy(gContestResources->boxBlinkTiles1, (u8*)(VRAM + 0x10000 + gSprites[spriteId1].oam.tileNum * 32),
                    0x800, 1);

    RequestDma3Copy(gContestResources->boxBlinkTiles2, (u8*)(VRAM + 0x10000 + gSprites[spriteId2].oam.tileNum * 32),
                    0x800, 1);
...
void SortContestants(bool8 useRanking) {
    ...
    if (!useRanking) {
        ...
        for (i = 0; i < CONTESTANT_COUNT; i++) {
            ...
            for (v3 = 0; v3 < i; v3++) {
                if (gContestMonRound1Points[gContestantTurnOrder[v3]] < gContestMonRound1Points[i] ||
                    (gContestMonRound1Points[gContestantTurnOrder[v3]] == gContestMonRound1Points[i] &&
        ...
        // the contestant is the data. After inverting the list, `gContestantTurnOrder[3] = 1`. The contestant is the
        // index, and the turn is the data.
...
static void DrawContestantWindows(void) {
    ...
    for (i = 0; i < CONTESTANT_COUNT; i++) {
        s32 windowId = i + 5;
        LoadPalette(eUnknownHeap1A004.cachedWindowPalettes[windowId], (gContestantTurnOrder[i] + 5) * 16,
    ...
    if (IsContestantAllowedToCombo(contestant)) {
        ...
        if (completedCombo && eContestantStatus[contestant].hasJudgesAttention) {
            ...
            eContestantStatus[contestant].comboAppealBonus =
                eContestantStatus[contestant].baseAppeal * eContestantStatus[contestant].completedCombo;
...
static void PrintAppealMoveResultText(u8 contestant, u8 stringId) {
    ...
    else if (gContestMoves[eContestantStatus[eContestAppealResults.contestant].currMove].contestCategory ==
...
static void UpdateApplauseMeter(void) {
    ...
    for (i = 0; i < APPLAUSE_METER_SIZE; i++) {
        ...
        CpuCopy32(src, (void*)(VRAM + 0x10000 + (gSprites[eContest.applauseMeterSpriteId].oam.tileNum + 17 + i) * 32),
                  32);
        CpuCopy32(src + 32,
                  (void*)(VRAM + 0x10000 + (gSprites[eContest.applauseMeterSpriteId].oam.tileNum + 25 + i) * 32), 32);
    ...
    for (i = 0; i < CONTESTANT_COUNT; i++) {
        if (eContestantStatus[i].turnOrderMod != 0 && show) {
            CpuCopy32(GetTurnOrderNumberGfx(i),
                      (void*)(VRAM + 0x10000 + (gSprites[eContestGfxState[i].nextTurnSpriteId].oam.tileNum + 6) * 32),
                      32);
...
static void ContestBG_FillBoxWithIncrementingTile(u8 bg, u16 firstTileNum, u8 x, u8 y, u8 width, u8 height,
                                                  u8 paletteSlot, s16 tileNumData) {
        ...
        case CONTEST_RANK_MASTER:
            if (b != 0) {
                for (i = NUM_CONTEST_HALL_WINNERS - 1; i >= 1; i--)
                    memcpy(&gSaveBlock1Ptr->contestWinners[i], &gSaveBlock1Ptr->contestWinners[i - 1],
...
static void SetContestLiveUpdateFlags(u8 contestant) {
    ...
    if (eContest.applauseLevel == 4 && !eContestExcitement.frozen && eContestExcitement.moveExcitement < 0) {
...
static u8 GetMonNicknameLanguage(u8* nickname) {
    ...
    if (StringLength(nickname) < PLAYER_NAME_LENGTH - 1) {
        while (*nickname != EOS) {
            if ((*nickname >= CHAR_A && *nickname <= CHAR_z) || (*nickname >= CHAR_0 && *nickname <= CHAR_9) ||
                *nickname == CHAR_SPACE || *nickname == CHAR_PERIOD || *nickname == CHAR_COMMA ||
                *nickname == CHAR_EXCL_MARK || *nickname == CHAR_QUESTION_MARK || *nickname == CHAR_MALE ||
                *nickname == CHAR_FEMALE || *nickname == CHAR_SLASH || *nickname == CHAR_HYPHEN ||
                *nickname == CHAR_ELLIPSIS || *nickname == CHAR_DBL_QUOT_LEFT || *nickname == CHAR_DBL_QUOT_RIGHT ||
