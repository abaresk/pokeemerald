...
#endif
    for (data_p = &gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[4]; numSlots != 0; data_p += 4) {
        bm_slot_id = data_p[2];
...
void rfu_REQ_disconnect(u8 bmDisconnectSlot) {
    ...
    if ((gRfuLinkStatus->connSlotFlag | gRfuLinkStatus->linkLossSlotFlag) & bmDisconnectSlot) {
        ...
        } else if (gRfuStatic->SCStartFlag && (STWI_set_Callback_M(rfu_CB_defaultCallback), STWI_send_SC_EndREQ(),
...
u16 rfu_changeSendTarget(u8 connType, u8 slotStatusIndex, u8 bmNewTgtSlot) {
    ...
    if (connType == 0x20) {
        ...
        if ((slotStatusNI->send.state & SLOT_BUSY_FLAG) && (slotStatusNI->send.state & SLOT_SEND_FLAG)) {
...
static void rfu_NI_checkCommFailCounter(void) {
    ...
    if (gRfuLinkStatus->sendSlotNIFlag | gRfuLinkStatus->recvSlotNIFlag) {
        ...
        for (bm_slot_id = 0; bm_slot_id < RFU_CHILD_MAX; ++bm_slot_id) {
            bm_slot_flag = 1 << bm_slot_id;
            if (gRfuLinkStatus->sendSlotNIFlag & bm_slot_flag && !(gRfuStatic->recvRenewalFlag & bm_slot_flag))
