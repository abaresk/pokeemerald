...
static void Task_LinkContest_StartCommunicateRngEm(u8 taskId) {
    SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateRngEm,
                                Task_LinkContest_StartCommunicateLeaderIdsEm);
}

static void Task_LinkContest_StartCommunicateLeaderIdsEm(u8 taskId) {
    SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateLeaderIdsEm,
...
static void Task_LinkContest_CommunicateAIMonsEm(u8 taskId) {
    ...
    switch (gTasks[taskId].data[0]) {
        ...
        case 0:
            if (GetMultiplayerId() == 0) {
                ...
                if (LinkContest_SendBlock(&gContestMons[gNumLinkContestPlayers],
                                          (CONTESTANT_COUNT - gNumLinkContestPlayers) *
                                              sizeof(struct ContestPokemon)) == 1)
                    gTasks[taskId].data[0]++;
            } else {
                gTasks[taskId].data[0]++;
            }
        ...
        case 1:
            if (LinkContest_GetBlockReceived(0)) {
                memcpy(&gContestMons[gNumLinkContestPlayers], gBlockRecvBuffer[0],
