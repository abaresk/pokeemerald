...
void Task_LinkContest_StartCommunicationEm(u8 taskId)
{
    ...
    switch (gTasks[taskId].tCategory)
    {
        case CONTEST_CATEGORY_COOL:
            gHighestRibbonRank =
                GetMonData(&gPlayerParty[gContestMonPartyIndex], MON_DATA_COOL_RIBBON);
            break;
        ...
        case CONTEST_CATEGORY_CUTE:
            gHighestRibbonRank =
                GetMonData(&gPlayerParty[gContestMonPartyIndex], MON_DATA_CUTE_RIBBON);
            break;
        ...
        case 0:
            if (IsLinkTaskFinished())
            {
                if (LinkContest_SendBlock(
                        &gContestMons[gContestPlayerMonIndex], sizeof(struct ContestPokemon))
                    == 1)
        ...
        case 1:
            if (LinkContest_GetBlockReceived(0))
            {
                ...
                for (i = gNumLinkContestPlayers; i < CONTESTANT_COUNT; i++)
                    StripPlayerAndMonNamesForLinkContest(
                        &gContestMons[i], gLinkPlayers[0].language);
