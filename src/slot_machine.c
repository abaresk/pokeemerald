...
static const struct WindowTemplate sWindowTemplate_InfoBox = {
    .bg = 0, .tilemapLeft = 1, .tilemapTop = 3, .width = 20, .height = 13, .paletteNum = 13, .baseBlock = 1
};
...
static bool8 (*const sAwardPayoutActions[])(struct Task* task) = { AwardPayoutAction0,
...
static bool8 (*const sDecideReelTurns_BiasTag[NUM_REELS])(void) = { DecideReelTurns_BiasTag_Reel1,
                                                                    DecideReelTurns_BiasTag_Reel2,
                                                                    DecideReelTurns_BiasTag_Reel3 };

static void (*const sDecideReelTurns_NoBiasTag[NUM_REELS])(void) = { DecideReelTurns_NoBiasTag_Reel1,
...
static bool8 (*const sDecideReelTurns_BiasTag_Reel1_Bets[MAX_BET])(u8 tag1, u8 tag2) = {
    DecideReelTurns_BiasTag_Reel1_Bet1, DecideReelTurns_BiasTag_Reel1_Bet2or3, DecideReelTurns_BiasTag_Reel1_Bet2or3
};

static bool8 (*const sDecideReelTurns_BiasTag_Reel2_Bets[MAX_BET])(void) = { DecideReelTurns_BiasTag_Reel2_Bet1or2,
...
static bool8 (*const sDecideReelTurns_BiasTag_Reel3_Bets[MAX_BET])(u8 biasTag) = {
    DecideReelTurns_BiasTag_Reel3_Bet1or2, DecideReelTurns_BiasTag_Reel3_Bet1or2, DecideReelTurns_BiasTag_Reel3_Bet3
};

static void (*const sDecideReelTurns_NoBiasTag_Reel2_Bets[MAX_BET])(void) = { DecideReelTurns_NoBiasTag_Reel2_Bet1,
                                                                              DecideReelTurns_NoBiasTag_Reel2_Bet2,
                                                                              DecideReelTurns_NoBiasTag_Reel2_Bet3 };

static void (*const sDecideReelTurns_NoBiasTag_Reel3_Bets[MAX_BET])(void) = { DecideReelTurns_NoBiasTag_Reel3_Bet1,
                                                                              DecideReelTurns_NoBiasTag_Reel3_Bet2,
                                                                              DecideReelTurns_NoBiasTag_Reel3_Bet3 };

static void (*const sReelStopButtonFuncs[])(struct Task* task, u8 taskId) = { StopReelButton_Press, StopReelButton_Wait,
                                                                              StopReelButton_Unpress };

static const s16 sReelButtonOffsets[NUM_REELS] = { 5, 10, 15 };

static void (*const sPikaPowerBoltFuncs[])(struct Task* task) = { PikaPowerBolt_Idle, PikaPowerBolt_AddBolt,
                                                                  PikaPowerBolt_WaitAnim, PikaPowerBolt_ClearAll };
...
static void (*const sReelTimeActions[])(struct Task* task) = { ReelTime_Init,
...
static void SlotMachineSetup_InitSlotMachineStruct(void) {
    ...
    for (i = 0; i < NUM_REELS; i++) {
        ...
        sSlotMachine->reelPixelOffsets[i] =
            SYMBOLS_PER_REEL * REEL_SYMBOL_HEIGHT - sSlotMachine->reelPositions[i] * REEL_SYMBOL_HEIGHT;
...
static bool8 SlotAction_CheckMatches(struct Task* task) {
    ...
    if (sSlotMachine->matchedSymbols) {
        ...
        // if you matched 777...
        if (sSlotMachine->matchedSymbols &
            ((1 << MATCHED_777_MIXED) | (1 << MATCHED_777_BLUE) | (1 << MATCHED_777_RED))) {
            sSlotMachine->luckyFlags &= ~(LUCKY_BIAS_777 | LUCKY_BIAS_MIXED_777);
            if (sSlotMachine->matchedSymbols & ((1 << MATCHED_777_BLUE) | (1 << MATCHED_777_RED))) {
                sSlotMachine->reelTimeSpinsLeft = 0;
                sSlotMachine->reelTimeSpinsUsed = 0;
...
static void AdvanceSlotReel(u8 reelIndex, s16 value) {
    ...
    sSlotMachine->reelPositions[reelIndex] =
        SYMBOLS_PER_REEL - sSlotMachine->reelPixelOffsets[reelIndex] / REEL_SYMBOL_HEIGHT;
}
...
static bool8 SlotReelAction_DecideWhereToStop(struct Task* task) {
    ...
    if (sSlotMachine->reelTimeSpinsLeft == 0 &&
        (sSlotMachine->luckyFlags == 0 || !sSlotMachine->isLuckySpin || !sDecideReelTurns_BiasTag[task->tReelId]())) {
        sSlotMachine->isLuckySpin = FALSE;
    ...
    for (i = 1; i < 5; i++) {
        ...
        if (biasedCopy || !AreCherriesOnScreen_Reel1(i)) {
            ...
            if (AreTagsAtPosition_Reel1(1 - i, tag1, tag2)) {
                //...and if it only took 1 turn and the lucky tag could also be the bottom row of a screen with no
                // cherries...
                ...
                //...or if it isn't the last turn and the lucky tag could be in the center row of a screen with no
                // cherries...
...
static bool8 DecideReelTurns_BiasTag_Reel2_Bet3(void) {
    ...
    if (DecideReelTurns_BiasTag_Reel2_Bet1or2()) {
        //...and if the biasTag is not in middle row of reel 1 and if biasTag appears in middle row of reel 2 in 2 or 3
        // turns...
        if (sSlotMachine->winnerRows[0] != 2 && sSlotMachine->reelExtraTurns[1] > 1 &&
...
static bool8 TagsDontMatchOrHaveAny7s(u8 tag1, u8 tag2, u8 tag3) {
    if ((tag1 == GFXTAG_7_RED && tag2 == GFXTAG_7_BLUE && tag3 == GFXTAG_7_RED) ||
        ...
        (tag1 == GFXTAG_7_BLUE && tag2 == GFXTAG_7_BLUE && tag3 == GFXTAG_7_RED) || (tag1 == tag2 && tag1 == tag3)) {
        return FALSE;
    }
    return TRUE;
}
...
static void DecideReelTurns_NoBiasTag_Reel3_Bet1(void) {
    ...
    if (tag1 == tag2) {
        ...
        while (1) {
            u8 tag3;
            if (!(tag1 == (tag3 = GetTag(RIGHT_REEL, 2 - i)) || (tag1 == GFXTAG_7_RED && tag3 == GFXTAG_7_BLUE) ||
...
static void DecideReelTurns_NoBiasTag_Reel3_Bet2(void) {
    ...
    u8 tag2;
    u8 tag3;
    if (sSlotMachine->winnerRows[1] != 0 && sSlotMachine->winnerRows[0] == sSlotMachine->winnerRows[1] &&
        sSlotMachine->luckyFlags & LUCKY_BIAS_777) {
    ...
    while (1) {
        ...
        for (i = 1, loopExit = 0; i < 4; i++) {
            ...
            if (!TagsDontMatchOrHaveAny7s(tag1, tag2, tag3) &&
                (!AreTagsMixed777(tag1, tag2, tag3) || !(sSlotMachine->luckyFlags & LUCKY_BIAS_777))) {
                loopExit++;
                break;
            }
        }
...
static void SpriteCB_FlashMatchingLines(struct Sprite* sprite) {
    ...
    if (sprite->sFlashing) {
        ...
        MultiplyPaletteRGBComponents(sMatchLinePalOffsets[sprite->sMatchLineId], sprite->sColor, sprite->sColor,
                                     sprite->sColor);
    }
}
...
static void CreateDigitalDisplayScene(u8 id) {
    ...
    for (i = 0; sDigitalDisplayScenes[id][i].spriteTemplateId != 255; i++) {
        u8 spriteId;
        spriteId = CreateStdDigitalDisplaySprite(sDigitalDisplayScenes[id][i].spriteTemplateId,
...
static void SpriteCB_ReelTimePikachuAura(struct Sprite* sprite) {
    ...
    if (sprite->sFlashPal && --sprite->sDelayTimer <= 0) {
        MultiplyInvertedPaletteRGBComponents((IndexOfSpritePaletteTag(PALTAG_PIKA_AURA) << 4) + 0x103,
                                             colors[sprite->sColorIdx], colors[sprite->sColorIdx],
                                             colors[sprite->sColorIdx]);
        ++sprite->sColorIdx;
        sprite->sColorIdx &= 1;
        sprite->sDelayTimer = sprite->sDelay;
    }
...
static u8 CreateStdDigitalDisplaySprite(u8 templateIdx, u8 dispInfoId, s16 spriteId) {
    return CreateDigitalDisplaySprite(templateIdx, sDigitalDisplay_SpriteCallbacks[dispInfoId],
...
static void SpriteCB_DigitalDisplay_PokeballShining(struct Sprite* sprite) {
    if (sprite->sCounter < 3) {
        LoadPalette(sPokeballShiningPalTable[sprite->sCounter],
                    (IndexOfSpritePaletteTag(PALTAG_DIG_DISPLAY) << 4) + 0x100, 32);
        if (++sprite->data[2] >= 4) {
            sprite->data[1]++;
            sprite->data[2] = 0;
        }
...
static const u8 sReelSymbolTileTags[NUM_REELS][SYMBOLS_PER_REEL] = {
    [LEFT_REEL] = { GFXTAG_7_RED,  GFXTAG_CHERRY, GFXTAG_AZURILL, GFXTAG_REPLAY, GFXTAG_POWER,   GFXTAG_LOTAD,
                    GFXTAG_7_BLUE, GFXTAG_LOTAD,  GFXTAG_CHERRY,  GFXTAG_POWER,  GFXTAG_REPLAY,  GFXTAG_AZURILL,
                    GFXTAG_7_RED,  GFXTAG_POWER,  GFXTAG_LOTAD,   GFXTAG_REPLAY, GFXTAG_AZURILL, GFXTAG_7_BLUE,
                    GFXTAG_POWER,  GFXTAG_LOTAD,  GFXTAG_REPLAY },
    [MIDDLE_REEL] = { GFXTAG_7_RED,  GFXTAG_CHERRY, GFXTAG_REPLAY,  GFXTAG_LOTAD, GFXTAG_AZURILL, GFXTAG_CHERRY,
                      GFXTAG_REPLAY, GFXTAG_POWER,  GFXTAG_POWER,   GFXTAG_LOTAD, GFXTAG_7_BLUE,  GFXTAG_LOTAD,
                      GFXTAG_REPLAY, GFXTAG_CHERRY, GFXTAG_AZURILL, GFXTAG_LOTAD, GFXTAG_REPLAY,  GFXTAG_CHERRY,
                      GFXTAG_LOTAD,  GFXTAG_REPLAY, GFXTAG_CHERRY },
    [RIGHT_REEL] = { GFXTAG_7_RED,   GFXTAG_POWER, GFXTAG_7_BLUE, GFXTAG_REPLAY,  GFXTAG_LOTAD,   GFXTAG_AZURILL,
                     GFXTAG_REPLAY,  GFXTAG_LOTAD, GFXTAG_POWER,  GFXTAG_AZURILL, GFXTAG_REPLAY,  GFXTAG_LOTAD,
                     GFXTAG_AZURILL, GFXTAG_POWER, GFXTAG_REPLAY, GFXTAG_LOTAD,   GFXTAG_AZURILL, GFXTAG_POWER,
                     GFXTAG_REPLAY,  GFXTAG_LOTAD, GFXTAG_CHERRY },
};

static const u8 sReelTimeTags[] = { 1, 0, 5, 4, 3, 2 };

static const s16 sInitialReelPositions[NUM_REELS][2] = {
    [LEFT_REEL] = { 0, 6 }, [MIDDLE_REEL] = { 0, 10 }, [RIGHT_REEL] = { 0, 2 }
};

static const u8 sLuckyRoundProbabilities[][3] = { { 1, 1, 12 }, { 1, 1, 14 }, { 2, 2, 14 },
                                                  { 2, 2, 14 }, { 2, 3, 16 }, { 3, 3, 16 } };

static const u8 sLuckyFlagProbabilities_Top3[][6] = { { 25, 25, 30, 40, 40, 50 },
                                                      { 25, 25, 30, 30, 35, 35 },
                                                      { 25, 25, 30, 25, 25, 30 } };

static const u8 sLuckyFlagProbabilities_NotTop3[][6] = { { 20, 25, 25, 20, 25, 25 },
                                                         { 12, 15, 15, 18, 19, 22 },
                                                         { 25, 25, 25, 30, 30, 40 },
                                                         { 25, 25, 20, 20, 15, 15 },
                                                         { 40, 40, 35, 35, 40, 40 } };
...
static const u16 sReelTimeExplodeProbability[] = { 128, 175, 200, 225, 256 };

static const u16 sReelIncrementTable[][2] = { { 10, 5 }, { 10, 10 }, { 10, 15 }, { 10, 25 }, { 10, 35 } };

static const u16 sReelTimeBonusIncrementTable[] = { 0, 5, 10, 15, 20 };

// tentative name
static const u8 sBiasTags[] = { GFXTAG_REPLAY, GFXTAG_CHERRY, GFXTAG_LOTAD, GFXTAG_AZURILL,
                                GFXTAG_POWER,  GFXTAG_7_RED,  GFXTAG_7_RED, GFXTAG_7_RED };

static const u16 sLuckyFlagSettings_Top3[] = { LUCKY_BIAS_777, LUCKY_BIAS_REELTIME, LUCKY_BIAS_MIXED_777 };

static const u16 sLuckyFlagSettings_NotTop3[] = { LUCKY_BIAS_POWER, LUCKY_BIAS_AZURILL, LUCKY_BIAS_LOTAD,
                                                  LUCKY_BIAS_CHERRY, LUCKY_BIAS_REPLAY };

static const u8 sSymToMatch[] = {
    [GFXTAG_7_RED] = MATCHED_777_RED, [GFXTAG_7_BLUE] = MATCHED_777_BLUE, [GFXTAG_AZURILL] = MATCHED_AZURILL,
    [GFXTAG_LOTAD] = MATCHED_LOTAD,   [GFXTAG_CHERRY] = MATCHED_1CHERRY,  [GFXTAG_POWER] = MATCHED_POWER,
    [GFXTAG_REPLAY] = MATCHED_REPLAY
};

static const u16 sSlotMatchFlags[] = {
    [MATCHED_1CHERRY] = 1 << MATCHED_1CHERRY,     [MATCHED_2CHERRY] = 1 << MATCHED_2CHERRY,
    [MATCHED_REPLAY] = 1 << MATCHED_REPLAY,       [MATCHED_LOTAD] = 1 << MATCHED_LOTAD,
    [MATCHED_AZURILL] = 1 << MATCHED_AZURILL,     [MATCHED_POWER] = 1 << MATCHED_POWER,
    [MATCHED_777_MIXED] = 1 << MATCHED_777_MIXED, [MATCHED_777_RED] = 1 << MATCHED_777_RED,
    [MATCHED_777_BLUE] = 1 << MATCHED_777_BLUE
};

static const u16 sSlotPayouts[] = { [MATCHED_1CHERRY] = 2,    [MATCHED_2CHERRY] = 4,   [MATCHED_REPLAY] = 0,
                                    [MATCHED_LOTAD] = 6,      [MATCHED_AZURILL] = 12,  [MATCHED_POWER] = 3,
                                    [MATCHED_777_MIXED] = 90, [MATCHED_777_RED] = 300, [MATCHED_777_BLUE] = 300 };
...
static const struct DigitalDisplaySprite sDigitalDisplay_Win[] = { { DIG_SPRITE_WIN, DIG_DISPINFO_WIN, 0 },
                                                                   { DIG_SPRITE_POKE_BALL,
                                                                     DIG_DISPINFO_POKE_BALL_SHINING, 0 },
                                                                   DIG_SPRITE_DUMMY };

static const struct DigitalDisplaySprite sDigitalDisplay_Lose[] = {
    { DIG_SPRITE_LOSE, DIG_DISPINFO_LOSE, 0 },      { DIG_SPRITE_SMOKE, DIG_DISPINFO_SMOKE_NW, 0 },
    { DIG_SPRITE_SMOKE, DIG_DISPINFO_SMOKE_NE, 1 }, { DIG_SPRITE_SMOKE, DIG_DISPINFO_SMOKE_SW, 2 },
    { DIG_SPRITE_SMOKE, DIG_DISPINFO_SMOKE_SE, 3 }, DIG_SPRITE_DUMMY
};

static const struct DigitalDisplaySprite sDigitalDisplay_ReelTime[] = { { DIG_SPRITE_REEL, DIG_DISPINFO_REEL, 0 },
                                                                        { DIG_SPRITE_TIME, DIG_DISPINFO_TIME, 0 },
                                                                        { DIG_SPRITE_NUMBER, DIG_DISPINFO_NUMBER,
                                                                          0 }, // Number of reel time spins left
                                                                        DIG_SPRITE_DUMMY };
...
static const struct SpriteFrameImage sImageTable_ReelTimeNumbers[] = {
    { gSlotMachineReelTimeNumber0, 0x80 }, { gSlotMachineReelTimeNumber1, 0x80 }, { gSlotMachineReelTimeNumber2, 0x80 },
    { gSlotMachineReelTimeNumber3, 0x80 }, { gSlotMachineReelTimeNumber4, 0x80 }, { gSlotMachineReelTimeNumber5, 0x80 },
};
...
static const union AnimCmd sAnim_SingleFrame[] = { ANIMCMD_FRAME(0, 1), ANIMCMD_END };

static const union AnimCmd sAnim_ReelTimeDuck[] = { ANIMCMD_FRAME(0, 1), ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_ReelTimePikachu_Still[] = { ANIMCMD_FRAME(0, 16), ANIMCMD_END };

static const union AnimCmd sAnim_ReelTimePikachu_ChargingSlow[] = { ANIMCMD_FRAME(1, 16), ANIMCMD_FRAME(0, 16),
                                                                    ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_ReelTimePikachu_ChargingMedium[] = { ANIMCMD_FRAME(1, 8), ANIMCMD_FRAME(0, 8),
                                                                      ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_ReelTimePikachu_ChargingFast[] = { ANIMCMD_FRAME(1, 4), ANIMCMD_FRAME(0, 4),
                                                                    ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_ReelTimePikachu_Cheering[] = { ANIMCMD_FRAME(2, 32), ANIMCMD_FRAME(3, 32),
                                                                ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_ReelTimePikachu_FellOver[] = { ANIMCMD_FRAME(4, 1), ANIMCMD_END };

static const union AnimCmd sAnim_ReelTimeNumber_0[] = { ANIMCMD_FRAME(0, 1), ANIMCMD_END };

static const union AnimCmd sAnim_ReelTimeNumber_1[] = { ANIMCMD_FRAME(1, 1), ANIMCMD_END };

static const union AnimCmd sAnim_ReelTimeNumber_2[] = { ANIMCMD_FRAME(2, 1), ANIMCMD_END };

static const union AnimCmd sAnim_ReelTimeNumber_3[] = { ANIMCMD_FRAME(3, 1), ANIMCMD_END };

static const union AnimCmd sAnim_ReelTimeNumber_4[] = { ANIMCMD_FRAME(4, 1), ANIMCMD_END };

static const union AnimCmd sAnim_ReelTimeNumber_5[] = { ANIMCMD_FRAME(5, 1), ANIMCMD_END };

static const union AnimCmd sAnim_ReelTimeBolt[] = { ANIMCMD_FRAME(0, 4), ANIMCMD_FRAME(1, 4), ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_ReelTimeExplosion[] = { ANIMCMD_FRAME(0, 16), ANIMCMD_FRAME(1, 16), ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_DigitalDisplay_AButton_Flashing[] = { ANIMCMD_FRAME(0, 30), ANIMCMD_FRAME(1, 30),
                                                                       ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_DigitalDisplay_AButton_Static[] = { ANIMCMD_FRAME(1, 1), ANIMCMD_END };

static const union AnimCmd sAnim_DigitalDisplay_DPad_Flashing[] = { ANIMCMD_FRAME(0, 30), ANIMCMD_FRAME(1, 30),
                                                                    ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_DigitalDisplay_Pokeball_Rocking[] = { ANIMCMD_FRAME(0, 16), ANIMCMD_FRAME(1, 16),
                                                                       ANIMCMD_FRAME(0, 16),
                                                                       ANIMCMD_FRAME(1, 16, .hFlip = TRUE),
                                                                       ANIMCMD_JUMP(0) };

static const union AnimCmd sAnim_DigitalDisplay_Pokeball_Static[] = { ANIMCMD_FRAME(0, 1), ANIMCMD_END };

static const union AnimCmd sAnim_DigitalDisplay_Number_1[] = { ANIMCMD_FRAME(0, 1), ANIMCMD_END };

static const union AnimCmd sAnim_DigitalDisplay_Number_2[] = { ANIMCMD_FRAME(1, 1), ANIMCMD_END };

static const union AnimCmd sAnim_DigitalDisplay_Number_3[] = { ANIMCMD_FRAME(2, 1), ANIMCMD_END };

static const union AnimCmd sAnim_DigitalDisplay_Number_4[] = { ANIMCMD_FRAME(3, 1), ANIMCMD_END };

static const union AnimCmd sAnim_DigitalDisplay_Number_5[] = { ANIMCMD_FRAME(4, 1), ANIMCMD_END };

static const union AnimCmd* const sAnims_SingleFrame[] = { sAnim_SingleFrame };

static const union AnimCmd* const sAnims_ReelTimeDuck[] = { sAnim_ReelTimeDuck };

static const union AnimCmd* const sAnims_ReelTimePikachu[] = {
    sAnim_ReelTimePikachu_Still,        sAnim_ReelTimePikachu_ChargingSlow, sAnim_ReelTimePikachu_ChargingMedium,
...
static const union AnimCmd* const sAnims_ReelTimeNumbers[] = { sAnim_ReelTimeNumber_0, sAnim_ReelTimeNumber_1,
                                                               sAnim_ReelTimeNumber_2, sAnim_ReelTimeNumber_3,
                                                               sAnim_ReelTimeNumber_4, sAnim_ReelTimeNumber_5 };

static const union AnimCmd* const sAnims_ReelTimeBolt[] = { sAnim_ReelTimeBolt };

static const union AnimCmd* const sAnims_ReelTimeExplosion[] = { sAnim_ReelTimeExplosion };

static const union AnimCmd* const sAnims_DigitalDisplay_AButton[] = { sAnim_DigitalDisplay_AButton_Flashing,
                                                                      sAnim_DigitalDisplay_AButton_Static };

static const union AnimCmd* const sAnims_DigitalDisplay_DPad[] = { sAnim_DigitalDisplay_DPad_Flashing };

static const union AnimCmd* const sAnims_DigitalDisplay_Pokeball[] = { sAnim_DigitalDisplay_Pokeball_Rocking,
                                                                       sAnim_DigitalDisplay_Pokeball_Static };

static const union AnimCmd* const sAnims_DigitalDisplay_Number[] = {
    sAnim_DigitalDisplay_Number_1, sAnim_DigitalDisplay_Number_2, sAnim_DigitalDisplay_Number_3,
    sAnim_DigitalDisplay_Number_4, sAnim_DigitalDisplay_Number_5
};

static const union AffineAnimCmd sAffineAnim_ReelTimeSmoke[] = { AFFINEANIMCMD_FRAME(16, 16, 0, 0),
                                                                 AFFINEANIMCMD_LOOP(0), AFFINEANIMCMD_FRAME(1, 1, 0, 1),
                                                                 AFFINEANIMCMD_LOOP(0xFF), AFFINEANIMCMD_END };

static const union AffineAnimCmd* const sAffineAnims_ReelTimeSmoke[] = { sAffineAnim_ReelTimeSmoke };

// Spin as it appears
static const union AffineAnimCmd sAffineAnim_PikaPowerBolt[] = {
    AFFINEANIMCMD_FRAME(0, 0, 8, 32),  AFFINEANIMCMD_FRAME(0, 0, 6, 32),
    AFFINEANIMCMD_FRAME(0, 0, 4, 16),  AFFINEANIMCMD_FRAME(0, 0, 12, 2),
    AFFINEANIMCMD_FRAME(0, 0, -12, 4), AFFINEANIMCMD_FRAME(0, 0, 12, 2),
    AFFINEANIMCMD_FRAME(0, 0, 12, 2),  AFFINEANIMCMD_FRAME(0, 0, -12, 4),
    AFFINEANIMCMD_FRAME(0, 0, 12, 2),  AFFINEANIMCMD_END
};

static const union AffineAnimCmd* const sAffineAnims_PikaPowerBolt[] = { sAffineAnim_PikaPowerBolt };

static const struct SpriteTemplate sSpriteTemplate_ReelSymbol = { .tileTag = GFXTAG_SYMBOLS_START,
...
static const struct SpriteTemplate sSpriteTemplate_CoinNumber = { .tileTag = GFXTAG_NUMBERS_START,
...
static const struct SpriteTemplate sSpriteTemplate_ReelBackground = { .tileTag = GFXTAG_REEL_BG,
                                                                      .paletteTag = PALTAG_REEL,
                                                                      .oam = &sOam_64x64,
                                                                      .anims = sAnims_SingleFrame,
                                                                      .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimePikachu = { .tileTag = 0xFFFF,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeMachineAntennae = { .tileTag = 0xFFFF,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeMachine = { .tileTag = 0xFFFF,
...
static const struct SpriteTemplate sSpriteTemplate_BrokenReelTimeMachine = { .tileTag = 0xFFFF,
                                                                             .paletteTag = PALTAG_REEL_TIME_MACHINE,
                                                                             .oam = &sOam_8x16,
                                                                             .anims = sAnims_SingleFrame,
                                                                             .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeNumbers = { .tileTag = 0xFFFF,
                                                                       .paletteTag = PALTAG_MISC,
                                                                       .oam = &sOam_16x16,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeShadow = { .tileTag = 0xFFFF,
                                                                      .paletteTag = PALTAG_MISC,
                                                                      .oam = &sOam_16x16,
                                                                      .anims = sAnims_SingleFrame,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeNumberGap = { .tileTag = 0xFFFF,
                                                                         .paletteTag = PALTAG_MISC,
                                                                         .oam = &sOam_16x16,
                                                                         .anims = sAnims_SingleFrame,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeBolt = { .tileTag = 0xFFFF,
                                                                    .paletteTag = PALTAG_MISC,
                                                                    .oam = &sOam_16x32,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimePikachuAura = { .tileTag = 0xFFFF,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeExplosion = { .tileTag = 0xFFFF,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeDuck = { .tileTag = 0xFFFF,
                                                                    .paletteTag = PALTAG_MISC,
                                                                    .oam = &sOam_8x8,
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeSmoke = { .tileTag = 0xFFFF,
                                                                     .paletteTag = PALTAG_MISC,
                                                                     .oam = &sOam_16x16,
                                                                     .anims = sAnims_SingleFrame,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Reel = { .tileTag = 0xFFFF,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Time = { .tileTag = 0xFFFF,
                                                                           .paletteTag = PALTAG_DIG_DISPLAY,
                                                                           .oam = &sOam_8x8,
                                                                           .anims = sAnims_SingleFrame,
                                                                           .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Insert = { .tileTag = 0xFFFF,
                                                                             .paletteTag = PALTAG_DIG_DISPLAY,
                                                                             .oam = &sOam_8x8,
                                                                             .anims = sAnims_SingleFrame,
                                                                             .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Stop = { .tileTag = 18,
                                                                           .paletteTag = PALTAG_DIG_DISPLAY,
                                                                           .oam = &sOam_8x8,
                                                                           .anims = sAnims_SingleFrame,
                                                                           .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Win = { .tileTag = 0xFFFF,
                                                                          .paletteTag = PALTAG_DIG_DISPLAY,
                                                                          .oam = &sOam_64x32,
                                                                          .anims = sAnims_SingleFrame,
                                                                          .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Lose = { .tileTag = 0xFFFF,
                                                                           .paletteTag = PALTAG_DIG_DISPLAY,
                                                                           .oam = &sOam_64x32,
                                                                           .anims = sAnims_SingleFrame,
                                                                           .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Bonus = { .tileTag = 19,
                                                                            .paletteTag = PALTAG_DIG_DISPLAY,
                                                                            .oam = &sOam_8x8,
                                                                            .anims = sAnims_SingleFrame,
                                                                            .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Big = { .tileTag = 20,
                                                                          .paletteTag = PALTAG_DIG_DISPLAY,
                                                                          .oam = &sOam_8x8,
                                                                          .anims = sAnims_SingleFrame,
                                                                          .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Reg = { .tileTag = 21,
                                                                          .paletteTag = PALTAG_DIG_DISPLAY,
                                                                          .oam = &sOam_8x8,
                                                                          .anims = sAnims_SingleFrame,
                                                                          .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_AButton = { .tileTag = 0xFFFF,
                                                                              .paletteTag = PALTAG_DIG_DISPLAY,
                                                                              .oam = &sOam_32x32,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Smoke = { .tileTag = 0xFFFF,
                                                                            .paletteTag = PALTAG_DIG_DISPLAY,
                                                                            .oam = &sOam_8x8,
                                                                            .anims = sAnims_SingleFrame,
                                                                            .images = NULL,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Number = { .tileTag = 0xFFFF,
                                                                             .paletteTag = PALTAG_DIG_DISPLAY,
                                                                             .oam = &sOam_16x16,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_Pokeball = { .tileTag = 0xFFFF,
                                                                               .paletteTag = PALTAG_DIG_DISPLAY,
                                                                               .oam = &sOam_8x8,
...
static const struct SpriteTemplate sSpriteTemplate_DigitalDisplay_DPad = { .tileTag = 0xFFFF,
                                                                           .paletteTag = PALTAG_DIG_DISPLAY,
                                                                           .oam = &sOam_8x8,
...
static const struct SpriteTemplate sSpriteTemplate_PikaPowerBolt = { .tileTag = 0xFFFF,
                                                                     .paletteTag = PALTAG_MISC,
                                                                     .oam = &sOam_8x8,
                                                                     .anims = sAnims_SingleFrame,
...
static const struct SubspriteTable sSubspriteTable_ReelBackground[] = { ARRAY_COUNT(sSubsprites_ReelBackground),
...
static const struct Subsprite sSubsprites_ReelTimeMachineAntennae[] = {
    ...
    { .x = 0, .y = 4, .shape = SPRITE_SHAPE(32x8), .size = SPRITE_SIZE(32x8), .tileOffset = 20, .priority = 1 }
};
...
static const struct SubspriteTable sSubspriteTable_ReelTimeMachine[] = { ARRAY_COUNT(sSubsprites_ReelTimeMachine),
...
static const struct SubspriteTable sSubspriteTable_ReelTimeShadow[] = { ARRAY_COUNT(sSubsprites_ReelTimeShadow),
...
static const struct SubspriteTable sSubspriteTable_ReelTimeNumberGap[] = { ARRAY_COUNT(sSubsprites_ReelTimeNumberGap),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_Win[] = { ARRAY_COUNT(sSubsprites_DigitalDisplay_Win),
...
static const u16 sUnused[] = {
    0x6F7B, 0x6968, 0x36AB, 0x7FFF, 0x5750, 0x7EC0, 0x02BA, 0x02BA, 0x01FD, 0x01FD,
};
...
static const u16* const sLitMatchLinePalTable[NUM_MATCH_LINES] = {
    [MATCH_MIDDLE_ROW] = sMiddleRowLit_Pal, [MATCH_TOP_ROW] = sTopRowLit_Pal,     [MATCH_BOTTOM_ROW] = sBottomRowt_Pal,
    [MATCH_NWSE_DIAG] = sNWSEDiagLit_Pal,   [MATCH_NESW_DIAG] = sNESWDiagLit_Pal,
};

static const u16* const sDarkMatchLinePalTable[NUM_MATCH_LINES] = {
    [MATCH_MIDDLE_ROW] = &gSlotMachineMenu_Pal[74], [MATCH_TOP_ROW] = &gSlotMachineMenu_Pal[75],
    [MATCH_BOTTOM_ROW] = &gSlotMachineMenu_Pal[76], [MATCH_NWSE_DIAG] = &gSlotMachineMenu_Pal[77],
...
// Flashing lights at top of slot machine, brightest point inside light goes from toward center of machine, to middle,
// to toward edges
...
static const u32 sReelTimeGfx[] =
    INCBIN_U32("graphics/slot_machine/reel_time_gfx.4bpp.lz"); // reel_time_machine and reel_time_pikachu
