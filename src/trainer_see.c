...
static const struct SpriteFrameImage sSpriteImageTable_ExclamationQuestionMark[] = {
    { .data = sEmotion_ExclamationMarkGfx, .size = 0x80 }, { .data = sEmotion_QuestionMarkGfx, .size = 0x80 }
};

static const struct SpriteFrameImage sSpriteImageTable_HeartIcon[] = { { .data = sEmotion_HeartGfx, .size = 0x80 } };
...
static const union AnimCmd* const sSpriteAnimTable_Icons[] = { sSpriteAnim_Icons1, sSpriteAnim_Icons2 };
...
bool8 CheckForTrainersWantingBattle(void) {
    ...
    if (gNoOfApproachingTrainers == 1) {
        ...
        for (i = 0; i < gNoOfApproachingTrainers; i++, gApproachingTrainerId++) {
            ConfigureTwoTrainersBattle(gApproachingTrainers[i].objectEventId, gApproachingTrainers[i].trainerScriptPtr);
        }
...
static u8 GetTrainerApproachDistance(struct ObjectEvent* trainerObj) {
    ...
    } else // TRAINER_TYPE_SEE_ALL_DIRECTIONS, TRAINER_TYPE_BURIED
    {
        for (i = 0; i < ARRAY_COUNT(sDirectionalApproachDistanceFuncs); i++) {
            approachDistance =
                sDirectionalApproachDistanceFuncs[i](trainerObj, trainerObj->trainerRange_berryTreeId, x, y);
            if (CheckPathBetweenTrainerAndPlayer(trainerObj, approachDistance,
...
static bool8 TrainerExclamationMark(u8 taskId, struct Task* task, struct ObjectEvent* trainerObj) {
    ...
    ObjectEventGetLocalIdAndMap(trainerObj, &gFieldEffectArguments[0], &gFieldEffectArguments[1],
...
static bool8 PlayerFaceApproachingTrainer(u8 taskId, struct Task* task, struct ObjectEvent* trainerObj) {
    ...
    TryOverrideTemplateCoordsForObjectEvent(trainerObj,
                                            GetTrainerFacingDirectionMovementType(trainerObj->facingDirection));
...
static void sub_80B44C8(u8 taskId) {
    ...
    if (task->data[0] == 3 && !FieldEffectActiveListContains(FLDEFF_ASH_PUFF)) {
        ...
        TryOverrideTemplateCoordsForObjectEvent(objEvent,
                                                GetTrainerFacingDirectionMovementType(objEvent->facingDirection));
        DestroyTask(taskId);
    } else {
...
void PlayerFaceTrainerAfterBattle(void) {
    ...
    if (gTrainerApproachedPlayer == TRUE) {
        ...
        ScriptMovement_StartObjectMovementScript(OBJ_EVENT_ID_PLAYER, gSaveBlock1Ptr->location.mapNum,
                                                 gSaveBlock1Ptr->location.mapGroup, gPostBattleMovementScript);
    } else {
