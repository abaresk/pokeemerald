...
static void PrintOnWindow_Font1(u8 windowId, const u8* src, u8 x, u8 y, u8 letterSpacing, u8 lineSpacing, u8 speed,
                                u8 colorTableId);
static void PrintOnWindow_Font7(u8 windowId, const u8* src, u8 x, u8 y, u8 letterSpacing, u8 lineSpacing, u8 speed,
                                u8 colorTableId);
...
static void PyramidBagMoveCursorFunc(s32 itemIndex, bool8 onInit, struct ListMenu* list) {
    ...
    if (gPyramidBagResources->unk814 == 0xFF) {
        ...
        if (itemIndex != LIST_CANCEL)
            ShowItemImage(gSaveBlock2Ptr->frontier.pyramidBag.itemId[gSaveBlock2Ptr->frontier.lvlMode][itemIndex],
...
static void PrintItemQuantity(u8 windowId, s32 itemIndex, u8 y) {
    ...
    ConvertIntToDecimalStringN(
        gStringVar1, gSaveBlock2Ptr->frontier.pyramidBag.quantity[gSaveBlock2Ptr->frontier.lvlMode][itemIndex],
...
static void PrintItemDescription(s32 listMenuId) {
    ...
    if (listMenuId != LIST_CANCEL) {
        desc = ItemId_GetDescription(
            gSaveBlock2Ptr->frontier.pyramidBag.itemId[gSaveBlock2Ptr->frontier.lvlMode][listMenuId]);
    } else {
...
static void AddScrollArrow(void) {
    if (gPyramidBagResources->scrollIndicatorsTaskId == 0xFF)
        gPyramidBagResources->scrollIndicatorsTaskId = AddScrollIndicatorArrowPairParameterized(
            2, 172, 12, 148, gPyramidBagResources->listMenuCount - gPyramidBagResources->listMenuMaxShown, 0xB5E, 0xB5E,
...
static void sub_81C56F8(void) {
    ...
    data[0] = ListMenuInit(&gMultiuseListMenuTemplate, gPyramidBagCursorData.scrollPosition,
    ...
    if (MenuHelpers_CallLinkSomething() != TRUE && !gPaletteFade.active) {
        if (gMain.newKeys & SELECT_BUTTON) {
            if (gPyramidBagCursorData.unk4 != 2) {
                ListMenuGetScrollAndRow(data[0], &gPyramidBagCursorData.scrollPosition,
                                        &gPyramidBagCursorData.cursorPosition);
                if (gPyramidBagCursorData.scrollPosition + gPyramidBagCursorData.cursorPosition !=
            ...
            switch (listId) {
                ...
                default:
                    PlaySE(SE_SELECT);
                    gSpecialVar_ItemId =
                        gSaveBlock2Ptr->frontier.pyramidBag.itemId[gSaveBlock2Ptr->frontier.lvlMode][listId];
                    data[1] = listId;
...
static void sub_81C5EAC(u8 windowId) {
    AddItemMenuActionTextPrinters(windowId, 7, 8, 1, 0, 0x10, gPyramidBagResources->menuActionsCount, sMenuActions,
...
static void sub_81C5F08(u8 windowId, u8 horizontalCount, u8 verticalCount) {
    sub_8198DBC(windowId, 7, 8, 1, 0x38, horizontalCount, verticalCount, sMenuActions,
                gPyramidBagResources->menuActionIds);
...
void TryStoreHeldItemsInPyramidBag(void) {
    ...
    memcpy(newItems, gSaveBlock2Ptr->frontier.pyramidBag.itemId[gSaveBlock2Ptr->frontier.lvlMode],
           PYRAMID_BAG_ITEMS_COUNT * sizeof(u16));
    memcpy(newQuantities, gSaveBlock2Ptr->frontier.pyramidBag.quantity[gSaveBlock2Ptr->frontier.lvlMode],
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (heldItem != ITEM_NONE && !AddBagItem(heldItem, 1)) {
            ...
            memcpy(gSaveBlock2Ptr->frontier.pyramidBag.itemId[gSaveBlock2Ptr->frontier.lvlMode], newItems,
                   PYRAMID_BAG_ITEMS_COUNT * sizeof(u16));
            memcpy(gSaveBlock2Ptr->frontier.pyramidBag.quantity[gSaveBlock2Ptr->frontier.lvlMode], newQuantities,
                   PYRAMID_BAG_ITEMS_COUNT * sizeof(u8));
            Free(newItems);
            Free(newQuantities);
...
static void ShowItemImage(u16 itemId, u8 itemSpriteArrayId) {
    ...
    if (*spriteId == 0xFF) {
        ...
        itemSpriteId =
            AddItemIconSprite(ITEM_IMAGE_TAG + 1 + itemSpriteArrayId, ITEM_IMAGE_TAG + 1 + itemSpriteArrayId, itemId);
