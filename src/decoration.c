...
static const u8* const sDecorationCategoryNames[] = { gText_Desk, gText_Chair,  gText_Plant, gText_Ornament,
...
static const u8* const sSecretBasePCMenuItemDescriptions[] = { gText_PutOutSelectedDecorItem,
                                                               ...
                                                               gText_ThrowAwayUnwantedDecors, gText_GoBackPrevMenu };
...
static const struct WindowTemplate sDecorationWindowTemplates[WINDOW_COUNT] = {
    { .bg = 0,
      ...
      .paletteNum = 15,
      .baseBlock = 0x0001 },
    { .bg = 0, .tilemapLeft = 1, .tilemapTop = 1, .width = 13, .height = 18, .paletteNum = 13, .baseBlock = 0x0091 },
    { .bg = 0, .tilemapLeft = 17, .tilemapTop = 1, .width = 12, .height = 2, .paletteNum = 15, .baseBlock = 0x017b },
    { .bg = 0, .tilemapLeft = 16, .tilemapTop = 13, .width = 13, .height = 6, .paletteNum = 15, .baseBlock = 0x0193 }
};
...
static const union AnimCmd sDecorSelectorAnimCmd0[] = { ANIMCMD_FRAME(0, 0, FALSE, FALSE), ANIMCMD_END };
...
static const struct SpriteTemplate sDecorWhilePlacingSpriteTemplate = {
    0x0000, 0x0000, &sDecorSelectorOam, sDecorSelectorAnimCmds, NULL, gDummySpriteAffineAnimTable, SpriteCallbackDummy
};
...
static const u16 sDecorShapeSizes[] = { 0x04, 0x08, 0x10, 0x20, 0x10, 0x08, 0x10, 0x20, 0x40, 0x20 };
...
static void AddDecorationActionsWindow(void) {
    ...
    InitMenuInUpperLeftCornerPlaySoundWhenAPressed(windowId, ARRAY_COUNT(sDecorationMainMenuActions),
...
static void ReinitDecorationCategoriesWindow(u8 taskId) {
    ...
    PrintDecorationCategoryMenuItems(taskId);
    InitMenuInUpperLeftCornerPlaySoundWhenAPressed(sDecorMenuWindowIds[WINDOW_DECORATION_CATEGORIES],
                                                   DECORCAT_COUNT + 1, sCurDecorationCategory);
    gTasks[taskId].func = HandleDecorationCategoriesMenuInput;
}
...
static void PrintDecorationCategoryMenuItems(u8 taskId) {
    ...
    AddTextPrinterParameterized(windowId, 1,
...
static void InitDecorationItemsMenuScrollAndCursor(void) {
    sub_812225C(&sDecorationsScrollOffset, &sDecorationsCursorPos, sDecorationItemsMenu->maxShownItems,
                sDecorationItemsMenu->numMenuItems);
}

static void InitDecorationItemsMenuScrollAndCursor2(void) {
    sub_8122298(&sDecorationsScrollOffset, &sDecorationsCursorPos, sDecorationItemsMenu->maxShownItems,
                sDecorationItemsMenu->numMenuItems, 8);
}
...
static void AddDecorationItemsScrollIndicators(void) {
    if (sDecorationItemsMenu->scrollIndicatorsTaskId == 0xFF) {
        sDecorationItemsMenu->scrollIndicatorsTaskId = AddScrollIndicatorArrowPairParameterized(
            SCROLL_ARROW_UP, 0x3c, 0x0c, 0x94, sDecorationItemsMenu->numMenuItems - sDecorationItemsMenu->maxShownItems,
            0x6e, 0x6e, &sDecorationsScrollOffset);
    }
}
...
static void WarpToInitialPosition(u8 taskId) {
    DrawWholeMapView();
    SetWarpDestination(gSaveBlock1Ptr->location.mapGroup, gSaveBlock1Ptr->location.mapNum, -1, gTasks[taskId].tInitialX,
    ...
    for (j = 0; j < decHeight; j++) {
        ...
        for (i = 0; i < decWidth; i++) {
            ...
            if (MetatileBehavior_IsSecretBaseImpassable(behavior) == TRUE ||
                (gDecorations[decoration].permission != DECORPERM_PASS_FLOOR && (behavior >> METATILE_ELEVATION_SHIFT)))
            ...
            if (elevation != 0xFFFF)
                MapGridSetMetatileEntryAt(x, y,
                                          (gDecorations[decoration].tiles[j * decWidth + i] + (0x200 | overlapsWall)) |
                                              impassableFlag | elevation);
            else
                MapGridSetMetatileIdAt(
                    x, y, (gDecorations[decoration].tiles[j * decWidth + i] + (0x200 | overlapsWall)) | impassableFlag);
        }
    }
}
...
void SetDecoration(void) {
    ...
    for (i = 0; i < NUM_DECORATION_FLAGS; i++) {
        if (FlagGet(FLAG_DECORATION_1 + i) == TRUE) {
            ...
            VarSet(VAR_OBJ_GFX_ID_0 + (gMapHeader.events->objectEvents[j].graphicsId - OBJ_EVENT_GFX_VAR_0),
            ...
            TryMoveObjectEventToMapCoords(gSpecialVar_0x8005, gSaveBlock1Ptr->location.mapNum,
                                          gSaveBlock1Ptr->location.mapGroup, gSpecialVar_0x8006, gSpecialVar_0x8007);
            TryOverrideObjectEventTemplateCoords(gSpecialVar_0x8005, gSaveBlock1Ptr->location.mapNum,
...
static void Task_PlaceDecoration(u8 taskId) {
    switch (gTasks[taskId].tState) {
        ...
        case 1:
            ...
            ConfigureCameraObjectForPlacingDecoration(&sPlaceDecorationGraphicsDataBuffer,
...
static void SetUpPlacingDecorationPlayerAvatar(u8 taskId, struct PlaceDecorationGraphicsDataBuffer* data) {
    ...
    x = 16 * (u8)gTasks[taskId].tDecorWidth + sDecorationMovementInfo[data->decoration->shape].cameraX -
        ...
        case DECORPERM_PASS_FLOOR:
            for (i = 0; i < mapY; i++) {
                ...
                for (j = 0; j < mapX; j++) {
                    ...
                    behaviorBy = GetBehaviorByMetatileId(0x200 + decoration->tiles[(mapY - 1 - i) * mapX + j]) &
...
static void SetDecorSelectionMetatiles(struct PlaceDecorationGraphicsDataBuffer* data) {
    ...
    for (i = 0; i < gUnknown_085A71B0[shape].size; i++) {
        data->tiles[gUnknown_085A71B0[shape].tiles[i]] =
            GetMetatile(data->decoration->tiles[gUnknown_085A71B0[shape].y[i]] * 8 + gUnknown_085A71B0[shape].x[i]);
    }
}
...
static u8 gpu_pal_decompress_alloc_tag_and_upload(struct PlaceDecorationGraphicsDataBuffer* data, u8 decor) {
    ...
    CopyPalette(data->palette,
                ((u16*)gTilesetPointer_SecretBaseRedCave->metatiles)[(data->decoration->tiles[0] * 8) + 7] >> 12);
...
static u8 AddDecorationIconObjectFromObjectEvent(u16 tilesTag, u16 paletteTag, u8 decor) {
    ...
    if (sPlaceDecorationGraphicsDataBuffer.decoration->permission != DECORPERM_SPRITE) {
        ...
        *template = sDecorWhilePlacingSpriteTemplate;
        ...
        spriteId =
            AddPseudoObjectEvent(sPlaceDecorationGraphicsDataBuffer.decoration->tiles[0], SpriteCallbackDummy, 0, 0, 1);
    }
    return spriteId;
}
...
void PutAwayDecorationIteration(void) {
    ...
    } else if (gDecorations[sDecorationContext.items[sDecorRearrangementDataBuffer[gSpecialVar_0x8004].idx]]
            ...
            for (y = 0; y < sDecorRearrangementDataBuffer[i].height; y++) {
                for (x = 0; x < sDecorRearrangementDataBuffer[i].width; x++) {
                    MapGridSetMetatileEntryAt(
                        posX + 7 + x, posY + 7 - y,
                        gMapHeader.mapLayout->map[posX + x + gMapHeader.mapLayout->width * (posY - y)] | 0x3000);
                }
            }
...
static void SetDecorRearrangementFlagIdIfFlagUnset(void) {
    ...
    for (i = 0; i < OBJECT_EVENT_TEMPLATES_COUNT; i++) {
        if (gSaveBlock1Ptr->objectEventTemplates[i].x == xOff && gSaveBlock1Ptr->objectEventTemplates[i].y == yOff &&
...
static void MarkSpriteDecorsInBoundsForRemoval(u8 left, u8 top, u8 right, u8 bottom) {
    ...
    for (i = 0; i < sDecorationContext.size; i++) {
        ...
        if (decor != DECOR_NONE && gDecorations[decor].permission == DECORPERM_SPRITE && left <= xOff && top <= yOff &&
