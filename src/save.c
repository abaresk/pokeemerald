...
#define SAVEBLOCK_OFFSET(struct, sector) \
    { sector *SECTOR_DATA_SIZE, min(sizeof(struct) - sector * SECTOR_DATA_SIZE, SECTOR_DATA_SIZE) }
...
static u32 RestoreSaveBackupVars(const struct SaveBlockChunk* chunk) // only ever called once, and gSaveBlock2 is passed
                                                                     // to this function. chunk is unused
{
    gFastSaveSection = &gSaveDataBuffer;
    gLastKnownGoodSector = gFirstSaveSector;
    gPrevSaveCounter = gSaveCounter;
...
static u8 WriteSomeFlashByte0x25ToCurSector(u16 chunkId, const struct SaveBlockChunk* chunk) {
    ...
    if (ProgramFlashByte(sector, sizeof(struct UnkSaveSection), 0x25)) {
        // sector is damaged, so SECTOR_DAMAGED the bit in gDamagedSaveSectors and restore the last written sector and
        // save counter.
        SetSectorDamagedStatus(SECTOR_DAMAGED, sector);
        gFirstSaveSector = gLastKnownGoodSector;
        gSaveCounter = gPrevSaveCounter;
...
static u8 ReadAllSaveSectorsCurrentSlot(u16 a1, const struct SaveBlockChunk* chunk) {
    ...
    for (i = 0; i < SAVE_SLOT_SECTORS; i++) {
        ...
        if (gFastSaveSection->security == FILE_SIGNATURE && gFastSaveSection->checksum == checksum) {
            u16 j;
    ...
    switch (saveType) {
        ...
        case SAVE_HALL_OF_FAME:
            ...
            if (status == SAVE_STATUS_OK)
                status = ReadSomeUnknownSectorAndVerify(SECTOR_HOF_2, gDecompressionBuffer + SECTOR_DATA_SIZE,
                                                        SECTOR_DATA_SIZE);
            break;
    }
