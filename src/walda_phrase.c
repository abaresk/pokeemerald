...
static bool32 TryCalculateWallpaper(u16* backgroundClr, u16* foregroundClr, u8* iconId, u8* patternId, u16 trainerId,
                                    u8* phrase);
...
static const u8 sWaldaLettersTable[] = { CHAR_B, CHAR_C, CHAR_D, CHAR_F, CHAR_G, CHAR_H, CHAR_J, CHAR_K,
...
enum { PHRASE_GIVEN_NEW, PHRASE_NO_CHANGE, PHRASE_FIRST_ATTEMPT };
...
u16 TryGetWallpaperWithWaldaPhrase(void) {
    ...
    gSpecialVar_Result =
        TryCalculateWallpaper(&backgroundClr, &foregroundClr, &iconId, &patternId, trainerId, GetWaldaPhrasePtr());
