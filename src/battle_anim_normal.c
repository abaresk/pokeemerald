...
u32 UnpackSelectedBattleAnimPalettes(s16 selector)
{
    ...
    return sub_80A75AC(
        battleBackground, attacker, target, attackerPartner, targetPartner, arg5, arg6);
}
...
// Task data for AnimTask_BlendColorCycle, AnimTask_BlendColorCycleExclude, and
...
// Many uses of this task only set a tNumBlends of 2, which has the effect of blending to a color
// and back once
...
{
    u32 selectedPalettes =
        ((u16)gTasks[taskId].tPalSelectorHi << 16) | (u16)gTasks[taskId].tPalSelectorLo;
    BeginNormalPaletteFade(selectedPalettes,
        gTasks[taskId].tDelay,
        startBlendAmount,
        targetBlendAmount,
...
// Flashes the specified anim tag with given color. Used e.g. to flash the particles red in Hyper
// Beam
void AnimTask_FlashAnimTagWithColor(u8 taskId)
{
    ...
    BeginNormalPaletteFade(
        1 << (paletteIndex + 16), 0, gBattleAnimArgs[4], gBattleAnimArgs[4], gBattleAnimArgs[3]);
...
void sub_8115F94(u8 taskId)
{
    ...
    TintPlttBuffer(
        selectedPalettes, gTasks[taskId].data[5], gTasks[taskId].data[6], gTasks[taskId].data[7]);
...
// Can shake battle terrain back and forth on the X or down and back to original pos on Y (cant
// shake up from orig pos) arg0: x offset of shake arg1: y offset of shake arg2: number of shakes
