...
#define ILLEGAL_BATTLE_TYPES                                                                                  \
    ((BATTLE_TYPE_LINK | BATTLE_TYPE_SAFARI | BATTLE_TYPE_FIRST_BATTLE | BATTLE_TYPE_WALLY_TUTORIAL |         \
      BATTLE_TYPE_ROAMER | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_KYOGRE_GROUDON | BATTLE_TYPE_LEGENDARY | \
      BATTLE_TYPE_REGI | BATTLE_TYPE_RECORDED | BATTLE_TYPE_TRAINER_HILL | BATTLE_TYPE_SECRET_BASE |          \
...
void sub_8184E58(void) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK) {
        ...
        sPlayers[0].trainerId = (gSaveBlock2Ptr->playerTrainerId[0]) | (gSaveBlock2Ptr->playerTrainerId[1] << 8) |
...
u8 RecordedBattle_GetBattlerAction(u8 battlerId) {
    ...
    if (sRecordedBytesNo[battlerId] >= BATTLER_RECORD_SIZE ||
        sBattleRecords[battlerId][sRecordedBytesNo[battlerId]] == 0xFF) {
...
static bool32 RecordedBattleToSave(struct RecordedBattleSave* battleSave, struct RecordedBattleSave* saveSection) {
    ...
    if (TryWriteSpecialSaveSection(SECTOR_BATTLE_RECORD, (void*)(saveSection)) != SAVE_STATUS_OK)
        return FALSE;
    else
        return TRUE;
}
...
bool32 MoveRecordedBattleToSaveData(void) {
    ...
    if (gTrainerBattleOpponent_A >= TRAINER_RECORD_MIXING_FRIEND &&
        gTrainerBattleOpponent_A < TRAINER_RECORD_MIXING_APPRENTICE) {
        for (i = 0; i < PLAYER_NAME_LENGTH + 1; i++)
            battleSave->recordMixFriendName[i] =
                gSaveBlock2Ptr->frontier.towerRecords[gTrainerBattleOpponent_A - TRAINER_RECORD_MIXING_FRIEND].name[i];
        battleSave->recordMixFriendClass =
            gSaveBlock2Ptr->frontier.towerRecords[gTrainerBattleOpponent_A - TRAINER_RECORD_MIXING_FRIEND]
                .facilityClass;

        if (sBattleOutcome == B_OUTCOME_WON) {
            for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
                battleSave->easyChatSpeech[i] =
                    gSaveBlock2Ptr->frontier.towerRecords[gTrainerBattleOpponent_A - TRAINER_RECORD_MIXING_FRIEND]
                        .speechLost[i];
        } else {
            for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
        ...
        battleSave->recordMixFriendLanguage =
            gSaveBlock2Ptr->frontier.towerRecords[gTrainerBattleOpponent_A - TRAINER_RECORD_MIXING_FRIEND].language;
    } else if (gTrainerBattleOpponent_B >= TRAINER_RECORD_MIXING_FRIEND &&
               gTrainerBattleOpponent_B < TRAINER_RECORD_MIXING_APPRENTICE) {
        for (i = 0; i < PLAYER_NAME_LENGTH + 1; i++)
            battleSave->recordMixFriendName[i] =
                gSaveBlock2Ptr->frontier.towerRecords[gTrainerBattleOpponent_B - TRAINER_RECORD_MIXING_FRIEND].name[i];
        battleSave->recordMixFriendClass =
            gSaveBlock2Ptr->frontier.towerRecords[gTrainerBattleOpponent_B - TRAINER_RECORD_MIXING_FRIEND]
                .facilityClass;
        ...
        battleSave->recordMixFriendLanguage =
            gSaveBlock2Ptr->frontier.towerRecords[gTrainerBattleOpponent_B - TRAINER_RECORD_MIXING_FRIEND].language;
    } else if (gPartnerTrainerId >= TRAINER_RECORD_MIXING_FRIEND &&
               gPartnerTrainerId < TRAINER_RECORD_MIXING_APPRENTICE) {
        for (i = 0; i < PLAYER_NAME_LENGTH + 1; i++)
            battleSave->recordMixFriendName[i] =
                gSaveBlock2Ptr->frontier.towerRecords[gPartnerTrainerId - TRAINER_RECORD_MIXING_FRIEND].name[i];
        battleSave->recordMixFriendClass =
            gSaveBlock2Ptr->frontier.towerRecords[gPartnerTrainerId - TRAINER_RECORD_MIXING_FRIEND].facilityClass;

        battleSave->recordMixFriendLanguage =
            gSaveBlock2Ptr->frontier.towerRecords[gPartnerTrainerId - TRAINER_RECORD_MIXING_FRIEND].language;
    }

    if (gTrainerBattleOpponent_A >= TRAINER_RECORD_MIXING_APPRENTICE) {
        battleSave->apprenticeId =
            gSaveBlock2Ptr->apprentices[gTrainerBattleOpponent_A - TRAINER_RECORD_MIXING_APPRENTICE].id;
        for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
            battleSave->easyChatSpeech[i] =
                gSaveBlock2Ptr->apprentices[gTrainerBattleOpponent_A - TRAINER_RECORD_MIXING_APPRENTICE].speechWon[i];
        battleSave->apprenticeLanguage =
            gSaveBlock2Ptr->apprentices[gTrainerBattleOpponent_A - TRAINER_RECORD_MIXING_APPRENTICE].language;
    } else if (gTrainerBattleOpponent_B >= TRAINER_RECORD_MIXING_APPRENTICE) {
        battleSave->apprenticeId =
            gSaveBlock2Ptr->apprentices[gTrainerBattleOpponent_B - TRAINER_RECORD_MIXING_APPRENTICE].id;
        for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
            battleSave->easyChatSpeech[i] =
                gSaveBlock2Ptr->apprentices[gTrainerBattleOpponent_B - TRAINER_RECORD_MIXING_APPRENTICE].speechWon[i];
        battleSave->apprenticeLanguage =
            gSaveBlock2Ptr->apprentices[gTrainerBattleOpponent_B - TRAINER_RECORD_MIXING_APPRENTICE].language;
        ...
        battleSave->apprenticeLanguage =
            gSaveBlock2Ptr->apprentices[gPartnerTrainerId - TRAINER_RECORD_MIXING_APPRENTICE].language;
    }
            ...
            if (arg0 == 1) {
                ...
                if (sBattleRecords[battlerId][sRecordedBytesNo[battlerId]] == ACTION_MOVE_CHANGE) {
                    ...
                    if (!(gBattleMons[battlerId].status2 & STATUS2_TRANSFORMED)) {
                        for (j = 0; j < MAX_MON_MOVES; j++) {
                            ppBonuses[j] = ((GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                                                        MON_DATA_PP_BONUSES, NULL) &
                                             ((3 << (j << 1)))) >>
                                            (j << 1));
                        }
                        for (j = 0; j < MAX_MON_MOVES; j++) {
                            movePp.moves[j] = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                                                         MON_DATA_MOVE1 + array1[j], NULL);
                            movePp.pp[j] = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                                                      MON_DATA_PP1 + array1[j], NULL);
                            array3[j] = ppBonuses[array1[j]];
                        }
                        for (j = 0; j < MAX_MON_MOVES; j++) {
                            SetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]], MON_DATA_MOVE1 + j,
                                       &movePp.moves[j]);
                    ...
                    gChosenMoveByBattler[battlerId] =
                        gBattleMons[battlerId].moves[*(gBattleStruct->chosenMovePositions + battlerId)];
                }
            }
        }
    }
