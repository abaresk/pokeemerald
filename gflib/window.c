...
bool16 InitWindows(const struct WindowTemplate* templates) {
    ...
    for (i = 0, allocatedBaseBlock = 0, bgLayer = templates[i].bg; bgLayer != 0xFF && i < 0x20;
         ++i, bgLayer = templates[i].bg) {
        if (gUnneededFireRedVariable == 1) {
            allocatedBaseBlock =
                DummiedOutFireRedLeafGreenTileAllocFunc(bgLayer, 0, templates[i].width * templates[i].height, 0);
        ...
        if (gUnneededFireRedVariable == 1) {
            ...
            DummiedOutFireRedLeafGreenTileAllocFunc(bgLayer, allocatedBaseBlock,
                                                    templates[i].width * templates[i].height, 1);
        }
    }
...
u16 AddWindow(const struct WindowTemplate* template) {
    ...
    if (gUnneededFireRedVariable == 1) {
        ...
        DummiedOutFireRedLeafGreenTileAllocFunc(bgLayer, allocatedBaseBlock,
                                                gWindows[win].window.width * gWindows[win].window.height, 1);
    }
...
void RemoveWindow(u8 windowId) {
    ...
    if (gUnneededFireRedVariable == 1) {
        DummiedOutFireRedLeafGreenTileAllocFunc(bgLayer, gWindows[windowId].window.baseBlock,
    ...
    if (w != 0 && h != 0) {
        ...
        switch (mode) {
            case 1:
                ...
                break;
            case 2:
                LoadBgTiles(windowLocal.window.bg, windowLocal.tileData + (rectPos * 32), rectSize,
                            windowLocal.window.baseBlock + rectPos);
                break;
            case 3:
                LoadBgTiles(windowLocal.window.bg, windowLocal.tileData + (rectPos * 32), rectSize,
                            windowLocal.window.baseBlock + rectPos);
...
void PutWindowTilemap(u8 windowId) {
    ...
    WriteSequenceToBgTilemapBuffer(
        windowLocal.window.bg, GetBgAttribute(windowLocal.window.bg, BG_ATTR_BASETILE) + windowLocal.window.baseBlock,
        windowLocal.window.tilemapLeft, windowLocal.window.tilemapTop, windowLocal.window.width,
        windowLocal.window.height, windowLocal.window.paletteNum, 1);
}
...
void PutWindowRectTilemapOverridePalette(u8 windowId, u8 x, u8 y, u8 width, u8 height, u8 palette) {
    ...
    for (i = 0; i < height; ++i) {
        WriteSequenceToBgTilemapBuffer(windowLocal.window.bg, currentRow, windowLocal.window.tilemapLeft + x,
                                       windowLocal.window.tilemapTop + y + i, width, 1, palette, 1);
...
void ClearWindowTilemap(u8 windowId) {
    ...
    FillBgTilemapBufferRect(windowLocal.window.bg, gTransparentTileNumber, windowLocal.window.tilemapLeft,
                            windowLocal.window.tilemapTop, windowLocal.window.width, windowLocal.window.height,
...
void PutWindowRectTilemap(u8 windowId, u8 x, u8 y, u8 width, u8 height) {
    ...
    for (i = 0; i < height; ++i) {
        WriteSequenceToBgTilemapBuffer(windowLocal.window.bg, currentRow, windowLocal.window.tilemapLeft + x,
                                       windowLocal.window.tilemapTop + y + i, width, 1, windowLocal.window.paletteNum,
                                       1);
...
void BlitBitmapRectToWindow(u8 windowId, const u8* pixels, u16 srcX, u16 srcY, u16 srcWidth, int srcHeight, u16 destX,
                            u16 destY, u16 rectWidth, u16 rectHeight) {
...
static void BlitBitmapRectToWindowWithColorKey(u8 windowId, const u8* pixels, u16 srcX, u16 srcY, u16 srcWidth,
                                               int srcHeight, u16 destX, u16 destY, u16 rectWidth, u16 rectHeight,
                                               u8 colorKey) {
    struct Bitmap sourceRect;
    struct Bitmap destRect;
...
void BlitBitmapRectToWindow4BitTo8Bit(u8 windowId, const u8* pixels, u16 srcX, u16 srcY, u16 srcWidth, int srcHeight,
                                      u16 destX, u16 destY, u16 rectWidth, u16 rectHeight, u8 paletteNum) {
    struct Bitmap sourceRect;
    struct Bitmap destRect;
