...
struct EventObject
{
    ...
    u32 disableCoveringGroundEffects : 1; // disables ground effects that cover parts of the
                                          // object's sprite
    u32 landingJump : 1;
    ...
    /*0x22*/ u8 playerCopyableMovement; // used as an index to gCopyPlayerMovementFuncs for the
                                        // "copy player" movement types
    /*size = 0x24*/
};
...
{
    /*0x00*/ u8 flags;
    /*0x01*/ u8 unk1; // used to be named bike, but its definitely not that. seems to be some
                      // transition flags
